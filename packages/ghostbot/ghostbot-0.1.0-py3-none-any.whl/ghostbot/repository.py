...
class Repository(Basis):
    REPOSITORY = "repository.db"
    DDL = "ddl/sqlite/repository.sql"
    ...
    def __init__(self):
        ...
        self._dsn = fs.join(GhostbotProfile.assets_database, self.REPOSITORY)
    ...
    def load_project(self):
        if self.is_ready():
            ...
            if fs.exists(projects_select):
                ...
                conn = db.open(self._dsn)
                with conn.cursor() as cursor:
                    row = cursor.query(db.load(projects_select)).fetchone()
                    if row:
