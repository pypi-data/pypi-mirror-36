...
class BrainwaveWebservice(Webservice):
    ...
    """
    _expected_payload_keys = ['name', 'description', 'tags', 'createdTime', 'computeType', 'keys', 'properties'
                                                                                                   'sslEnabled',
                              'ipAddress', 'port', 'numReplicas']
    ...
    def run(self, input_data):
        ...
        if self.client is None:
            self.client = PredictionClient(self.address, self.port, False, "")
