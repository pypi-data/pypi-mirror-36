...
class ACLBuilder(object):
    ...
    def buildACL(self):
        ...
        l.info("acl built: {acl}".format(acl = acl))
        return acl
...
class AuthController():
    ...
    def editUser(self):
        ...
        userid = self.userid
        user = self.user
        #l.info("method name = " + self.request.matched_route.name)
        #l.info("session = {session}".format(session=repr(self.request.session)))
        if self.request.matched_route.name == 'ppss:user:editself':
            userid = self.request.session['user']['id']
            user = self.getUserById(userid)
            self.userid = userid
            self.user = user
        l.info("***{id} -> {user}".format(user=user,id=userid))
        #raise Exception("stoca")
        ...
        if self.request.POST:
            ...
            if newpassword:
                l.info("*****changing password to {pwd}".format(pwd=newpassword) )
                l.warn("checkPassword result = {res}".format(res = checkPassword(user,newpassword) ))
            ...
            l.info("group={groups}".format(groups=groups ))
    ...
    def editGroup(self):
        ...
        if self.request.POST:  #editing group
            ...
            return render_to_response(  Conf.editgrouptemplate,
                dict(retdict,**{'group':group,'allperm':allperm,'msg':""}),
                self.request )
        #return HTTPFound(self.request.route_url("ppss:group:list") )
