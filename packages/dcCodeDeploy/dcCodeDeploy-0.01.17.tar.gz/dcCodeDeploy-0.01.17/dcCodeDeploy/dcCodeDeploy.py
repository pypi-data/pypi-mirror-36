...
@task
def update_app_utils(app, instanceType, branch):
    ...
    # tar it up
    TAR_NAME = "app-utils"
    local('tar -czf %s.tar.gz %s' % (TAR_NAME, [APP_UTILS_DIR + "/config", APP_UTILS_DIR + "/environment"] ))

    # and move the tarball to the destination and expand it
    # set the upload path to the home directory
    UPLOAD_CODE_PATH = "$HOME/" + app 
    unpack_code()

    # run the "instanceType"-command.sh 
    cmdToRun = "$HOME/" + app + "/" + APP_UTILS_DIR + "/" + ENVIRONMENT + "/" + instanceType + "-commands.sh"
    sudo(cmdToRun)

    # run deployenv on the instance to get the new values into /etc/enviornmnet
    sudo("deployenv.sh --type instance -a " + app + " -e " + ENVIRONMENT)
