...
@task
def update_app_utils(appname, instanceType, branch):
    ...
    local('tar -czf %s.tar.gz %s' % (TAR_NAME, DIRS_TO_TAR))
    ...
    cmd = "mkdir -p " + UPLOAD_CODE_PATH
    sudo(cmd)
    put('%s.tar.gz' % TAR_NAME, '%s' % UPLOAD_CODE_PATH, use_sudo=True, temp_dir="/tmp")
    with cd('%s' % UPLOAD_CODE_PATH):
        sudo('tar zxf %s.tar.gz' % TAR_NAME)

    # run the "instanceType"-command.sh 
    cmdToRun = "$HOME/" + appname + "/" + APP_UTILS_DIR + "/" + ENVIRONMENT + "/" + instanceType + "-commands.sh"
    sudo(cmdToRun)

    # run deployenv on the instance to get the new values into /etc/enviornmnet
    sudo("deployenv.sh --type instance -a " + appname + " -e " + ENVIRONMENT)
