...
@task
def update_app_utils(appname, branch):
    ...
    CONFIG_ENV_DIR = "$HOME/" + appname + "/" + APP_UTILS_DIR + "/config/" + ENVIRONMENT
    with cd('%s' % CONFIG_ENV_DIR):
        run("git checkout " + branch)
        #cmdToRun = "$HOME/" + appname + "/" + APP_UTILS_DIR + "/config/" + ENVIRONMENT + "/${dc_TAG_Type}-commands.sh"
        cmdToRun = CONFIG_ENV_DIR + "/${dc_TAG_Type}-commands.sh"
        sudo(cmdToRun)
