...
class BoxCoxTransformation(Transformation):
    ...
    def __init__(self, lambda2: float = 0):
    ...
    def back_transform(self, x: np.array) -> np.array:
        try:
            ...
            if w.__str__() == "invalid value encountered in power" and (x < 0).all() and self.params.lambda1 < 1:
                ...
                raise ValueError("Back-transform failed for y(x, lambda1, lambda2, lambda3) with:\n"
                                 "x = %s\n"
                                 "lambda1 = %s\n"
                                 "lambda2 = %s\n"
                                 "lambda3 = %s\n"
                                 "warning = %s" % (x, self.params.lambda1, self.params.lambda2, self.params.lambda3, w))
        return y
