...
def echo(line, *args, output = None, nl = True, **kwargs):
	if output is not None:
		output.write(ansi_escape.sub('', line) + ('\n' if nl else ''))
	return click.echo(line, *args, nl = nl, **kwargs)

def secho(line, *args, output = None, nl = True, **kwargs):
	if output is not None:
		output.write(ansi_escape.sub('', line) + ('\n' if nl else ''))
	return click.secho(line, *args, nl=nl, **kwargs)
...
def print_line(row, widths, ellipsize = False, align = 'left', **kwargs):
	...
	for last, col in mark_last(enumerate(colored)):
		...
		if not last or len(widths) == 1:
			echo(pad(text, widths[i], align = align) + '  ', nl = last, **kwargs)
		else:
			echo(text, nl = last, **kwargs)
...
def print_section(section, **kwargs):
	...
	for row in section:
		print_line(row, widths, align = align, **kwargs)
	# map(render, section)

def print_sections(sections, **kwargs):
	for heading, section in sections.items():
		print_header(heading, **kwargs)
		print_section(section, **kwargs)
