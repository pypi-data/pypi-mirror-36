...
from .print import print_error, print_header, print_sections, print_info, pad, TERM_WIDTH
...
def k8s(conf):
	...
	if hs.pull_status():
		...
		click.echo('')
		print_sections(hs.repr, output=conf.output)
	else:
...
def orbit(conf):
	try:
		...
		print_error(str(e))
		sys.exit(1)
	print_sections(oc.repr, output=conf.output)
...
def backends(ctx, deep):
	...
	if len(checked) == 1:
		checked = NO_PROBLEMS
	print_sections({'backends': checked}, output=conf.output)
	if deep:
		ctx.invoke(buckets)
...
def buckets(conf):
	...
	if len(checked) == 1:
		checked = NO_PROBLEMS
	print_sections({'buckets': checked}, output = conf.output)
