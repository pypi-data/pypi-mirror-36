...
class Container(object):
  ...
  #        json.dump(data,stdout)
        ...
        action = data["Action"]
        if action == 'create':
          Container.register_service(id)
        if action == 'remove':
          Container.unregister_service(id,name)
        if action == 'update':
          Container.update_service(id)
    except Exception as ex:
      globals.logger.warn ("caught error in Container process(): " + data_t )
      globals.logger.warn ("ex: " +  str(ex) )
      pass
  ...
  def load():
    try:
      ...
      for s in _services.list():
        ...
        service = Container ( s.id )
        service.name = s.name
        if "Labels" in s.attrs['Spec']:
          for k,  v  in  s.attrs['Spec']['Labels'].items():
            labels[k] = v

        published_ports = Container.get_ports(s.attrs)
...
#      print("Unregistering service {} ({})\n".format(name, id))
      Container.remove ( id )
      Callbacks.process ("service", "unregister")
