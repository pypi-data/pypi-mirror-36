...
def message_handler(bot: Bot, update: Update, bot_instance: TelegramBot):
    chat_id = bot_instance.settings['chat_id']
    m = update.effective_message
    if hasattr(m, 'audio') and m.audio:
        a = m.audio
        r = bot.send_audio(chat_id, a.file_id, a.duration, a.performer, a.title, m.caption_html, parse_mode='html')
    elif hasattr(m, 'document') and m.document:
        d = m.document
        r = bot.send_document(chat_id, d.file_id, d.file_name, m.caption_html, parse_mode='html')
    elif hasattr(m, 'photo') and m.photo:
        p = m.photo
        r = bot.send_photo(chat_id, p[-1].file_id, m.caption_html, parse_mode='html')
    elif hasattr(m, 'sticker') and m.sticker:
        s = m.sticker
        r = bot.send_sticker(chat_id, s.file_id)
    elif hasattr(m, 'video') and m.video:
        v = m.video
        r = bot.send_video(chat_id, v.file_id, v.duration, m.caption_html, parse_mode='html')
    elif hasattr(m, 'voice') and m.voice:
        v = m.voice
        r = bot.send_voice(chat_id, v.file_id, v.duration, m.caption_html, parse_mode='html')
