...
class ProxyURI(object):
    ...
    async def prepare_ciphers_and_headers(self, reader_remote, writer_remote, host, port, handler):
        if not self.direct:
            ...
            if handler:
                if self.cipher and self.cipher not in handler.ready:
                    handler.ready.add(self.cipher)
                    _, writer_cipher_r = self.cipher(reader_remote, writer_remote, DUMMY, DUMMY, DUMMY, DUMMY)
            else:
