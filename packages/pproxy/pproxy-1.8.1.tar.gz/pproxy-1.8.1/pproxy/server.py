...
class ProxyURI(object):
    def __init__(self, **kw):
        ...
        self.udpmap = {}
        self.handler = None
        self.handler_lock = asyncio.Lock()
    ...
    async def open_connection(self, host, port, local_addr, lbind):
        ...
        elif self.reuse and self.handler:
            return self.handler.streams
        elif self.unix:
    ...
    async def prepare_connection(self, reader_remote, writer_remote, host, port):
        if self.reuse and not self.handler:
            async with self.handler_lock:
                if not self.handler:
                    handler = self.rproto.get_handler(reader_remote, writer_remote, DUMMY)
                    ret = await self.prepare_ciphers_and_headers(reader_remote, writer_remote, host, port, handler)
                    self.handler = handler
                    return ret
        return await self.prepare_ciphers_and_headers(reader_remote, writer_remote, host, port, self.handler)
    async def prepare_ciphers_and_headers(self, reader_remote, writer_remote, host, port, handler):
        if not self.direct:
            ...
            if not handler or not handler.ready:
                ...
                writer_cipher_r = None
            whost, wport = (host, port) if self.relay.direct else (self.relay.host_name, self.relay.port)
            sid = await self.rproto.connect(reader_remote=reader_remote, writer_remote=writer_remote, rauth=self.auth, host_name=whost, port=wport, writer_cipher_r=writer_cipher_r, sock=writer_remote.get_extra_info('socket'))
            if sid:
                reader_remote, writer_remote = handler.get_streams(sid)
