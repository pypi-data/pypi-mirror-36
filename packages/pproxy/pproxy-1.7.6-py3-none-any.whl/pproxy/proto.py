    ...
    def get_handler(self, reader, writer, verbose, tcp_handler=None, udp_handler=None):
        class Handler:
                ...
                try:
                    while True:
                        ...
                        if header in (0x01, 0x03, 0x04, 0x11, 0x13, 0x14):
                            ...
                            elif header == 0x30 and sid in self.sessions and udp_handler:
                                host_name, port, sendto = self.sessions[sid]
            ...
            def get_datagram(self, sid, host_name, port):
                ...
                self.sessions[sid] = (host_name, port, sendto)
                return self.sessions[sid]
        return Handler()
    async def connect(self, reader_remote, writer_remote, host_name, port, **kw):
        sid = os.urandom(8)
        writer_remote.write(b'\x03' + sid + packstr(host_name.encode()) + port.to_bytes(2, 'big'))
        return sid
