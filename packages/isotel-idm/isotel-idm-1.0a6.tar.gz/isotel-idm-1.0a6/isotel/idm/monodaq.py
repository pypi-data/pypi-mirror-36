...
from isotel.idm import gateway, signal
import socket
import struct
import time
import requests
...
class MonoDAQ_U(gateway.Device):
    USER_HEADER_LEN = 2
    DIGITAL_MASK = 0x80000000   # digital stream enabled
    ANALOG_MASK = 0x7fffffff    # all analog channels present in a stream
    ANUSED_MASK = 0x3FFFFF      # used (selected) channels
    ...
    def settime(self, finetune=True):
        ...
        try:
            ...
            while ax != None or dx != None:
                if ax != None and ax == dx:
                    ...
                    yield (ax,ay),(None,(float('nan'),))
                    ax = next(Axi, None)
                    ay = next(Ayi, None)
                    i += 1
                elif ax == None or (dx != None and ax > dx):
                    yield (None,(float('nan'),)),(dx,dy)
                    dx = next(Dxi, None)
                    dy = next(Dyi, None)
                    if Ax == []:
                        i += 1
