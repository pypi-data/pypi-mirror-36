    ...
    def redirect_request(self):
        ...
        try:
            ...
            for record in active_redirects:
                if self.request_uri.startswith(u'{protocol}://{record}'.format(protocol=self.request_uri.scheme,
                                                                               record=record)):
            ...
            replace_len = len(self.request_uri.scheme) + 3 + len(best_match_name)
            self.request_uri = u'{replace}{remaining}'.format(replace=best_match[redirect.URI],
                                                              remaining=self.request_uri.url[replace_len:])

            logging.info(self.prefix_message(u'Redirected URI: {uri}'.format(uri=self.request_uri.url)))
