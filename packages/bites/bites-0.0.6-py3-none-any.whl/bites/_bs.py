...
class Bs:
    ...
    def extto(self, n):
        ...
        - if `bs._r` is set, return `bs.repto(n)`
        - else if `bs._p` is set, return `bs.padto(n, bs._p)`
        - else raise error
        '''
        if self._r:
            return self.repto(n)
    ...
    def every(self, n, m=lambda x: x, f=lambda x: True):
        # Wow!
        return list(
            map(
                m,
    ...
    def each(self, f=lambda x: x):
        return [f(Bs(i)) for i in self]
