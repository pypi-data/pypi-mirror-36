...
def mosaic_list_from_parser(args):
    ...
    for filelist in os.listdir(args.local):
        if filelist.endswith('.geojson'):
            ...
            idl(infile=os.path.join(args.local, filelist),
...
def main(args=None):
    ...
    parser_idm = subparsers.add_parser('download_metadata',
    ...
    parser_shp2gj = subparsers.add_parser('shp2geojson',
            help='Convert all shapefiles in folder to GeoJSON')
    parser_shp2gj.add_argument('--source', help='Choose Source Folder')
    parser_shp2gj.add_argument('--destination',
