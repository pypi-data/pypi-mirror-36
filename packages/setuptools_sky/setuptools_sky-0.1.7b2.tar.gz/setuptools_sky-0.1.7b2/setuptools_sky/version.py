...
def get_latest_version(branch_prefix='origin/release'):
    ...
    for branch_name in release_branches:
        if branch_prefix not in branch_name:
            continue

        tokens = map(lambda x: int(x), branch_name.split(branch_prefix+'/')[1].split('.'))
    ...
    if branch_prefix == 'origin/legacy':
        return '{}.{}.{}'.format(max_version[0], max_version[1], max_version[2]+1)
    return '{}.{}.{}'.format(max_version[0], max_version[1], max_version[2])


def guess_next_dev_version(version):
    log.warn("guess next version")
    if version.exact:
        return version.format_with("{tag}")
    else:
        branch_name = os.environ.get('BRANCH_NAME', "develop")
        log.warn("BRANCH NAME: %s" % branch_name)

        if branch_name.startswith('PR'):
            target_branch = os.environ.get('CHANGE_TARGET', 'develop')
            log.warn("TARGET BRANCH: %s" % target_branch)

            if target_branch.startswith('master') or target_branch.startswith('legacy'):
        ...
        elif branch_name.startswith('legacy'):
            return get_latest_version('origin/legacy')
        else:
            return '%s.%s' % (version.tag.base_version, version.format_with('alpha.{distance}'))
