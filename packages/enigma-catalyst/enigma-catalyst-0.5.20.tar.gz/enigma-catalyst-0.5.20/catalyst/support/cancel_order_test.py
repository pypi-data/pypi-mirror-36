...
from catalyst.api import order_target, record, symbol, cancel_order, order
import pandas as pd
...
def handle_data(context, data):
    if not context.blotter.open_orders:
        if(context.portfolio.positions and context.portfolio.positions[context.asset].amount >= 2):
            order_target(context.asset, 0, limit_price=(
                data.current(context.asset, 'price') + 0.00002))
        else:
            context.id_o = order_target(context.asset, 50, limit_price=(
                data.current(context.asset, 'price') - 0.00002))
    else:
        cancel_order(context.id_o,context.asset)
        # for order_one in context.blotter.open_orders[context.asset]:
        #     cancel_order(order_one, context.asset)
    record(btc=data.current(context.asset, 'price'))
