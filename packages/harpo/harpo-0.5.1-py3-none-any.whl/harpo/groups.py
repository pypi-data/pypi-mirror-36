...
from harpo.util import mkdir_p as mkdir_p
from harpo.util import list_uniq as list_uniq
...
class GroupManager(object):
    ...
    def save(self):
        with open(self.groups_file, 'w') as stream:
            try:
                data = json.dumps(self.groups, sort_keys=True, indent=2)
                stream.write(data)
            except (json.JSONDecodeError, OSError) as e:
    ...
    def __getitem__(self, gid):
        try:
            ...
            raise HarpoException("No such group: {}".format(gid))
    ...
    def create(self, gid):
        if gid in self.groups.keys():
            raise HarpoException("Group already exists: {}".format(gid))

        logging.info("Creating group %s", gid)
        self.groups[gid] = {"users": []}
        self.save()

    def remove(self, gid):
        if gid not in self.groups.keys():
            raise HarpoException("Group doesn't exist: {}".format(gid))

        logging.info("Removing group %s", gid)
        self.groups.pop(gid)
        self.save()
...
class Group(dict):
    # Inherit it from dict and initialize as one
    ...
    def update_dict(self):
        dict.__init__(self, users=list_uniq(self.users))
