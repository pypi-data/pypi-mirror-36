...
from harpo.util import HarpoException as HarpoException
...
class DomainManager(object):
    ...
    def create(self, domain_name):
        ...
        :return: path
        :raises: HarpoException when already exists
        """
        if self.exists(domain_name):
            raise HarpoException("Domain already exists: {}".format(domain_name))
    ...
    def remove(self, domain_name):
        ...
        if not self.exists(domain_name):
            raise HarpoException("Domain doesn't exist: {}".format(domain_name))
    ...
    def save_recipients(self):
        try:
            with open(self.recipients_file, 'w+') as stream:
                stream.write(json.dumps(self.recipients))
        except OSError as e:
            raise HarpoException(e)
