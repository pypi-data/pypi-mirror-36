...
`}static get properties(){return{hass:Object,_currentMessage:Object,_errorMsg:String,_loading:{type:Boolean,value:!1},_opened:{type:Boolean,value:!1}}}showDialog({hass,message}){this.hass=hass;this._loading=!0;this._errorMsg=null;this._currentMessage=message;this._opened=!0;this.$.transcribe.innerText=message.message;const platform=message.platform,mp3=this.$.mp3;mp3.src=null;const url=`/api/mailbox/media/${platform}/${message.sha}`;this.hass.fetchWithAuth(url).then(response=>{if(response.ok){return response.blob()}return Promise.reject({status:response.status,statusText:response.statusText})}).then(blob=>{this._loading=!1;mp3.src=window.URL.createObjectURL(blob);mp3.play()}).catch(err=>{this._loading=!1;this._errorMsg=`Error loading audio: ${err.statusText}`})}openDeleteDialog(){if(confirm(this.localize("ui.panel.mailbox.delete_prompt"))){this.deleteSelected()}}deleteSelected(){const msg=this._currentMessage;this.hass.callApi("DELETE",`mailbox/delete/${msg.platform}/${msg.sha}`);this._dialogDone()}_dialogDone(){this.$.mp3.pause();this.setProperties({_currentMessage:null,_errorMsg:null,_loading:!1,_opened:!1})}_openedChanged(ev){if(!ev.detail.value){this._dialogDone()}}}customElements.define("ha-dialog-show-audio-message",HaDialogShowAudioMessage)}}]);
