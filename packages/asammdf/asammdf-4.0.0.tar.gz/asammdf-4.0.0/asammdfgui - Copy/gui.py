...
class Plot(pg.PlotWidget):
    ...
    def setSignalEnable(self, index, state):
        ...
        selected_items = [index for index, sig in enumerate(self.signals) if sig.enable]
...
class FileWidget(QWidget):
    def __init__(self, file_name, memory, step_mode, with_dots, *args, **kwargs):
        ...
        target = MDF
        kwargs = {"name": file_name, "memory": memory, "callback": self.update_progress}

        self.mdf = run_thread_with_progress(
            self, target=target, kwargs=kwargs, factor=33, offset=0, progress=progress
        )
