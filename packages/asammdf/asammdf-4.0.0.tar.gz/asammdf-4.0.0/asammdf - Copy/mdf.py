...
class MDF(object):
    ...
    def __init__(self, name=None, memory="full", version="4.10", callback=None):
        if name:
            ...
            if version in MDF2_VERSIONS:
                self._mdf = MDF3(version=version, memory=memory, callback=callback)
            elif version in MDF3_VERSIONS:
                self._mdf = MDF3(version=version, memory=memory, callback=callback)
            elif version in MDF4_VERSIONS:
                self._mdf = MDF4(version=version, memory=memory, callback=callback)
            else:
                message = (
        ...
        for i, group in enumerate(self.groups):
            ...
            for idx, fragment in enumerate(data):
                ...
                if idx == 0:
                    ...
                    for j in included_channels:
                        ...
                        if version < "4.00" and sig.samples.dtype.kind == "S":
                            strsig = self.get(group=i, index=j, samples_only=True)
                ...
                # samples records to the data block
                    ...
                    for j in included_channels:
                        sig = self.get(
                            group=i, index=j, data=fragment, raw=True, samples_only=True
                        )
    ...
    def cut(self, start=None, stop=None, whence=0):
        ...
        """
        out = MDF(version=self.version, memory=self.memory)
        ...
        for i, group in enumerate(self.groups):
            ...
            for fragment in data:
                ...
                if idx == 0:
                    sigs = []
                    for j in included_channels:
                        sig = self.get(group=i, index=j, data=fragment, raw=True).cut(
    ...
    def export(self, fmt, filename=None, **kargs):
        ...
        **kwargs
        ...
        header_items = ("date", "time", "author", "organization", "project", "subject")
        ...
        if fmt == "hdf5":
                ...
                if single_time_base:
                    ...
                    for i, grp in enumerate(self.groups):
                        ...
                        for col, _ in enumerate(grp["channels"]):
                            ...
                            try:
                                sheet.write_column(
                                    1, col + offset, sig.samples.astype(str)
                                )
                            except:
                                vals = [str(e) for e in sig.samples]
            ...
            if format == "7.3":
                savemat(name, mdict, long_field_names=True, format="7.3")
            else:
    ...
    def filter(self, channels, memory="full"):
        ...
        *channels* argument
        ...
        mdf = MDF(version=self.version, memory=memory)
        ...
        for new_index, (group_index, indexes) in enumerate(gps.items()):
            ...
            for idx, fragment in enumerate(data):
                ...
                if idx == 0:
                    ...
                    for j in indexes:
                        sig = self.get(
                            group=group_index, index=j, data=fragment, raw=True
                        )
                        if self.version < "4.00" and sig.samples.dtype.kind == "S":
                            strsig = self.get(
                                group=group_index, index=j, samples_only=True
                            )
                            sig.samples = sig.samples.astype(strsig.dtype)
                            del strsig
                        if not sig.samples.flags.writeable:
            ...
            for offset, group, mdf in zip(offsets, groups, files):
                ...
                for fragment in data:
                    ...
                    if idx == 0:
                        signals = []
                        for j in included_channels:
                            sig = mdf.get(group=i, index=j, data=fragment, raw=True)
                            ...
                            if version < "4.00" and sig.samples.dtype.kind == "S":
                                ...
                                for tmp_mdf in files:
                                    strsig = tmp_mdf.get(
                                        group=i, index=j, samples_only=True
                                    )
                    ...
                    if idx == 0:
                        ...
                        for j in included_channels:
                            ...
                            if version < "4.00" and sig.samples.dtype.kind == "S":
                                ...
                                strsig = mdf.get(group=i, index=j, samples_only=True)
                                string_dtypes.append(strsig.dtype)
                                del strsig
            ...
            for fragment in data:
                ...
                for j, _ in enumerate(group["channels"]):
                    if j == master_index:
                        continue
                    sigs[idx].append(
                        self.get(group=i, index=j, data=fragment, samples_only=True)
                    )
                    idx += 1
        ...
        for i, group in enumerate(self.groups):
            ...
            for idx, fragment in enumerate(data):
                if idx == 0:
                    ...
                    for j in included_channels:
                        sig = self.get(
                            group=i, index=j, data=fragment, raw=True, raster=raster
                        )
                        if self.version < "4.00" and sig.samples.dtype.kind == "S":
                            strsig = self.get(group=i, index=j, samples_only=True)
                            sig.samples = sig.samples.astype(strsig.dtype)
                    ...
                    mdf.append(
                        sigs, "Resampled to {}s".format(raster), common_timebase=True
                    )
