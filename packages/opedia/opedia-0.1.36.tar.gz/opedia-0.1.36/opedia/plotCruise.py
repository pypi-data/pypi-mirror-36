...
def main():
    ...
    source = sys.argv[2]      
    cruise = sys.argv[3]      
    resampTau = sys.argv[4]
    shapeFlag = bool(int(sys.argv[5]))
    cololocalizeFalg = bool(int(sys.argv[6]))
    fname = sys.argv[7] 

    df = getCruiseTrack(DB_Cruise, source, cruise)
    df = resample(df, resampTau) 
    if shapeFlag:       ## generates cruise track shapefile     
        try:            
            dumpCruiseShape(df.copy(), source, cruise, fname)
        except Exception as e:              
            print("Error in generating the cruise shape file: ")
            print(e)

    if cololocalizeFalg:        ## generates along track plot
        tables = sys.argv[8].split(',')
        variables = sys.argv[9].split(',')      
        spatialTolerance = float(sys.argv[10])         #spatial tolerance (+/- degrees) 
        depth1 = 0
        depth2 = 5        
