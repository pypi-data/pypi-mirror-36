...
from edam.reader.models import Station, Sensors, AbstractObservables, UnitsOfMeasurement, \
    HelperTemplateIDs
from edam.reader.utilities import parse_for_iterations, extract_data_from_preamble, check_if_path_exists, parse_observables_with_reasoner
...
class SourceConfiguration:
    ...
    def __init__(self, input_yaml, input_file_data=io.StringIO(), input_preamble=io.StringIO(),
                 template_preamble=io.StringIO(), template_name=None):
        """
        """
    ...
    def set_station(self, parse_from_yaml):
        ...
        if exists:
            ...
            if station.latitude is not None and station.longitude is not None:
                try:
                    test_point = Point("%s;%s" % (station.latitude, station.longitude))
                    station.latitude = test_point.latitude
                    station.longitude = test_point.longitude
    ...
    def set_observables(self, iter_or_not=None):
        ...
        if iter_or_not is None:
            ...
            parse_observables_with_reasoner(observables=observables, template_name=self.template_name)
        else:
