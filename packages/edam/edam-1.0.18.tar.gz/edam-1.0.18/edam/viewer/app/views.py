...
def specific_station(station):
    ...
    try:
        ...
        raise InvalidUsage('Station with id *%s* does not exist' % station, status_code=410)
...
def get_data():
    ...
    valid, template, station, chunk = calculate_data_and_render_from_template(target_template=template_name,
                                                                              station_id=station_id)
    # Template_id: Yucheng. The actual file is edam/Yucheng.tmpl
    if type(valid) is not InvalidUsage:
        response = make_response(render_template("edam/" + template_name + '.tmpl', chunk=chunk, station=station))
        response.headers['Content-type'] = 'text/plain'
        ...
        raise valid
        # if station:
        #     raise InvalidUsage('%s template and %s station are not compatible' % (template_name, station.name),
        #                    status_code=410)
        # else:
