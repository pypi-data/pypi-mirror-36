...
def check_if_path_exists(filename=None):
    ...
    """
    filename = generate_uri(filename)[0]
...
def check_if_ontology_class_exists_and_create(ontology, term, belongsTo):
    """
    :param ontology: The ontology to which we will register the class
    :param term: E.g. "http://edam.gr#Term"
    :return: the added class
    """
    term_exists = ontology.search(iri=term)
    term_name = term.split("#")[-1]
    if not term_exists:
        term_class = types.new_class(term_name, (belongsTo,), )
        return term_class, term_name, False
    else:
        term_class = term_exists[0]
        return term_class, term_name, True


def backup_ontology():
    os.remove("%s/edam.owl" % home_directory)
    file_src = '%s/backup.owl' % home_directory
    f_src = open(file_src, 'rb')
    
    file_dest = '%s/edam.owl' % home_directory
    f_dest = open(file_dest, 'wb')
    
    shutil.copyfileobj(f_src, f_dest)


def parse_observables_with_reasoner(observables, template_name):
    onto = get_ontology("file://%s/edam.owl" % home_directory)
    with onto:
        quantity, _, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                   term="http://edam.gr#Observables",
                                                                   belongsTo=Thing)
        gtemplate, _, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                    term="http://edam.gr#Templates",
                                                                    belongsTo=Thing)
        gquali, _, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(), term="http://edam.gr#Qualifiers",
                                                                 belongsTo=Thing)
        temporal_qualifiers, _, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                              term="http://edam.gr#Temporal",
                                                                              belongsTo=gquali)
        statistical_qualifiers, _, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                                 term="http://edam.gr#Statistical",
                                                                                 belongsTo=gquali)
        
        AllDisjoint([gtemplate, quantity, gquali])
        AllDisjoint([temporal_qualifiers, statistical_qualifiers])
        
        class hasObservable(FunctionalProperty, gtemplate >> quantity):
            pass
    
    namespace_edam = onto.get_namespace("http://%s.gr" % template_name)
    template, _, exists = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                    term="http://edam.gr#%s" % template_name,
                                                                    belongsTo=gtemplate)
    if exists:
        return
    with onto:
        observable_classes = list()
        
        for observable in observables:
            last_used_observable = None
            try:
                observable_class, observable_name, _ = check_if_ontology_class_exists_and_create(ontology=onto.load(),
                                                                                                 term=observable[
                                                                                                     'ontology'],
                                                                                                 belongsTo=quantity)
                stored_qualifiers = list()
                # E.g.['max', 'daily']: This will be used to create a unique class of both plus the quantity
                # E.g. maxdailyTemperature
                qualifier_names_as_string = list()
                try:
                    qualifiers_from_yaml = observable['qualifiers']
                    
                    for qualifier in qualifiers_from_yaml.split(','):
                        if qualifier.split("#")[-1] in ["daily", "yearly", "hourly", "minutes", "quarterly"]:
                            qualifier_class, qualifier_name, _ = check_if_ontology_class_exists_and_create(
                                ontology=onto.load(),
                                term=qualifier,
                                belongsTo=temporal_qualifiers)
                        else:
                            qualifier_class, qualifier_name, _ = check_if_ontology_class_exists_and_create(
                                ontology=onto.load(),
                                term=qualifier,
                                belongsTo=statistical_qualifiers)
                        qualifier_names_as_string.append(qualifier_name)
                        stored_qualifiers.append(qualifier_class)
                    # Here we will create the combined class Qualifiers + quantity
                    if qualifier_names_as_string:
                        combined_class = ''.join(qualifier_names_as_string) + observable_name
                        observable_qualifiers_class, _, _ = check_if_ontology_class_exists_and_create(
                            ontology=onto.load(),
                            term=combined_class,
                            belongsTo=observable_class)
                        observable_classes.append(observable_qualifiers_class)
                        last_used_observable = observable_qualifiers_class
                    # else:
                    #     observable_classes.append(observable_class)
                
                except Exception as e:
                    observable_classes.append(observable_class)
                    last_used_observable = observable_class
                    utilities_logger.debug('Error: ' + str(e))
                    pass
            ...
            with namespace_edam:
                test = last_used_observable(observable['observable_id'])
        
        observable_classes = list(map(lambda a: hasObservable.some(a), observable_classes))
        observable_classes.append(gtemplate)
        
        if not template.equivalent_to:
            template.equivalent_to = [functools.reduce(lambda a, b: a & b, observable_classes)]
        
        onto.save(file="%s/edam.owl" % home_directory)


def create_relation_among_units(source: dict, target: dict, mapping: dict):
    ureg = UnitRegistry()
    Q_ = ureg.Quantity
    final_mapping_tuples = list()
    for source_id, target_id in mapping.items():
        try:
            final_mapping_tuples.append(
                (source_id, target_id, Q_(source[source_id]).to(Q_(target[target_id])).magnitude))
        except Exception as e:
            print(str(e))
            final_mapping_tuples.append((source_id, target_id, 1))
    
    return final_mapping_tuples


def analyse_instance(template_class, list_with_instances: list):
    arbitrary_dict = dict()
    for item in list_with_instances:
        try:
            concept = list(filter(lambda x: x.is_a[0] != template_class, item.is_a))
            arbitrary_dict[concept[0]] = item.get_name()
        except Exception as e:
            print(e.args)
            print(template_class)
            print(list_with_instances)
            print(item.is_a)
            exit()
...
def match_different_named_instances(source_dataset_dict: dict, target_dataset_dict: dict):
    updated_mapping = dict()
    
    for concept, value in target_dataset_dict.items():
        updated_mapping[source_dataset_dict[concept]] = value
    return updated_mapping


def check_compatibility_among_templates(source_template, target_template):
    onto = get_ontology("file://%s/edam.owl" % home_directory).load()
    namespace_edam = onto.get_namespace("http://edam.gr")
    source_instances = onto.search(iri="http://%s.gr#*" % source_template)
    target_instances = onto.search(iri="http://%s.gr#*" % target_template)
    error_to_return = None
    try:
        source_class = onto.search(iri="*edam.owl#%s" % source_template)[0]
    except:
        error_to_return = '*%s* template does not exist' % source_template
        return False, None, error_to_return
    try:
        target_class = onto.search(iri="*edam.owl#%s" % target_template)[0]
    except:
        error_to_return = '*%s* template does not exist' % target_template
        return False, None, error_to_return
    
    with namespace_edam:
        sync_reasoner(debug=0, keep_tmp_file=True)
        
        # They are compatible!
        if target_class in source_class.is_a or target_class == source_class:
            source = analyse_instance(namespace_edam.Templates, source_instances)
            target = analyse_instance(namespace_edam.Templates, target_instances)
            mathcing_concepts = match_different_named_instances(source, target)
            return True, mathcing_concepts, error_to_return
        else:
...
if __name__ == "__main__":
    for file in os.listdir("%s/metadata" % home_directory):
        filename = os.path.join("%s/metadata" % home_directory, file)
        if fnmatch.fnmatch(file, 'Agmip.yaml') or fnmatch.fnmatch(file, 'Yucheng.yaml') or fnmatch.fnmatch(file,
                                                                                                           'wofost.yaml') or fnmatch.fnmatch(
            file, 'dssat.yaml') or fnmatch.fnmatch(file, 'bioma.yaml') or fnmatch.fnmatch(file,
                                                                                          'knmi_limited.yaml'):
            with open(filename, 'r') as stream:
                observables = yaml.load(stream)['Observables']
                parse_observables_with_reasoner(observables, template_name=file.split(".")[0])
