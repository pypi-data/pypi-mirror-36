...
_default_field_labels = {
    ...
    'authentication_code': _('Authentication code')
}
...
class LoginForm(Form, LoginFormMixin, NextFormMixin):
...
class TFAForm(Form, NextFormMixin):
    """The default two factor authetication form"""

    authentication_code = StringField(get_form_field_label('authentication_code'), validators=[Required(message='AUTHENTICATION_CODE_NOT_PROVIDED')])
    email = HiddenField(get_form_field_label('email'), validators=[Required(message='EMAIL_NOT_PROVIDED')])
    password = HiddenField(get_form_field_label('password'), validators=[password_required])
    remember = HiddenField(get_form_field_label('remember_me'))
    submit = SubmitField(get_form_field_label('login'))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.next.data:
            self.next.data = request.args.get('next', '')
        self.user = _datastore.get_user(request.form['email'])

    def validate(self):
        if not super(TFAForm, self).validate():
            return False
        if not self.user:
            return False
        
        user = self.user
        code = self.authentication_code.data

        if not user.verify_totp(code):
            self.authentication_code.errors.append(get_message('INVALID_AUTHENTICATION_CODE')[0])
            return False
        return True