...
import pkg_resources, base64, urllib, onetimepass
...
from .forms import ChangePasswordForm, ConfirmRegisterForm, \
    ...
    ResetPasswordForm, SendConfirmationForm, TFAForm
from .utils import config_value as cv
from .utils import _, get_config, hash_data, localize_callback, string_types, \
    url_for_security, verify_hash, generate_otp_secret
...
_default_config = {
    ...
    'FLASH_MESSAGES': True,
    'I18N_DOMAIN': Domain(
        pkg_resources.resource_filename('ff_flask_security', 'translations'),
    ...
    'TFA_VIEW': '/login',
    'TFA_OTP_APP_NAME': 'FLASK-SECURITY',
    'UNAUTHORIZED_VIEW': lambda: None,
    'FORGOT_PASSWORD_TEMPLATE': 'security/forgot_password.html',
    'LOGIN_USER_TEMPLATE': 'security/login_user.html',
    'TFA_TEMPLATE': 'security/tfa_login.html',
...
_default_messages = {
    ...
    'AUTHENTICATION_CODE_NOT_PROVIDED': (
        _('Please enter Authentication Code from the Google Authenticator app'), 'error'),
    'INVALID_AUTHENTICATION_CODE': (
        _('Invalid Authentication Code'), 'error'),
}
...
class UserMixin(BaseUserMixin):
    ...
    def get_totp_uri(self):
        ...
        name = urllib.parse.quote(cv('TFA_OTP_APP_NAME'), safe='')
        return 'otpauth://totp/{0}:{1}?secret={2}&issuer={3}' \
            .format(name, self.email, otp_secret, name)
