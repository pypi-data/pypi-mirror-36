...
from .utils import config_value, do_flash, get_message, \
    ...
    get_post_register_redirect, get_tfa_redirect, get_url, login_user, logout_user, \
    slash_url_suffix
...
def login():
    ...
    login_form_class = _security.login_form
    login_form = login_form_class(MultiDict(request.get_json())) if request.is_json else login_form_class(request.form)

    if login_form.validate_on_submit():
        if login_form.user.tfa == 'ga': # and state.tfaable: - Добавить это. Если в настройках приложения включена 2fa ('SECURITY_TFAABLE' = True)
            return tfa_view(login_form)
...
@anonymous_user_required
def tfa_view(login_form):
    """View function for 2FA login view"""

    tfa_form_class = _security.tfa_form
    tfa_form = tfa_form_class(MultiDict(request.get_json())) if request.is_json else tfa_form_class(request.form)
    
    if 'authentication_code' in request.form and tfa_form.validate_on_submit():
        print('valid', tfa_form.user)
        login_user(tfa_form.user, remember=tfa_form.remember.data)
        after_this_request(_commit)

        if not request.is_json:
            return redirect(get_post_login_redirect(tfa_form.next.data))

    if request.is_json:
        return _render_json(tfa_form, include_auth_token=True)
    # return redirect(get_url(tfa_view))
    return _security.render_template(config_value('TFA_TEMPLATE'),
                                    tfa_form=tfa_form,
                                     **_ctx('tfa_login'))
...
def create_blueprint(state, import_name):
    ...
    if state.tfaable:
        bp.route(state.tfa_view,
                 methods=['GET', 'POST'],
                 endpoint='tfa_view')(tfa_view)
