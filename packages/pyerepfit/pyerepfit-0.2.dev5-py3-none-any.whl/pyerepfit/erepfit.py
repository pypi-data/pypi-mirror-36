...
class Erepfit:
    ...
    def evaluate_elec_data(self):
        ...
        with open('evaluated.json', 'w') as f:
            ...
            print(js, file=f)
            print('Evaluated input file has been saved as evaluated.json')
    ...
    def write_bond_statistic_graphs(self, filename='bond_length.pdf'):
        ...
        for uuid, dist_matrix in self.builder.dist_matrix_uuids.items():
            ...
            for index_pair, dist_vector in dist_matrix.items():
                ...
                if elem1 > elem2 :
                    continue
                distances = [x[0] for x in dist_vector if x[0] <= self.final_repulsive_potentials['{}-{}'.format(elem1, elem2)].knots[-1]]
