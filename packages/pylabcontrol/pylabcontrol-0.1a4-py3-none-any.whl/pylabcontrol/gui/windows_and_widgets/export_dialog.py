...
class ExportDialog(QDialog, Ui_Dialog):
    ...
    def __init__(self):
        ...
        print(importlib.util.find_spec('pylabcontrol').submodule_search_locations[0] + '\\scripts\\')
        print(os.path.exists(importlib.util.find_spec('pylabcontrol').submodule_search_locations[0] + '\\scripts\\'))

        pylab_spec = importlib.util.find_spec('pylabcontrol')
        #if pylabcontrol in the python path, finds the path to its base folder, navigates to scripts, and uses that
        #as the default location
        if pylab_spec:
            self.source_path.setText(pylab_spec.submodule_search_locations[0] + '\\scripts\\')
