...
def connect(dock_serial_number, baudrate=115200, modbus_id=247):
    ...
    # first open it with a short time out, so we don't hang for long if nothin is attached
    pod = Pod(portname, baudrate=baudrate, modbus_id=modbus_id, timeout=0.1)
    try:
...
def connectR(dock_serial_number, baudrate=115200, modbus_id=247):
    portname = dock_port(dock_serial_number)[:-1]+'B'
    # first open it with a short time out, so we don't hang for long if nothin is attached
...
def connectL(dock_serial_number, baudrate=115200, modbus_id=247):
    portname = dock_port(dock_serial_number)[:-1]+'A'
    # first open it with a short time out, so we don't hang for long if nothin is attached
    pod = Pod(portname, modbus_id=modbus_id, baudrate=baudrate, timeout=0.1)
    try:
        # if we can read the first two bytes without throwing an exception, we're good
