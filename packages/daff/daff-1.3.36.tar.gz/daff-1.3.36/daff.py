...
class Coopy(object):
	...
	def run(self,args,io = None):
		if (io is None):
			io = TableIO()
		if (io is None):
			haxe_Log.trace("No system interface available",_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 724, 'className': "Coopy", 'methodName': "run"}))
			return 1
		self.init()
		self.io = io
		more = True
		...
		while (more):
			...
			while ((_g1 < _g)):
				...
				if (tag == "--output"):
					...
					if (pos22 < 0):
						pos22 = 0
					res22 = args[pos22:(pos22 + 2)]
					del args[pos22:(pos22 + 2)]
					res22
					break
		...
		try:
			...
			if (cmd1 == "git"):
				...
				if (pos29 < 0):
					pos29 = 0
				res29 = args[pos29:(pos29 + len)]
				del args[pos29:(pos29 + len)]
				types = res29
			...
			if git:
				...
				if (pos30 < 0):
					pos30 = 0
				res30 = args[pos30:(pos30 + ct)]
				del args[pos30:(pos30 + ct)]
				git_args = res30
				...
				if (pos31 < 0):
					pos31 = 0
				res31 = args[pos31:(pos31 + len1)]
				del args[pos31:(pos31 + len1)]
				res31
				offset = 0
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 1173, 'className': "Coopy", 'methodName': "show"}))
