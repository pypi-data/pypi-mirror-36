...
set i_mpi_ofi_library_internal=""
...
while ($#argv > 0)
    switch ($argv[1])
        ...
        case -h:
            ...
            echo "      release_mt      "
            echo ""
            echo "-ofi_internal specifies whether to use libfabric from the Intel MPI Library."
            echo ""
...
switch ($i_mpi_ofi_library_internal)
    ...
    case enable:
        ...
        breaksw
    default:
        ${I_MPI_ROOT}/intel64/libfabric/bin/fi_info --version >&/dev/null
        if ($status != 0) then
            setenv PATH ${I_MPI_ROOT}/intel64/libfabric/bin:${PATH}
            setenv LD_LIBRARY_PATH ${I_MPI_ROOT}/intel64/libfabric/lib:${LD_LIBRARY_PATH}
            if !($?LIBRARY_PATH) then
