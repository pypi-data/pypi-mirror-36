...
class AsmStatement(object):
    ...
    def __init__(self, instr, stack, evmcode):
        ...
        if self.stack:
            self.args = tuple(zip(instr.args, [get_z3_value_hex(self.stack[len(self.stack) - 1 - i]) for i in range(len(instr.args))]))
            self.args_as_dict = dict(self.args)

        if self.stack_next:
            self.returns = tuple(zip(instr.returns, [get_z3_value_hex(self.stack_next[len(self.stack_next) - 1 - i]) for i in range(len(instr.returns))]))
        else:
            self.returns = tuple(zip(instr.returns, ['?'] * len(instr.returns)))
