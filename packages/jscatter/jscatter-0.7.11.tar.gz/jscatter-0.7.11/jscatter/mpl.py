...
def contourImage(x,y=None,z=None,xdim=None,levels=8,fontsize=8,colorMap='jet',lineMap=None):
    ...
    borders=[np.min(x),np.max(x),np.min(y),np.max(y)]
    im = ax.imshow(z,cmap=cmap,extent=borders) # drawing the function
    # adding the Contour lines with labels
    try:
        im.cset = pyplot.contour(z,levels,linewidths=1,cmap=lmap,extent=borders)
        im.labels=pyplot.clabel(im.cset,inline=True,fmt='%1.1f',fontsize=10)
    except:
        im.cset = pyplot.contour(z,levels, linewidths=1,extent=borders)
