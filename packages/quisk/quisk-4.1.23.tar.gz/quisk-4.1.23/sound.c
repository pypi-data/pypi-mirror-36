...
{  // called in an infinite loop by the main program
	...
	if (rxMode == DGT_U || rxMode == DGT_L || rxMode == DGT_IQ || rxMode == DGT_FM)
	...
	if (rxMode == DGT_U || rxMode == DGT_L || rxMode == DGT_IQ || rxMode == DGT_FM) {		// Discard previous samples and use digital samples
