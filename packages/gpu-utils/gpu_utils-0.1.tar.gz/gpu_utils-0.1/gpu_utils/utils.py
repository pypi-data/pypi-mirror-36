...
GPU = namedtuple('GPU', ['num', 'mem_used', 'mem_free', 'util_used', 'util_free'])  # mem in MiB, util as % used
...
def get_gpus(skip_gpus: Sequence[int]=(), smi_info: str='', keep_all: bool=False) -> List[GPU]:
    """
...
def kill_interrupted_processes(sudo: bool=False) -> None:
    ...
    for gpu_id in range(n_gpus):
        stdout = run(sudo + f"lsof -t /dev/nvidia{gpu_id}".split(" "), stdout=PIPE).stdout
