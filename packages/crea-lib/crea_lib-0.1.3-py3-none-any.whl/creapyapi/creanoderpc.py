...
from creapybase.chains import known_chains
import logging
import warnings
...
class CreaNodeRPC(GrapheneWebsocketRPC):
    """ This class allows to call API methods synchronously, without
        ...
        * history
        ...
        .. code-block:: python
            ...
            ws = CreaNodeRPC("ws://10.0.0.16:1776")
    ...
    def account_history(self, account, first=99999999999,
        ...
        warnings.warn(
            "The block_stream() call has been moved to `crea.account.Account.rawhistory()`",
            DeprecationWarning
        )
        from creapy.account import Account
        return Account(account, crea_instance=self.crea).rawhistory(
    ...
    def block_stream(self, start=None, stop=None, mode="irreversible"):
        warnings.warn(
            "The block_stream() call has been moved to `crea.blockchain.Blockchain.blocks()`",
            DeprecationWarning
        )
    ...
    def stream(self, opNames, *args, **kwargs):
        warnings.warn(
            "The stream() call has been moved to `crea.blockchain.Blockchain.stream()`",
            DeprecationWarning
        )
        from creapy.blockchain import Blockchain
    ...
    def list_accounts(self, start=None, step=1000, limit=None, **kwargs):
        warnings.warn(
            "The list_accounts() call has been moved to `crea.blockchain.Blockchain.get_all_accounts()`",
            DeprecationWarning
        )
        from creapy.blockchain import Blockchain
    ...
    def rpcexec(self, payload):
        """ Execute a call by sending the payload.
            ...
            In here, we mostly deal with Crea specific error handling
        ...
        try:
            ...
            return super(CreaNodeRPC, self).rpcexec(payload)
            ...
            elif msg == "Voting weight is too small, please accumulate more voting power or CREA power.":
    ...
    def __getattr__(self, name):
        """ Map all methods to RPC calls and pass through the arguments.
            It makes use of the GrapheneRPC library.
        """
        return super(CreaNodeRPC, self).__getattr__(name)
