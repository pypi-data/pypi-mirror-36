...
class Account(dict):
    ...
    def refresh(self):
        account = self.crea.rpc.get_account(self.name)
        if not account:
    ...
    def converter(self):
        if not self._converter:
            self._converter = Converter(self.crea)
    ...
    def get_balances(self, as_float=False):
        my_account_balances = self.crea.get_balances(self.name)
        balance = {
            "CREA": my_account_balances["balance"],
            "CBD": my_account_balances["cbd_balance"],
            "VESTS": my_account_balances["vesting_shares"],
            "SAVINGS_CREA": my_account_balances["savings_balance"],
            "SAVINGS_CBD": my_account_balances["savings_cbd_balance"]
        }
        if as_float:
    ...
    def _get_followers(self, direction="follower", last_user=""):
        if direction == "follower":
            followers = self.crea.rpc.get_followers(self.name, last_user, "blog", 100, api="follow")
        elif direction == "following":
            followers = self.crea.rpc.get_following(self.name, last_user, "blog", 100, api="follow")
    ...
    def virtual_op_count(self):
        try:
            last_item = self.crea.rpc.get_account_history(self.name, -1, 0)[0][0]
        except IndexError:
            return 0
        else:
            return last_item
    ...
    def get_account_votes(self):
        return self.crea.rpc.get_account_votes(self.name)

    def get_withdraw_routes(self):
        return self.crea.rpc.get_withdraw_routes(self.name, 'all')

    def get_conversion_requests(self):
        return self.crea.rpc.get_conversion_requests(self.name)
    ...
    def history(self, filter_by=None, start=0):
        ...
        while True:
            if i == start_index:
                ...
                limit = batch_size - 1
            history = self.crea.rpc.get_account_history(self.name, i, limit)
            for item in history:
                index = item[0]
                if index >= max_index:
                    return
    ...
    def rawhistory(
        ...
        while first > 0:
            # RPC call
            txs = self.crea.rpc.get_account_history(self.name, first, _limit)
            for i in txs[::-1]:
