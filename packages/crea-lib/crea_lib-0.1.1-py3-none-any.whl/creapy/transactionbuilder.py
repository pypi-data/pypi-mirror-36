...
from creapybase import transactions, operations
from creapybase.account import PrivateKey
from creapybase.operations import Operation
from creapybase.signedtransactions import Signed_Transaction
...
class TransactionBuilder(dict):
    ...
    def __init__(self, tx={}, crea_instance=None):
    ...
    def appendSigner(self, account, permission):
        ...
        def fetchkeys(account, level=0):
            ...
            for authority in account[permission]["key_auths"]:
                wif = self.crea.wallet.getPrivateKeyForPublicKey(authority[0])
                if wif:
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(authority[0], crea_instance=self.crea)
    ...
    def constructTx(self):
        ...
        expiration = transactions.formatTimeFromNow(self.crea.expiration)
        ref_block_num, ref_block_prefix = transactions.getBlockParams(self.crea.rpc)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.crea.rpc:
            operations.default_prefix = self.crea.rpc.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.crea.rpc.chain_params)
    ...
    def verify_authority(self):
        ...
        """
        try:
            if not self.crea.rpc.verify_authority(self.json()):
    ...
    def broadcast(self):
        ...
        try:
            self.crea.rpc.broadcast_transaction(self.json(), api="network_broadcast")
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        accountObj = Account(account, crea_instance=self.crea)
        ...
        for account_auth in authority["account_auths"]:
            account_auth_account = Account(account_auth[0], crea_instance=self.crea)
        ...
        self["blockchain"] = self.crea.rpc.chain_params
    ...
    def appendMissingSignatures(self, wifs):
        ...
        for pub in missing_signatures:
            wif = self.crea.wallet.getPrivateKeyForPublicKey(pub)
            if wif:
                self.appendWif(wif)
