...
simulatorDescription=("Spectre", "Cadence Spectre")
"""
...
def save_property(devices, params, indices=None):
	...
	if indices is None:
		...
		raise PyOpusError(DbgMsg("SPSI", "Device properties do not have indices in SPECTRE."))
	...
	*name* gives the name of the instance whose *parameter* is swept. 
	...
	if (
		...
		if sweep=='lin':
			...
			raise PyOpusError(DbgMsg("SPSI", "SPECTRE supports only 'lin' and 'dec' sweep in a PyOPUS common sweep."))
	...
	* ``'dec'`` - logarithmic sweep with points per decade 
	...
	if (
		...
		if sweep=='lin':
			...
			raise PyOpusError(DbgMsg("SPSI", "SPECTRE supports only 'lin' and 'dec' in a PyOPUS common sweep"))
...
def an_noise(start=None, stop=None, sweep=None, points=None, input=None, outp=None, outn=None, ptsSum=1, **kwargs):
	...
	if (
		...
		if outp is None:
			raise PyOpusError(DbgMsg("SPSI", "Need at least outp for noise analysis"))
...
class Spectre(Simulator):
	...
	* ``param`` - a dictionary containing the members of the ``params`` entry 
	...
	def __init__(self, binary=None, args=[], debug=0, saveSplit=False):
		...
		# Index of job group whose results were loaded last
		self.loadedResultsGroup=None
		# Loaded results for the loadedResultsGroup
		self.loadedResults=None
		# Converter from jobIndex to result index in group
		job2resultsIndex={}
	...
	def findSimulator(cls):
		"""
		Finds the simulator. Location is defined by the SPECTRE 
		...
		if 'SPECTRE_BINARY' in environ:
			...
			spectrebinary=shutil.which("spectre")
		
		# Verify binary
		if spectrebinary is None:
			return None
		elif os.path.isfile(spectrebinary):
			return spectrebinary
		else:
			return None
	...
	def _compile(self):
		...
		* constructs the binary name for invoking the simulator
		...
		if self.binary is None:
			self.binary=Spectre.findSimulator()
		...
		*saveDirectives* list. *variables* is a dictionary of extra 
		...
		for saveDirective in saveDirectives:
			...
			if type(saveList) is not list: 
				raise PyOpusError(DbgMsg("SPSI", "Save directives must evaluate to a list of strings."))
	...
	def writeFile(self, i):
		...
		with open(fileName, 'w') as f:
			...
			for j in jobGroup:
				...
				for (param, value) in analysisParams.items():
					if value is True:
						strValue='1'
					elif value is False:
						strValue='0'
	...
	def readResults(self, jobIndex, runOK=None):
		...
		*runOK* specifies the status returned by the :meth:`runJobGroup` 
		...
		Returns an object of the class :class:`SpiceOpusSimulationResults`. 
		...
		if runOK:
			# Because the results of all jobs in a job group are in one file, 
			# we convert jobIndex to groupIndex
			groupIndex=self.jobGroupIndex[jobIndex]
			
			# Do we already have these results loaded
			if self.loadedResultsGroup!=groupIndex:
				# No, load them
				# Get the list of jobs in this group
				jobsInGroup=self.jobSequence[groupIndex]
				# Build converter to job index in results list
				self.job2resultsIndex={}
				for ii in range(len(jobsInGroup)):
					self.job2resultsIndex[jobsInGroup[ii]]=ii
				
				fileName=self.simulatorID+"_group"+str(groupIndex)+'.raw'
				
				if self.debug>1: 
					DbgMsgOut("SPSI", "Reading results from '"+fileName+"'.")
				
				# Load group of results
				try:
					# Load the file, reverse bytes in double values (those Cadence folks and endianness don't mix well)
					self.loadedResults=raw_read(
						fileName, 
						reverse=1
					)
					self.loadedResultsGroup=groupIndex
				except:
					self.loadedResults=None
					self.loadedResultsGroup=None
		else:
		...
		# Now pick the job results out of the list
		job=self.jobList[jobIndex]
		if self.loadedResults is None:
			# Failed to load results
			retVal=None
		else:
			# Get consecutive index of requested job
			ii=self.job2resultsIndex[jobIndex]
			
			# Index out of bounds
			if ii<0 or ii>=len(self.loadedResults):
				retVal=None
			else:
				params={}
				params.update(self.inputParameters)
				params.update(job['params'])
				retVal=SpectreSimulationResults(
					self.loadedResults[ii], params=params, variables=job['variables']
				)
				
		if self.debug>0:
			if retVal is not None:
				DbgMsgOut("SPSI", "Job '"+str(job['name'])+"' OK.")
			else:
				DbgMsgOut("SPSI", "Job '"+str(job['name'])+"' FAILED.")
...
class SpectreSimulationResults(SimulationResults):
	"""
	Objects of this class hold Spectre simulation results. 
	"""
	...
	def vectorNames(self):
		...
		"""
		return list(self.rawData[0].keys())
		
	def vector(self, name):
		"""
		Returns vector named *name*. 
		"""
		resGrp=self.rawData
		if name in resGrp[0]:
	...
	def scale(self, vecName=None):
		"""
		If *vecName* is specified returns the scale corresponding to 
		the specified vector. Usually this is the default scale. 
		
		If *vecName* is not specified the default scale is returned. 
		"""
		name=self.scaleName(vecName)
		return self.vector(name)
	
	def v(self, node1, node2=None):
		"""
		Retrieves the voltage corresponding to *node1* (voltage between 
		nodes *node1* and *node2* if *node2* is also given). 
		"""
		if node2 is None:
			return self.vector(node1)
		else:
			return self.vector(node1)-self.vector(node2)
	
	def i(self, name):
		"""
		Retrieves the current flowing through instance *name*. 
		"""
		return self.vector(name+":p")
	
	def p(self, name, parameter, index=None):
		"""
		Retrieves the *index*-th component of property named *parameter* 
		belonging to  instance named *name*. *index* must always be 
		``None`` because Spectre does not support vector properties. 
		...
		if index is None:
			return self.vector(name+':'+parameter)
		else:
			raise PyOpusError(DbgMsg("SPSI", "Device properties do not have indices in SPECTRE."))
	
	def ns(self, reference, name=None, contrib=None):
		"""
		...
		*reference*). 
		...
		In the simulator output file partial noise spectra are stored as 
		squared noise (V^2/Hz or A^2/Hz). Total noise spectra are stored 
		as V/sqrt(Hz) or A/sqrt(Hz). 
		This function always returns squared noise with V^2/Hz or A^2/Hz 
		as unit. 
		...
		if name is None:
			...
			if reference=='input':
				spec=self.vector('in')**2
			elif reference=='output':
				spec=self.vector('out')**2
			elif reference=='gain':
				# gain is out/in
				spec=self.vector('gain')
			else:
				raise PyOpusError("Bad noise reference.")
		else:
			# Partial spectrum
			if reference=='input':
				A=(self.vector('out')/self.vector('in'))**2
			elif reference=='output':
				A=1.0
			else:
				raise PyOpusError("Bad noise reference.")
			...
			if contrib is None:
				spec=self.vector(str(name)+".total")/A
			else:
				spec=self.vector(str(name)+"."+str(contrib))/A
