...
class Quadratic(GlobalProblem):
	...
	def __init__(self, n=30):
		...
		if self.n<1:
			raise TestFunctionError(DbgMsg("GLBC", "Bad n."))
...
class Shekel(GlobalProblem):
	...
	def __init__(self, n=4, m=5):
		...
		if self.m!=5 and self.m!=7 and self.m!=10:
			raise TestFunctionError(DbgMsg("GLBC", "Bad m."))
...
class Bohachevsky(GlobalProblem):
	...
	def __init__(self, j=1):
		...
		if j!=1 and j!=2 and j!=3:
			raise TestFunctionError(DbgMsg("GLBC", "Bad j."))
	...
	def __init__(self, n=4, b=None):
		...
		if b is None:
			if self.n==4:
				...
				raise TestFunctionError(DbgMsg("GLBC", "Need vector b of length n."))
		elif len(b.shape)!=1 or b.shape[0]!=n:
			raise TestFunctionError(DbgMsg("GLBC", "Vector b must have n components."))
		else:
			self.b=b
...
class Zakharov(GlobalProblem):
	...
	def __init__(self, n=10, K=2):
		...
		if self.K<2:
			raise TestFunctionError(DbgMsg("GLBC", "Bad K."))
