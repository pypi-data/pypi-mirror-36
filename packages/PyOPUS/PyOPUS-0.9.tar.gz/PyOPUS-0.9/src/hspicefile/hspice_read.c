					 ...
					 int header)
{
	...
	if (debugMode>=2) {
		fprintf(debugFile, "Got block trailer, itemsize=%d, count=%d, bytes=%d,\n", sizeof(int), 1, sizeof(int)*1);
	}
					...
					int *bufOffset)
{
	...
	if (debugMode>=2) {
		fprintf(debugFile, "Reading header block @0x%x\n", ftell(f));
	}
				  ...
				  int *rawDataOffset)
{
	...
	if (debugMode>=2) {
		fprintf(debugFile, "Reading data block @0x%x\n", ftell(f));
	}
...
static PyObject *HSpiceRead(PyObject *self, PyObject *args)
{
	...
	date = PyUnicode_InternFromString(&buf[dateStartPosition]);	// Get creation date.
	if(date == NULL)
	{
		if(debugMode)
