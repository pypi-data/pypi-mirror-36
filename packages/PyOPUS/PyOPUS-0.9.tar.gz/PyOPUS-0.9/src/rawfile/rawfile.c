...
static PyMethodDef module_methods[] = {
	{"raw_write", raw_write, METH_VARARGS, "Write SPICE3 raw file.\n"},
	{"raw_read", raw_read, METH_VARARGS, "Read SPICE3 raw file.\n"}, 
	{NULL, NULL, 0, NULL}     // Marks the end of this structure
};

static struct PyModuleDef module = {
    PyModuleDef_HEAD_INIT,
    "_rawfile", /* name of module */
    "SPICE3 raw file import/export module.\n", /* module documentation, may be NULL */
...
PyMODINIT_FUNC PyInit__rawfile()  {
...
__inline npy_intp validateKeyData(PyObject *key, PyObject *value, const char **keyString) {
	...
	*keyString=PyUnicode_AsUTF8(key); 
...
static PyObject *raw_write(PyObject *self, PyObject *args) {
	...
	// Parsing with s specifier accepts a unicode string and returns char * which is UTF-8 encoded unicode
	// Returned objects need not be freed
	...
	if (!status) {
		...
		while (1) {
			...
			if (didDflVec && !strcmp(PyUnicode_AsUTF8(key), PyUnicode_AsUTF8(dflVecKey))) {
				continue;
			}
				
			// Dump it
			fprintf(f, "\t%d\t%s\t%s", i, PyUnicode_AsUTF8(key), "notype");
			
			// Check if this vector has a scale vector (scales dictionary)
			scaleObj=PyDict_GetItem(scales, key);
			if (scaleObj && PyUnicode_Check(scaleObj)) {
				scaleName=PyUnicode_AsUTF8(scaleObj);
...
static PyObject *raw_read(PyObject *self, PyObject *args) {
	...
	while (!status) {
		...
		if (matchPrefix("title:", &p, 1)) {
			...
			skipSpaces(&p);
			plotTitle=PyUnicode_InternFromString(p);
			...
			skipSpaces(&p);
			plotDate=PyUnicode_InternFromString(p);
			...
			skipSpaces(&p);
			plotName=PyUnicode_InternFromString(p);
			...
			for(i=0;i<count;i++) {
				...
				*p2=0;
				...
				while (*pp) {
					...
					if (matchPrefix("scale=", &pp, 1)) {
						...
						*p4=0;
						...
						tmpStr=PyUnicode_InternFromString(p3);
						if (!tmpStr) {
							// Out of memory
							status=1;
							if (debugMode) {
				...
				if (i==0) {
					dflScale=PyUnicode_InternFromString(p1);
					if (!dflScale) {
						// Out of memory
						status=1;
						if (debugMode) {
		...
		if (storeResult) {
			...
			if (!status) {
				...
				if (!plotTitle)
					plotTitle=PyUnicode_InternFromString("");
				if (!plotDate)
					plotDate=PyUnicode_InternFromString("");
