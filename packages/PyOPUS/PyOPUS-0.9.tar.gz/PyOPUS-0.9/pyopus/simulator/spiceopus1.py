...
simulatorDescription=("SpiceOpus", "Spice Opus")
"""
...
def an_dc(start, stop, sweep, points, name, parameter, index=None):
	...
	*name* gives the name of the instance whose *parameter* is swept. If the 
	...
	if index is None:
		if name is None:
			if parameter=='temperature':
				...
				raise PyOpusError(DbgMsg("SOSI", "Bad sweep parameter."))
		else:
	...
	if sweep == 'lin':
		...
		raise PyOpusError(DbgMsg("SOSI", "Bad sweep type."))
...
class SpiceOpus(Simulator):
	...
	* ``param`` - a dictionary containing the members of the ``params`` entry 
	...
	def findSimulator(cls):
		"""
		Finds the simulator. Location is defined by the OPUSHOME 
		...
		if 'OPUSHOME' in environ:
			...
			if platform.system()=='Windows':
				opusbinary=shutil.which("spiceopus.exe")
			else:
				opusbinary=shutil.which("spiceopus.bin")
		
		# Verify binary
		if opusbinary is None:
			return None
		elif os.path.isfile(opusbinary):
			return opusbinary
		else:
			return None
	...
	def _compile(self):
		...
		* constructs the binary name for invoking the simulator
		...
		if self.binary is None:
			self.binary=SpiceOpus.findSimulator()
			
			# Last resort - use local folder
			if self.binary is None: 
				if platform.system()=='Windows':
					self.binary=os.path.join(".", 'spiceopus.exe')
				else:
					self.binary=os.path.join(".", 'spiceopus.bin')
		...
		*saveDirectives* list. *variables* is a dictionary of extra 
		...
		for saveDirective in saveDirectives:
			...
			if type(saveList) is not list: 
				raise PyOpusError(DbgMsg("SOSI", "Save directives must evaluate to a list of strings."))
...
#            bla.close()
	...
	def cleanupResults(self, i): 
		...
		for j in jobGroup:
			...
			try:
#                bla = open("log.txt",'a')
#                bla.write("\nCleaning: " + str(self.simulatorID+"_job"+str(j)+'_'+job['name']+'.raw'))
#                bla.close()
	...
	def runFile(self, fileName):
		...
		try:
			...
			# Send quit command
			try:
				self.messages=p.communicate(bytes('set noaskquit\nquit\n', 'utf-8'),timeout=self.timeOut)
			except subprocess.TimeoutExpired:
				print("Timeout fail")
				p.kill()
				self.messages=p.communicate()
				spawnOK = False
	...
	def readResults(self, jobIndex, runOK=None):
		...
		*runOK* specifies the status returned by the :meth:`runJobGroup` 
		...
		if runOK:
			fileName=self.simulatorID+"_job"+str(jobIndex)+'_'+job['name']+'.raw'
			
			if self.debug>1: 
				DbgMsgOut("SOSI", "Reading results from '"+fileName+"'.")
			...
			try:
				...
				retVal=SpiceOpusSimulationResults(
		...
		if self.debug>0:
			if retVal is not None:
				DbgMsgOut("SOSI", "Job '"+str(job['name'])+"' OK")
			else:
				DbgMsgOut("SOSI", "Job '"+str(job['name'])+"' FAILED")
		...
		for job in self.jobList:
			for option in job['options'].keys():
				if option.lower()=='temp':
					raise PyOpusError(DbgMsg("SOSI", "TEMP option is not allowed. Use temperature parameter."))
			if 'temperature' in job['params']: 
...
class SpiceOpusSimulationResults(SimulationResults):
	"""
	...
	def i(self, name, resIndex=0):
		...
		*resIndex*-th plot. 
		...
		"""
		return self.vector(name+"#branch", resIndex=resIndex)
	...
	def p(self, name, parameter, index=None, resIndex=0):
		...
		*index* can be ommitted. The property is retrieved from *resIndex*-th 
		...
		"""
		if index is None:
			return self.vector('@'+name+'['+parameter+']', resIndex=resIndex)
		else:
			return self.vector('@'+name+'['+parameter+']['+str(index)+']', resIndex=resIndex)
	...
	def ns(self, reference, name=None, contrib=None, resIndex=0):
		...
		*reference*). 
		...
		if name is None:
			...
			if contrib is None:
				spec=self.vector("onoise_"+str(name), resIndex=resIndex)/A
			else:
				spec=self.vector("onoise_"+str(name)+"_"+str(contrib), resIndex=resIndex)/A
