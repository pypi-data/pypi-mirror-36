...
Conversion between list form and dictionary form of parameter description 
			...
			for depend in measure['depends']:
				...
				for depMeasureName in dependsOnMeasures: 
					...
					if 'analysis' not in measures[depMeasureName]:
						raise PyOpusError(DbgMsgOut("MI", "Dependent measures can't depend on dependent measures."))
...
def dictParamDesc(inputList, nameList, indices=None):
	...
	*nameList* is a list of parameter names corresponding to individual 
	...
	for ii in ndx:
		name=nameList[ii]
		output[name]={}
		for prop,propList in inputList.items():
			val=propList[ii]
			if val is not None:
