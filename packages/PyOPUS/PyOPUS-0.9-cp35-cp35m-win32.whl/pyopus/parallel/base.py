...
def translateToAbstractPath(path):
	"""
...
def translateToActualPath(index, relPath):
	"""
...
class VirtualMachine(object):
	...
	*mirrorMap* is a dictionary specifying filesystem objects (files and 
	...
	def cleanupEnvironment(self, taskStorage, forceCleanup=False):
		"""
...
if 'PARALLEL_MIRRORED_STORAGE' in environ:
	...
	if platform.platform().startswith('Windows'):
		...
		ParallelMirroredStorage=[os.getcwd()]
	else:
		# Reasonable default for Linux is the user's home
		ParallelMirroredStorage=[os.path.expanduser("~")]
