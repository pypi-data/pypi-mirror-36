	...
	* :attr:`fmin`    - best known minimum
	...
	def __init__(self, name=None, number=None):
		if number is None and name is None:
			raise TestFunctionError(DbgMsg("LVNS", "Must specify name or number."))
			
		if number is not None and name is not None:
			raise TestFunctionError(DbgMsg("LVNS", "Name and number cannot be specified at the same time."))
		
		if number is not None:
			self.number=number
			if number<0 or number>24:
				raise TestFunctionError(DbgMsg("LVNS", "Bad problem number."))
			self.name=self.names[number]
		
		if name is not None:
			if name not in self.functionNumber:
				raise TestFunctionError(DbgMsg("LVNS", "Function not found."))
