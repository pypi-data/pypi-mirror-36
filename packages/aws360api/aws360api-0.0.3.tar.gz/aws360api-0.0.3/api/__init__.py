...
if "FLASK_ENV" in os.environ:
    if os.environ["FLASK_ENV"] == 'prod':
        app.config.from_object('api.config.prod.ProductionConfig')
    else:
...
from api.scrapper.controllers import scrapper
from api.badger.controllers import badger
from api.heartbeat.controllers import heartbeat
from api.info.controllers import info
...
def index(error=None):
    return redirect(url_for('badger.generate'))
