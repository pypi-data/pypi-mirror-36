...
from dpaygoapi.node import Nodes
...
class Blockchain(object):
    ...
    def blocks(self, start=None, stop=None, max_batch_size=None, threading=False, thread_num=8, only_ops=False, only_virtual_ops=False):
        ...
        if threading:
            ...
            for i in range(thread_num - 1):
                dpay_instance.append(stm.DPay(node=nodelist,
                                                num_retries=self.dpay.rpc.num_retries,
            ...
            if threading and not head_block_reached:
                ...
                for blocknum in range(start, head_block + 1, thread_num):
                    ...
                    if FUTURES_MODULE is not None:
                        futures = []
                    block_num_list = []
                    # freeze = self.dpay.rpc.nodes.freeze_current_node
                    num_retries = self.dpay.rpc.nodes.num_retries
                    # self.dpay.rpc.nodes.freeze_current_node = True
                    ...
                    # self.dpay.rpc.nodes.freeze_current_node = freeze
                ...
                for blocknumblock in range(start, head_block + 1, batches):
                    ...
                    for blocknum in range(blocknumblock, blocknumblock + batches - 1):
                        if only_virtual_ops:
                            if self.dpay.rpc.get_use_appbase():
                                # self.dpay.rpc.get_ops_in_block({"block_num": blocknum, 'only_virtual': only_virtual_ops}, api="account_history", add_to_queue=True)
                    ...
                    if latest_block <= head_block:
                        if only_virtual_ops:
                            if self.dpay.rpc.get_use_appbase():
                                # self.dpay.rpc.get_ops_in_block({"block_num": blocknum, 'only_virtual': only_virtual_ops}, api="account_history", add_to_queue=False)
    ...
    def ops_statistics(self, start, stop=None, add_to_ops_stat=None, with_virtual_ops=True, verbose=False):
        ...
        if add_to_ops_stat is None:
            ...
            ops_stat = dpaygobase.operationids.operations.copy()
            for key in ops_stat:
                ops_stat[key] = 0
        else:
    ...
    def get_all_accounts(self, start='', stop='', steps=1e3, limit=-1, **kwargs):
        ...
        if not self.dpay.is_connected():
            raise OfflineHasNoRPCException("No RPC available in offline mode!")
        if self.dpay.rpc.get_use_appbase() and start == "":
            lastname = None
        else:
            lastname = start
