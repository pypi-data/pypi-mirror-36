...
from dpaygoapi.exceptions import ApiNotSupported, MissingRequiredActiveAuthority
...
class Account(BlockchainObject):
    ...
    def refresh(self):
        ...
        if self.dpay.rpc.get_use_appbase():
                account = self.dpay.rpc.find_accounts({'accounts': [self.identifier]}, api="database")
        else:
            if self.full:
                account = self.dpay.rpc.get_accounts(
                    [self.identifier])
            else:
                account = self.dpay.rpc.lookup_account_names(
                    [self.identifier])
        if self.dpay.rpc.get_use_appbase() and "accounts" in account:
        ...
        # self.dpay.refresh_data()

        super(Account, self).__init__(account, id_item="name", lazy=self.lazy, full=self.full, dpay_instance=self.dpay)

    def _parse_json_data(self, account):
        parse_int = [
            "bbd_seconds", "savings_bbd_seconds", "average_bandwidth", "lifetime_bandwidth", "lifetime_market_bandwidth", "reputation",
        ]
        for p in parse_int:
        ...
        parse_times = [
            ...
            "last_account_recovery", "last_vote_time", "bbd_seconds_last_update", "bbd_last_interest_payment",
            "savings_bbd_seconds_last_update", "savings_bbd_last_interest_payment", "next_vesting_withdrawal",
        ...
        for p in amounts:
            if p in account and isinstance(account.get(p), (string_types, list, dict)):
                account[p] = Amount(account[p], dpay_instance=self.dpay)
        return account

    def json(self):
        output = self.copy()
        parse_int = [
            "bbd_seconds", "savings_bbd_seconds",
        ]
    ...
    def print_info(self, force_refresh=False, return_str=False, use_table=False, **kwargs):
        ...
        if use_table:
            ...
            t.add_row(["Vote Value", "%.2f $" % (self.get_voting_value_BBD())])
            t.add_row(["Last vote", "%s ago" % last_vote_time_str])
            t.add_row(["Full in ", "%s" % (self.get_recharge_time_str())])
            t.add_row(["BEX Power", "%.2f BEX" % (self.get_dpay_power())])
    ...
    def get_reputation(self):
        """ Returns the account reputation in the (dSite) normalized form
        """
        if not self.dpay.is_connected():
            return None
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            rep = self.dpay.rpc.get_account_reputations({'account_lower_bound': self["name"], 'limit': 1}, api="follow")['reputations']
            if len(rep) > 0:
    ...
    def get_vote_pct_for_bbd(self, bbd, voting_power=None, dpay_power=None, not_broadcasted_vote=True):
        ...
        if voting_power is None:
            voting_power = self.get_voting_power()
        if dpay_power is None:
            dpay_power = self.get_dpay_power()

        if isinstance(bbd, Amount):
            bbd = Amount(bbd, dpay_instance=self.dpay)
        elif isinstance(bbd, string_types):
            bbd = Amount(bbd, dpay_instance=self.dpay)
        else:
            bbd = Amount(bbd, 'BBD', dpay_instance=self.dpay)
        if bbd['symbol'] != 'BBD':
            raise AssertionError('Should input BBD, not any other asset!')

        vote_pct = self.dpay.rshares_to_vote_pct(self.dpay.bbd_to_rshares(bbd, not_broadcasted_vote=not_broadcasted_vote), voting_power=voting_power * 100, dpay_power=dpay_power)
        return vote_pct
    ...
    def get_recharge_timedelta(self, voting_power_goal=100):
        ...
        if missing_vp < 0:
            return 0
        recharge_seconds = missing_vp * 100 * DPAY_VOTE_REGENERATION_SECONDS / DPAY_100_PERCENT
    ...
    def get_feed(self, start_entry_id=0, limit=100, raw_data=False, short_entries=False, account=None):
        ...
        if not self.dpay.is_connected():
            return None
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if raw_data and short_entries and self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_feed_entries({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        elif raw_data and short_entries and not self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_feed_entries(account, start_entry_id, limit, api='follow')
            ]
        elif raw_data and self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_feed({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        elif raw_data and not self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_feed(account, start_entry_id, limit, api='follow')
            ]
        elif not raw_data and self.dpay.rpc.get_use_appbase():
            from .comment import Comment
            return [
                Comment(c['comment'], dpay_instance=self.dpay) for c in self.dpay.rpc.get_feed({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["feed"]
            ]
        else:
            from .comment import Comment
            return [
                Comment(c['comment'], dpay_instance=self.dpay) for c in self.dpay.rpc.get_feed(account, start_entry_id, limit, api='follow')
            ]
    ...
    def get_blog(self, start_entry_id=0, limit=100, raw_data=False, short_entries=False, account=None):
        ...
        if raw_data and short_entries and self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_blog_entries({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["blog"]
            ]
        elif raw_data and short_entries and not self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_blog_entries(account, start_entry_id, limit, api='follow')
            ]
        elif raw_data and self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_blog({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["blog"]
            ]
        elif raw_data and not self.dpay.rpc.get_use_appbase():
            return [
                c for c in self.dpay.rpc.get_blog(account, start_entry_id, limit, api='follow')
            ]
        elif not raw_data and self.dpay.rpc.get_use_appbase():
            from .comment import Comment
            return [
                Comment(c["comment"], dpay_instance=self.dpay) for c in self.dpay.rpc.get_blog({'account': account, 'start_entry_id': start_entry_id, 'limit': limit}, api='follow')["blog"]
            ]
        else:
            from .comment import Comment
            return [
                Comment(c["comment"], dpay_instance=self.dpay) for c in self.dpay.rpc.get_blog(account, start_entry_id, limit, api='follow')
            ]
    ...
    def get_blog_authors(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.get_blog_authors({'blog_account': account}, api='follow')['blog_authors']
        else:
            return self.dpay.rpc.get_blog_authors(account, api='follow')
    ...
    def get_follow_count(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.get_follow_count({'account': account}, api='follow')
        else:
            return self.dpay.rpc.get_follow_count(account, api='follow')
    ...
    def get_followers(self, raw_name_list=True, limit=100):
        ...
        if raw_name_list:
            ...
            return Accounts(name_list, dpay_instance=self.dpay)
        ...
        while limit_reached:
            ...
            if self.dpay.rpc.get_use_appbase():
                ...
                if direction == "follower":
                    followers = self.dpay.rpc.get_followers(query, api='follow')['followers']
                elif direction == "following":
                    followers = self.dpay.rpc.get_following(query, api='follow')['following']
            else:
                if direction == "follower":
                    followers = self.dpay.rpc.get_followers(self.name, last_user, what, limit, api='follow')
                elif direction == "following":
                    followers = self.dpay.rpc.get_following(self.name, last_user, what, limit, api='follow')
            if cnt == 0:
    ...
    def available_balances(self):
        """ List balances of an account. This call returns instances of
            :class:`dpaygo.amount.Amount`.
        """
        amount_list = ["balance", "bbd_balance", "vesting_shares"]
        available_amount = []
    ...
    def saving_balances(self):
        savings_amount = []
        amount_list = ["savings_balance", "savings_bbd_balance"]
        for amount in amount_list:
            if amount in self:
    ...
    def reward_balances(self):
        amount_list = ["reward_dpay_balance", "reward_bbd_balance", "reward_vesting_balance"]
        rewards_amount = []
        for amount in amount_list:
            if amount in self:
    ...
    def get_balance(self, balances, symbol):
        ...
        for b in balances:
            if b["symbol"] == symbol:
                return b
        return Amount(0, symbol, dpay_instance=self.dpay)
    ...
    def interest(self):
        ...
        interest_rate = self.dpay.get_dynamic_global_properties()[
            "bbd_interest_rate"] / 100  # percent
        interest_amount = (interest_rate / 100) * int(
            int(self["bbd_seconds"]) / (60 * 60 * 24 * 356)) * 10**-3
        return {
    ...
    def get_account_bandwidth(self, bandwidth_type=1, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            # return self.dpay.rpc.get_account_bandwidth({'account': account, 'type': 'post'}, api="witness")
            return self.dpay.rpc.get_account_bandwidth(account, bandwidth_type)
        else:
            return self.dpay.rpc.get_account_bandwidth(account, bandwidth_type)
    ...
    def get_bandwidth(self):
        ...
        reserve_ratio = self.dpay.get_reserve_ratio()
        ...
        total_vesting_shares = Amount(global_properties["total_vesting_shares"], dpay_instance=self.dpay).amount
        ...
        if not not self.dpay.is_connected() and self.dpay.rpc.get_use_appbase():
    ...
    def get_owner_history(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_owner_histories({'owner': account}, api="database")['owner_auths']
        else:
            return self.dpay.rpc.get_owner_history(account)
    ...
    def get_conversion_requests(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_bbd_conversion_requests({'account': account}, api="database")['requests']
        else:
            return self.dpay.rpc.get_conversion_requests(account)
    ...
    def get_vesting_delegations(self, start_account="", limit=100, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_vesting_delegations({'account': account}, api="database")['delegations']
        else:
            return self.dpay.rpc.get_vesting_delegations(account, start_account, limit)
    ...
    def get_withdraw_routes(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_withdraw_vesting_routes({'account': account, 'order': 'by_withdraw_route'}, api="database")['routes']
        else:
            return self.dpay.rpc.get_withdraw_routes(account, 'all')
    ...
    def get_savings_withdrawals(self, direction="from", account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_savings_withdrawals({'account': account}, api="database")['withdrawals']
        elif direction == "from":
            return self.dpay.rpc.get_savings_withdraw_from(account)
        elif direction == "to":
            return self.dpay.rpc.get_savings_withdraw_to(account)
    ...
    def get_recovery_request(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_account_recovery_requests({'account': account}, api="database")['requests']
        else:
            return self.dpay.rpc.get_recovery_request(account)
    ...
    def get_escrow(self, escrow_id=0, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_escrows({'from': account}, api="database")['escrows']
        else:
            return self.dpay.rpc.get_escrow(account, escrow_id)
    ...
    def verify_account_authority(self, keys, account=None):
        ...
        try:
            if self.dpay.rpc.get_use_appbase():
                return self.dpay.rpc.verify_account_authority({'account': account, 'signers': keys}, api="database")
            else:
                return self.dpay.rpc.verify_account_authority(account, keys)
    ...
    def get_tags_used_by_author(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.get_tags_used_by_author({'author': account}, api="tags")['tags']
        else:
            return self.dpay.rpc.get_tags_used_by_author(account, api="tags")
    ...
    def get_expiring_vesting_delegations(self, after=None, limit=1000, account=None):
        ...
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.find_vesting_delegation_expirations({'account': account}, api="database")['delegations']
        else:
            return self.dpay.rpc.get_expiring_vesting_delegations(account, formatTimeString(after), limit)
    ...
    def get_account_votes(self, account=None):
        ...
        self.dpay.rpc.set_next_node_on_empty_reply(False)
        if self.dpay.rpc.get_use_appbase():
            return self.dpay.rpc.get_account_votes(account, api="condenser")
        else:
            return self.dpay.rpc.get_account_votes(account)
    ...
    def get_vote(self, comment):
        ...
        c = Comment(comment, dpay_instance=self.dpay)
    ...
    def _get_account_history(self, account=None, start=-1, limit=0):
        ...
        if self.dpay.rpc.get_use_appbase():
            try:
                ret = self.dpay.rpc.get_account_history({'account': account["name"], 'start': start, 'limit': limit}, api="account_history")['history']
            except ApiNotSupported:
                ret = self.dpay.rpc.get_account_history(account["name"], start, limit, api="condenser")
        else:
            ret = self.dpay.rpc.get_account_history(account["name"], start, limit, api="database")
        return ret
    ...
    def update_account_metadata(self, metadata, account=None, **kwargs):
        ...
        return self.dpay.finalizeOp(op, account, "active", **kwargs)
    ...
    def approvewitness(self, witness, account=None, approve=True, **kwargs):
        ...
        #     witness = Witness(witness, dpay_instance=self)
    ...
    def update_memo_key(self, key, account=None, **kwargs):
        ...
        PublicKey(key, prefix=self.dpay.prefix)
    ...
    def transfer(self, to, amount, asset, memo="", account=None, **kwargs):
        ...
        if account is None:
            ...
            account = Account(account, dpay_instance=self.dpay)
        amount = Amount(amount, asset, dpay_instance=self.dpay)
        to = Account(to, dpay_instance=self.dpay)
        if memo and memo[0] == "#":
    ...
    def transfer_to_vesting(self, amount, to=None, account=None, **kwargs):
        ...
        if to is None:
            ...
            to = Account(to, dpay_instance=self.dpay)
        amount = self._check_amount(amount, "BEX")
    ...
    def convert(self, amount, account=None, request_id=None):
        ...
        if account is None:
            ...
            account = Account(account, dpay_instance=self.dpay)
        amount = self._check_amount(amount, "BBD")
        if request_id:
    ...
    def _check_amount(self, amount, symbol):
        if isinstance(amount, (float, integer_types)):
            amount = Amount(amount, symbol, dpay_instance=self.dpay)
        elif isinstance(amount, string_types) and amount.replace('.', '', 1).replace(',', '', 1).isdigit():
            amount = Amount(float(amount), symbol, dpay_instance=self.dpay)
        else:
    ...
    def claim_reward_balance(self,
        ...
        reward_dpay = self._check_amount(reward_dpay, "BEX")
        reward_bbd = self._check_amount(reward_bbd, "BBD")
        reward_vests = self._check_amount(reward_vests, "VESTS")

        if reward_dpay.amount == 0 and reward_bbd.amount == 0 and reward_vests.amount == 0:
            reward_dpay = account.balances["rewards"][0]
            reward_bbd = account.balances["rewards"][1]
            reward_vests = account.balances["rewards"][2]

        op = operations.Claim_reward_balance(
            **{
                "account": account["name"],
                "reward_dpay": reward_dpay,
                "reward_bbd": reward_bbd,
    ...
    def delegate_vesting_shares(self, to_account, vesting_shares,
        ...
        if account is None:
            ...
            account = Account(account, dpay_instance=self.dpay)
        to_account = Account(to_account, dpay_instance=self.dpay)
    ...
    def set_withdraw_vesting_route(self,
        ...
        op = operations.Set_withdraw_vesting_route(
            **{
                ...
                "to_account": to,
                "percent": int(percentage * DPAY_1_PERCENT),
                "auto_vest": auto_vest
            })
    ...
    def allow(
        ...
        try:
            pubkey = PublicKey(foreign, prefix=self.dpay.prefix)
            ...
            try:
                foreign_account = Account(foreign, dpay_instance=self.dpay)
        ...
        if threshold:
            ...
            self.dpay._test_weights_treshold(authority)
        ...
        if permission == "owner":
            return self.dpay.finalizeOp(op, account, "owner", **kwargs)
        else:
            return self.dpay.finalizeOp(op, account, "active", **kwargs)
    ...
    def blog_history(self, limit=None, start=-1, reblogs=True, account=None):
        """ stream the blog entries done by an account in reverse time order.
            ...
            :param bool reblogs: (optional) if set `True` (default)
                reblogs / reposts are included. If set `False`,
...
class Accounts(AccountsObject):
    """ Obtain a list of accounts
        ...
        :param dpay dpay_instance: DPay() instance to use when
            accessing a RPC
    """
    def __init__(self, name_list, batch_limit=100, lazy=False, full=True, dpay_instance=None):
        self.dpay = dpay_instance or shared_dpay_instance()
        if not self.dpay.is_connected():
            return
        accounts = []
        ...
        while name_cnt < len(name_list):
            self.dpay.rpc.set_next_node_on_empty_reply(False)
            if self.dpay.rpc.get_use_appbase():
                accounts += self.dpay.rpc.find_accounts({'accounts': name_list[name_cnt:batch_limit + name_cnt]}, api="database")["accounts"]
            else:
                accounts += self.dpay.rpc.get_accounts(name_list[name_cnt:batch_limit + name_cnt])
            name_cnt += batch_limit

        super(Accounts, self).__init__(
            [
                Account(x, lazy=lazy, full=full, dpay_instance=self.dpay)
                for x in accounts
            ]
        )
