    ...
    def appendSigner(self, account, permission):
        ...
        def fetchkeys(account, perm, level=0):
            ...
            for authority in account[perm]["key_auths"]:
                try:
                    wif = self.dpay.wallet.getPrivateKeyForPublicKey(
                        authority[0])
                    if wif:
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[perm]["account_auths"]:
                    auth_account = Account(
                        authority[0], dpay_instance=self.dpay)
        ...
        if account["name"] not in self.signing_accounts:
            ...
            if isinstance(account, PublicKey):
                self.wifs.add(
                    self.dpay.wallet.getPrivateKeyForPublicKey(
                        str(account)
                    )
                )
            else:
    ...
    def broadcast(self, max_block_age=-1):
        ...
        try:
            ...
            if self.dpay.use_dpid:
                ...
                self.dpay.rpc.broadcast_transaction(
                    args, api=broadcast_api)
        except Exception as e:
