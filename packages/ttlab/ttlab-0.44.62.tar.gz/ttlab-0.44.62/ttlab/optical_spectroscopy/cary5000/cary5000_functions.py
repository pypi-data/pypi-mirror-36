...
class Cary5000Functions:
    ...
    @staticmethod
    def find_peak(wavelength, transmission, range, show_fit=False):
    ...
    def _create_peak_array(data, range, show_fit=False):
        ...
        for key in data.get_samples():
            if key == 'Baseline 100%T':
                continue
            tmp_peak, tmp_error = Cary5000Functions.find_peak(data.get_wavelength(key), data.get_transmission(key), range, show_fit=show_fit)
