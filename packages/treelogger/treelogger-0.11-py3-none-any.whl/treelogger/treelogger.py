...
class Tag():
    ...
    def __str__(self):
        ...
        return '<%s %s>  %s  </%s>' % (tag, attstring, self.message, tag)
    ...
    def open(self):
        ...
        starttag = '<%s %s>' % (self.message, attstring)

        return starttag

    def close(self):

        endtag = '</%s>' % self.message
...
class TextFileOutput(OutputObject):
    ...
    def close_log(self, message):
        ...
        if not self.autoclosing:
            text = open(self.filepath).read()

            reparsed = minidom.parseString(text).toprettyxml(indent = '\t')

            open(self.filepath, "w").write(reparsed)
...
#Create a tree instance for modules to import
tree = None
TreeLogger(outputs = [SysStdOutput()])
