...
class BaseSpider:
    ...
    def _do_task(self,task):
        ...
        # 判断是否需要限流
        target_url_host = parse.urlparse(url).netloc
        current_proxy = self.fetcher.current_proxy()
        last_execute_key = "%s_%s" % (target_url_host, current_proxy)
        last_execute = self.last_execute_record.get(last_execute_key, 0)
        delta = time.time() - last_execute
        throttle = self.throttle(target_url_host, current_proxy)
        logger.info("Last execute time: %s, current delta: %s, throttle: %s " %(last_execute, delta, throttle))
        ...
        self.last_execute_record.update({last_execute_key: time.time()})
