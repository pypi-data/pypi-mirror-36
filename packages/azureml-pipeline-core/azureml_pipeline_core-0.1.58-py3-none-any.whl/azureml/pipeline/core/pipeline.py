...
def _submit_pipeline(pipeline, workspace, experiment_name, **kwargs):
    ...
    for key, value in kwargs.items():
        if key == 'continue_on_node_failure':
            continue_on_node_failure = value
...
class Pipeline(object):
    ...
    :param default_datastore_name: The default datastore name to use for data connections.
    ...
    def __init__(self, workspace, steps, description=None,
                 default_datastore_name=None, default_source_directory=None, resolve_closure=True,
        ...
        # construct dictionary of MLC computes
        targets = dict([(target.name, target) for target in workspace.compute_targets()])
        ...
        self._graph_builder = _PipelineGraphBuilder(resolve_closure=resolve_closure,
                                                    ...
                                                    default_datastore_name=default_datastore_name)
    ...
    def submit(self, experiment_name, pipeline_parameters=None, continue_on_node_failure=False,
               regenerate_outputs=False):
        """
        ...
        :param continue_on_node_failure: Whether to continue pipeline execution if a node fails.
