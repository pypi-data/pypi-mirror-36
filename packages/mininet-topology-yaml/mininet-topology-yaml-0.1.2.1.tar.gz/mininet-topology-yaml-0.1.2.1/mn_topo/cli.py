...
def format_customer_option(customer):
    if customer:
        return [merge_two_dicts(DEFAULT_CUSTOMERS[0], {'customer': u_to_ascii(i[0]), 'connects_to': u_to_ascii(i[1]), 'hostname': u_to_ascii(i[2]), 'port': int(u_to_ascii(i[3]))}) for i in customer]
