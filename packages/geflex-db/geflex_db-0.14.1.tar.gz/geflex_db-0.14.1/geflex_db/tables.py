...
from .utils import Combiner, DotDict
...
class InnerObject:
    ...
    def __setitem__(self, key, value):
        self.__pos[self.__columns.index(key)] = value

    def __getitem__(self, key):
        return self.__pos[self.__columns.index(key)]
    ...
    def __str__(self):
        s = '{'
        for key, value in zip(self.__columns, self.__pos):
            s += f'{key}: {value}, '
        return s + '}'

    def __repr__(self):
        return f'{self.__class__}({self.__pos}, {self.__columns})'
...
class Table(Combiner):
    ...
    # ------------------------------------------------------------------------
    #   Dict-items operations
    # ------------------------------------------------------------------------

    def dicts(self):
        for item in self._items:
            yield DotDict(item, self._keys)

    def get_dict(self, index):
        return DotDict({c: v for c, v in zip(self._items[index], self._columns)})

    def contains_dict(self, dct, keys=None):
        return self.contains(dct.values(), dct.keys(), keys=keys)

    def index_dict(self, dct, keys=None):
        return self.index(dct.values(), dct.keys(), keys=keys)

    def insert_dict(self, dct, keys=None, funcs=None):
        return self.insert(dct.values(), dct.keys(), keys=keys, funcs=funcs)

    def unique_str_dict(self, dct, keys=None):
        if keys is None:
            keys = self._keys
    ...
    def positions(self):
        for item in self._items:
            yield InnerObject(item, self._keys)

    def get_pos(self, index):
        return InnerObject(self._items[index], self._items)

    def index_dict(self, pos, keys=None):
        return self.index(pos._InnerObject__pos, pos._InnerObject__columns, keys=keys)

    def contains_pos(self, pos, keys=None):
        return self.contains(pos._InnerObject__pos, pos._InnerObject__columns, keys=keys)

    def insert_pos(self, pos, keys=None, funcs=None):
        self.insert(pos._InnerObject__pos, pos._InnerObject__columns, keys=keys, funcs=funcs)

    def unique_str_pos(self, pos, keys=None):
        if keys is None:
            keys = self._keys
        return self.unique_str(pos._InnerObject__pos, pos._InnerObject__columns, keys=keys)

    def remove_pos(self, pos, keys=None):
        self.remove(pos._InnerObject__pos, pos._InnerObject__columns, keys=keys)
