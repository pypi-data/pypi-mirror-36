...
class Table:
    ...
    # ------------------------------------------------------------------------
    #   Сomparing operations
    # ------------------------------------------------------------------------

    def is_contained(self, other, keys=None):
        keys = self._prepare_keys(keys)
        key_indices = self._key_indices(other._columns, keys)

        for pos in self._items:
            for npos in other._items:
                if not self._pos_isequal(pos, npos, key_indices):
                    return False
        return True

    def contains_table(self, other, keys=None):
        return other.is_contained(self, keys=keys)

    def equal(self, other, keys=None):
        keys = self._prepare_keys(keys)
        key_indices = self._key_indices(other._columns, keys)

        if not len(self._items) == len(other._items):
            return False
    ...
    def __ge__(self, other):
        return self.contains_table(other)
    ...
    def with_columns(self, *columns, clmns=None):
        """Возвращает таблицу ТОЛЬКО с переданными столбцами"""
        if clmns is not None:
            clmns = list(clmns)
            columns = list(columns) + [c for c in clmns if c not in columns]
        indices = [self._columns.index(c) for c in columns]
    ...
    def __reversed__(self):
        ...
        :rtype: reversed
        """
        return reversed(list(self.dicts()))
    ...
    def __contains__(self, obj):
        ...
        :type obj: Union[dict, Dotdict, InnerObject, list, Table]
        """
        if isinstance(obj, (dict, DotDict, InnerObject)):
            return self.contains_pos(obj)
        elif isinstance(obj, list):
            return self.contains(obj)
        elif isinstance(obj, Table):
            return self.contains_table(obj)
        else:
            return TypeError(f'obj parameter must be dict, DotDict, InnerObject, list or Table, but not {type(obj)}')
    ...
    def where(self, dct=None, **kwargs):
        ...
        for key, value in dct.items():
            splitted = key.split('.')
            if len(splitted) == 1:
                column_name = splitted[0]
                func_name = '__eq__'
            else:
                column_name = '.'.join(splitted[:-1])
                func_name = splitted[-1].strip()

            if func_name in method_names:
                func_name = method_names[func_name]

            d.append((column_name, func_name, value))

        def condition(pos: dict):
            for column, f_name, val in d:
                if not getattr(pos[column], f_name)(val):
                    return False
            return True
