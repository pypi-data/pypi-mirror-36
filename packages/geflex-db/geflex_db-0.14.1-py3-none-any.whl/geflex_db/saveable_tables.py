...
class FileInnerObject(metaclass=ObjectWrapperDecorated):
    ...
    def __repr__(self):
        return f'{self.__class__}({self.obj}, {self.parent})'
...
class TableFile(Table, File):
    ...
    def __iter__(self):
        return self.dicts(saving=True)

    def dicts(self, saving=False):
        for item in super().dicts():
            if not saving:
                yield item
            else:
                yield FileInnerObject(item, self)

    def lists(self, saving=False):
        for item in super().lists():
            if not saving:
                yield item
            else:
                yield FileInnerObject(item, self)
