...
def commit_tree(object_store, blobs):
    ...
    def add_tree(current_path):
        if current_path in trees:
            return trees[current_path]

        dir_name, base_name = pathsplit(current_path)
        t = add_tree(dir_name)

        assert isinstance(base_name, bytes)
        newtree = {}
        t[base_name] = newtree
        trees[current_path] = newtree
    ...
    def build_tree(current_path):
        current_tree = Tree()
        for base_name, entry in trees[current_path].items():
            if isinstance(entry, dict):
                current_mode = stat.S_IFDIR
                current_sha = build_tree(pathjoin(current_path, base_name))
            else:
                current_mode, current_sha = entry

            current_tree.add(base_name, current_mode, current_sha)

        object_store.add_object(current_tree)
...
def index_entry_from_stat(stat_val, hex_sha, flags, mode=None):
    ...
    return (stat_val.st_ctime, stat_val.st_mtime, stat_val.st_dev,
            ...
            stat_val.st_gid, stat_val.st_size, hex_sha, flags, url)
