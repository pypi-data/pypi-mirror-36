...
from cfme_testcases import cfme_parsereq, cli_utils, consts, gen_xmls
...
def get_requirements_from_testcases():
...
def gen_requirements(config, transform_func=None):
    ...
    try:
        ...
        requirements_data = get_requirements_from_testcases()
    return gen_xmls.gen_requirements_xml_str(requirements_data, config, transform_func)


def get_requirements_xml_root(config, transform_func=None):
    """Gets the requirements XML root."""
    req_xml_str = gen_requirements(config, transform_func)
...
def get_requirements_from_log(req_logname):
    ...
    with io.open(req_logname, encoding="utf-8") as input_file:
        return parselogs.parse_requirements(input_file, req_logname)
...
def get_requirements_mapping(args, submit_args, config, req_xml_root):
    ...
    requirements = get_requirements_from_log(req_logname)
