    ...
    try:
        ...
        if not args.no_requirements:
            requirements_root = cli_requirements.get_requirements_xml_root(
                config, requirements_transform_func
            )
        ...
        testsuites_root = cli_testcases.get_testsuites_xml_root(
            args, config, testsuites, testsuites_transform_func
        )
        testcases_root = cli_testcases.get_testcases_xml_root(
            config, requirements_mapping, testcases, testcases_transform_func
        )

        missing = cli_testcases.get_missing(
            args, submit_args, config, testsuites_root, testcases_root
        )
        filtered_xmls = filters.get_filtered_xmls(
            testcases_root, testsuites_root, missing, trust_source=args.trust_source
        )
