...
def gen_testcases_xml_str(
    ...
    try:
        testcases_data = _load_json(testcases_json_filename)["results"]
...
def gen_testsuite_xml_str(testsuite_json_filename, testrun_id, config, transform_func=None):
    """Generates the testcases XML string."""
    assert testrun_id
    try:
        results = _load_json(testsuite_json_filename)["results"]
    except Exception as err:
        raise TestcasesException(
            "Cannot load results from {}: {}".format(testsuite_json_filename, err)
        )
