...
import json
import threading
from utils import add_to_job_requests, get_data
...
class DroneApiConnector():
    def __init__(self, token, vehicle_id,ip_address='cloud',timeout=60, time_constant=20, conn_time=10, wait_for_drone_response=False):
        """
        ...
        job_request = dict(type="NEW_CONN", namespace=self.namespace, id=self.id, vehicleID=vehicle_id,
                           ip_address=self.ip_address, timeout=self.timeout, header=header, conn_time = self.conn_time)
    ...
    def handle_return(self, path, place):
        ...
        for item in self.pubsub.listen():
            ...
            try:
                ...
                if 'waypoints' in c['values']:
                    d = {
                        "success": c['values']['success'],
                        "message": c['values']['message'],
                        "waypoints": c['values']['waypoints']
                    }
