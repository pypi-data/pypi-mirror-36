...
CHANGELOG_FILE = os.path.join(LOCATION, "CHANGELOG.md")
...
def _bump_changelog(baseVersion):
    ...
    log, count = re.subn("\[Unreleased\]: (.*)master", "[{0}]: \g<1>v{0}"
    ...
    log, count = re.subn("## \[Unreleased\]", "## [{}]".format(baseVersion),log)
...
def bump_version(*parts):
    ...
    if len(parts) == 2:
        ...
        raise Exception(
            "Version format must be MAJOR.MINOR or MAJOR.MINOR.PATCH.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--commit-prefix", metavar="STR", type=str,
        default=".post", help="prefix for number of commits since last release")
