    ...
    def transform_widget_run_data(self, runs):
        ...
        for run in runs:
            ...
            transformed_run = {'run_id': run.id,
                               ...
                               'duration': self._get_run_duration(top_level_properties['status'],
...
class _AutoMLDataTransformer(_DataTransformer):
    ...
    def _get_additional_properties(self, run):
        ...
        if top_level_properties['status'] == 'Completed' and 'run_properties' in property_bag \
                and 'run_preprocessor' in property_bag and 'run_algorithm' in property_bag:
            run_properties = property_bag['run_properties']
            if property_bag['run_preprocessor'] is not None and property_bag['run_preprocessor'] != "":
                run_name = "{0}, {1}".format(property_bag['run_preprocessor'], property_bag['run_algorithm'])
            else:
                run_name = property_bag['run_algorithm']
        else:
            run_properties = top_level_properties['status']
