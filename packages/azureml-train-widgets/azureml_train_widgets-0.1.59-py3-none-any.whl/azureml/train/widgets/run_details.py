...
class _UserRun(_WidgetBase):
    ...
    def get_widget_data(self, widget_settings=None):
        ...
        self.run_properties['distributed_processes'] = distributed_processes
        ...
        self._get_run_logs_async(log_files,
                                 ...
                                 self.error,
                                 distributed_processes[0] if distributed_processes else None)

        child_runs = self._get_child_runs()

        # cache child runs. Cached runs will be merged with runs retrieved from the backend
        tmp_cache = []
        for (a, b) in itertools.zip_longest(sorted(child_runs, key=operator.itemgetter('run_number')),
                                            self._child_runs_cache, fillvalue={}):
            tmp_cache.append({**b, **a})
        self._child_runs_cache = tmp_cache
        self.widget_instance.child_runs = self._child_runs_cache

        transformed_children_metrics = {}
        if child_runs:
            run_ids = [x['run_id'] for x in child_runs]

            metrics = {}

            # get list of not cached runs for which we will retrieve metrics
            not_cached_runs = [x for x in child_runs if x['run_id'] not in _UserRun._metrics_cache]
            if not_cached_runs:
                metrics = self._get_metrics(not_cached_runs)

                # populate cache with metrics from completed runs
                completed_run_ids = [x['run_id'] for x in child_runs
                                     if x['status'].lower() in ('completed', 'failed', 'canceled')]
            ...
            mapped_metrics = self._get_mapped_metrics(metrics, child_runs)
            ...
            updated_runs = self._update_children_with_metrics(child_runs, transformed_children_metrics)
            if updated_runs:
                self._child_runs_cache = sorted(updated_runs, key=operator.itemgetter("run_number"))
                self.widget_instance.child_runs = self._child_runs_cache
    ...
    def _get_run_logs(self, log_files, status, error, process):
        ...
        logs = "Log is empty or no log files found." \
            if _status in ["completed", "failed", "canceled"] \
        ...
        if error:
            ...
            _logs = "{0}\nError occured: {1}".format(_logs, _error)
    ...
    def _get_child_runs(self):
        child_runs = self.run_instance.get_children(_rehydrate_runs=False)
    ...
    def _should_stop_refresh(self, widget_data):
        return widget_data['run_properties']['status'] is not None and \
            widget_data['run_properties']['status'].lower() in ["completed", "failed", "canceled"]
    ...
    def _on_selected_run_id_change(self, change):
        ...
        if self.widget_instance.child_runs:
            ...
            if len(run_list) > 0:
                run = Run(self.experiment, change.new)
...
class _HyperDrive(_UserRun):
    ...
    def _get_child_runs(self):
        child_runs = super()._get_child_runs()
...
class _AutoML(_UserRun):
    ...
    def __init__(self, run_instance):
        ...
        """
        super().__init__(run_instance, "automl", refresh_time=MULTIRUN_WIDGET_REFRESH_SLEEP_TIME,
