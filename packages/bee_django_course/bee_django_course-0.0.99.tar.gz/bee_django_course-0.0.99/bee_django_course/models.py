...
class CourseSectionMid(models.Model):
    ...
    pionts = models.IntegerField(default=0, verbose_name='通过后获得的M币')
...
class UserCourseSection(models.Model):
    ...
    def auto_pass_check(self):
        ...
        # 所有检查条件都通过，才可自动通过
        if textwork_check & imagework_check & videowork_check:
            self.status = 2
            self.updated_at = timezone.now()
            self.save()
