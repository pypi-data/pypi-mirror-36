...
def copy_script_template(form):
    ...
    filename = f"{output_dir}\{folder_name}\script.py" if output_dir else 'script.py'
...
def parse_error_emails(form):
    return form.form['error_email'].split(';')
...
def write_script_configuration_file(form):
    ...
    filename = f"{output_dir}\{folder_name}\script_config.py" if output_dir else 'script_config.py'
...
def write_query(form):
    output_dir = parse_output_dir(form)
    folder_name = parse_folder_name(form)
    filename = f"{output_dir}\{folder_name}\query.sql" if output_dir else 'query.sql'
...
def write_scheduler_configuration_file(form):
    ...
    output_dir = parse_output_dir(form)
    folder_name = parse_folder_name(form)
    filename = f"{output_dir}\{folder_name}\config.txt" if output_dir else 'config.txt'

    with open(filename, 'w') as f:
        if not error_emails:
            raise ValueError
        else:
            f.write(f"error_email: {', '.join(error_emails.strip())}\n")
        if days:
            f.write(f"days: {days}\n")
        if weekdays:
