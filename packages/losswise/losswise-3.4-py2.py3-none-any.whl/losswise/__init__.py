        ...
        for key, val in iteritems(data_new):
            ...
            for kind in kind_list:
                ...
                if key not in self.stats:
                    self.stats[key] = {kind: val_new}
                else:
                    self.stats[key][kind] = val_new
        if self.max_iter is not None:
            xmin = self.stats['x']['min']
            self.stats['xper'] = {'max': min(1., (x + 1. - xmin) / (self.max_iter - xmin))}
        work_queue.put((x, y, self.stats, int(self.now()), self.graph_id, self.tracker.session_id))
        self.x = self.x + 1
