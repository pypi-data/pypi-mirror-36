...
# global variables
dataset_path = 'data'
color = {'centroid': 'OrangeRed', 'ctc': 'Crimson', 'nuclei': 'LightSkyBlue', '+': 'Orange'}
...
page.layout = html.Div([
        ...
        html.Div(className='container row', children=[
            ...
            html.Div(className='two columns', children=[
                ...
                html.Div(className='card', children=[
                    html.Details([
                        html.Summary('', className='arrow'),
                        dsi.RadioItemRows(
                            id='lock-zoom',
                            options=[[{'label': i, 'value': i} for i in ['Lock', 'Refresh']]],
                            value='Lock'
                        ),
                    ]),
                ]),
...
def update_items(ds):
    ...
    if not ds:
        return []
    return [
        {'label': shorten(f.name, n=20, suffix=True), 'value': f.name}
...
@page.callback(
    ...
    [Input('save', 'n_clicks')],
    [State('dataset', 'value'), State('uid', 'value'), 
     State('tile', 'value'), State('graph', 'figure')],
)
def save_data(btn, ds, uid, tile, figure):
...
def traces(path, size):
    ...
    '''
    items = []
    mask_path = os.path.join(dataset_path, path, 'masks')
    metadata = read_metadata(path)
    # parse centroid markers
    if 'centroid' in metadata:
        pixels = np.array(metadata['centroid'])
        items.append(dict(
            showlegend = True,
    ...
    if not os.path.isdir(mask_path):
        return items
    def trace(show=False, group='nulcei', name='', x=[0], y=[0]):
        return dict(
            showlegend = not show,
            legendgroup = group,
            name = shorten(name),
            ...
            x = x,
            y = y,
            fill = 'toself' if show else 'none',
            opacity = 0.3 if show else 1.0,
            hoverinfo = 'name' if show else 'skip',
        )
    # non-interactive legend placeholders
    items.append(trace(False, 'nuclei', 'nuclei'))
    items.append(trace(False, 'ctc', 'ctc'))
    items.append(trace(False, '+', '+'))
    # iterate contours
    for m, c in iter_contour(mask_path, size):
        g = 'ctc' if 'ctc' in metadata and m in metadata['ctc'] else 'nuclei'
        c.append(c[0]) # closure shape to first pixel
        c = np.array(c)
        items.append(trace(True, g, m, c[:, 1], c[:, 0]))
    return items
...
@cache.memoize()
def b64img(path, mode, quality=25):
    # base64 encoding
