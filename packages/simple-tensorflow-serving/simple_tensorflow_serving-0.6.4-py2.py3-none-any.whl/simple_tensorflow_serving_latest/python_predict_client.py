...
def predict_json(json_data, port=8500):
  ...
  try:
    ...
    logger.debug("Get predict result:{}".format(predict_result))
  except Exception as e:
    logger.error("Get result: {} and exception: {}".format(result, e.message))
...
def get_gen_json_and_clients(model_name="default", signature_name="serving_default", language="json", port=8500):
  ...
  try:
    ...
    logger.debug("Get predict result:{}".format(return_result))
  except Exception as e:
    logger.error("Get exception: {}".format(e.message))
