...
Debug = False
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw, all_cooked_rtt,random_seed=RANDOM_SEED):
    ...
    def poisson_distribution(self, t, n = 0):
        
        # Poisson distribution P(N(t) = n) =  (lamda * t) ** n * e **( -lamda * t) / num!
        P = ((lamda * t) ** n)  * (math.exp(-lamda * t)) / math.factorial(n)
        is_get_value = np.random.randint(1, 100) 
        if is_get_value > P * 100:
            return True
        else:
            return False
