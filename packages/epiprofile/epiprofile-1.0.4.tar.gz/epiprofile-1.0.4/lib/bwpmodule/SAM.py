...
import pysam
from bwpmodule import mystat
from bwpmodule import fasta
from bwpmodule import cigar
...
class ParseBAM:
	...
	def configure_experiment(self,refbed,sample_size = 200000, cutoff=0.80):
		...
		if len(p_strandness) >0 and len(s_strandness) ==0 :
			...
			if spec1 >= spec2 and spec1 >=cutoff:
				strandness ='1++,1--,2+-,2-+'
				specificity = "%.2f%%" % (spec1*100.0)
			if spec2 >= spec1 and spec2 >=cutoff:
				strandness ='1+-,1-+,2++,2--'
				specificity = "%.2f%%" % (spec2*100.0)
			...
			protocol="Mixture"
		return [protocol,strandness,specificity]

		
	def calculate_rpkm(self,geneFile,strand_rule='1++,1--,2+-,2-+'):
		...
		#get all exons regions from reference bed file. The purpose is to calculate the totoal
		#number of reads. intergenic reads will be excluded.
		
		ref_ranges={}
		tmp_obj = BED.ParseBED(geneFile)
		ref_exons = tmp_obj.getExon()
		for l in ref_exons:
			#chrom = l[0]
			#start = l[1]
			#end = l[2]
			if l[0] not in ref_ranges:
				ref_ranges[l[0]] = Intersecter()
			else:
				ref_ranges[l[0]].add_interval(Interval(int(l[1]), int(l[2]) ))
		...
		try:
			while(1):
				...
				for ex in exon_blocks:
					if ex[0] not in ref_ranges:		#if reads not located in exon regions
						continue
					elif len(ref_ranges[ex[0]].find(ex[1],ex[2])) >0:	#total reads only include those reads mapped to exon region. 
						total_tags += 1
						
				#construct bitset
				if strand_rule is not None:															# This is strand-specific sequencing
					if aligned_read.is_paired:														#This is pair end sequencing
						# PairEnd + strand-specific
						if aligned_read.is_read1:read_id = '1'
						if aligned_read.is_read2:read_id = '2'
						if aligned_read.is_reverse:map_strand = '-'
						else:map_strand = '+'
						if strandRule[read_id + map_strand] == '+':
							for block in exon_blocks:
								if chrom not in plus_ranges:plus_ranges[chrom] = Intersecter()
								else:plus_ranges[chrom].add_interval( Interval( block[1], block[2]) )
								
						elif strandRule[read_id + map_strand] == '-':
							for block in exon_blocks:
								if chrom not in minus_ranges:minus_ranges[chrom] = Intersecter()	
								else:minus_ranges[chrom].add_interval( Interval( block[1], block[2]) )
								
					if not aligned_read.is_paired:													#This is single end sequencing
						if aligned_read.is_reverse:map_strand = '-'
						else: map_strand = '+'
						if strandRule[map_strand] == '-':
							for block in exon_blocks:
								if chrom not in minus_ranges: minus_ranges[chrom] = Intersecter()
								else: minus_ranges[chrom].add_interval( Interval( block[1], block[2]) )
						elif strandRule[map_strand] == '+':
							for block in exon_blocks:
								if chrom not in plus_ranges:plus_ranges[chrom] = Intersecter()
								else:plus_ranges[chrom].add_interval( Interval( block[1], block[2]) )
				elif strand_rule is None:															# This is NOT strand specific sequencing
					for block in exon_blocks:
						if chrom not in unstrand_ranges:unstrand_ranges[chrom] = Intersecter()
						else:unstrand_ranges[chrom].add_interval( Interval( block[1], block[2]) )
			...
			for st,end in zip(exon_start,exon_end):
				if strand_rule is None:																# Not strand specific
					if chrom in unstrand_ranges:
						mRNA_count += len(unstrand_ranges[chrom].find(st,end))
					else:
						mRNA_count=0
				else:																				#strand specific
					if strand=='+':
						if chrom in plus_ranges:
							mRNA_count += len(plus_ranges[chrom].find(st,end))
						else:
							mRNA_count=0
					if strand=='-':
						if chrom in minus_ranges:
							mRNA_count += len(minus_ranges[chrom].find(st,end))
						else:
							mRNA_count=0
			try:
				mRNA_RPKM = (mRNA_count * 1000000000.0)/(mRNA_len * total_tags)
			except:
				mRNA_RPKM ="nan"
			rpkm_value[line_id]=mRNA_RPKM
		print >>sys.stderr, "Finished"
		return rpkm_value		
	...
	def peakRootPoint(self,bwfile,flankSize):
		'''detect candicate peak roots from bam and bigwig file. peak root is 5' end that over-
		...
		try:
			while(1):
				...
				peak_root[chrom + ":" + strand +':' + str(read5end)] +=1
		except StopIteration:
			print >>sys.stderr, "Finished"			
						
	def countReads(self,chrom_sizes, skip_multi=True,q_cut=30):
		"""count uniquely alined reads in BAM file"""
		
		print >>sys.stderr, "Count reads ... "
		readsum = 0.0
		read_id=''
	...
	def orphanReads(self,chrom_sizes,count=2000000,q_cut=30):
		"""generate orphan reads from BAM file. orphan reads are those reads not overlapped
		with any other reads. BAM file must be sorted"""
		...
		for chr_name, chr_size in chrom_sizes.items():		#iterate each chrom
			...
			for aligned_read in alignedReads:
				...
				if aligned_read.is_unmapped:continue		#skip unmap read
				if aligned_read.mapq < q_cut: continue
				if aligned_read.pos < progress_pos: continue
				read_seq = aligned_read.seq.upper()
				if 'N' in read_seq: continue
				
				ref_start = aligned_read.pos
				ref_end = aligned_read.pos + aligned_read.qlen
				progress_pos = ref_end
				
				overlap_count=0
				for i in self.samfile.fetch( chr_name, ref_start,ref_end ):
					overlap_count +=1
					if overlap_count > 1:break
				if overlap_count != 1:	#this is NOT orphan reads
					continue
				if aligned_read.is_reverse: 
					read_seq2 = read_seq.translate(transtab)[::-1]
				else:
					read_seq2 = read_seq
				readcount += 1
				#yield (chr_name, ref_start, ref_end, read_seq2)
				yield (read_seq2)
	
	def batchIterBam(self,q_cut=30,batchSize = 100000):
		'''return size alignments each time from BAM file'''
		...
		try:
			while (1):
				...
				for n in xrange(batchSize):
					tmp = self.samfile.next()
					if tmp.is_qcfail:continue
					if tmp.is_unmapped:continue
					if tmp.is_paired and tmp.is_read2: continue		#only consider read1
					read_seq = tmp.seq.upper()
					try:
						chrom = self.samfile.getrname(tmp.tid)
					except:
						continue
					if tmp.is_reverse:
						read_seq =  read_seq.translate(transtab)[::-1]
					values.append((chrom,tmp.pos,read_seq))
				for i in xrange(1,len(values)-1):
					cur_chrom = values[i][0]
					pre_chrom = values[i-1][0]
					aft_chrom = values[i+1][0]
					
					cur_st = values[i][1]
					cur_end = values[i][1] + len(values[i][2])
					
					#pre_st = values[i-1][1]
					pre_end = values[i-1][1] + len(values[i-1][2])
					
					aft_st = values[i+1][1]					
					#aft_end = values[i+1][1] + len(values[i+1][2])

					if cur_chrom == pre_chrom and cur_chrom == aft_chrom and cur_st > pre_end and cur_end < aft_st:
						if "N" in values[i][2]:continue				
