...
class ParseBED:
	...
	def segmentBED(self,maxdist=100,outfile='aaa'):
		'''segment bed entries according the distance between two adjacent entries. Within
		a cluster, the distance between any adjacent entries cannot exceed maxdist'''
		
		OUT = open (outfile,'w')
		#build interval tree
		chr2gpos = collections.defaultdict(list)		#chr vs genomic position, 1-based
		for line in self.f:
			...
			chr2gpos[chrom].append(end)
		
		for chr in chr2gpos:
			seg_start = min(chr2gpos[chr])-1
			break_pos = 0
			for dist in numpy.diff(chr2gpos[chr]):
				break_pos += 1
				if dist > maxdist:
					seg_end = chr2gpos[chr][break_pos-1]
					print >>OUT, chr + '\t' + str(seg_start) + '\t' + str(seg_end)
					seg_start = chr2gpos[chr][break_pos]-1
		OUT.close()	
			
	def uniformBED(self,chromSizes,unisize='median'):
		'''make every bed entry the same size. This size could be user specified, or if NOT
		specified, the median size of all bed entries. Note the unisize should be a even, integer'''
		
		import random
		half_window=0
		try:
			float(unisize)
			half_window = int(int(unisize)/2.0)
		except ValueError:
			tmp=[]
			for line in self.f:
				if line.startswith(("#","track","browser")):continue	#remove track information
			...
			self.f.seek(0)
			half_window = int(numpy.median(tmp)/2.0)
		print >>sys.stderr, "Half window size is: " + str(half_window)
		...
		for line in self.f:
			...
			window_start = start + int((end-start)/2.0) - half_window
			window_end = start + int((end-start)/2.0) + half_window
			
			ctrl_start = random.randint(half_window, chromSizes[chrom] - half_window)
			
			yield(chrom,(window_start, window_end),(ctrl_start - half_window, ctrl_start + half_window  ))

	def midBED(self,half_window_size,head_num):
		'''add window size to upstream/downstream of middle point of BED. if half_window_size =0
		return bed entry AS IS, otherwise, add half_window_size to both size of middle point of bed entry'''
		
		lst=[]
		count = 0
		half_window_size = int(half_window_size)
		head_num = int(head_num)
		for line in self.f:
			if line.startswith(("#","track","browser")):
				print >>sys.stderr, 'skip ' + line,
				continue	#remove track information
			if not line.strip():continue	#remove blank line
			fields=line.rstrip('\n ').split()
			if len(fields)<3: 
				print >>sys.stderr, 'skip ' + line,		#invalid bed format
				continue
			try:
				strand = f[5]
			except:
			...
			if fields[1].isdigit() and  fields[2].isdigit():
				...
				if start <0:
					print >>sys.stderr, 'skip ' + line,   #coordinates negative
					continue
			else:
				print >>sys.stderr, 'skip ' + line,		#coordinates not digit
				continue
			
			if half_window_size == 0:
				count +=1
				lst.append( (chrom, start, end, strand))
				if count == head_num: return lst
					
			elif half_window_size >0:
				if (end - start) % 2 ==1:	#odd number
					mid_point = start + (end - start +1)/2
				if (end - start) % 2 ==0:
					mid_point = start + (end - start)/2
				window_start = mid_point - 1 - half_window_size
				if window_start<0:
					print >>sys.stderr, "Warning: negative values when add window size to " + line
					continue
				window_end = mid_point + half_window_size
				lst.append( (chrom,window_start, window_end, strand))
				count +=1
				if count == head_num: return lst
				
			else:
				print >>sys.stderr, "Half window size cannot be negative."
		return lst
...
def consumeBed3(lst):
	...
	'''remove entry that is completely included in other entries'''
	
	ranges={}
	return_lst=[]
	#build interval tree
	for l in lst:
		if l[0] not in ranges:
			ranges[l[0]] = Intersecter()
		ranges[l[0]].add_interval(Interval(l[1],l[2]))
	...
	for l in lst:
		...
		if l[0] not in ranges:
			...
			overlap = ranges[l[0]].find(l[1],l[2])
			for x in overlap:
				if (x.start < l[1] and x.end > l[2]):
					contained = True
					break
