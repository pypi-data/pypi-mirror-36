...
def sequence_range_errors(test, runner):
    ...
    for case in test.cases:
        ...
        if response.status_code == _output:
            ...
            if (response.status_code == 200 or response.status_code == 206) and 'via' in response.headers and 'squid' in response.headers['via'].lower():
                res = 0
            print("Range: {} == Result: {}".format(_input[1], res))
