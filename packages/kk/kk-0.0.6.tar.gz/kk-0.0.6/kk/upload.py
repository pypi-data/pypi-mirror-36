...
define("dev", default=False, help="dev mode", type=bool)
define("all", default=True, help="show all files", type=bool)
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def get_args(self, **kwargs):
        ...
        self.pagebar = Dict(args)
        self.args = Dict(filter(lambda x: x[0] not in ['page', 'count', 'sort', 'order'], args.items()))
    ...
    def query(self, collection, doc, clean=[], fields=[], schema={}):
        self.clean(doc, *clean)
        self.fields(doc, *fields)
        self.schema(doc, **schema)
        page, count, sort, order = self.pagebar.page, self.pagebar.count, self.pagebar.sort, self.pagebar.order
        cursor = self.db[collection].find(doc).skip((page - 1) * count).limit(count).sort(sort, order)
        self.pagebar.total = cursor.count()
        self.pagebar.pages = int(math.ceil(self.pagebar.total / float(count)))
...
class TableHandler(BaseHandler):
    ...
    def get(self, name):
        ...
        self.pagebar.fields = list(map(lambda item: item.split(':')[0], meta['fields']))
        self.pagebar.searchs = meta.get('search', [])
        self.pagebar.marks = meta.get('mark', [])
        self.pagebar.options = {
            'sort': self.pagebar.fields,
            'order': ['1:asc', '-1:desc'],
        }
        self.render('table.html', entries=entries, pagebar=self.pagebar, title=name)
...
class HomeHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    def listdir(self, root):
        ...
        if root in self.cache and st_mtime == self.cache[root][0]:
            ...
            if root.exists():
                ...
                entries.sort(key=lambda x: str(x[1]).lower())
                self.cache[root] = [st_mtime, entries]
                if len(self.cache) >= 10000:
                    self.cache.popitem()

        page, count = self.pagebar.page, self.pagebar.count
        self.pagebar.total = len(entries)
        self.pagebar.pages = int(math.ceil(len(entries) / count))
    ...
    def execute(self, path):
        ...
        logging.info(f'execute command: {command}, path: {path}')
...
<script>hljs.initHighlightingOnLoad()</script>
        ...
        if path.is_file():
            if path.suffix.lower() in ['.yml', '.yaml']:
                ...
                with tempfile.NamedTemporaryFile('w+', suffix=f'.html', delete=True) as fp:
                    ...
                    os.system(command)
                    self.finish(fp.read())
            elif path.suffix.lower() in ['.py', '.sh', '.h', '.c', '.cpp', '.js', '.css', '.html', '.java', '.go', '.ini']:
                finish(f'''<pre><code>{ tornado.escape.xhtml_escape(path.read_text()) }</code></pre>''')
            else:
                await super().get(name, include_body)
        else:
            entries = await self.listdir(path)
            self.render('index.html', entries=entries, pagebar=self.pagebar)
    ...
    def prepare(self):
        ...
        if self.request.method == 'PUT':
            ...
            HomeHandler._stream_request_body = False
            super().prepare()
...
class Application(tornado.web.Application):
    ...
    def scan(self):
        logging.info(f'scaning {self.path}')
        for root, dirs, files in os.walk(self.path):
            entries = []
            path = Path(root)
            st_mtime = path.stat().st_mtime
            if path in self.cache and st_mtime == self.cache[path][0]:
                continue
            ...
            for f in dirs + files:
                ...
                if not item.exists():
                    continue
                if not options.all and f.name.startswith('.'):
                    continue
                name = item.relative_to(self.path)
                stat = item.stat()
                filesize = stat.st_size
                if filesize / (1024 * 1024 * 1024.0) >= 1:
                    value = '%.1f GB' % (filesize / (1024 * 1024 * 1024.0))
                    ...
                    value = '%.1f KB' % (filesize / 1024.0)
                mtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(stat.st_mtime))
                entries.append([name, mtime, value])
            entries.sort(key=lambda x: x[0].name.lower())
            self.cache[path] = [st_mtime, entries]


def main():
    options.parse_command_line()
    logging.info(f'Debug: {options.dev}, Running: http://{get_ip()}:{options.port}')
