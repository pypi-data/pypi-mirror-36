...
class GCSFS(FS):
    ...
    def __init__(self,
                 bucket_name: str,
                 root_path: str = STANDARD_DELIMITER,
                 project: str = None,
    ...
    def client(self) -> Client:
        if not hasattr(self._tlocal, "gcs"):
            self._tlocal.gcs = storage.Client(project=self.project, credentials=self.credentials)
        return self._tlocal.gcs

    @property
    def bucket(self) -> Bucket:
        return self.client.get_bucket(self._bucket_name)
    ...
    def _check_and_fix_dir(self, path: str):
        ...
        if not self.bucket.get_blob(dir_key):
            child_blobs = list(self.bucket.list_blobs(prefix=dir_key, delimiter="/"))
    ...
    def makedir(self, path, permissions=None, recreate=False):
        ...
        # Adding empty object, see https://fs-s3fs.readthedocs.io/en/latest/#limitations
    ...
    def exists(self, path):
        ...
        directory = self.bucket.get_blob(self._path_to_dir_key(_path))
        if directory:
            return True
