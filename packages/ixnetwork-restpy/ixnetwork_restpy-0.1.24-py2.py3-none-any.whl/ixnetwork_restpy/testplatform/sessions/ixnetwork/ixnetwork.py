...
class Ixnetwork(Base):
	...
	def find(self):
		"""Finds and retrieves / data from the server.

		All named parameters support regex and can be used to selectively retrieve / data from the server.
		By default the find method takes no parameters and will retrieve all / data from the server.

		Returns:
			self: This instance with found / data from the server available through an iterator or index
