...
def clusterSM(cpout,modelname,score,pc,bdpc,clnum=None,pcnum=None,VamModel=None,BuildModel=None):
    ...
    if BuildModel:
        ...
        R=cluster.hierarchy.dendrogram(Z,p=0,truncate_mode='mlab',orientation='bottom',ax=None,color_threshold=0)
    ...
    n=np.multiply(n,100)
    n=np.around(n,2)
    plt.bar(x=np.delete(bins,0),height=n,width=0.5,align='center',color='blue',edgecolor='black')
    plt.xticks(np.delete(bins,0))
    plt.ylabel('%')
# clusterSM.m:237
    #bjff3
    #xlim(cat(0,clnum + 1))
