            ...
            if fea.type == 'CDS':
                ...
                if 'translation' in fea.qualifiers:
                    ...
                    if '*' in translation:
                        ...
                        if logger:
                            logger.info('{0}, {1}, positions on protein: {2}'.format(rec.id, gene, poses))
                        ...
                        if strand > 0:
                            ...
                            if logger:
                                logger.info('{0}, {1}, positions on NT sequence: {2}'.format(rec.id, gene, internalStopCodon_starts))
                            ...
                            if logger:
                                logger.info('\n>{0} {1} {2} protein sequence\n{3}\n'.format(rec.id, gene, fea.location, translation))
                                logger.info('\n>{0} {1} {2} CDS sequence\n{3}\n'.format(rec.id, gene, fea.location, rec.seq[start: end]))
                                logger.info('\n>{0} {1} {2} updated CDS sequence\n{3}\n'.format(rec.id, gene, fea.location, seq[start: end]))
                        ...
                        # the - strand
                            ...
                            if logger:
                                logger.info('\n{0}, {1}, positions on protein: {2}'.format(rec.id, gene, poses))
...
def polish_fasta(infasta=None, NewInternalStopCodonNT='NNN', table=2, logger=None):
    ...
    for rec in SeqIO.parse(ingb, 'fasta'):
        ...
        if '*' in translation:
            ...
            if logger:
                logger.info('{0}: positions on protein: {1}'.format(rec.id, poses))
            ...
            if logger:
                logger.info('{0}: positions on NT sequence: {1}'.format(rec.id, internalStopCodon_starts))
            ...
            if logger:
                logger.info('{0} protein sequence:\n{1}'.format(rec.id, translation))
                logger.info('{0} CDS sequence:\n{1}'.format(rec.id, rec.seq))
                logger.info('{0} updated CDS sequence:\n{1}'.format(rec.id, seq))
