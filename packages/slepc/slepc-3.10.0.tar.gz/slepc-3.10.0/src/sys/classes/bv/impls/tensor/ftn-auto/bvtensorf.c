...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  bvtensorbuildfirstcolumn_(BV V,PetscInt *k, int *__ierr){
*__ierr = BVTensorBuildFirstColumn(
	(BV)PetscToPointer((V) ),*k);
}
PETSC_EXTERN void PETSC_STDCALL  bvtensorcompress_(BV V,PetscInt *newc, int *__ierr){
*__ierr = BVTensorCompress(
	(BV)PetscToPointer((V) ),*newc);
}
PETSC_EXTERN void PETSC_STDCALL  bvtensorgetdegree_(BV bv,PetscInt *d, int *__ierr){
*__ierr = BVTensorGetDegree(
	(BV)PetscToPointer((bv) ),d);
}
PETSC_EXTERN void PETSC_STDCALL  bvcreatetensor_(BV U,PetscInt *d,BV *V, int *__ierr){
