...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  vecnormalizecomplex_(Vec xr,Vec xi,PetscBool *iscomplex,PetscReal *norm, int *__ierr){
*__ierr = VecNormalizeComplex(
	(Vec)PetscToPointer((xr) ),
	(Vec)PetscToPointer((xi) ),*iscomplex,norm);
}
PETSC_EXTERN void PETSC_STDCALL  vecduplicateempty_(Vec v,Vec *newv, int *__ierr){
