...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  lmesetfromoptions_(LME lme, int *__ierr){
*__ierr = LMESetFromOptions(
	(LME)PetscToPointer((lme) ));
}
PETSC_EXTERN void PETSC_STDCALL  lmesetproblemtype_(LME lme,LMEProblemType *type, int *__ierr){
*__ierr = LMESetProblemType(
	(LME)PetscToPointer((lme) ),*type);
}
PETSC_EXTERN void PETSC_STDCALL  lmegetproblemtype_(LME lme,LMEProblemType *type, int *__ierr){
*__ierr = LMEGetProblemType(
	(LME)PetscToPointer((lme) ),type);
}
PETSC_EXTERN void PETSC_STDCALL  lmesettolerances_(LME lme,PetscReal *tol,PetscInt *maxits, int *__ierr){
*__ierr = LMESetTolerances(
	(LME)PetscToPointer((lme) ),*tol,*maxits);
}
PETSC_EXTERN void PETSC_STDCALL  lmegetdimensions_(LME lme,PetscInt *ncv, int *__ierr){
*__ierr = LMEGetDimensions(
	(LME)PetscToPointer((lme) ),ncv);
}
PETSC_EXTERN void PETSC_STDCALL  lmesetdimensions_(LME lme,PetscInt *ncv, int *__ierr){
*__ierr = LMESetDimensions(
	(LME)PetscToPointer((lme) ),*ncv);
}
PETSC_EXTERN void PETSC_STDCALL  lmeseterrorifnotconverged_(LME lme,PetscBool *flg, int *__ierr){
*__ierr = LMESetErrorIfNotConverged(
	(LME)PetscToPointer((lme) ),*flg);
}
PETSC_EXTERN void PETSC_STDCALL  lmegeterrorifnotconverged_(LME lme,PetscBool *flag, int *__ierr){
