...
extern "C" {
#endif
PETSC_EXTERN void PETSC_STDCALL  lmesolve_(LME lme, int *__ierr){
*__ierr = LMESolve(
	(LME)PetscToPointer((lme) ));
}
PETSC_EXTERN void PETSC_STDCALL  lmegetiterationnumber_(LME lme,PetscInt *its, int *__ierr){
*__ierr = LMEGetIterationNumber(
	(LME)PetscToPointer((lme) ),its);
}
PETSC_EXTERN void PETSC_STDCALL  lmegetconvergedreason_(LME lme,LMEConvergedReason *reason, int *__ierr){
*__ierr = LMEGetConvergedReason(
	(LME)PetscToPointer((lme) ),reason);
}
PETSC_EXTERN void PETSC_STDCALL  lmegeterrorestimate_(LME lme,PetscReal *errest, int *__ierr){
*__ierr = LMEGetErrorEstimate(
	(LME)PetscToPointer((lme) ),errest);
}
PETSC_EXTERN void PETSC_STDCALL  lmecomputeerror_(LME lme,PetscReal *error, int *__ierr){
