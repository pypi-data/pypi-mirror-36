...
PETSC_EXTERN void PETSC_STDCALL  epspowersetnonlinear_(EPS eps,PetscBool *nonlinear, int *__ierr){
*__ierr = EPSPowerSetNonlinear(
	(EPS)PetscToPointer((eps) ),*nonlinear);
}
PETSC_EXTERN void PETSC_STDCALL  epspowergetnonlinear_(EPS eps,PetscBool *nonlinear, int *__ierr){
*__ierr = EPSPowerGetNonlinear(
	(EPS)PetscToPointer((eps) ),nonlinear);
}
PETSC_EXTERN void PETSC_STDCALL  epspowersetupdate_(EPS eps,PetscBool *update, int *__ierr){
*__ierr = EPSPowerSetUpdate(
	(EPS)PetscToPointer((eps) ),*update);
}
PETSC_EXTERN void PETSC_STDCALL  epspowergetupdate_(EPS eps,PetscBool *update, int *__ierr){
*__ierr = EPSPowerGetUpdate(
	(EPS)PetscToPointer((eps) ),update);
}
PETSC_EXTERN void PETSC_STDCALL  epspowersetsnes_(EPS eps,SNES snes, int *__ierr){
*__ierr = EPSPowerSetSNES(
	(EPS)PetscToPointer((eps) ),
	(SNES)PetscToPointer((snes) ));
}
PETSC_EXTERN void PETSC_STDCALL  epspowergetsnes_(EPS eps,SNES *snes, int *__ierr){
