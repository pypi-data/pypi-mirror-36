...
path1=os.path.join(sppath,"ai_tools/data/clf.pkl")
print(path1)
...
def get_9hist(image):
    ...
    #zprint(hist9)
    return hist9
def get_imglist_histfeatre(imglist):
    if len(imglist)==0:
    ...
    return histimg
def lumi_classify(histimg):
    histimg*=255.0
    histimg.astype("uint8") 
    x=histimg.reshape((histimg.shape[0]*histimg.shape[1]))
    clf1=joblib.load("clf.pkl")
    clf=clf1
    y_pred = clf.predict_proba(x)
    y_pred_sort_index=np.argsort(y_pred)
    y_pred_l=y_pred_sort_index[:,y.max()]
    return y_pred_l
