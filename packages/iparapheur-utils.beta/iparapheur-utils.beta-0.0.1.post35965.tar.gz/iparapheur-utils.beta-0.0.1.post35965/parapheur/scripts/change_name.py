...
pprint.warning("ATTENTION ! Le certificat configuré dans le fichier /etc/nginx/conf.d/parapheur_ssl.conf ne correspond "
               "potentiellement plus avec le nouveau nom du parapheur. Il convient de remplacer ce certificat "
               "(localisé dans le dossier /etc/nginx/ssl/) pour que le parapheur soit totalement fonctionnel.\n", True)

pprint.success("Propriétés à modifier dans le fichier de configuration /etc/nginx/conf.d/parapheur_ssl.conf :")
pprint.header("- ssl_certficiate /etc/nginx/ssl/test.pem;     # Partie publique")
pprint.header("- ssl_certficiate_key /etc/nginx/ssl/test.key; # Partie privée")
