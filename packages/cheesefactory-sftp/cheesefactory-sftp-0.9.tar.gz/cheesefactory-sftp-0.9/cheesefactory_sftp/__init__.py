...
class SFTP:
    ...
    def __post_init__(self):
        ...
        self._local_directory = '/'  # Default local directory for transferred files.
    ...
    def __is_this_a_new_directory(self, directory: str):
        ...
        if os.path.isdir(self._local_directory, directory):
            self.__logger.debug(f'Directory exists: {self._local_directory}/{directory} (regex: {self._ignore_regex})')
    ...
    def __is_this_a_new_file(self, filename: str):
        ...
        """
        local_file: Path = Path(self._local_directory, Path(filename).name)

        if re.match(self._ignore_regex, filename):
