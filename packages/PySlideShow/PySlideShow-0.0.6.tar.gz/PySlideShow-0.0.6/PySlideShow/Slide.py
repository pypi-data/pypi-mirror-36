...
class Slide:
    ...
    def set_anchor(self, anchor_configuration):
        ...
        if not 'offset-x' in anchor_configuration.keys():
            anchor_configuration['offset-x'] = 0
        if not 'offset-y' in anchor_configuration.keys():
            anchor_configuration['offset-y'] = 0

        anchor = AnchorSet()

        # size of the textured object based on base scale and zoom
        ...
        # helper variables
        alpha = anchor_configuration['offset-x']
        beta = anchor_configuration['offset-y']
        assert 0 <= alpha and alpha <= 1
        assert 0 <= beta and beta <= 1
        D_w = self.display_width
        D_h = self.display_height
        A_w = obj_width - D_w
        A_h = obj_height - D_h

        # distance to the sides of the object sides
        left = -alpha * A_w
        right = D_w + (1-alpha) * A_w
        top = D_h + beta * A_h
        bottom = -(1-beta) * A_h

        # combine to 4 corner points
        anchor.bottom_left  = np.array((left, bottom))
        anchor.bottom_right = np.array((right, bottom))
        anchor.top_right    = np.array((right, top))
        anchor.top_left     = np.array((left, top))
