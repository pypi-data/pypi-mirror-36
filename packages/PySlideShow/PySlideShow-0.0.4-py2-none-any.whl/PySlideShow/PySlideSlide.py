...
class Slide:
    ...
    def __init__(self, slide_index, configuration, display):
        self.slide_index = slide_index
        self.fn = configuration['file']
        self.display = display
        if not self.dry:
            print "Loading image '%s' ..." % self.fn
            self.load_texture(self.fn)
            self.parseConfirguration(configuration)

    def setAnchor(self, anchorConfiguration):

        # set implicit default parameters
        if not 'zoom' in anchorConfiguration.keys():
            anchorConfiguration['zoom'] = 1
        if not 'position' in anchorConfiguration.keys():
            anchorConfiguration['position'] = 'top_left'
        if not 'offset_x' in anchorConfiguration.keys():
            anchorConfiguration['offset_x'] = 'top_left'

        anchor = AnchorSet()
        obj_width = self.base_scale * self.txt_width * anchorConfiguration['zoom']
        obj_height = self.base_scale * self.txt_height * anchorConfiguration['zoom']
        if anchorConfiguration['position'] == 'top_left':
        ...
        elif anchorConfiguration['position'] == 'top_right':
        ...
        elif anchorConfiguration['position'] == 'bottom_left':
            ...
            print "Configuration Error Slide %d: Position '%s' not supported." % (self.slide_index, anchorConfiguration['position'])
            sys.exit(1)
        return anchor
    ...
    def parseConfirguration(self, configuration):
        ...
        self.anchor_begin = self.setAnchor(configuration['begin'])
        self.anchor_end = self.setAnchor(configuration['end'])
        pass
