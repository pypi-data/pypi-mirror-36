...
from dst_server_deploy.helpers import id_generator, ask_yes_no, ask_path, choose_preset, input_int, input_csv, fileio_csv
...
class ServerCommon(ABC):
    ...
    def __init__(self,
        ...
        # TODO - Add :param bool player_control_enabled: Should we set a non-standard maximum number of players, adminlist, whitelist or blocklist?
    ...
    def _singleton_init(cls,
        ...
        :param bool access_controls_enabled: Should the server have access controls (password / steam group)?
        :param bool offline_settings_enabled: Should the server not be listed online or only accessable from lan?
        :param bool set_gamemode_enabled: Should the server use a particular gamemode or server intentions?
        ...
        if cls._instance is None:
            ...
            cls.game_intention_types = ['social', 'cooperative', 'competitive', 'competitive']
    ...
    def _input_access_controls(cls):
        ...
        if ask_yes_no("Should the server be password protected?"):
    ...
    def _write_common_env_params(cls, path):
        ...
        with open(path, "w+") as file_obj:
            ...
            if cls.mods_enabled:
                file_obj.write("\nMODS=%s\n" %cls.mod_list_csv)
                file_obj.write("MOD_OVERRIDES=%s\n\n" %cls.mod_override)
