""" Forest Server Deployement fo ruse with  dstacademy's dontstarvetogether docker image.
...
from dst_server_deploy import ServerCommon, DockerComposer
...
def main():
    ...
    pvp_enabled = input("Should PvP be enabled [y/n] ")
    pvp_enabled = (pvp_enabled[0].lower() == 'y')
    caves_enabled = input("Should caves be enabled [y/n] ")
    caves_enabled = (caves_enabled[0].lower() == 'y')
    mods_enabled = input("Should mods be enabled? [y/n] ")
    mods_enabled = (mods_enabled[0].lower() == 'y')
    world_customization = input("Do you want to customize the world generation? [y/n] ")
    world_customization = (world_customization[0].lower() == 'y')

    # Build overworld object
    underworld = None
    overworld = ForestServer(server_token, world_name, world_desc, pvp_enabled,
                       caves_enabled, mods_enabled, world_customization)
    # Write env file.
    overworld.write_env_param()

    #if caves_enabled:
        # Build underworld object
        #underworld = Cave(server_token, world_name, world_desc, pvp_enabled,
        #                   caves_enabled, mods_enabled, world_customization)
        # Write env file.
        #underworld.write_env()
