...
def ask_path(question, validate_file_exists=True, invalid=False):
    ...
    """
    if invalid:
        print('Sorry, that path is invalid/no file exists. Please try again (be careful with whitespaces).')
...
def choose_preset(presets, invalid=False):
    ...
    selection = input('Please select a preset by number/index\n' \
                      'Valid Responses include: {}'.format(list(enumerate(presets_list))))
...
def input_csv(message, valid_delimiter=None):
    ...
    return ",".join(filter(len, response.split(",")))
