...
for algorithm, periods in plot_data.items():
	...
	bar = ax.bar(xticks + shift[index], periods[:,3], width, color=algorithm.color)
	bars.append(bar)
...
ax.set_ylabel('Cost Computations Count')
ax.legend( legend_data, legend_labels)
# for bar in bars:
# 	autolabel(bar)
# for conf, content in record_csv_content:

plt.savefig(config.images.cost_count, bbox_inches='tight', transparent=True)

print("Latex code:")

print(r"\begin{tabular}{|r|r|r|}")
print("\hline")
print("Period & HMM & GSMM")
print(r"\tabularnewline")
print("\hline")
print("\hline")

for index, row in enumerate(plot_data[Algorithm.HMM]):
	row_our = plot_data[Algorithm.PROPOSED][index]
	print("{} & {} & {}".format(int(row[2]), '{:,}'.format(int(row[3])).replace(',', ' '),
								'{:,}'.format(int(row_our[3])).replace(',', ' ')))
	print(r"\tabularnewline")
	print("\hline")
