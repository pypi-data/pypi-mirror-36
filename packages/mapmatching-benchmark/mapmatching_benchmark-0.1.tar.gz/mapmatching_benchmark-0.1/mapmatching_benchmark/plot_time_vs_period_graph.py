...
for algorithm, periods in plot_data.items():
	...
	# 			c=algorithm.color)
	shift = width / 2 if index else - width / 2
	bar = ax.bar(xticks + shift, periods[:,1] / 1000, width, color=algorithm.color)
	bars.append(bar)
...
ax.set_ylabel('Map matching speed [km/s]')
ax.legend( legend_data, legend_labels)
# for bar in bars:
# 	autolabel(bar)
# for conf, content in record_csv_content:

plt.savefig(config.images.time_period, bbox_inches='tight', transparent=True)
