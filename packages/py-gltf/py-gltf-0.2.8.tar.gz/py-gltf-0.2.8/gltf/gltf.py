...
class GLTF:
    ...
    def center(self):
        ...
        if not np.allclose(transformation, np.identity(4)):
            ...
            self.apply_all_transforms(transformation)

    def render(self, binary=False, embed=None):
        embed = binary if embed is None else embed
        self.buffers = []
        self.buffer_views = []
    ...
    def apply_all_transforms(self, transform=None, rotation=None):
        ...
        for n in self.get_parent_nodes():
