...
class _freeze_time(object):
    ...
    def __call__(self, func):
        if self.is_python_agent_call():
            return self.noop_decorator(func)
    ...
    def is_python_agent_call(self):
        call_stack = inspect.stack()
        stack_limit = min(len(call_stack), 20)
        # Start at 1 to ignore the current frame (index 0)
        for i in range(1, stack_limit):
            mod = inspect.getmodule(call_stack[i][0])
            if mod.__name__.startswith(python_agent.__name__):
                return True
        return False
