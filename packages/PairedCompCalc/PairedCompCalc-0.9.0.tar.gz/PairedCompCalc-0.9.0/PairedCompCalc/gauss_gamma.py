...
used for predictive distributions derived from a GaussainRV instance.
...
class GaussianRV:
    ...
    # def __str__(self):
    #     return ('GaussianRV object with properties\n' +
    #             f' mean= {self.mean}\n' +
    #             f' prec= {self.prec}\n')
    ...
    @property
    def var(self):
        """E{ |x - self.loc|^2 | self.mean, self.prec }
    ...
    def mean_logpdf(self, x):
        ...
        return (- z2 - self.size / self.mean.learned_weight
                + np.sum(self.prec.mean_log) - self.size * np.log(2 * np.pi)
                ) / 2
...
class GaussianGivenPrecisionRV:
    ...
    def __repr__(self):
        return ('GaussianGivenPrecisionRV(' +
                f'\n\tloc= {repr(self.loc)},' +
                f'\n\tlearned_weight= {repr(self.learned_weight)},' +
                '\n\tprec= prec)')

    # def __str__(self):
    #     return f'GaussianGivenPrecisionRV(loc= {self.loc}, learned_weight= {self.learned_weight})'
...
class PrecisionRV:  # ********** might be subclass of scipy.stats.gamma ??? **********
    ...
    # @scale.setter
    # def scale(self, scale):  # ***** works only for complete property, NOT sub-scripted
    #     self.b = 1. / scale
    ...
    def mean_log(self):
        ...
        # return multi_psi(self.df/2, self.D) + self.D * np.log(2) + s * log_det_scale
