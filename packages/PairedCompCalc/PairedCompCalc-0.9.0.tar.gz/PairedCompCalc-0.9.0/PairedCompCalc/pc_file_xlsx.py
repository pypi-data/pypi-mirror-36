...
*2: a combination of (choice, response_label), where
    ...
    The response_label must be an element in the list PairedCompFrame.response_labels.
...
response_label == PairedCompFrame.response_labels[0], if forced_choice == False.


*** Usage Example:

pcf=PairedCompFrame(systems=['HA-1', 'HA-2', 'HA-3'],
...
The location of subject, attribute, and test factors can be either a column address
...
This parameter may be omitted if pcf.systems includes exactly TWO elements,
and then it is assumed that every pair == pcf.systems.
...
class PairedCompFile(pc_base.PairedCompFile):
    ...
    def __init__(self, file_path, pcf,
        ...
        :param pair: (optional) tuple with two column addresses defining (stim0, stim1)
            may be None if there are only two systems in total for comparison
        ...
        :param test_factors: (optional) dict with elements (tf, location), where
            ...
            location is a string with a column (top) address like 'D4',
    ...
    def __iter__(self):
        ...
        if len(sheets) == 0:
            raise FileFormatError('No sheets found in {file_path.stem}')
    ...
    def _get_pair(self, ws, row):
        ...
        if self.pair is None:
            if len(self.pcf.systems) == 2:
                return tuple(self.pcf.systems)
            else:
    ...
    def _get_response(self, ws, row, pair):
        ...
        if self.difference is None:
            ...
            try:
                response_magn = self.pcf.response_labels.index(response_label)
...
def _get_value(ws, col, row):
    ...
    if col == 'sheet':
        return ws.title
    # c = ws[col + str(row)]
    c = row[column_index_from_string(col) - 1]
    return c.value
