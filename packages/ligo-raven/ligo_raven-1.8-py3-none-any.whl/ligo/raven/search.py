...
def calc_signif_gracedb(se, exttrig, tl, th, incl_sky=False):
    ...
    if incl_sky:
        ...
        far = (th - tl) * gcn_rate * se.far / skymap_overlap_integral
        ...
        message = "RAVEN: Spatiotemporal coincidence with external trigger <a href='{0}".format(gracedb_events_url + link1)
        ...
        message = "RAVEN: Spatiotemporal coincidence with superevent <a href='{0}".format(gracedb_events_url + link2)
        message += "{0}'>{1}</a> gives a coincident FAR = {2} Hz".format(se.graceid, se.graceid, far)
        exttrig.submit_gracedb_log(message, tag_name=["ext_coinc"])
        return
    ...
    # Otherwise, proceed with only time coincidence.
        ...
        message = "RAVEN: Temporal coincidence with superevent <a href='{0}".format(gracedb_events_url + link2)
        message += "{0}'>{1}</a> gives a coincident FAR = {2} Hz".format(se.graceid, se.graceid, far)
        exttrig.submit_gracedb_log(message, tag_name=["ext_coinc"])
        return
