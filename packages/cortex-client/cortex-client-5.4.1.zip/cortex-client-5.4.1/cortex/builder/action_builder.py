...
class ActionBuilder:
    ...
    def _get_docker_repository(self, use_prefix=False):
        # If the image name is explicitly set, we ignore the registry property and also override our default image naming scheme
    ...
    def _dump_file(to_dir, file_name, file_contents):
        with closing(Path(to_dir, file_name).open('w')) as f:
            f.write(file_contents)

    @staticmethod
    def _docker_build_and_push(build_dir, repository):
        d = docker.from_env()
        cli = d.api
        ...
        try:
            with Timer() as t:
                stream = cli.push(repository, stream=True)
                for obj in json_stream(stream):
                    if 'status' in obj:
                        match = re.search(
    ...
    def _deploy_daemon_image(self, source, func_name, global_code):
        ...
        try:
            ...
            self._docker_build_and_push(temp_dir, self._get_docker_repository())
        ...
        with tempfile.NamedTemporaryFile(mode='w+b') as temp:
            ...
            if not content_client.exists(model_key):
                ...
                with closing(open(temp.name, 'rb')) as f:
                    content_client.upload(key=model_key, stream_name='model', content_type='application/octet-stream',
                                          stream=f)
