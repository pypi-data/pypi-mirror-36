...
def _call_scons_target(target, command, extra_args=None):
    """ Call scons to run specific command """
    if os.path.exists('ucube.py') == True:
        make_args = ['scons -f ucube.py COMMAND=' + command]
    else:
        make_args = ['scons -f build/ucube.py COMMAND=' + command]
...
def upload(target, work_path=None, bin_dir=None):
    ...
    _call_scons_target(target, 'upload', extra_args)
...
def debug(target, work_path=None, bin_dir=None, start_client=False, gdb_args=None):
    ...
    _call_scons_target(target, 'debug', extra_args)
