...
class Installer(build_ext):
	...
	def get_default_cmake_args(self, output_dir):
		...
		if "CXX" in os.environ.keys() or "CC" in os.environ.keys():
			cxx = os.environ["CXX"] if "CXX" in os.environ.keys() else os.environ["CC"]
			cmake_args.append("-DCMAKE_CXX_COMPILER:FILEPATH='{}'".format(cxx))
