    ...
    if constants.Metric.PrecisionRecall in metrics:
        ...
        try:
            if class_labels is not None:
                ...
                all_recall = np.unique(np.concatenate([recall['class_' + str(label)] for label in class_labels]))
                mean_precision = np.zeros_like(all_recall)
                for label in class_labels:
                    if label in y_test_labels:
                        label = str(label)
                        f = scipy.interpolate.interp1d(recall['class_' + label], precision['class_' + label],
                                                       kind='zero')
        ...
        try:
            if class_labels is not None:
                ...
                all_false_positive_rate = \
                    np.unique(np.concatenate([false_positive_rate['class_' + str(label)] for label in class_labels]))
                mean_true_positive_rate = np.zeros_like(all_false_positive_rate)
                for label in class_labels:
                    if label in y_test_labels:
                        label = str(label)
                        f = scipy.interpolate.interp1d(
                            false_positive_rate['class_' + label], true_positive_rate['class_' + label],
