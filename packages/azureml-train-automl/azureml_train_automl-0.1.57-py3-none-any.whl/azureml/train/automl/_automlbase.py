...
class _AutoMLBase(object):
            ...
            if os.path.exists(data_script):
                ...
                if self.automl_settings.path is not None:
                    ...
                    if os.path.exists(script_path):
                        ...
                        raise DataException(
                            "Could not find data script with name '{0}' or '{1}'".format(data_script, script_path))
            try:
        ...
        if run_configuration is None:
            ...
            if compute_target is not None:
                ...
                self._console_logger.write(
                    'No run_configuration provided, running on {0} with default configuration\n'.format(
                        compute_target.name))
            else:
                self._console_logger.write(
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        if X is None and y is None and data is None:
            ...
            if isinstance(X, pd.DataFrame):
                ...
                X_valid = _dataprep_utilities.try_retrieve_pandas_dataframe(
                    X_valid)
                y_valid = _dataprep_utilities.try_retrieve_numpy_array(
                    y_valid)
        ...
        if hasattr(self.automl_settings, 'enable_ensembling') and self.automl_settings.enable_ensembling:
            enable_ensembling = True

        if not existing_run:
            iteration_count = self.automl_settings.iterations
        ...
        try:
            ...
            bad_scores = _metrics.get_worst_values(
    ...
    def _ensembling_iteration(self, ci, X, y, sample_weight, X_valid, y_valid, sample_weight_valid,
                              cv_splits_indices=None):
        ci.print_start("Ensembling")
        start_iter_time = datetime.utcnow()
        validation_scores = {'metrics': None}
        validation_scores = automl.fit_ensemble(self.automl_settings,
                                                self.parent_run_id,
                                                X,
                                                y,
                                                sample_weight=sample_weight,
        ...
        ci.print_pipeline("Model Ensembling")
    ...
    def _fit_remote(self, run_configuration, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
        ...
        _AutoMLBase._modify_run_configuration(run_config_object)
        ...
        self.logger.info("Starting a snapshot run (snapshotId : {0}) with following compute definition: {1}".format(
    ...
    def _modify_run_configuration(run_config):
        ...
        # TODO: this will replace user extra index, we should support multiple
        # indecies
        if not any("azuremlsdk" in op for op in dependencies.pip_options):
            dependencies.set_pip_dependencies(
                "--extra-index-url {0}".format(CondaDependencies.sdk_origin_url()))
