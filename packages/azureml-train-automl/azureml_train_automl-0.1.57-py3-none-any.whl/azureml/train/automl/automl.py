...
from . import _pipeline_spec
from ._automl_settings import _AutoMLSettings
from ._datasets import _ClientDatasets
from .ensemble import Ensemble
from .model_wrappers import LightGBMClassifier, SparseNormalizer, TruncatedSVDWrapper  # noqa F401
from .model_wrappers import CalibratedModel, FastICA, LightGBMRegressor, LinearSVMWrapper, NBWrapper  # noqa F401
from .model_wrappers import NuSVCWrapper, SGDClassifierWrapper, SVCWrapper  # noqa F401
from .exceptions import DataException, ServiceException
from ._logging import get_logger
from ._metrics import minimize_or_maximize
from .preprocess import DataTransformer, LaggingTransformer
from ._resource_limits import default_resource_limits, safe_enforce_limits
from ._limit_function_call_for_win import enforce_time_limit
from ._runner_legacy import _ClientRunner
...
from sklearn import pipeline, preprocessing, linear_model, decomposition, ensemble  # noqa F401
from sklearn.metrics import recall_score, precision_score  # noqa F401
from sklearn.model_selection import cross_val_score  # noqa F401
from sklearn.pipeline import make_pipeline
from azureml.core import Experiment, Run
from azureml.telemetry import set_diagnostics_collection, get_telemetry_log_handler
...
def set_problem_info(X, y, task_type, current_run=None, workspace=None,
    ...
    if preprocess and not scipy.sparse.issparse(X):
        _, _, X, _ = _transform_data(None, X, y, None, task_type,
                                     x_raw_column_names, lag_length)
...
def _transform_data(logger, X, y, X_valid, task_type, x_raw_column_names,
                    lag_length):
    """
    Apply data transfomers for input data

    :param logger: Logger for intermediate messages
    :param X_valid: Validation data
    :param task_type: Validation labels
    :param x_raw_column_names: Column headers for the data
    :param lag_length: Length by which to lag the data
    :return:
    """
    if logger:
    ...
    try:
        X = _add_raw_column_names_to_X(x_raw_column_names, X)
        transformer, X = _get_transformer_x(X, y, task_type, logger)
    except ValueError:
        raise Exception(
    ...
    if X_valid is not None:
        try:
            X_valid = _add_raw_column_names_to_X(x_raw_column_names, X_valid)
    ...
    if lag_length > 0:
        lag_transformer = LaggingTransformer(lag_length)
        X = lag_transformer.fit_transform(X)
        if X_valid is not None:
            X_valid = lag_transformer.fit_transform(X_valid)
        if logger:
            logger.info("lagging transformer is enabled with length {}.".format(lag_length))

    if isinstance(X, pd.DataFrame):
        # X should be a numpy array
        X = X.values
    return transformer, lag_transformer, X, X_valid


def fit_ensemble(automl_settings,
                 parent_run_id,
                 X,
                 y,
                 sample_weight=None,
                 ...
                 logger=None):
    """
    Runs the ensembling iteration for the AutoML experiment.

    :param automl_settings: Settings for running this fit.
    :type automl_settings: str or dict
    :param parent_run_id: Parent run id for this AutoML experiment.
    :type parent_run_id: str
    :param X: Input data.
    :type X: numpy.ndarray or panda.DataFrame
    ...
    :param X_valid: X validation.
    :type X_valid: numpy.ndarray or pandas.DataFrame
    :param y_valid: y validation.
    :type y_valid: numpy.ndarray or pandas.DataFrame
    :param sample_weight_valid: validation set sample weights.
    :type sample_weight: numpy.ndarray or pandas.DataFrame
    :param cv_splits_indices: cross validation split indices.
    ...
    :type iteration: int
    :param parent_run: The parent run for this AutoML experiment.
    :type parent_run: azureml.core.run.Run
    ...
    :return: dict of child run properties for the ensemble run.
    :rtype: dict
    """
    ...
    if isinstance(automl_settings, dict):
        automl_settings = _AutoMLSettings(
            experiment=experiment, **automl_settings)

    if hasattr(automl_settings, 'ensemble_iterations'):
        ensemble_iterations = automl_settings.ensemble_iterations
    else:
    ...
    ensemble_run_id = parent_run_id + "_ensemble"
    if remote:
        # the ensembling run is already created
        current_run = Run.get_submitted_run()
        parent_run = Run(current_run.experiment, parent_run_id)
    elif parent_run is None:
        raise ValueError("parent_run parameter is required")
    else:
        # create a child run for the ensembling iteration
        logger.info("Creating ensembling child run ({0})".format(ensemble_run_id))
        current_run = parent_run.child_run(name="Ensembling run", run_id=ensemble_run_id)
        properties = {
    ...
    run_properties = parent_run.get_properties()
    child_runs = parent_run.get_children(_rehydrate_runs=False)
    primary_metric = run_properties['primary_metric']
    ...
    for child_run in child_runs:
        ...
        if child_run.id == ensemble_run_id:
            continue
        try:
    ...
    estimators = zip(algo_names, fitted_models)
    ens = Ensemble(logger=logger,
                   estimators=estimators,
                   task_type=task_type,
                   primary_metric=primary_metric,
                   iterations=ensemble_iterations)

    ensembling_pipeline = make_pipeline(ens)
    ...
    try:
        ...
        results, status = runner.run_ensembling(dataset=dataset,
                                                pipeline=ensembling_pipeline,
        ...
        logger.info(
            "Ensembling execution finished with a score of {0}".format(score))

        if remote:
            ensemble_run = Run.get_submitted_run()
        else:
            ensemble_run = Run(experiment=experiment, run_id=ensemble_run_id)

        fit_output = {
            "staticProperties": {},
            "score": score,
            "run_preprocessor": "",
            "run_algorithm": "EnsemblingAlgo",
            "pipeline_id": "0",
            "metrics": score_valid,
            "training_type": training_type,
            ...
            "pipeline_python_obj": ensembling_pipeline,
        }

        dependencies = {"dependencies_versions": None}
        dependencies['dependencies_versions'] = json.dumps(get_sdk_dependencies())

        logger.info("Start logging metrics for ensembling run.")
        _populate_scores_in_fit_output(ensemble_run, fit_output, score_valid)

        fit_output_str = _sanitize_fit_output(fit_output)
        ensemble_run.set_tags(fit_output_str)

        with(open(model_output.name, 'wb')):
            pickle.dump(fitted_pipeline, model_output)
            model_output.flush()
        with(open(model_output.name, 'rb')):
            ensemble_run.upload_file(constants.MODEL_PATH, model_output.name)
        model_output.close()
        ensemble_run.complete()
    except Exception as e:
        ...
        if ensemble_run is not None and fit_output_str is not None:
            ensemble_run.add_properties(fit_output_str)
            ensemble_run.add_properties(dependencies)
...
def fit_pipeline(pipeline_script,
    ...
    :param project: Deprecated parameter - replaced by experiment.
    ...
    if isinstance(automl_settings, dict):
        telemetry_verbosity = automl_settings.get('telemetry_verbosity', logging.getLevelName(logging.NOTSET))
        send_telemetry = automl_settings.get('send_telemetry', False)
        automl_settings = _AutoMLSettings(
            experiment=experiment, **automl_settings)
        automl_settings.send_telemetry = send_telemetry
        automl_settings.telemetry_verbosity = telemetry_verbosity
    ...
    if remote:
        child_run_metrics = Run.get_submitted_run()
    else:
    ...
    try:
        ...
        transformer, lag_transformer = None, None
        if preprocess is True or preprocess == "True":
            # Read raw feature names if they are available
            if fit_iteration_parameters_dict is not None and 'x_raw_column_names' in fit_iteration_parameters_dict:
                x_raw_column_names = fit_iteration_parameters_dict['x_raw_column_names']
            else:
                x_raw_column_names = None

            transformer, lag_transformer, X, X_valid = \
                _transform_data(logger, X, y, X_valid,
                                automl_settings.task_type, x_raw_column_names,
                                automl_settings.lag_length)
        ...
        try:
            if enforce_time_on_win_required:
                ...
                results, status = _train_pipeline(runner=runner, dataset=dataset, pipeline_spec=pipeline_spec,
                                                  training_type=training_type)
            ...
            if results is not None and len(results) > 2:
                if training_type in \
                    ...
                    results = results[0], results[1], results[2], result_full[3]
        except Exception as e:
        ...
        _populate_scores_in_fit_output(child_run_metrics, fit_output, score_valid)
        ...
        if (transformer is not None or lag_transformer is not None) and \
            ...
            fitted_pipeline = _add_transformer_x(
                transformer, lag_transformer, fitted_pipeline)
        ...
        with(open(model_output.name, 'wb')):
            pickle.dump(fit_output['fitted_pipeline'], model_output)
            model_output.flush()
        with(open(model_output.name, 'rb')):
            child_run_metrics.upload_file(
                constants.MODEL_PATH, model_output.name)
        model_output.close()
...
def _train_pipeline(runner, dataset, pipeline_spec, training_type):
    return runner.run_type(dataset, pipeline_spec, training_type=training_type)
...
def _get_training_args(metrics,
    ...
    if pipeline_script:
        _, pipeline_spec = _get_pipeline(
            pipeline_script, problem_info, is_sparse)
    runner = _ClientRunner(dataset, metrics=metrics, task_type=task_type, runtime_constraints=runtime_constraints)
...
def _train_pipeline_on_win_spawn_process(metrics,
    ...
    from ._datasets import _ClientDatasets  # noqa: F401,F811
    from .model_wrappers import LightGBMClassifier, SparseNormalizer, TruncatedSVDWrapper  # noqa: F401,F811
    from .model_wrappers import CalibratedModel, FastICA, LightGBMRegressor, LinearSVMWrapper  # noqa: F401,F811
    from .model_wrappers import NBWrapper, NuSVCWrapper, SGDClassifierWrapper, SVCWrapper  # noqa: F401,F811
    from .exceptions import DataException, ServiceException  # noqa: F401,F811
    from ._metrics import minimize_or_maximize  # noqa: F401,F811
    from .preprocess import DataTransformer, LaggingTransformer  # noqa: F401,F811
    from ._runner_legacy import _ClientRunner   # noqa: F401,F811
    ...
    try:
        runner, dataset, _, training_type = \
            _get_training_args(metrics,
                               ...
                               y_min,
                               y_max)
        return _train_pipeline(runner=runner, dataset=dataset, pipeline_spec=pipeline_spec,
                               training_type=training_type)
    except Exception as e:
        return None, e
...
def _train_pipeline_enforce_time_limit_on_windows(metrics,
    ...
    :return:
    '''
    dict = {"metrics": metrics, "X": X, "y": y, "sample_weight": sample_weight,
            "X_valid": X_valid, "y_valid": y_valid, "sample_weight_valid": sample_weight_valid,
            "frac_valid": frac_valid, "cv_splits_indices": cv_splits_indices, "num_cv_folds": num_cv_folds,
            "num_classes": num_classes, "task_type": task_type, "y_min": y_min, "y_max": y_max,
            "pipeline_spec": pipeline_spec}
    return enforce_time_limit(max_time_sec, _train_pipeline_on_win_spawn_process, dict)
    ...
    if X_valid is not None:
        ...
        if(num_cv_folds == 0 or num_cv_folds is None) and cv_splits_indices is None:
            training_type = _get_training_type(
                constants.TrainingType.TrainAndValidation)
            if frac_valid <= 0.0:
                assert_failures.append('frac_valid must be greater than zero.')
        else:
...
def _get_pipeline(pipeline_script, problem_info, is_sparse):
    """
    ...
    try:
        pipeline_script = pipeline_script.replace(SOURCE_WRAPPER_MODULE,
                                                  MAPPED_WRAPPER_MODULE)
...
def _get_pipeline_from_dict(pipeline_dict, problem_info, is_sparse):
    """
    ...
    scaler = [o for o in pipeline_dict["objects"]
              if o['spec_class'] == _pipeline_spec.PREPROC_NAME and o['class_name'] == 'StandardScaler']
    if len(scaler) == 1:
        scaler[0]['class_name'] = 'StandardScalerWrapper'
        scaler[0]['module'] = MAPPED_WRAPPER_MODULE

    spec = _pipeline_spec.PipelineSpec.from_dict(pipeline_dict)
    pipeline = spec.instantiate_pipeline_spec(
        problem_info, is_sparse=is_sparse)
...
def _add_raw_column_names_to_X(x_raw_column_names, X):
    ...
    if x_raw_column_names is not None:
        if x_raw_column_names.shape[0] != X.shape[1]:
            raise DataException("Number of raw column names " +
                                x_raw_column_names.shape[0] +
...
def _populate_scores_in_fit_output(child_run, fit_output, score_valid):
    delete_from_score_valid = {}
    for m in score_valid:
        fit_output[m] = score_valid[m]
        if isinstance(score_valid[m], float):
            ...
            delete_from_score_valid[m] = score_valid[m]
    ...
    for key in delete_from_score_valid:
        del score_valid[key]
