...
#      compute_metrics, get_default_metrics)
#  from automl.client.core.common.resource_limits import safe_enforce_limits
from . import constants
from . import _resource_limits
from .constants import TrainingResultsType
from ._metrics import compute_metrics, get_default_metrics
from ._resource_limits import safe_enforce_limits
...
class _ClientRunner(object):
    ...
    def __init__(self,
        ...
        """
        :param pipelines:
        :param datasets:  A _ClientDatasets object.
        :param metrics:
        ...
        if metrics is None:
            metrics = get_default_metrics(self.task)
    ...
    def _run_train_valid(self, dataset, pipeline_spec,
        ...
        X_valid, y_valid = dataset.get_valid_set()
    ...
    def _run_cv(self, dataset, pipeline_spec, problem_info,
        ...
        for X_train, y_train, X_test, y_test in dataset.get_CV_splits():
            if y_min is None:
    ...
    def _run_cv_mean(self, dataset, pipeline_spec, problem_info,
        ...
        # TODO// marking fit_model as None to pass code autopep8 test for
        # checkin only. Confirmed with madhu that this file needs to be
        # merged and is not currently in the active flow
        fit_models = None
        return mean_scores, mean_time, models, fit_models

    def _get_subsampled_dataset(self, original_dataset, subsample_percent):
        """
        ...
        dataset = copy.copy(original_dataset)
        original_n = len(original_dataset['train indices'])
        subsample_frac = float(subsample_percent) / 100.0
        subsample_n = int(original_n * subsample_frac)
        dataset['train indices'] = dataset['train indices'][:subsample_n]
        assert subsample_n == len(dataset['train indices'])
        # make sure we didn't modify original_dataset
        assert original_n == len(original_dataset['train indices'])

        # for CV
        dataset['train CV splits'] = copy.copy(original_dataset['train CV splits'])
        for i, item in enumerate(dataset['train CV splits']):
            train, test = item
            original_n = len(train)
            subsample_n = int(original_n * subsample_frac)
            sub_train = train[:subsample_n]
            dataset['train CV splits'][i] = (sub_train, test)

            assert subsample_n == len(dataset['train CV splits'][i][0])
            assert original_n == len(original_dataset['train CV splits'][i][0])
    ...
    def _run(self, dataset, pipeline_spec, problem_info, sets_to_run,
        ...
        if subsample_percent is not None:
            ...
            results[TrainingResultsType.TRAIN_PERCENT] = subsample_percent
        ...
        if constants.TrainingType.TrainValidateTest in sets_to_run:
            ...
            try:
                ...
                results[TrainingResultsType.TRAIN_METRICS] = compute_metrics(
                ...
                results[TrainingResultsType.TEST_METRICS] = compute_metrics(
                    y_pred_test, y_test, num_classes=num_classes,
                    metrics=self.metrics, task=self.task, y_max=y_train.max(),
                    y_min=y_train.min())
            except ValueError as e:
    ...
    def _run_prewrap(self, fn, dataset, pipeline_spec, problem_info,
        ...
        if pipeline_spec.supports_constrained_fit():
            c = _resource_limits.default_resource_limits
