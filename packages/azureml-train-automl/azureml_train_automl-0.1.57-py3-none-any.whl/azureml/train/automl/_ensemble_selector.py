...
class EnsembleSelector(object):
    ...
    def select(self,
        ...
        Arguments:
            X_valid {np.ndarray} -- The test samples
        ...
        for i in range(self.model_count):
            single_model_scores[i] = \
                self._get_model_score(
                    predictions[:, :, i], y_valid, num_classes)
        self.logger.info(
        ...
        for iteration in range(number_of_models_needed):
            ...
            for j, model in enumerate(self.fitted_models):
                ...
                scores[j] = self._get_model_score(
                    temp_predictions, y_valid, num_classes)
    ...
    def _get_model_score(self,
        ...
        all_metrics = _metrics.compute_metrics(y_pred=y_pred,
                                               y_test=y_valid,
