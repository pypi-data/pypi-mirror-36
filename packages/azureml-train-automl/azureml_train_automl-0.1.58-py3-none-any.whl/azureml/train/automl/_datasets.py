...
# from automl.client.core.common import problem_info
# from automl.client.core.common.utils import sparse_std, sparse_isnan
from . import constants
from . import _problem_info
from ._utils import sparse_std, sparse_isnan


class _ClientDatasets(object):
    """
    ...
    def get_full_set(self):
        ...
        sample_weight = self._dataset['sample_weight']
    ...
    def get_valid_set(self):
        """
        returns teh validation part of the dataset
        :return:
        """
        X, y = self._dataset['X'], self._dataset['y']
        sample_weight = self._dataset['sample_weight']
        ...
        if 'X_valid' in self._dataset:
            ...
            X_valid, y_valid = X[self._dataset['valid indices']],\
                y[self._dataset['valid indices']]
            sample_weight_valid = None \
                if sample_weight is None \
                else sample_weight[self._dataset['valid indices']]

        return X_valid, y_valid, sample_weight_valid

    def get_test_set(self):
        """
        returns the test part of dataset
        :return:
        """
        X, y = self._dataset['X'], self._dataset['y']
        sample_weight = self._dataset['sample_weight']
        if 'X_test' in self._dataset:
            ...
            sample_weight_test = sample_weight[self._dataset['test indices']]
    ...
    def get_CV_splits(self):
        ...
        :return:
        """
        sample_wt = self._dataset['sample_weight']
        for train_ind, test_ind in self._dataset['train CV splits']:
    ...
    def add_data(self, attr, val):
        ...
        :param attr: teh attribute to set the value
    ...
    def get_problem_info(self):
        ...
        :return:
        """
        return _problem_info._ProblemInfo(
    ...
    def _init_dataset(self, name, X, y, task, sample_weight=None, categorical=None,
                      openml_id=None, num_classes=None, y_min=None, y_max=None,
                      init_all_stats=True):
        """
    ...
    def parse_simple_train_validate(self,
        ...
        self._init_dataset(
            name, X, y, sample_weight=sample_weight, task=task, num_classes=num_classes)
        self._init_dataset(
            name, X, y, sample_weight=sample_weight, task=task,
    ...
    def parse_data(self, name, X, y, task, sample_weight=None,
        ...
        self._init_dataset(name=name, X=X, y=y, task=task,
        ...
        if cv_splits_indices is not None:
            ...
            _ClientDatasets._validiate_cv_splits(max_index=N, max_size=len(remaining_ind),
                                                 cv_splits_indices=cv_splits_indices, test_index=test_ind)
            self._dataset['train CV splits'] = cv_splits_indices
            if task == constants.Tasks.REGRESSION:
                self._dataset['bin_info'] = self.get_bin_info(np.min([len(split[1]) for split in cv_splits_indices]),
                                                              self._dataset['y'])
            ...
            for train_index, valid_index in kf.split(remaining_ind):
                ...
                self._dataset['train CV splits'] = splits
                if task == constants.Tasks.REGRESSION:
                    self._dataset['bin_info'] = self.get_bin_info(int(self._dataset['num_samples'] / CV),
                                                                  self._dataset['y'])
            ...
            for i in range(CV):
                remaining_ind = remaining_ind[np.random.permutation(
                    remaining_size)]
                splits.append((remaining_ind[:int(
                    N * (1 - perc_valid))], remaining_ind[int(N * (1 - perc_valid)):]))
                assert np.intersect1d(splits[-1][0], test_ind).shape[0] == 0
            self._dataset['train CV splits'] = splits
            if task == constants.Tasks.REGRESSION:
                self._dataset['bin_info'] = self.get_bin_info(int(self._dataset['num_samples'] * perc_valid),
                                                              self._dataset['y'])

        # Split train in train and validation

        # TODO: Remove below legacy code and refactor/fix all the code paths that rely on it (such as refit)
        if not perc_valid:
            perc_valid = 0.1
    ...
    def get_bin_info(self, n_val, y_test):
        """
        creates ditctionary y binned with points in y.
        :param n_val:  number of items
        :param y_test:  label to be binned.
        :return:
        """
        default_num_bins, min_points_per_bin, percentile, extra_outlier_bins = 100, 10, 1, 2
        if int(n_val / default_num_bins) >= min_points_per_bin:
            num_bins_major_perc = default_num_bins
        else:
            num_bins_major_perc = int(n_val / min_points_per_bin) - extra_outlier_bins
        num_bins = num_bins_major_perc + extra_outlier_bins
        first_bin_end, last_bin_start = np.percentile(y_test, [percentile, 100 - percentile])
        bin_dict = {'number_of_bins': num_bins, 'bin_starts': np.zeros(num_bins), 'bin_ends': np.zeros(num_bins)}
        bin_width_major_perc = (last_bin_start - first_bin_end) / num_bins_major_perc
        bin_dict['bin_starts'][0], bin_dict['bin_starts'][-1] = y_test.min(), last_bin_start
        bin_dict['bin_ends'][0], bin_dict['bin_ends'][-1] = first_bin_end, y_test.max()
        bin_dict['bin_starts'][1:num_bins - 1] = [first_bin_end + x * bin_width_major_perc
                                                  for x in range(num_bins_major_perc)]
        bin_dict['bin_ends'][1:num_bins - 1] = bin_dict['bin_starts'][2:num_bins]
        return bin_dict
    ...
    def get_subsampled_dataset(self, subsample_percent):
        ...
        :return: Another _ClientDatasets object that is a subsample.
        """
        assert subsample_percent in set(range(5, 105, 5))
        # shallow copy the dataset
        ret = _ClientDatasets()
        ret._dataset = copy.copy(self._dataset)
        original_n = len(self._dataset['train indices'])
        subsample_frac = float(subsample_percent) / 100.0
        subsample_n = int(original_n * subsample_frac)
        ret._dataset['train indices'] = (
            self._dataset['train indices'][:subsample_n])
        assert subsample_n == len(ret._dataset['train indices'])
        # make sure we didn't modify original_dataset
        assert original_n == len(self._dataset['train indices'])

        # for CV
        ret._dataset['train CV splits'] = copy.copy(
            self._dataset['train CV splits'])
        for i, item in enumerate(ret._dataset['train CV splits']):
            train, test = item
            original_n = len(train)
            subsample_n = int(original_n * subsample_frac)
            sub_train = train[:subsample_n]
            ret._dataset['train CV splits'][i] = (sub_train, test)

            assert subsample_n == len(ret._dataset['train CV splits'][i][0])
            assert original_n == len(self._dataset['train CV splits'][i][0])
    ...
    @staticmethod
    def get_dataset(self, name, X, y, task, sample_weight=None, missing_data=False, categorical=None,
                    seed=123, perc_test=0.1, perc_valid=0.1, CV=10, cv_splits_indices=None,
                    openml_id=None, y_min=None, y_max=None):
        """
        ...
        :return:
        """
        _ClientDatasets._check_data(X, y, categorical, missing_data)
        if categorical is not None:
            X, y = _ClientDatasets._encode_data(X, y, categorical)

        ret = _ClientDatasets()
        self.parse_data(name=name, X=X, y=y, sample_weight=sample_weight, categorical=categorical,
                        task=task, seed=seed,
                        perc_test=perc_test, perc_valid=perc_valid, CV=CV,
                        cv_splits_indices=cv_splits_indices, openml_id=openml_id, y_min=y_min, y_max=y_max)
        return ret

    @staticmethod
    def _validiate_cv_splits(max_index, max_size, cv_splits_indices, test_index=None):
        """
        ...
        for train, valid in cv_splits_indices:
            ...
            if len(train) + len(valid) > max_size:
                raise ValueError(
                    "train index or valid index can not exceed size {0}".format(max_size))
            if np.intersect1d(train, valid).shape[0] != 0:
                raise ValueError("train index  in cv split index and valid index have common values,"
                                 "cv-split indices should be disjoint")

            if test_index is not None and len(test_index) > 0:
                if np.intersect1d(train, test_index).shape[0] != 0:
                    raise ValueError("train index  in cv split index and test_index have common values, "
                                     "cv-split indices and test index should be disjoint")
                if np.intersect1d(valid, test_index).shape[0] != 0:
                    raise ValueError("validation index  in cv split index and test_index have common values cv-split, "
                                     "indices and test index should be disjoint")
