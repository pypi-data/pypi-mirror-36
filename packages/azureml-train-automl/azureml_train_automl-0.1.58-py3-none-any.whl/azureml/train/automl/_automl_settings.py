...
from ._metrics import minimize_or_maximize, get_default_metrics
...
class _AutoMLSettings(object):
    ...
    def __init__(
        ...
        if self.service_url is None and workspace is not None:
            self.service_url = service_discovery.get_history_service_url(workspace._auth_object,
                                                                         workspace.get_details()['id'])
        # Deprecated param
        self.metrics = None
    ...
    def _verify_settings(self):
        ...
        if self.n_cross_validations is not None and self.validation_size is not None:
            if not (self.n_cross_validations > 0) and not (self.validation_size > 0.0):
        ...
        if self.validation_size is not None:
            if self.validation_size > 1.0 or self.validation_size < 0.0:
                raise ValueError("validation_size parameter must be between 0 and 1 when specified.")

        if self.n_cross_validations is not None:
            if self.n_cross_validations == 1 or self.n_cross_validations < 0:
                raise ValueError("n_cross_validations must be greater than or equal to 2 when specified.")

        if self.primary_metric is not None and self.task_type is not None and \
                self.primary_metric not in get_default_metrics(self.task_type):
            raise ValueError("{0} is not a supported metric for {1}. Please use one of the following: {2}".format(
                self.primary_metric,
                self.task_type,
                get_default_metrics(self.task_type)))

        if self.iterations > constants.MAX_ITERATIONS:
            raise ValueError("Number of iterations cannot be larger than {0}.".format(constants.MAX_ITERATIONS))
        ...
        if self.primary_metric is None:
            ...
            if self.task_type is not None:
                if self.primary_metric not in get_default_metrics(self.task_type):
                    raise ValueError("Invalid primary metric specified for {0}. Please use on of: {1}".format(
                        self.task_type, get_default_metrics(self.task_type)
                    ))
            else:
        ...
        if self.enforce_time_on_windows:
            try:
                import psutil
            except Exception:
                self.enforce_time_on_windows = False
                logging.warning("psutil not found, disabling enforce_time_on_windows. "
                                "enforce_time_on_windows requires psutil library to be installed.")
        else:
            if os.name == 'nt':
                try:
                    import psutil
                    self.enforce_time_on_windows = True
                except Exception:
                    pass
