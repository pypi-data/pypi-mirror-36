...
class LightGBMClassifier(ClassifierMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=None, n_jobs=1, **kwargs):
        ...
        if self._min_data_str not in kwargs and self._min_child_samples not in kwargs:
    ...
    def get_model(self):
        ...
        :return: Returns the fitted model if fit method has been called. Else returns None.
        """
        return self.model
...
class FastICA(_AbstractModelWrapper):
    """
    Fast ICA class.
    :param random_state:
        RandomState instance or None, optional (default=None)
...
class SparseNormalizer(TransformerMixin, _AbstractModelWrapper):
    ...
    :param norm: Type of normalization to perform - l1’, ‘l2’, or ‘max’, optional (‘l2’ by default).
    :type norm: str
    """
...
class NBWrapper(BaseEstimator, _AbstractModelWrapper):
    """
    ...
    def fit(self, X, y, **kwargs):
        ...
        if is_sparse and np.any(X < 0).max():
            clf = sklearn.pipeline.Pipeline([('MinMax scaler', SparseScaleZeroOne()),
                                             (self.model_name + 'NB', base_clf)])
            model = clf
        # regular matrix with negative cells
        elif not is_sparse and np.any(X < 0):
            clf = sklearn.pipeline.Pipeline([('MinMax scaler',
                                              sklearn.preprocessing.MinMaxScaler(feature_range=(0, X.max()))),
                                             (self.model_name + 'NB', base_clf)])
            model = clf
...
class TruncatedSVDWrapper(BaseEstimator, TransformerMixin, _AbstractModelWrapper):
    ...
    def __init__(self, min_components=2, max_components=200, random_state=None, **kwargs):
        """
        ...
        self._min_components = 2
        self._max_components = 200
    ...
    def fit(self, X, y=None):
        ...
        """
        args = dict(self.args)
        args[self.n_components_str] = min(self._max_components,
...
class SVCWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    Read more at: http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html.
...
class NuSVCWrapper(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    Read more at: http://scikit-learn.org/stable/modules/generated/sklearn.svm.NuSVC.html.
...
class SGDClassifierWrapper(BaseEstimator, ClassifierMixin):
    ...
    Wrapper around SGD Classifier to support predict probabilities on loss functions
    other than log loss and modified huber loss. This breaks partial_fit on
    loss functions other than log and modified_huber since the calibrated model
    ...
    def predict_proba(self, X):
        ...
        :param X: Input data.
        :type X: numpy.ndarray
        :return: Prediction probability values from SGD Classifier Wrapper model.
        """
        return self.model.predict_proba(X)
...
class CalibratedModel(BaseEstimator, ClassifierMixin, _AbstractModelWrapper):
    ...
    Read more at: http://scikit-learn.org/stable/modules/generated/sklearn.calibration.CalibratedClassifierCV.html.
    """
    ...
    def fit(self, X, y, **kwargs):
        ...
        self.args = kwargs
        out_arrays = train_test_split(*arrays, train_size=self._train_ratio,
...
class LightGBMRegressor(RegressorMixin, _AbstractModelWrapper):
    ...
    def __init__(self, random_state=None, n_jobs=1, **kwargs):
        ...
        if self._min_data_in_leaf not in kwargs and self._min_child_samples not in kwargs:
            raise ValueError("min_data_in_leaf not passed")
    ...
    def get_model(self):
        ...
        :return: :return: Returns the fitted model if fit method has been called. Else returns None
        """
        return self.model
...
class PipelineWithYTransformations(sklearn.pipeline.Pipeline):
    ...
    def fit(self, X, y, y_min=None):
        """
        Fit function for pipeline transform. Perform the fit_transform of y_transformer,
        then fit into the sklearn.pipeline.Pipeline.
    ...
    def predict(self, X):
        ...
        Perform the prediction of sklearn.pipeline.Pipeline, then do the inverse transform
        from y_transformer.
...
class QuantileTransformerWrapper(BaseEstimator, TransformerMixin):
    ...
    :param n_quantiles: Number of quantiles to be computed. It corresponds to the number \
    ...
    Read more at: http://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.QuantileTransformer.html.
    ...
    def __init__(self, n_quantiles=1000, output_distribution="uniform"):
        ...
        :type output_distribution: string
        """
        self.transformer = preprocessing.QuantileTransformer(n_quantiles=n_quantiles,
...
class LogTransformer(BaseEstimator, TransformerMixin):
    ...
    :param safe: If true, truncate values outside the transformer's domain to the nearest point in the domain.
    :type safe: boolean
    ...
    def __str__(self):
        ...
        """
        return "LogTransformer(base=e, y_min=%.5f, scaler=%s, safe=%s)" % (self.y_min if self.y_min is not None else 0,
                                                                           self.scaler,
                                                                           self.safe)
    ...
    def fit(self, y, y_min=None):
        ...
        if self.y_min > self.lower_range:
            ...
            self.scaler = preprocessing.StandardScaler(copy=False, with_mean=False,
                                                       with_std=False).fit(y.reshape(-1, 1))
        else:
            y_max = np.max(y)
            self.scaler = preprocessing.MinMaxScaler(feature_range=(self.lower_range, 1)).fit(
    ...
    def transform(self, y):
        ...
        elif np.min(y) < self.y_min and np.min(self.scaler.transform(y.reshape(-1, 1)).reshape(-1, )) <= 0:
            if self.safe:
                warnings.warn("y_min greater than observed minimum in y, clipping y to "
                              "domain")
                y_copy = y.copy()
                y_copy[y < self.y_min] = self.y_min
                return self.return_y(self.scaler.transform(y_copy.reshape(-1, 1)).reshape(-1, ))
            else:
                raise Exception('y_min greater than observed minimum in y')
        else:
            return self.return_y(self.scaler.transform(y.reshape(-1, 1)).reshape(-1, ))
    ...
    def inverse_transform(self, y):
        ...
        if self.y_min is None:
            ...
            return self.scaler.inverse_transform(np.exp(y).reshape(-1, 1)).reshape(-1, )
...
class PowerTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, power=1, safe=True):
        ...
        if self.power > 0 and (((self.power % 2 == 1) or (1 / self.power % 2 == 1)) or
            ...
            self.scaler = preprocessing.StandardScaler(copy=False,
                                                       with_mean=False,
                                                       with_std=False).fit(np.array([1], dtype=float).reshape(-1, 1))
    ...
    def return_y(self, y, power, invert=False):
        ...
        :type power: double
        :param invert: A boolean whether or not to perform the inverse transform.
        :type invert: boolean
        ...
        if invert:
            return self.scaler.inverse_transform(y_inter.reshape(-1, 1)).reshape(-1, )
        else:
            return y_inter
...
class BoxCoxTransformerScipy(BaseEstimator, TransformerMixin):
    ...
    :param lambda_val: Lambda value for Box Cox transform, will be inferred if not set.
    ...
    def __str__(self):
        ...
        return ("BoxCoxTransformer(lambda=%.3f, y_min=%.5f, scaler=%s, "
                "safe=%s)" % (self.lambda_val if self.lambda_val is not None else 0,
                              self.y_min if self.y_min is not None else 0,
                              self.scaler,
                              self.safe))
    ...
    def transform(self, y):
        ...
        if self.lambda_val is None:
            ...
            if self.safe:
                ...
                y_copy = y.copy()
                y_copy[y < self.y_min] = self.y_min
                return boxcox(self.scaler.transform(y_copy.reshape(-1, 1)).reshape(-1, ), self.lambda_val)
            else:
                raise Exception('y_min greater than observed minimum in y')
        else:
            return boxcox(self.scaler.transform(y.reshape(-1, 1)).reshape(-1, ), self.lambda_val)
    ...
    def inverse_transform(self, y):
        ...
        if self.lambda_val is None:
            ...
            return self.scaler.inverse_transform(inv_boxcox(y, self.lambda_val).reshape(-1, 1)).reshape(-1, )
...
class PreFittedSoftVotingClassifier(VotingClassifier):
    ...
    :param flatten_transform: If True, transform method returns matrix with \
    ...
    def __init__(self, estimators, weights=None, flatten_transform=None, classification_labels=None):
        r"""
        ...
        self.estimators_ = [tuple[1] for tuple in estimators]
...
class PreFittedSoftVotingRegressor(PreFittedSoftVotingClassifier):
    ...
    def __init__(self, estimators, weights=None, flatten_transform=None):
        ...
        :type flatten_transform: boolean
        """
        super().__init__(estimators, weights, flatten_transform, classification_labels=[0])
