...
try:
    ...
    import sys
    import time
    from ._killable_subprocess import kill_process_tree_in_linux
    ...
    def subprocess_func(func, pipe, logger, mem_in_mb, cpu_time_limit_in_s,
        ...
        def handler(signum, frame):
            ...
            if (signum == signal.SIGXCPU):
                # when process reaches soft limit --> a SIGXCPU signal is sent (it
        ...
        # catching all signals at this point turned out to interfer with the
        ...
        if cpu_time_limit_in_s is not None:
            # From the Linux man page: When the process reaches the soft limit, it
        ...
        try:
            ...
            try:
                ...
                # this part should only fail if the parent process is alread dead,
                # so there is not much to do anymore :)
                pass
            finally:
                # recursively kill all children
                pid = os.getpid()
                kill_process_tree_in_linux(pid)
    ...
    class enforce_limits(object):
        ...
        def __init__(self, mem_in_mb=None, cpu_time_in_s=None, wall_time_in_s=None,
            ...
            if self.mem_in_mb is not None:
                self.logger.debug(
                    "Restricting your function to {} mb memory.".format(
                        self.mem_in_mb))
            ...
            if self.grace_period_in_s is not None:
                self.logger.debug(
                    "Allowing a grace period of {} seconds.".format(
                        self.grace_period_in_s))
        ...
        def __call__(self, func):
            ...
            class function_wrapper(object):
                ...
                def __call__(self2, *args, **kwargs):
                    ...
                    try:
                    ...
                    except EOFError as e:  # Don't see that in the unit tests :(
                        self.logger.error(
                        ...
                        # subproc.join hangs in mac, due to empty queue deadlock
                        # join with timeout doesn't work either, so forcing terminate.
                        # finally is called only after the timeout period
