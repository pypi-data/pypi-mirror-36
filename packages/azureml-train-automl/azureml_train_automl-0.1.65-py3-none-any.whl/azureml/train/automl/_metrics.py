...
def get_worst_values(task):
    """Creates a dictionary of the worst possible values for each metric
    A metric that should be maximized gets zero, otherwise the largest float
    :param task: string "classification" or "regression"
    :return: returns a dictionary of the worst metrics for the task
    """
    metrics = get_default_metric_with_objective(task)
    _MAX = constants.OptimizerObjectives.MAXIMIZE
    bad = {m: 0.0 if obj == _MAX else sys.float_info.max
           for m, obj in metrics.items()}
    bad[constants.Metric.R2Score] = -1000.0  # R2 is different
    bad[constants.Metric.Spearman] = -1.0

    # add default metrics bad score here
    bad['default'] = 0.0
    return bad
...
def compute_metrics(y_pred, y_test, metrics=None, num_classes=None,
    ...
    if task == constants.Tasks.CLASSIFICATION:
        return compute_metrics_classification(y_pred, y_test, metrics,
                                              num_classes=num_classes, sample_weight=sample_weight)
    elif task == constants.Tasks.REGRESSION:
        if y_min is None:
            y_min = np.min(y_test)
        if y_max is None:
...
def compute_metrics_classification(y_pred_probs, y_test, metrics,
                                   num_classes=None, sample_weight=None, class_labels=None):
    ...
    if class_labels is not None:
        y_test_binarized = sklearn.preprocessing.label_binarize(y_test, class_labels)
    ...
    if constants.Metric.Accuracy in metrics:
        results[constants.Metric.Accuracy] = try_calculate_metric(score=sklearn.metrics.accuracy_score, y_true=y_test,
    ...
    if constants.Metric.NormMacroRecall in metrics:
        ...
        # for the AutoML challenge https://competitions.codalab.org/competitions/2321#learn_the_details-evaluation
        ...
        cmat = try_calculate_metric(sklearn.metrics.confusion_matrix, y_true=y_test,
                                    y_pred=y_pred_bin, sample_weight=sample_weight)
        if isinstance(cmat, float):
            results[constants.Metric.NormMacroRecall] = float('nan')
        else:
            R = 1 / num_classes
            cms = cmat.sum(axis=1)
            if cms.sum() == 0:
                ...
                results[constants.Metric.NormMacroRecall] = max(
                    0.0,
                    (np.mean(cmat.diagonal() / cmat.sum(axis=1)) - R) / (1 - R))

    if constants.Metric.BalancedAccuracy in metrics:
        classes = np.unique(y_test)
        bac = np.nanmean([try_calculate_metric(sklearn.metrics.roc_auc_score,
                                               y_true=(y_test == k),
                                               y_score=(y_pred_bin == k),
    ...
    if constants.Metric.ConfusionMatrices in metrics:
        ...
        try:
            if class_labels is not None:
                missing_labels = [item for item in class_labels if item not in y_test_labels]
                cm = try_calculate_metric(sklearn.metrics.confusion_matrix, y_true=y_test, y_pred=y_pred_bin,
                                          sample_weight=sample_weight)
                if len(missing_labels) != 0:
                    cm = _add_missing_labels_to_confusion_matrix(cm, missing_labels, class_labels, y_test_labels)
                bac = cm
            ...
            if class_labels is not None:
                ...
                for label in class_labels:
                    ...
                    precision['class_' + label], recall['class_' + label], _ = \
                        _precision_recall_curve(y_test_binarized[:, class_index], y_pred_probs2[:, class_index])
                recall['macro'], precision['macro'] = \
                    _pr_or_roc_macro_average(recall, precision, class_labels, y_test_labels)
                precision['micro'], recall['micro'], _ = \
                    _precision_recall_curve(y_test_binarized.ravel(), y_pred_probs2.ravel())
                bac = reformat_table({'precision': precision, 'recall': recall}, 'precision', 'recall')
            else:
                bac = np.nan
        except ValueError as e:
            bac = np.nan
        ...
        try:
            if class_labels is not None:
                ...
                for label in class_labels:
                    class_index = np.where(class_labels == label)[0][0]
                    label = str(label)
                    false_positive_rate['class_' + label], true_positive_rate['class_' + label], _ = \
                        _roc_curve(y_test_binarized[:, class_index], y_pred_probs2[:, class_index])
                false_positive_rate['macro'], true_positive_rate['macro'] = \
                    _pr_or_roc_macro_average(false_positive_rate, true_positive_rate, class_labels, y_test_labels)
                false_positive_rate['micro'], true_positive_rate['micro'], _ = \
                    _roc_curve(y_test_binarized.ravel(), y_pred_probs2.ravel())
                bac = reformat_table({'true_positive_rate': true_positive_rate,
    ...
    for m in metrics:
        if 'AUC' in m:
            results[m] = try_calculate_metric(sklearn.metrics.roc_auc_score, y_true=y_test_bin,
                                              y_score=y_pred_probs, average=m.replace('AUC_', ''),
                                              sample_weight=sample_weight)

        if 'f1_score' in m:
            results[m] = try_calculate_metric(sklearn.metrics.f1_score, y_true=y_test, y_pred=y_pred_bin,
                                              average=m.replace('f1_score_', ''), sample_weight=sample_weight)

        if 'precision_score' in m and 'average' not in m:
            results[m] = try_calculate_metric(sklearn.metrics.precision_score, y_true=y_test, y_pred=y_pred_bin,
                                              average=m.replace('precision_score_', ''), sample_weight=sample_weight)

        if 'recall_score' in m:
            results[m] = try_calculate_metric(sklearn.metrics.recall_score, y_true=y_test, y_pred=y_pred_bin,
                                              average=m.replace('recall_score_', ''), sample_weight=sample_weight)

        if 'average_precision_score' in m:
            results[m] = try_calculate_metric(sklearn.metrics.average_precision_score, y_true=y_test_bin,
...
def _add_missing_labels_to_confusion_matrix(cm, missing_labels, class_labels, y_test_labels):
...
def confusion_matrices(matrix):
    ...
    :return: dictionary containing 'confusion_matrix' and 'normalized_confusion_matrix'
    """
    try:
        ...
        for row in matrix:
            ...
            for cell in row:
                normalized_row = np.append(
                    normalized_row, cell / row_sum if (row_sum != 0) else row_sum)
        ...
        return {'confusion_matrix': matrix, 'normalized_confusion_matrix': normalized_matrix}
    except Exception as e:
        return float('nan')
...
def _pr_or_roc_avg(dicts, yaxis, xaxis):
    ...
    for key in dicts[0]:
        if xaxis in key:
            key_add = '_' if xaxis + '_macro' == key or xaxis + '_micro' == key else '_class_'
            class_label = key.replace(xaxis + key_add, '')
            yaxis_label = yaxis + key_add + str(class_label)
            all_xaxis[key] = np.unique(np.concatenate([dicts[i][key] for i in range(num_cross_vals)]))
            tmp = np.zeros_like(all_xaxis[key])
            for i in range(num_cross_vals):
                if not (np.isnan(dicts[i][key]).any() or np.isnan(dicts[i][yaxis_label]).any()):
                    f = scipy.interpolate.interp1d(dicts[i][key], dicts[i][yaxis_label],
                                                   kind='zero', fill_value="extrapolate")
                    tmp += f(all_xaxis[key])
            mean_yaxis[yaxis_label] = tmp / num_cross_vals
            all_xaxis[key], mean_yaxis[yaxis_label] = _curve_downsample(all_xaxis[key], mean_yaxis[yaxis_label])
    combined_dict = {**all_xaxis, **mean_yaxis}
    return combined_dict


def _precision_recall_curve(y_true, probas_pred, pos_label=None, sample_weight=None):
    precision, recall, thresholds = \
        sklearn.metrics.precision_recall_curve(y_true, probas_pred, pos_label, sample_weight)
...
def _roc_curve(y_true, y_score, pos_label=None, sample_weight=None, drop_intermediate=True):
    ...
    false_positive_rate, true_positive_rate = _curve_downsample(false_positive_rate, true_positive_rate)
...
def _pr_or_roc_macro_average(x_dict, y_dict, class_labels, y_test_labels):
    all_x_values = np.unique(np.concatenate([x_dict['class_' + str(label)] for label in class_labels]))
    mean_y_values = np.zeros_like(all_x_values)
    for label in class_labels:
        if label in y_test_labels:
            label = str(label)
            f = scipy.interpolate.interp1d(x_dict['class_' + label], y_dict['class_' + label], kind='zero')
            mean_y_values += f(all_x_values)
    mean_y_values /= len(y_test_labels)
    return _curve_downsample(all_x_values, mean_y_values)
...
def compute_metrics_classification_legacy(y_pred, y_test, metrics=None, num_classes=None, sample_weight=None):
    """Legacy function to complete classification metrics
    :rtype: object
    :param y_pred:
    :param y_test:
    :param metrics:
    ...
    if metrics is None:
        metrics = get_default_metrics(task="classification")
    ...
    if 'AUC_macro' in metrics or 'AUC_weighted' in metrics:
        ...
        results['AUC_macro'] = sklearn.metrics.roc_auc_score(y_test_bin, y_pred_bin, average='macro',
                                                             sample_weight=sample_weight)
        results['AUC_weighted'] = sklearn.metrics.roc_auc_score(y_test_bin, y_pred_bin, average='weighted',
                                                                sample_weight=sample_weight)
    ...
    if "norm_macro_recall" in metrics:
        ...
        confusion_matrix = sklearn.metrics.confusion_matrix(
            y_test, y_pred, sample_weight=sample_weight)
        r = 1 / num_classes
        results['norm_macro_recall'] = (np.mean(confusion_matrix.diagonal() /
                                                confusion_matrix.sum(axis=1)) - r) / (1 - r)
    if "balanced_accuracy" in metrics:
        classes = np.unique(y_test)
        bac = np.mean([sklearn.metrics.roc_auc_score(y_test == k, y_pred == k, sample_weight=sample_weight)
                       for k in classes])
        results['balanced_accuracy'] = bac
    # for the new metrics not in the legacy list and each metric to the result to make it compatable.
...
def compute_metrics_regression(y_pred, y_test, metrics, y_max, y_min, sample_weight=None, bin_info=None):
    ...
    if constants.Metric.R2Score in metrics:
        bac = sklearn.metrics.r2_score(
            y_test, y_pred, sample_weight=sample_weight, multioutput='uniform_average')
        results[constants.Metric.R2Score] = np.clip(bac, -10.0, 1.0)
    ...
    if constants.Metric.NormRMSE in metrics:
        bac = np.sqrt(
            sklearn.metrics.mean_squared_error(
                ...
                multioutput='uniform_average'))
        bac = bac / np.abs(y_max - y_min)
        results[constants.Metric.NormRMSE] = np.clip(bac, 0, 1)
    ...
    if constants.Metric.Predictions in metrics:
        ...
        try:
            if bin_info is not None:
                ...
                predvtruedict = {constants.Preds_Reg.TRUE_VALS_BINS_START: bin_info['bin_starts'],
                                 constants.Preds_Reg.TRUE_VALS_BINS_END: bin_info['bin_ends'],
                                 constants.Preds_Reg.AVG_PRED_VALS_PER_BIN: np.zeros(num_bins),
                                 constants.Preds_Reg.PRED_ERROR_PER_BIN: np.zeros(num_bins),
                                 constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN: np.zeros(num_bins)}
                for i in range(len(y_test)):
                    for bin_index in range(len(bin_info['bin_ends'])):
                        if y_test[i] <= bin_info['bin_ends'][bin_index]:
                            pred_indices_per_bin[bin_index] = np.append(pred_indices_per_bin[bin_index], int(i))
                            break
                predvtruedict[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN] = np.array([len(x)
                                                                                       for x in pred_indices_per_bin])
                predvtruedict[constants.Preds_Reg.AVG_PRED_VALS_PER_BIN] = np.array([np.mean(y_pred[x])
                                                                                     if len(x) != 0 else 0
                                                                                     for x in pred_indices_per_bin])
                predvtruedict[constants.Preds_Reg.PRED_ERROR_PER_BIN] = np.array([(np.std(y_pred[x]) ** 2) * len(x)
                                                                                  if len(x) != 0 else 0
                                                                                  for x in pred_indices_per_bin])
                bac = predvtruedict
            else:
...
def _predvstrue_mean(dicts):
    num_cv, tot_bin_count = len(dicts), len(dicts[0][constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN])
    predvtruedict = {constants.Preds_Reg.TRUE_VALS_BINS_START: np.zeros(tot_bin_count),
    ...
    for i in range(tot_bin_count):
        predvtruedict[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i] = \
            np.sum(np.array([dicts[j][constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i] for j in range(num_cv)]))
        if (predvtruedict[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i] != 0):
            weighted_means_sum = np.sum(np.array([dicts[j][constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i] *
            ...
            predvtruedict[constants.Preds_Reg.AVG_PRED_VALS_PER_BIN][i] = \
                weighted_means_sum / predvtruedict[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i]
            std_devs = np.sum(np.array([(dicts[j][constants.Preds_Reg.PRED_ERROR_PER_BIN][i] +
                                         dicts[j][constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i] *
                                         ((dicts[j][constants.Preds_Reg.AVG_PRED_VALS_PER_BIN][i] -
                                           predvtruedict[constants.Preds_Reg.AVG_PRED_VALS_PER_BIN][i]) ** 2))
                                        for j in range(num_cv)]))
            predvtruedict[constants.Preds_Reg.PRED_ERROR_PER_BIN][i] = \
                ((std_devs) / predvtruedict[constants.Preds_Reg.TRUE_VALS_COUNT_PER_BIN][i]) ** (0.5)
        else:
            predvtruedict[constants.Preds_Reg.AVG_PRED_VALS_PER_BIN][i] = 0
            predvtruedict[constants.Preds_Reg.PRED_ERROR_PER_BIN][i] = 0
    predvtruedict[constants.Preds_Reg.TRUE_VALS_BINS_START] = dicts[0][constants.Preds_Reg.TRUE_VALS_BINS_START]
    predvtruedict[constants.Preds_Reg.TRUE_VALS_BINS_END] = dicts[0][constants.Preds_Reg.TRUE_VALS_BINS_END]
