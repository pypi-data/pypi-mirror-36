...
from ._logging import get_logger, cleanup_log_map
from ._problem_info import _ProblemInfo
from .run import AutoMLRun
from ._systemusage_telemetry import SystemResourceUsageTelemetryFactory
from .utilities import _log_traceback, _y_nan_check, friendly_http_exception
...
class _AutoMLBase(object):
    ...
    def __init__(self,
        ...
        self._usage_telemetry = SystemResourceUsageTelemetryFactory.get_system_usage_telemetry(self.logger)
        ...
        if data_script is not None:
            if os.path.exists(data_script):
                ...
                if self.automl_settings.path is not None:
                    script_path = os.path.join(self.automl_settings.path, data_script)
            ...
            try:
                ...
                if not hasattr(self.user_script, 'get_data'):
                    raise DataException("User script does not contain get_data() function.")
                try:
                ...
                raise DataException("Could not import get_data() function from {0}".format(data_script)) from None
        else:
    ...
    def __del__(self):
        ...
        if self.logger is not None:
            while self.logger.handlers:
                handler = self.logger.handlers.pop()
                handler.flush()
                handler.close()
        cleanup_log_map(self.automl_settings.debug_log, self.automl_settings.verbosity)
    ...
    def start_experiment(self):
        ...
        try:
            ...
            self.experiment_id = self._jasmine_client.create_jasmine_experiment(self.automl_settings.name)
        except HttpOperationError as e:
            _log_traceback(e, self.logger)
            friendly_http_exception(e, constants.API.CreateExperiment)
        except Exception as e:
            _log_traceback(e, self.logger)
            raise ServiceException("Error when trying to create experiment in AutoML service.") from None

        self.logger.info('Jasmine created new experiment with id: \'{0}\''.format(self.experiment_id))
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        if X is None and y is None and data is None:
            ...
            if isinstance(X, pd.DataFrame):
                ...
                sample_weight = _dataprep_utilities.try_retrieve_numpy_array(sample_weight)
                X_valid = _dataprep_utilities.try_retrieve_pandas_dataframe(X_valid)
                y_valid = _dataprep_utilities.try_retrieve_numpy_array(y_valid)
                sample_weight_valid = _dataprep_utilities.try_retrieve_numpy_array(sample_weight_valid)
                cv_splits_indices = _dataprep_utilities.try_resolve_cv_splits_indices(cv_splits_indices)
        ...
        if not existing_run:
            ...
            parent_run_dto = CreateParentRunDto(target='local',
                                                ...
                                                raw_aml_settings_string=str(self.automl_settings.__dict__),
                                                aml_settings_json_string=json.dumps(self.automl_settings.__dict__))
            try:
                self.logger.info("Start creating parent run with DTO: {0}.".format(parent_run_dto))
                self.parent_run_id = self._jasmine_client.post_parent_run(self.automl_settings.name, parent_run_dto)
            except HttpOperationError as e:
                _log_traceback(e, self.logger)
        ...
        if not existing_run:
            self._jasmine_client.set_parent_run_status(self.parent_run_id, constants.RunState.START_RUN)
        ...
        try:
            ...
            raise
        if existing_run:
            self.current_iter = len(list(self.current_run.get_children(_rehydrate_runs=False)))
        ...
        try:
            ...
            bad_scores = _metrics.get_worst_values(self.automl_settings.task_type)
            bad_score = bad_scores["default"]
            ...
            while self.current_iter < self.automl_settings.iterations:
                ...
                if (self._status == constants.Status.Terminated):
                    self._console_logger.write("Stopping criteria reached at iteration {0}. Ending experiment.")
    ...
    def _ensembling_iteration(self, ci, X, y, sample_weight, X_valid, y_valid, sample_weight_valid,
        ...
        ensemble_run_id = "{0}_{1}".format(self.parent_run_id, self.current_iter)
        spec_dict = {
            "pipeline_id": pipeline_id,
            "objects": [{
                "module": "azureml.train.automl.ensemble",
                "class_name": "Ensemble",
                "spec_class": _pipeline_spec.SKLEARN_NAME,
                "param_args": (),
                "param_kwargs": {
        ...
        self.current_run.tag('iterations', str(self.automl_settings.iterations))

        current_run = self.current_run.child_run(name="Ensembling run", run_id=ensemble_run_id)
        properties = {
    ...
    def _fit_remote(self, run_configuration, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
        ...
        if isinstance(run_configuration, str):
            run_config_object = RunConfiguration.load(self.automl_settings.path, run_configuration)
        else:
            # we alread have a run configuration object
            run_config_object = run_configuration
            run_configuration = run_config_object.target

        run_config_object = _AutoMLBase._modify_run_configuration(run_config_object)
        ...
        if any(var is not None for var in df_value_list):
            ...
            dataprep_json = _dataprep_utilities.save_dataflows_to_json(dataflow_dict)
        ...
        try:
            ...
            _log_traceback(e, self.logger)
            raise ServiceException("Error occurred when tyring to create new parent run in AutoML service.") from None
        self.current_run = AutoMLRun(self.experiment, self.parent_run_id, host=self.automl_settings.service_url)
        self._console_logger.write("Parent Run ID: " + self.parent_run_id)
        self.logger.info("Parent Run ID: " + self.parent_run_id)

        snapshotId = self.current_run.take_snapshot(self.automl_settings.path)
        self.logger.info("Snapshotted folder: {0} with snapshot_id: {1}".format(self.automl_settings.path, snapshotId))
        ...
        if show_output:
            try:
                ...
                remote_printer = RemoteConsoleInterface(self._console_logger, self.logger)
                ...
                if self.automl_settings.blacklist_algos is None:
                    ...
                    for blacklist_algo in constants.MAX_SAMPLES_BLACKLIST_ALGOS:
                        if blacklist_algo not in self.automl_settings.blacklist_algos:
                            self.automl_settings.blacklist_algos.append(blacklist_algo)
    ...
    def _get_pipeline(self):
        ...
        """
        pipeline_out_dto = self._jasmine_client.get_pipeline(self.parent_run_id, self.current_iter)
        ...
        if self.automl_settings.task_type == constants.Tasks.REGRESSION:
            ...
            if (not self.automl_settings.y_min or not isinstance(self.automl_settings.y_min, float)):
                ...
                if isinstance(self.automl_settings.y_min, numpy_unserializable_ints):
                    self.automl_settings.y_min = int(self.automl_settings.y_min)
            if (not self.automl_settings.y_max or not isinstance(self.automl_settings.y_max, float)):
                ...
                if isinstance(self.automl_settings.y_max, numpy_unserializable_ints):
                    self.automl_settings.y_max = int(self.automl_settings.y_max)
    ...
    def _modify_run_configuration(run_config):
        """
        Takes a run_config object ensures our conda dependencies exist
        """
        run_config.script = "automl.py"
        run_config.auto_prepare_environment = True

        dependencies = run_config.environment.python.conda_dependencies

        if "numpy" not in dependencies.conda_packages:
            dependencies.add_conda_package("numpy")
        if ("azureml-train-automl" not in dependencies.pip_packages and
                "azureml-sdk[automl]" not in dependencies.pip_packages):
