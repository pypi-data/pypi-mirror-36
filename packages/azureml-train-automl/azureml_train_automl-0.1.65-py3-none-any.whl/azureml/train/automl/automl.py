...
def set_problem_info(X, y, task_type, current_run=None, workspace=None,
    ...
    current_run.add_properties({_constants_azureml.Properties.PROBLEM_INFO: problem_info_str})
...
def _transform_data(logger, X, y, X_valid, task_type, x_raw_column_names,
    ...
    try:
        ...
        raise Exception("Cannot preprocess training data. Run after processing manually.")
    ...
    if X_valid is not None:
        try:
            ...
            raise Exception("Cannot preprocess validation data. Run after processing manually.")
...
def fit_pipeline(pipeline_script,
    ...
    automl_settings = _AutoMLSettings.from_string_or_dict(automl_settings, experiment=experiment)
    ...
    try:
        ...
        logger.info("The following metrics have been logged for the child run: {0}.".format(score_valid))
        ...
        with(open(model_output.name, 'rb')):
            child_run_metrics.upload_file(constants.MODEL_PATH, model_output.name)
        model_output.close()
...
def _get_training_type(training_type, folds=0):
    ...
    if training_type not in valid_training_types:
        raise AssertionError("%s and %s are the only supported training types." % valid_training_types)
    ...
    if folds < 0 or folds == 1:
        raise AssertionError("Cross validation folds must be greater than 1, got %d" % folds)
...
def _get_training_args(metrics,
    ...
    pipeline_spec = None
    if pipeline_script:
        _, pipeline_spec = _get_pipeline(pipeline_script, problem_info, is_sparse)
...
def _train_pipeline_enforce_time_limit_on_windows(metrics,
    ...
    dict = {"metrics": metrics, "X": X, "y": y, "sample_weight": sample_weight,
            ...
            "pipeline_spec": pipeline_spec, "is_ensemble_iteration": is_ensemble_iteration}
...
def _get_dataset(X,
    ...
    dataset = _ClientDatasets()
    if X_valid is not None:
        training_type = _get_training_type(constants.TrainingType.TrainAndValidation)

        if not (num_cv_folds == 0 or num_cv_folds is None):
            assert_failures.append('n_cross_validations cannot be specified when X_valid is provided.')

        if not (frac_valid == 0.0 or frac_valid is None):
            assert_failures.append('validation_size cannot be specified when X_valid is provided.')

        if y_valid is None:
            assert_failures.append('y_valid must also be provided when X_valid is provided.')
        ...
        if y_valid is not None:
            assert_failures.append('y_valid should only be provided when X_valid is provided.')
        if(num_cv_folds == 0 or num_cv_folds is None) and cv_splits_indices is None:
            ...
            training_type = _get_training_type(constants.TrainingType.MeanCrossValidation, num_cv_folds)
...
def _get_pipeline_from_dict(pipeline_dict, problem_info, is_sparse):
    ...
    pipeline = spec.instantiate_pipeline_spec(problem_info, is_sparse=is_sparse)
...
def _add_transformer_x(transformer, lag_transformer, pipeline_spec):
    ...
    """
    transformers = filter(lambda x: x is not None, [transformer, lag_transformer])
            ...
            if isinstance(score_valid[m], float):
                ...
                if m == 'confusion_matrices':
                    for key in score_valid[m].keys():
                        score_valid[m][key] = (score_valid[m][key].flatten()).tolist()
                uniform_arr_len = np.max([len(score_valid[m][x]) for x in score_valid[m]])
                for key in score_valid[m]:
                    while len(score_valid[m][key]) != uniform_arr_len:
                        score_valid[m][key] = np.append(score_valid[m][key], np.nan)
