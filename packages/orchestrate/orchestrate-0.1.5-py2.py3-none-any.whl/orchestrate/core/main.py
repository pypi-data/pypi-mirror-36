...
""")
  ...
  def run(self):
    experiment_id = self.runner(local=False, quiet=self.app.pargs.quiet)
...
class OrchestrateClusterController(ArgparseController):
  ...
  def create(self):
    ...
    try:
      cluster = self.app.services.aws_service.create_kubernetes_cluster(options)
      print(safe_format('Successfully created kubernetes cluster: {}', cluster['name']))
    except Exception as e:
      try:
        self.app.services.aws_service.disconnect_kubernetes_cluster(options['cluster_name'])
      except Exception:
        pass
      six.raise_from(
        Exception(safe_format(
    ...
    filename = self.app.services.ec2_service.key_pair_location(cluster['name'])
    print('*Optional:')
    print('\tTo ssh into any ec2 node in your cluster, use the key pair located at:')
