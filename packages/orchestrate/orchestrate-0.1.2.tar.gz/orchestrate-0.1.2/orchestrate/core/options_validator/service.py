...
class OptionsValidatorService(Service):
  ...
  def validate_aws(
    ...
    if region is not None:
      assert is_string(region) and len(region) > 0, safe_format(
        'Please provide a string aws.region: {}',
        region
      )
      assert region in ('us-west-2', 'us-east-1'), safe_format(
        'Unsupported region: {}, supported regions are us-east-1 and us-west-2',
        region
      )
    if ecr is not None:
      assert is_mapping(ecr), safe_format(
        'aws.ecr is not a mapping: {}',
        ecr
      )
      repository = ecr.get('repository')
      assert is_string(repository) and len(repository) > 0, safe_format(
        'Please provided string aws.ecr.repository {}',
        repository
      )
