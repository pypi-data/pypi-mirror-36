...
class FollowPodsRunner(object):
  ...
  def follow(self, pod_name, function):
    ...
    If you'd like to see logs for a specific pod, you can use kubectl directly: `sigopt kubectl logs --follow {}`.
...
class JobLogsService(Service):
  ...
  def follow_logs(self, experiment_id, color_off=False):
    ...
    follow_pods_runner = FollowPodsRunner(self.services.kubernetes_service, kube_config, experiment_id, color_off)
    try:
      while True:
