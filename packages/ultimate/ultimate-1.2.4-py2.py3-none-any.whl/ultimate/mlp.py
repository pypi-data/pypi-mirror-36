# -*- coding: utf-8 -*-
from __future__ import print_function, division, unicode_literals, absolute_import

import sys, os, math

import numpy

_basedir = os.path.dirname(os.path.abspath(__file__))
sys.path.append(_basedir)
from mesh import Mesh, _C_FLOAT, _DTYPE
sys.path.pop()

class MLP(Mesh):
    def __init__(self, 
            mi=0,
            layer_size=[], 
            input_type='pointwise', 
            loss_type='mse', 
            output_range=[0, 1], 
            output_shrink=0.001, 
            importance_mul=0.001,
            bias_rate=[0.005], weight_rate=[], 
            regularization=1):

        self.init(mi)
        self.input_type = input_type
        self.loss_type = loss_type
        self.layer_size = layer_size

        self.set_conf({}) 
        for idx, size in enumerate(layer_size):
            arg = {'Shape': [1,1,size]}
            if idx == 0:
                arg['InputType'] = input_type
                arg['ImportanceMul'] = importance_mul
            if idx == len(layer_size) - 1:
                arg['Regularization'] = regularization
                arg['LossType'] = loss_type
                arg['OutputRange'] = output_range
                arg['OutputShrink'] = output_shrink
            
            self.set_tensor(idx, arg)

        for idx in range(len(layer_size) - 1):
            arg = {'Tin':idx, 'Tout':idx+1, 'Op':'conv'}
            if idx < len(bias_rate) and bias_rate[idx] is not None:
                arg['BiasRate'] = bias_rate[idx]
            if idx < len(weight_rate) and weight_rate[idx] is not None:
                arg['WeightRate'] = weight_rate[idx]
            self.set_filter(idx, arg)
        
        self.run_filler(-1)
    
    def check_arr(self, arr):
        if not isinstance(arr, numpy.ndarray):
            raise NotImplementedError()
        if len(arr.shape) == 1:
            arr = arr.reshape((-1,1))
        if arr.dtype != _DTYPE:
            arr = arr.astype(_DTYPE)
        if not arr.flags['C_CONTIGUOUS']: 
            arr = numpy.ascontiguousarray(arr)
        if len(arr.shape) != 2:
            raise NotImplementedError()
        return arr

    def train(self, in_arr, target_arr, 
        epoch_train=5, epoch_decay=1, iteration_log = 100,
        rate_init=0.06, rate_decay=0.9,
        importance_out=False,
        loss_mul=0.001, verbose=1, shuffle=True):

        in_arr = self.check_arr(in_arr)
        target_arr = self.check_arr(target_arr)
       
        SIZE = len(in_arr)
        if SIZE != len(target_arr):
            raise NotImplementedError()
        
        pos = numpy.arange(SIZE, dtype=numpy.int32)
        rate = rate_init
        loss = None

        for epoch in range(epoch_train):
            if epoch > 0 and epoch % epoch_decay == 0:
                rate *= rate_decay

            if shuffle: self.shuffle(pos)
            for idx in range(SIZE):
                idx_ = pos[idx]  
                loss_ = self.train_one(in_arr[idx_], target_arr[idx_], rate)
                if loss is None: loss = loss_
                else: loss += (loss_ - loss) * loss_mul
                if importance_out:
                    self.importance(0)

                if verbose > 1:
                    if (idx + 1) % iteration_log == 0:
                        print("Iteration %d/%d Epoch %d/%d\n    rate: %g loss: %g" % (idx+1, SIZE, epoch, epoch_train, rate, loss))
                        sys.stdout.flush()
                        if math.isnan(loss): raise ValueError("")
            
            if verbose > 0:
                print("Epoch %d/%d\n    rate: %g loss: %g" % (epoch+1, epoch_train, rate, loss))
                sys.stdout.flush()
                if math.isnan(loss): raise ValueError("")

        if importance_out:
            importance_buf = numpy.zeros((self.layer_size[0],), dtype=_DTYPE)
            self.read_tensor(0, importance_buf, self.FLAG_EX)
            return importance_buf
    
    def predict(self, in_arr, out_arr=None, verbose=0, iteration_log=100):
        if out_arr is None:
            out_arr = numpy.zeros((len(in_arr), self.layer_size[-1]), dtype=_DTYPE)
    
        in_arr = self.check_arr(in_arr)
        out_arr = self.check_arr(out_arr)

        SIZE = len(in_arr)
        if SIZE != len(out_arr):
            raise NotImplementedError()

        for idx in range(SIZE):
            self.predict_one(in_arr[idx], out_arr[idx])

            if verbose > 1:
                if (idx + 1) % iteration_log == 0:
                    print("Iteration %d/%d" % (idx+1, SIZE))

        return out_arr

    def train_one(self, in_buf, target_buf, rate):
        self.clear_tensor(-1, self.FLAG_V | self.FLAG_DV)
        self.clear_filter(-1, self.FLAG_DV)
        self.input(0, in_buf)

        self.forward()

        loss = self.loss(len(self.layer_size) - 1, target_buf)
        
        self.backward()
        self.renew(rate)

        return loss

    def predict_one(self, in_buf, out_buf):
        self.clear_tensor(-1, self.FLAG_V)
        self.input(0, in_buf)
        self.forward()
        
        flag = self.FLAG_V
        if self.loss_type in ["softmax", "hardmax"]:
            flag = self.FLAG_EX

        self.read_tensor(len(self.layer_size) - 1, out_buf, flag)

if __name__ == '__main__':
    m = MLP(layer_size=[2,1])
    print("version", m.info("version"))
    m.show_conf()
    m.show_tensor()
    m.show_filter()
    m.destroy()


