...
class SegmentSyncApp(QtWidgets.QMainWindow, mainwindow.Ui_MainWindow):
    def __init__(self, config):
        ...
        self.lineEdit.textChanged.connect(self.on_filter_changed)
        ...
        self.on_combobox_current_index_changed(0)
    ...
    def set_enable_ui(self, flag):
        ...
        self.refreshListPushButton.setEnabled(flag)
    ...
    def on_filter_changed(self, request):
        filter_text = str(self.lineEdit.text()).lower()
        for row in range(self.listWidget.count()):
            if filter_text in str(self.listWidget.item(row).text()).lower():
