...
from dpaybase.exceptions import AccountDoesNotExistsException
...
class Account(dict):
    ...
    def __init__(self, account_name, dpayd_instance=None):
    ...
    def refresh(self):
        account = self.dpayd.get_account(self.name)
        if not account:
    ...
    def converter(self):
        if not self._converter:
            self._converter = Converter(self.dpayd)
    ...
    def bp(self):
        ...
        return round(self.converter.vests_to_bp(vests), 3)
    ...
    def get_balances(self):
        available = {
            'BEX': Amount(self['balance']).amount,
            'BBD': Amount(self['bbd_balance']).amount,
            'VESTS': Amount(self['vesting_shares']).amount,
        }

        savings = {
            'BEX': Amount(self['savings_balance']).amount,
            'BBD': Amount(self['savings_bbd_balance']).amount,
        }

        rewards = {
            'BEX': Amount(self['reward_dpay_balance']).amount,
            'BBD': Amount(self['reward_bbd_balance']).amount,
            'VESTS': Amount(self['reward_vesting_balance']).amount,
        }

        totals = {
            'BEX':
                sum([available['BEX'], savings['BEX'], rewards['BEX']]),
            'BBD':
                sum([available['BBD'], savings['BBD'], rewards['BBD']]),
            'VESTS':
    ...
    def _get_followers(self, direction="follower", last_user=""):
        if direction == "follower":
            ...
            followers = self.dpayd.get_followers(self.name, last_user, "blog",
                                                  100)
        elif direction == "following":
            followers = self.dpayd.get_following(self.name, last_user, "blog",
                                                  100)
    ...
    def curation_stats(self):
        ...
        reward_7d = self.converter.vests_to_bp(reward_7d)
        reward_24h = self.converter.vests_to_bp(reward_24h)
        return {
            "24hr": reward_24h,
            "7d": reward_7d,
            "avg": reward_7d / 7,
    ...
    def virtual_op_count(self):
        try:
            last_item = self.dpayd.get_account_history(self.name, -1, 0)[0][0]
        except IndexError:
            return 0
        else:
            return last_item
    ...
    def get_account_votes(self):
        return self.dpayd.get_account_votes(self.name)

    def get_withdraw_routes(self):
        return self.dpayd.get_withdraw_routes(self.name, 'all')

    def get_conversion_requests(self):
        return self.dpayd.get_conversion_requests(self.name)
    ...
    def get_account_history(self,
                            ...
                            filter_by=None,
                            raw_output=False):
        """ A generator over dpayd.get_account_history.
        ...
        Args:
            ...
            raw_output (bool): (Defaults to False). If True, return history in
                dpayd format (unchanged).
        """
        history = self.dpayd.get_account_history(self.name, index, limit)
