...
from dpaybase.exceptions import (
    PostDoesNotExist,
    VotingInvalidOnArchivedPost,
)
from dpaybase.operations import CommentOptions
...
class Post(dict):
    """ This object gets instantiated by DPay.streams and is used as an
        ...
        Args:
            ...
            dpayd_instance (DPayd): DPayd node to connect to

    """

    def __init__(self, post, dpayd_instance=None):
        self.dpayd = dpayd_instance or shared_dpayd_instance()
        self.commit = Commit(dpayd_instance=self.dpayd)
    ...
    def refresh(self):
        ...
        post = self.dpayd.get_content(post_author, post_permlink)
        ...
        for p in bbd_amounts:
            post[p] = Amount(post.get(p, "0.000 BBD"))
    ...
    def reward(self):
        ...
        """
        return Amount(self.get("total_payout_value", "0 BBD")) + \
               Amount(self.get("pending_payout_value", "0 BBD"))
    ...
    def export(self):
        ...
        safe_dict = remove_from_dict(self, ['dpayd', 'commit'])
                ...
                "percent_dpay_dollars":
                    int(
                        options.get("percent_dpay_dollars",
                                    self["percent_dpay_dollars"] / 100) * 100),
                "allow_votes":
