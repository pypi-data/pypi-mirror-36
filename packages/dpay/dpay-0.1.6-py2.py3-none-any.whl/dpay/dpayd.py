...
from dpaybase.chains import known_chains
from dpaybase.http_client import HttpClient
from dpaybase.storage import configStorage
from dpaybase.transactions import SignedTransaction
from dpaybase.types import PointInTime
...
class DPayd(HttpClient):
    ...
    def __init__(self, nodes=None, **kwargs):
        if not nodes:
            nodes = get_config_node_list() or ['https://d.dpays.io']
    ...
    def get_replies(self, author, skip_own=True):
        ...
        for reply in replies:
            ...
            if skip_own and post["author"] == author:
                continue
            discussions.append(Post(post, dpayd_instance=self))
        return discussions
    ...
    def get_promoted(self):
        ...
        for p in promoted:
            ...
            r.append(Post(post, dpayd_instance=self))
        return r
    ...
    def get_posts(self, limit=10, sort="hot", category=None, start=None):
        ...
        for p in func(discussion_query):
            r.append(Post(p, dpayd_instance=self))
        return r
    ...
    def stream_comments(self, *args, **kwargs):
        ...
        for c in Blockchain(
            ...
            yield Post(c, dpayd_instance=self)
    ...
    def get_account(self, account):
        ...
        Args:
            account (str): BEX username that we are looking up.
    ...
    def get_all_usernames(self, last_user=''):
        """ Fetch the full list of BEX usernames. """
    ...
    def _get_blocks(self, blocks):
        """ Fetch multiple blocks from dpayd at once.
    ...
    def get_blocks(self, block_nums):
        """ Fetch multiple blocks from dpayd at once, given a range.
    ...
    def get_feed_history(self):
        """ Get the hourly averages of witness reported BEX/BBD prices.

        ::

            {'current_median_history':
                {'base': '0.093 BBD', 'quote': '1.010 BEX'},
             'id': 0,
             'price_history': [{'base': '0.092 BBD', 'quote': '1.010 BEX'},
              {'base': '0.093 BBD', 'quote': '1.020 BEX'},
              {'base': '0.093 BBD', 'quote': '1.010 BEX'},
              {'base': '0.094 BBD', 'quote': '1.020 BEX'},
              {'base': '0.093 BBD', 'quote': '1.010 BEX'},
    ...
    def get_account_count(self):
        """ How many accounts are currently registered on BEX? """
    ...
    def get_version(self):
        """ Get dpayd version of the node currently connected to. """
    ...
    def get_ticker(self):
        """ Returns the market ticker for the internal BBD:BEX market. """
    ...
    def get_trade_history(self, start, end,
                          limit):
        """ Returns the trade history for the internal BBD:BEX market. """
        return self.call(
            'get_trade_history', start, end, limit, api='market_history_api')

    def get_recent_trades(self, limit):
        """ Returns the N most recent trades for the internal BBD:BEX
        market. """
    ...
    def get_market_history(self, bucket_seconds, start,
        ...
        """ Returns the market history for the internal BBD:BEX market. """
        return self.call(
            'get_market_history',
            bucket_seconds,
            start,
