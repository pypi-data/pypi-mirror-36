...
def legacyentry():
    ...
    parser.add_argument(
        ...
        help='URL for public dPay API (default: "https://d.dpays.io")'
    )
    ...
    parser.add_argument(
        ...
        version='%(prog)s {version}'.format(
            version=pkg_resources.require("dpay")[0].version))
    ...
    parser_info = subparsers.add_parser(
        'info', help='Show basic BEX blockchain info')
    ...
    parser_powerup = subparsers.add_parser(
        'powerup', help='Power up (vest BEX as BEX POWER)')
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing BEX from BEX POWER)')
    ...
    parser_convert = subparsers.add_parser(
        'convert',
        help='Convert BEX Dollars to BEX (takes a week to settle)')
    parser_convert.set_defaults(command="convert")
    parser_convert.add_argument(
        'amount', type=float, help='Amount of BBD to convert')
    parser_convert.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_buy = subparsers.add_parser(
        'buy', help='Buy BEX or BBD from the internal market')
    ...
    parser_buy.add_argument(
        'price', type=float, help='Limit buy price denoted in (BBD per BEX)')
    parser_buy.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_sell = subparsers.add_parser(
        'sell', help='Sell BEX or BBD from the internal market')
    ...
    dpay = stm.DPay(no_broadcast=args.no_broadcast, **options)
    ...
    if args.command == "set":
        ...
        if not args.objects:
            ...
            median_price = dpay.get_current_median_history_price()
            dpay_per_mvest = (
        ...
        for obj in args.objects:
            ...
            # Public Key
                ...
                account = dpay.commit.wallet.getAccountFromPublicKey(obj)
                if account:
        ...
        if args.unsafe_import_key:
            ...
            while True:
                ...
                if not wifkey:
                    break
                try:
                    dpay.commit.wallet.addPrivateKey(wifkey)
                except Exception as e:
                    print(str(e))
                    continue

                installed_keys = dpay.commit.wallet.getPublicKeys()
                if len(installed_keys) == 1:
                    name = dpay.commit.wallet.getAccountFromPublicKey(
                        installed_keys[0])
                    print("=" * 30)
                    print("Would you like to make %s a default user?" % name)
                    print()
                    print("You can set it with with:")
                    print("    dpaycli set default_account <account>")
                    print("=" * 30)
        ...
        if confirm("Are you sure you want to delete keys from your wallet?\n"
            ...
            for pub in args.pub:
                dpay.commit.wallet.removePrivateKeyFromPublicKey(pub)
        ...
        print(dpay.commit.wallet.getPrivateKeyForPublicKey(args.pub))

    elif args.command == "listkeys":
        t = PrettyTable(["Available Key"])
        t.align = "l"
        for key in dpay.commit.wallet.getPublicKeys():
            t.add_row([key])
        print(t)
        ...
        t.align = "l"
        for account in dpay.commit.wallet.getAccounts():
            t.add_row([
        ...
        print_json(
            dpay.commit.transfer_to_vesting(
                args.amount, account=args.account, to=args.to))

    elif args.command == "powerdown":
        print_json(
            dpay.commit.withdraw_vesting(
                args.amount,
                account=args.account,
            ))

    elif args.command == "convert":
        print_json(dpay.commit.convert(
            args.amount,
            account=args.account,
        ))
        ...
        print_json(
            dpay.commit.set_withdraw_vesting_route(
                args.to,
        ...
        if args.account and isinstance(args.account, list):
            for account in args.account:
                ...
                t.add_row([
                    'Available',
                    a.balances['available']['BEX'],
                    a.balances['available']['BBD'],
                    a.balances['available']['VESTS'],
                ])
                t.add_row([
                    'Rewards',
                    a.balances['rewards']['BEX'],
                    a.balances['rewards']['BBD'],
                    a.balances['rewards']['VESTS'],
                ])
                t.add_row([
                    'Savings',
                    a.balances['savings']['BEX'],
                    a.balances['savings']['BBD'],
                    'N/A',
                ])
                t.add_row([
                    'TOTAL',
                    a.balances['total']['BEX'],
                    a.balances['total']['BBD'],
                    a.balances['total']['VESTS'],
                ])
                print(t)
        else:
            print("Please specify an account: dpaycli balance <account>")
        ...
        if not args.foreign_account:
            from dpaybase.account import PasswordKey
            pwd = get_terminal(
        ...
        if not args.key:
            ...
            if not args.no_broadcast:
                dpay.commit.wallet.addPrivateKey(memo_privkey)
        print_json(
            dpay.commit.update_memo_key(args.key, account=args.account))
        ...
        print_json(
            dpay.commit.create_account(
                args.accountname,
                creator=args.account,
                password=pw,
                delegation_fee_dpay=args.fee,
            ))
        ...
        if "owner" in args.roles:
            ...
            if owner_pubkey in [x[0] for x in account["owner"]["key_auths"]]:
                ...
                dpay.commit.wallet.addPrivateKey(owner_privkey)
                imported = True
        ...
        if "active" in args.roles:
            ...
            if active_pubkey in [x[0] for x in account["active"]["key_auths"]]:
                ...
                dpay.commit.wallet.addPrivateKey(active_privkey)
                imported = True
        ...
        if "posting" in args.roles:
            ...
            if posting_pubkey in [
                ...
                dpay.commit.wallet.addPrivateKey(posting_privkey)
                imported = True
        ...
        if args.file and args.file != "-":
            ...
            tx = sys.stdin.read()
        tx = eval(tx)
        print_json(dpay.commit.sign(tx))
        ...
        print_json(
            dpay.commit.approve_witness(args.witness, account=args.account))

    elif args.command == "disapprovewitness":
        print_json(
            dpay.commit.disapprove_witness(
                args.witness, account=args.account))

    elif args.command == "repost":
        print_json(dpay.commit.repost(args.identifier, account=args.account))
        ...
        print_json(
            dpay.commit.update_account_profile(
        ...
        if args.account_creation_fee:
            props["account_creation_fee"] = str(
                Amount("%f BEX" % args.account_creation_fee))
        ...
        props = {
            "account_creation_fee":
                str(Amount("%f BEX" % args.account_creation_fee)),
            "maximum_block_size":
                args.maximum_block_size,
            "bbd_interest_rate":
                int(args.bbd_interest_rate * 100)
        }
        print_json(
            dpay.commit.witness_update(
...
def dpaytailentry():
    parser = argparse.ArgumentParser(
        description="UNIX tail(1)-like tool for the dPay blockchain")
    parser.add_argument(
        '-f',
        '--follow',
