...
from dpaybase.exceptions import (InsufficientAuthorityError, MissingKeyError,
                                  InvalidKeyFormat)
from dpaybase import operations
from dpaybase.operations import Operation
from dpaybase.transactions import SignedTransaction, fmt_time_from_now, \
    get_block_params
...
class TransactionBuilder(dict):
    ...
    def __init__(self,
        ...
        self.wallet = wallet_instance or Wallet(self.dpayd)
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(
                        authority[0], dpayd_instance=self.dpayd)
    ...
    def constructTx(self):
        ...
        ref_block_num, ref_block_prefix = get_block_params(self.dpayd)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.dpayd:
            operations.default_prefix = self.dpayd.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.dpayd.chain_params)
    ...
    def broadcast(self):
        ...
        try:
            if not self.dpayd.verify_authority(self.json()):
        ...
        try:
            self.dpayd.broadcast_transaction(self.json())
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        accountObj = Account(account, dpayd_instance=self.dpayd)
        ...
        for account_auth in authority["account_auths"]:
            account_auth_account = Account(
                account_auth[0], dpayd_instance=self.dpayd)
        ...
        self["blockchain"] = self.dpayd.chain_params
