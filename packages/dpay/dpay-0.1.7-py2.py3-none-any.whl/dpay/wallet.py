...
from dpaybase import bip38
from dpaybase.account import PrivateKey
from dpaybase.exceptions import (InvalidWifError, WalletExists)
...
class Wallet:
    """ The wallet is meant to maintain access to private keys for
        ...
        :param DPay rpc: RPC connection to a DPay node
        ...
        * **Wallet Database**: Here, dpaylibs loads the keys from the
    ...
    def __init__(self, dpayd_instance=None, **kwargs):
        ...
        # Prefix
        if self.dpayd:
            self.prefix = self.dpayd.chain_params["prefix"]
        else:
        ...
        if "keys" in kwargs:
            ...
            """ If no keys are provided manually we load the SQLite
                keyStorage
            """
            from dpaybase.storage import (keyStorage, KeyEncryptionKey)
    ...
    def addPrivateKey(self, wif):
        ...
        # it could be either graphenebase or dpaypybase so we can't check
        # the type directly
    ...
    def getOwnerKeyForAccount(self, name):
        ...
        if "owner" in Wallet.keyMap:
            ...
            account = self.dpayd.get_account(name)
            if not account:
                return
