...
def main() -> None:
    ...
    try:
        ...
        logger.fatal("Importers error: {}".format(e))
        sys.exit(1)
    logger.info("Loading exporter plugins")
    try:
        export_plugins = load_exporters(config)
    except ConfigError as e:
        logger.fatal("Exporters error: {}".format(e))
        sys.exit(1)
    ...
    try:
        ...
        logger.warn("No resources file written")
        sys.exit(0)
...
def initialize_export_interfaces(config: dict, plugins: dict) -> dict:
    """
    Method to initialize the interfaces with the configuration.
