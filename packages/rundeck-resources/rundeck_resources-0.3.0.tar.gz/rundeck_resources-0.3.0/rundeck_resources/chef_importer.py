        ...
        try:
            for node in chef_nodes:
                try:
                    ...
                    self.logger.warn("KeyError encountered with node '{}': {}"
                                     .format(node['automatic']['hostname'],
                                             e))
                    continue
                nodes.update(_node)
                self.cache.cache(self.section, nodes)
        except AttributeError as e:
            self.logger.error("ChefAPI encountered an issues: {}".format(e))
            self.logger.warn("Retrieving data from cache")
