...
class CloudLogger:
    ...
    async def close(self, grace_seconds=10):
        if self.tasks:
            logger.warning('Remote logger is closing but still has %s tasks, will allow %s seconds of grace', len(self.tasks), grace_seconds)
            await asyncio.sleep(grace_seconds)
            logger.warning('Remote logger is closing and %s tasks, are still alive after the grace period', len(self.tasks))
    ...
    def _on_done(self, log_line: str, done_task: asyncio.Task):
        ...
        try:
            ...
            logger.warning('Line %s was not sent to server, the send task was cancelled', log_line)
        except asyncio.InvalidStateError:
            logger.warning("Line %s was not sent to server, the send task hasn't started", log_line)
        except aiohttp.client_exceptions.ClientConnectorError as ex:
            logger.warning('Failed to send "%s" to ML: %s', log_line, str(ex))
        except Exception:
