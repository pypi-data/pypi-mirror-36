...
class Backbone(object):
    ...
    def __init__(self, active_config=None, loop=None, **kwargs):
        ...
        logger.info('Connection endpoint: %s', self.endpoint)
    ...
    def job_completed_callback(self, job_id, task):
        if self.single_job_mode:
            logger.info('%s completed, exiting', job_id)
            self.cancel()
    ...
    async def single_job_timeout(self):
        if not self.single_job_mode:
            return
        logger.info('Scheduling Single Mode timer')
        await asyncio.sleep(self.recv_timeout)
        logger.info('Scheduling Single Mode timer.. timeout')
        if not self.api.get_running():
            logger.info('No running jobs and running in single mode... leaving')
            self.cancel()
