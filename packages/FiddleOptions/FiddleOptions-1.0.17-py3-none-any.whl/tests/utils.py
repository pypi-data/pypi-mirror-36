...
class TestUtils:
    ...
    def create_vertical_spread(self, opening_date):
        ...
        short_leg = OptionLeg(contract=contracts[1], direction=Direction.SHORT, quantity=1)
        long_leg = OptionLeg(contract=contracts[0], direction=Direction.LONG, quantity=1)
    ...
    def create_long_put_butterfly(self, opening_date):
        ...
        long_lower_leg = option_contracts[0].to_option_leg(direction=Direction.LONG, quantity=1)
        short_leg = short_contract.to_option_leg(direction=Direction.SHORT, quantity=2)
        long_upper_leg = option_contracts[-1].to_option_leg(direction=Direction.LONG, quantity=1)
        butterfly = Butterfly(
            symbol='SPX',
            direction=Direction.LONG,
            quantity=10,
