...
class TestTradeserviceApi(unittest.TestCase):
    ...
    def _create_iron_condor(self):
        ...
        return IronCondor(self.equity, self.open_date, Direction.LONG, 10,
                          out_leg_call=OptionLeg(contract=call_contracts[0], direction=Direction.LONG, quantity=1),
                          in_leg_call=OptionLeg(contract=call_contracts[1], direction=Direction.SHORT, quantity=1),
                          in_leg_put=OptionLeg(contract=put_contracts[0], direction=Direction.SHORT, quantity=1),
                          out_leg_put=OptionLeg(contract=put_contracts[1], direction=Direction.LONG, quantity=1))
    ...
    def _create_butterfly_position(self):
        ...
        """
        put_contracts = self._put_chain.contracts
        first_leg = OptionLeg(contract=put_contracts[0], direction=Direction.LONG, quantity=1)
        mid_leg = OptionLeg(contract=put_contracts[int(len(put_contracts) / 2)],
                            direction=Direction.SHORT, quantity=2)
        third_leg = OptionLeg(contract=put_contracts[-1], direction=Direction.LONG, quantity=1)
    ...
    def _create_condor_position(self, direction=Direction.LONG, type=OptionContractType.PUT):
        # sort contracts by strike, ascending
        contracts = self._put_chain.contracts
        contracts.sort(key=lambda n: n.strike)

        oll = OptionLeg(contract=contracts[0], direction=Direction.LONG, quantity=1)
        ilml = OptionLeg(contract=contracts[1], direction=Direction.SHORT, quantity=1)
        iuml = OptionLeg(contract=contracts[2], direction=Direction.SHORT, quantity=1)
        oul = OptionLeg(contract=contracts[3], direction=Direction.LONG, quantity=1)
    ...
    def test_iron_condor_strike_exception(self):
        with self.assertRaises(CondorValueException) as ctx:
            ic = IronCondor(self.equity, self.open_date, Direction.LONG, 10,
                            out_leg_call=OptionLeg(strike=1, direction=Direction.LONG, quantity=1),
                            in_leg_call=OptionLeg(strike=2, direction=Direction.SHORT, quantity=1),
                            in_leg_put=OptionLeg(strike=2, direction=Direction.SHORT, quantity=1),
                            out_leg_put=OptionLeg(strike=1, direction=Direction.LONG, quantity=1))
