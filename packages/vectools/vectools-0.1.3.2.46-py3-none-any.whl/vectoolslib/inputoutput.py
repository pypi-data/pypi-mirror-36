""" This file contains classes related to general input and output of data, was well as error catching and quitting.
"""
import sys
import numpy as np
import argparse
...
# Important! This variable determines what string should be used for denoting STDIN input.
STDIN_INDICATOR = "-"
...
# Handle printing in color, at the moment we only use red for errors.
HEADER = '\033[95m'
OKBLUE = '\033[94m'
OKGREEN = '\033[92m'
WARNING = '\033[93m'
...
def error_quit(quit_str):
    """ Print in red and exit with code 1.

    This function is for gracefully catching and quiting conditions that would cause errors.

    Note that the value 1 is passed to the the sys.exit function. This prevents a zero from being passed and denotes
    that an error has occurred during execution. This is especially important when using workflow managers as many
    check exit codes to determine if a program executed correctly. Typically a non-zero will cause the manager to
    delete the files produced by the job. This is a good idea as many programs including Vectools produce files
    iteratively so checking for empty files will not alert one to errors.

    :param quit_str: The string to display on error.
    """
    print(FAIL + quit_str + ENDC)
    sys.exit(1)


def check_file_existence(files, accept_stdin=True):
    """ Check for file or STDIN existence and exit if none available.
    ...
    # Data may be passed as a string or list in the case of multiple files.
    # So if a string is passed convert it to a single element list.
    if type(files) is str:
        files = [files]

    # Just in-case we get some other kind of iterable data structure.
    # This could indicate an error within the function passing the file names.
    assert type(files) is list

    # Iterate over the file names and check for file existence.
    # Note for STDIN input, STDIN input must be allowed and the STDIN indicator must be passed.
    # This indicator is needed as without it we must wait and see if we get any STDIN input.
    # This also lets us control the order of other files passed with STDIN.
    for file_name in files:
        if file_name == STDIN_INDICATOR:
            ...
            if not os.path.isfile(file_name):
                error_quit(FILE_NOT_FOUND_STR % files)


def cast_data_type(float_int_or_str):
    """
    :param float_int_or_str:
    :return:
    """
    # Check the highest data type to cast to
    try:
...
class VectorIO:
    ...
    def out(self, data_frame, index_label=None, print_column_titles=True, roundto=None, ix_has_name=False):
        ...
        elif roundto is not None:
            data_frame = np.round(data_frame, decimals=roundto)

        if print_column_titles:
            col_names = self.has_col_names
            self.has_col_names = self.col_indexes
        else:
            col_names = False

        '''
        if column_names is not False:
            # print(ix_has_name)
            # print(list(data_frame.columns))
            # data_frame.columns = column_names
            self.has_col_names = column_names
        '''
        if ix_has_name is not False and self.has_col_names and self.has_row_names:
            print(ix_has_name, end="")
    ...
    def iterative_out(self, row_title, vector, sliced_col_titles=False, column_titles=None, roundto=None,
                      output_type='sys.stdout'):
        """ Yields output of vectors and column and row titles as they are created.
        ...
        # Decide if the program should write to a file or print the results.
        # Usually we will print the file. However, in certain cases, such as machine learning, we need to return
        # multiple files.
    ...
    def yield_matrices(self, parse_str_or_list, widths_must_match=True, heights_must_match=True, dtype=None):
        ...
        for file_name in self._yield_file_names(parse_str_or_list):
            ...
            try:
                ...
                # Fix for when the row column has a name.
                if df.index.name != "ix" and self.ix_has_name is False:
                    self.ix_has_name = df.index.name
    ...
    def parse_matrices(self, parse_str_or_list, widths_must_match=True, heights_must_match=True):
        ...
        widths_match, heights_match = widths_must_match, heights_must_match
...
class ParseVectors:
    ...
    This function parses tsv files to a tuple of the containing matrix, col_titles, row_titles
    """
    ...
    def __handleinput(self):
        ...
        if self.file_name != 'sys.stdin' and self.file_name != "-":
...
def _shared_params(parser, enable_column_titles=True, enable_row_titles=True, enable_delimiter=True,
    ...
    Keyword arguments in the function allow for silencing (False) or enabling (True) an command-line argument.
    Note:
    1) The default settings of these keyword arguments are different.
    2) Passing a string instead of a boolean value will change the help string.

    :param parser:
    :param enable_column_titles: Determines if column titles are allowed to be specified.
    :param enable_row_titles: Determines if row titles are allowed to be specified.
    :param enable_delimiter: Determines if the delimiter separating columns many be set.
    :param only_apply_on: Determines if an argument allowing selection of subsets of columns may be set.
    :param random_state: Determines if an argument allowing a seed value for random functions may be set.
    :return: None, but adds arguments to the parser class.
    """
