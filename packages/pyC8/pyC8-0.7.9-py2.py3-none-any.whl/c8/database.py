...
# KARTIK : Disable below for now
#from c8.realtime import Realtime
...
class Database(APIWrapper):
    ...
    @property
    def realtime(self):
        """Return Real time API wrapper.

        :return: Realtime API wrapper.
        :rtype: c8.realtime.Realtime
        """
        return Realtime(self.tenant_name, self.db_name)
    ...
    def on_change(self, collection, callback=printdata):
        ...
        topic = "non-persistent://" + self.tenant_name + "/" + constants.STREAM_LOCAL_NS_PREFIX + self.tenant_name + "." + self.db_name + "/" + collection
        ...
        if self.pulsar_client:
            ...
            self.client = pulsar.Client('pulsar://' + constants.PLUSAR_URL_PREFIX + hostname + ":" + str(self.stream_port)) 
