...
class Database(APIWrapper):
    ...
    def streams(self):
        ...
        # KARTIK : 20181001 : Work around REST API issue where /streams does
        # NOT show all the streams. Currently, it only shows global streams,
        # Not local ones. So we comment out the original code below, and then
        # we call each of the streams enumeration endpoints for both local and
        # global, add them to a set to eliminate duplicates, and then finally
        # convert the set to a list and then return the list.
        # When the streams enumeration APIs get fixed, we can restore this 
        # original code below and remove the workaround section.
        # See github issue: https://github.com/Macrometacorp/C8DB/issues/188
        # NOTE: Even if the bug gets fixed, it's safe to leave the workaround
        # code in place since it will not harm anything and still behave
        # correctly.
        #def response_handler(resp):
        #    code = resp.status_code
        #    if resp.is_success:
        ...
        #
        #return self._execute(request, response_handler)
        # KARTIK : ORIG CODE ABOVE COMMENTED OUT, WORKAROUND STARTS BELOW.
        ###### KARTIK : 20181001 : C8DB#188 : BEGIN WORKAROUND ######
        tmpset = set()
        def response_handler(resp):
            code = resp.status_code
            if resp.is_success:
        ...
        for s in self._execute(request, response_handler):
            tmpset.add(s)

        # Get persistent streams, both global and local
        for s in self.persistent_streams():
            tmpset.add(s)
        for s in self.persistent_streams(local=True):
            tmpset.add(s)

        # Get nonpersistent streams, both global and local
        for s in self.nonpersistent_streams():
            tmpset.add(s)
        for s in self.nonpersistent_streams(local=True):
            tmpset.add(s)

        # Now convert the set to a list and return it
        return list(tmpset)
        ###### KARTIK : 20181001 : C8DB#188 : END WORKAROUND ######
    ...
    def has_stream(self, stream_name, persistent=True, local=False):
        """ Check if the list of streams has a stream with the given name.
        Since the streams() function returns the full stream URL, we need
        to extract the last bit of the full stream URL, which is the stream
        name. This is the 'STREAM_NAME' part of a full stream URL e.g. 
            'peristent://<TENANT>/<NAMESPACE>/<STREAM_NAME>'

        :param stream_name: The name of the stream for which to check. 
        :type stream_name: str | unicode
        :return: True=stream_name found; False=stream_name not found.
        :rtype: bool
        """
        ...
        if persistent:
            stream = "persistent://" + self.tenant_name + "/" + namespace + "/" + stream_name
        else:
            stream = "non-persistent://" + self.tenant_name + "/" + namespace + "/" + stream_name

        if stream in self.streams():
            return True
        #for s in self.streams():
        #    name_only = s.split('/')[-1]
            #print("   HAS_STREAM: name only: "+name_only)
        #    if stream_name == name_only:
        #        return True

        # If we got here, we didn't find it in the for loop above.
        return False
