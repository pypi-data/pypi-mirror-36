...
from pycostanza.steepest import _compute_neighbours, _validate_connectivity
from pycostanza.misc import merge, crop
...
def dilate(lab_img):
    ...
    B = scipy.ndimage.maximum_filter(lab_img, 3)
    B[lab_img != 0] = lab_img[lab_img != 0]
    return B


def remove_labels_size(lab_img, remove_bordering=False, min_size=None, max_size=None):
...
def merge_labels_depth(lab_img, int_img, threshold, connectivity=None, offset=None):
    ...
    while True:
        ...
        # check basin depths
        labels_maxima = labeled_max(int_img, lab_img)
        to_merge = []
        ...
        for label1, neighbours in enumerate(neighbouring_domains, start=1):
            for label2 in neighbours[neighbours > label1]:
                ...
                labels_max = np.max(labels_maxima[np.array([label1 - 1, label2 - 1])])
