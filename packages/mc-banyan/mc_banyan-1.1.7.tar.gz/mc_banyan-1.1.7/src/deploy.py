...
def build_deploy_script(project_name, config_name, only_structure=False, remote_dict = None):
    ...
    # write data to file
    write_playbook(roles_build(get_roles_data(), remote_name = None if remote_dict == None else 'remote'))
...
def deploy_config(project_name, config_name):
    ...
    def get_existing_items_from_host():
        return (F(filter, _.call("find", "=") != -1) \
            >> F(map, lambda item: (item.split("=")[0], item.split("=")[1].strip().replace('"', '').replace("'", ""))) \
