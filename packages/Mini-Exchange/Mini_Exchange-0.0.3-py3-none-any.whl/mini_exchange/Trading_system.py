...
class Account:
    ...
    def sharpe_ratio(self,start=None,end=None):
        '''
        annual sharpe ratio
        '''
        start=(self.history_value.index.min() if start is None else start)
        ...
        return value.pct_change().dropna().mean()/value.pct_change().dropna().std()*15.8745
...
class Log:
    ...
    def total_trade(self,start=None,end=None):
        start=(self.log.open_dt.min() if start is None else start)
        end=(self.log.close_dt.max() if end is None else end)
        return self.log.loc[(self.log['open_dt']>=start)&(self.log['close_dt']<=end)].shape[0]
    def pos_trade(self,start=None,end=None):
        start=(self.log.open_dt.min() if start is None else start)
        end=(self.log.close_dt.max() if end is None else end)
        return (self.log.loc[(self.log['open_dt']>=start)&(self.log['close_dt']<=end)].returns>=0).sum()
    def win_rate(self,start=None,end=None):
        return self.pos_trade(start,end)/self.total_trade(start,end)
