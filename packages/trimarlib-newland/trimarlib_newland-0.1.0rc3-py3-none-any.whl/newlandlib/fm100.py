...
class FM100(object):
    ...
    def discover(limit: int = None):
        ...
        for port in ports:
            if port.vid == FM100.VID and port.pid == FM100.PID:
                dev = serial.Serial(exclusive=True, timeout=.5)
        ...
        dev = serial.Serial(exclusive=True, timeout=.5, write_timeout=.5)
        for port in ports:
            dev.port = port.device
            for baudrate in [115200, 9600, 19200, 1200, 2400, 4800, 14400, 38400, 57600]:
                try:
                    ...
                    for stopbits in [serial.STOPBITS_ONE, serial.STOPBITS_TWO]:
                        ...
                        for bytesize in [8, 7]:
                            ...
                            try:
                                ...
                                break
                            try:
                                dev.write(FM100.PROG_MODE_ENTER)
                                dev.write(FM100.PROG_MODE_EXIT)
                                buf = bytearray()
                                ...
                                ndev = serial.Serial(exclusive=True, timeout=.5, baudrate=baudrate,
                                                     parity=parity, stopbits=stopbits, bytesize=bytesize)
                                ndev.port = port.device
                                ret.append(ndev)
                                break
                            except serial.SerialException as e:
    ...
    def _execute_commands(self, *commands):
        ...
        try:
            with self._port_lock:
                ...
                while not rxdata.endswith(self.PROG_MODE_ON):
                    tmp = self._port.read()
                    if tmp == b'':
                        raise TimeoutError('failed to enter programming mode, rxdata={}'.format(bytes(rxdata)))
                    rxdata += tmp
                ...
                for command in commands:
                    ...
                    self._port.flush()
                    while not rxdata.endswith(b''.join([command, b';'])):
                        tmp = self._port.read()
                        if tmp == b'':
                            raise TimeoutError('response to command {} not received, rxdata={}'.format(command,
                                                                                                       bytes(rxdata)))
                        rxdata += tmp
                ...
                while not rxdata.endswith(self.PROG_MODE_OFF):
                    tmp = self._port.read()
                    if tmp == b'':
                        raise TimeoutError('failed to exit programming mode, rxdata={}'.format(bytes(rxdata)))
                    rxdata += tmp
        ...
        rxdata = rxdata[len(self.PROG_MODE_ON):-len(self.PROG_MODE_OFF)]
        self._logger.debug('received data = %s', bytes(rxdata))
    ...
    def scan_mode(self, *, mode: str = None, reading_time: int = None, interval_length: int = None, sensitivity=None,
        ...
        if mode is not None:
            if isinstance(mode, (bytes, bytearray)):
                ...
                try:
                    ...
                    raise ValueError('invalid mode')
            self._execute_commands(b''.join([b'999001', mode]))
        ...
        if sensitivity is not None:
            if isinstance(sensitivity, (bytes, bytearray)):
                ...
                self._execute_commands(b''.join([b'999001', sensitivity]))
                ...
                try:
                    self._execute_commands(b''.join([b'999001', sensitivity_map[sensitivity.lower()]]))
                except KeyError:
                    raise ValueError('invalid sensitivity')
    ...
    def scan_start(self):
        ...
        :raises RuntimeError: if command execution failed
        """
        self._execute_commands(b'99900035')
    ...
    def scan_stop(self):
        ...
        :raises RuntimeError: if command execution failed
        """
        self._execute_commands(b'99900036')
    ...
    def beeper_mute(self, muted: bool):
        ...
        if muted:
            self._execute_commands(b'99900040')
        else:
            self._execute_commands(b'99900041')
    ...
    def query(self, what: str = 'all'):
        ...
        if isinstance(what, str):
            if what == 'all':
                ...
                try:
                    ret = self._execute_commands(self.QUERY_MAP[what])[0].decode()
                except KeyError:
