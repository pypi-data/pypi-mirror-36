...
from .metadata_db.version_visitor import LimitVisitor
...
from missinglink.core.json_utils import multi_line_json_from_data, normalize_item, dict_normalize
...
class DataSync(object):
    ...
    def download_all(self, query, root_folder, dest_pattern, batch_size, processes, save_meta=True, isolation_token=None):
        from tqdm import tqdm
    ...
    class DownloadIterResults(object):
        ...
        def __get_next_results(self):
            def do_query():
                ...
                return self.__repo.metadata.query(self.__query, is_async=self.__is_async, start_index=query_offset, isolation_token=self.__isolation_token)
    ...
    def upload_index_and_metadata(self, data_path, isolation_token=None):
        def with_disk_result():
            ...
            if self.repo.data_volume_config.embedded:
                files_to_upload = self.__get_files_to_upload(files_info, combined_index_files, isolation_token)
                same_files_count = len(files_info) - len(files_to_upload)

                return files_to_upload, same_files_count

            return self.__upload_full_index(combined_index_files, isolation_token), same_files_count
