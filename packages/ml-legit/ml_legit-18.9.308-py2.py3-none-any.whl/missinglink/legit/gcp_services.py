    ...
    def gcp_default_credentials(cls, scopes=None, **kwargs):
        ...
        def _get_credentials_from_files():
            ...
            if credential_type == cls._AUTHORIZED_USER_TYPE:
                file_credentials = google_credentials.Credentials.from_authorized_user_info(info, scopes=scopes)
            elif credential_type == cls._SERVICE_ACCOUNT_TYPE:
                file_credentials = service_account.Credentials.from_service_account_info(info, scopes=scopes, **kwargs)
            else:
                # noinspection PyUnresolvedReferences
