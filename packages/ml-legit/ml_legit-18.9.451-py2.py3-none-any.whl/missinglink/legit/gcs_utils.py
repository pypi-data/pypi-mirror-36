...
class GCSDownloadDirectDownload(GCSService):
    ...
    def _download_using_gcs_service(self, bucket_name, object_name):
    ...
    def _download_using_http(self, object_name):
        with GCPServices.get_auth_session('gcloud', self._credentials) as auth_session:
    ...
    def download(self, bucket_name, object_name):
        if bucket_name:
            return self._download_using_gcs_service(bucket_name, object_name)

        return self._download_using_http(object_name)
...
def __retry_if_retry_possible_error(exception):
    ...
    return not isinstance(exception, (ValueError, NonRetryException))
