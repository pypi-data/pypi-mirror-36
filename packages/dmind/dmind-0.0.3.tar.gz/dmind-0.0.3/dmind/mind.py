...
with open(path.join(here, 'static', 'kity.min.js'), encoding='utf-8') as fh:
    STRING_KITY = fh.read()

with open(
        path.join(here, 'static', 'kityminder.core.min.js'),
        encoding='utf-8') as fh:
    STRING_KITYMINDER_JS = fh.read()
with open(
        path.join(here, 'static', 'kityminder.core.css'),
        encoding='utf-8') as fh:
    STRING_KITYMINDER_CSS = fh.read()

with open(path.join(here, 'static', 'render.js'), encoding='utf-8') as fh:
    STRING_RENDER = fh.read()
