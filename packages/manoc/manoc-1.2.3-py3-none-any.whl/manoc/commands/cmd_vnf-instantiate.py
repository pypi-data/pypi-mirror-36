...
@click.command('vnf-instantiate')
@click.argument('id')
@click.option('--inputs', help='File with instantiation inputs.')
@pass_context
def cli(ctx, id, inputs):
    """Command on instantiate vnf"""
    url = "http://api-gateway-mano." + ctx.base_url \
          + "/vnf-manager/" + ctx.tenant + "/vnflcm/v1/vnf_instances/" + id + "/instantiate/"
