...
class Moxa(object):
    ...
    config = {
        ...
        'VTG_PATTERN':  '\$..VTG.*\*\w\w',
        'ZDA_PATTERN':  '\$..ZDA.*\*\w\w',
        'MMB_PATTERN':  '\$WIMMB[,]{3}[\d]{4}.\d,B\*\w\w',
        'MWV_PATTERN':  '\$WIMWV,\d*.\d,\w,\d*.\d,\w,\w\*\w\w',
        'MTA_PATTERN':  '\$WIMTA,[\d]*.\d,\w\*[\w]{2}',
        'MHU_PATTERN':  '\$WIMHU,.*\*[\w]{2}',
        'GPS_PORT':     957,
        'WX_PORT':      956,
    ...
    def cacheexpired(self, cachename: str) -> bool:
        ...
        if path.isfile(cachename) and \
                path.getmtime(cachename) > (time.time() - self.config['CACHE_AGE']):
            return False
    ...
    def pattern_to_cachename(self, pattern: str) -> str:
        ...
        return '{}/{}.txt'.format(self.config['CACHE_DIR'], pattern[4:7])
    ...
    def readmoxa(self, pattern, port):
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.settimeout(2)
                sock.connect((self.config['MOXA_IP'], port))
    ...
    def pressure(self) -> float:
        ...
        """
        parts = self.nmeasentence(self.config['MMB_PATTERN'], self.config['WX_PORT'])
        if len(parts) > 3:
    ...
    def relwind(self) -> dict:
        ...
        """
        parts = self.nmeasentence(self.config['MWV_PATTERN'], self.config['WX_PORT'])
        if len(parts) > 3:
            result = {
                'dir': float(parts[1]),
    ...
    def humidity(self) -> dict:
        ...
        """
        parts = self.nmeasentence(self.config['MHU_PATTERN'], self.config['WX_PORT'])
        if len(parts) > 3:
            result = {
    ...
    def temperature(self):
        """() -> float
        """
        parts = self.nmeasentence(self.config['MTA_PATTERN'], self.config['WX_PORT'])
        if len(parts) > 1:
    ...
    def course(self) -> float:
        parts = self.nmeasentence(self.config['VTG_PATTERN'], self.config['GPS_PORT'])
        if len(parts) > 8:
            return float(parts[1])
    ...
    def position(self) -> (float, float):
        parts = self.nmeasentence(self.config['GLL_PATTERN'], self.config['GPS_PORT'])
        if len(parts) > 6:
            lat = self.get_degrees(parts[1], parts[2])
            lon = self.get_degrees(parts[3], parts[4])
            return lat, lon
