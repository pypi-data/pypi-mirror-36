...
class GaussianMixtureRv(object):  # TODO: Change to support scipy rv interface
    def __init__(self, mu, sigma=None, weights=None):#, *args, **kwargs):
        self.mu = np.array(mu)
        self.sigma = np.eye(np.shape(mu)[1]) if sigma is None else np.array(sigma)
    ...
    def rvs(self, N=1):
        x = np.empty((N, np.shape(self.mu)[1]))
        for row in range(N):
