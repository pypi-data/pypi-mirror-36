...
class EfemerideaManager(models.Manager):
    ...
    def gaurkoak(self):
        gaur = datetime.today()
        return Efemeridea.objects.filter(eguna=gaur.day, hila=gaur.month,is_public=True)

    def egunbatekoak(self,date):
        return Efemeridea.objects.filter(eguna=date.day, hila=date.month,is_public=True)
