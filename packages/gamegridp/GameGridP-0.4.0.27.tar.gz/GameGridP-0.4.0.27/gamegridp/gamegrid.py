...
class GameGrid(object):
    ...
    def __init__(self, title, cell_size=32,
        ...
        y_res = self.grid_height_in_pixels + _actionbar_height + _console_height  # 100 pixels for actionbar
        if self.toolbar is not None:
        ...
        if img_path is not None:
            ...
            if img_path is not None and img_action == "scale":
                ...
                self.background = pygame.transform.scale(
                    self.background, (self.grid_width_in_pixels, self.grid_height_in_pixels))
            elif img_path is not None and img_action == "upscale":
                if self.background.get_width() < self.grid_width_in_pixels \
                        or self.background.get_height() < self.grid_height_in_pixels:
                    self.background = pygame.transform.scale(
                        self.background, (self.grid_width_in_pixels, self.grid_height_in_pixels))
    ...
    def is_rectangle_in_grid(self, rect) -> bool:
        ...
        if rect.topleft[0] < 0 \
                or rect.topleft[1] < 0 \
                or rect.topleft[0] + rect.width > self.grid_width_in_pixels \
                or rect.topleft[1] + rect.height > self.grid_height_in_pixels:
    ...
    def __listen(self):
        ...
        for event in pygame.event.get():
            ...
            # Event: Mouse-Button Down
                ...
                if pos[0] < self.grid_width_in_pixels and pos[
                    1] < self.grid_height_in_pixels and event.button == 1:
                    ...
                    self._logging.debug("gamegrid.__listen__(): Mouseclick left at grid-position:" +
                                       str(cell_clicked))
                elif pos[0] < self.grid_width_in_pixels and pos[
                    1] < self.grid_height_in_pixels and event.button == 3:
                    cell_location = self.__pixel_to_cell(pos)
                    self._logging.debug("gamegrid.__listen__() : " + str(cell_location))
                    column = cell_location[0]
                    row = cell_location[1]
                ...
                # Click is in status_bar
                    ...
                    if pos[0] > 5 and pos[0] < 60:
                        if not self._running:
                            ...
                            return True
                    elif pos[1] >= self.grid_height_in_pixels and pos[0] > 60 and pos[
                    ...
                    elif pos[1] >= self.grid_height_in_pixels and pos[0] > 120 and pos[0] < 220:
                        self._logging.debug("gamegrid.__listen__() : Reset")
                        self._running = False
                        self.reset()
                    elif pos[1] >= self.grid_height_in_pixels and pos[0] > 220 and pos[0] < 280:
                        ...
                        if self._info == True:
                            ...
                            self.dirty = 1
                            self._info = True
                    elif pos[1] >= self.grid_height_in_pixels and pos[0] > 285 and pos[0] < 345:
                        if self.speed > 0:
                            self.speed = self.speed - 1
                    elif pos[1] >= self.grid_height_in_pixels and pos[0] > 345 and pos[0] < 395:
                        if self.speed<60:
                            self.speed = self.speed + 1
        ...
        if mouse_x > self.grid_width_in_pixels:
