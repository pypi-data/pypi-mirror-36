...
class GameGrid(object):
    ...
    def __init__(self, title, cell_size=32,
        ...
        if self.console is not None:
            ...
            _console_height = 0
        x_res = self.__grid_width_in_pixels + _toolbar_width
        y_res = self.__grid_height_in_pixels + _actionbar_height + _console_height  # 100 pixels for actionbar
        if self.toolbar is not None:
            self.toolbar.set_height(y_res)
            self.toolbar.set_posx(self.__grid_width_in_pixels)
        if self.actionbar is not None:
            self.actionbar.set_width(x_res)
            self.actionbar.set_posy(self.__grid_height_in_pixels)
        if self.console is not None:
            self.console.set_width(x_res)
            self.console.set_posy(self.__grid_height_in_pixels + _actionbar_height)
        ...
        self.grid_surface = pygame.Surface((self.__grid_width_in_pixels, self.__grid_height_in_pixels))
        self.background = pygame.Surface((self.__grid_width_in_pixels, self.__grid_height_in_pixels))
    ...
    def set_image(self, img_path: str, img_action: str = "upscale", size=None):
        ...
        if img_path is not None:
            ...
            if img_path is not None and img_action == "scale":
                if size is None:
                    self.background = pygame.transform.scale(self.background,
                                                             (
                                                             self.__grid_width_in_pixels,
                                                             self.__grid_height_in_pixels))
                else:
                    self.background = pygame.transform.scale(self.background, (size[0], size[1]))
                self.background = pygame.transform.scale(
                    self.background, (self.__grid_width_in_pixels, self.__grid_height_in_pixels))
            elif img_path is not None and img_action == "upscale":
                if self.background.get_width() < self.__grid_width_in_pixels \
                        or self.background.get_height() < self.__grid_height_in_pixels:
                    self.background = pygame.transform.scale(
                        self.background, (self.__grid_width_in_pixels, self.__grid_height_in_pixels))
                ...
                i = 0
                j = 0
                surface = pygame.Surface((self.__grid_width_in_pixels, self.__grid_height_in_pixels))
                while (i < self.__grid_width_in_pixels):
                    j = 0
                    while (j < self.__grid_height_in_pixels):
            ...
            self.backround = pygame.Surface((self.__grid_width_in_pixels, self.__grid_height_in_pixels))

        # Crop surface
        cropped_surface = pygame.Surface((self.__grid_width_in_pixels, self.__grid_height_in_pixels))
        cropped_surface.fill((255, 255, 255))
        cropped_surface.blit(self.background, (0, 0),
                             (0, 0, self.__grid_width_in_pixels, self.__grid_height_in_pixels))
        ...
        if self._cell_margin > 0:
            ...
            while i <= self.__grid_width_in_pixels:
                pygame.draw.rect(self.background, self._background_color,
                                 [i, 0, self._cell_margin, self.__grid_height_in_pixels])
                i += self.cell_size + self._cell_margin
            i = 0
            while i <= self.__grid_height_in_pixels:
                pygame.draw.rect(self.background, self._background_color,
                                 [0, i, self.__grid_width_in_pixels, self._cell_margin])
                i += self.cell_size + self._cell_margin
        self.actors.clear(self.grid_surface, self.background)
    ...
    @property
    def __grid_width_in_pixels(self) -> int:
    ...
    def is_at_bottom_border(self, rect):
        ...
        :return: True, falls Ja, ansonsten False
        """
        if rect.topleft[1] + rect.height >= self.__grid_height_in_pixels:
            return True
        else:
            return False
    ...
    def is_at_right_border(self, rect):
        ...
        :return: True, falls Ja, ansonsten False
        """
        if rect.topleft[0] + rect.width >= self.__grid_width_in_pixels:
            return True
        else:
            return False
    ...
    def is_rectangle_in_grid(self, rect) -> bool:
        ...
        if rect.topleft[0] < 0 \
                or rect.topleft[1] < 0 \
                or rect.topleft[0] + rect.width > self.__grid_width_in_pixels \
                or rect.topleft[1] + rect.height > self.__grid_height_in_pixels:
    ...
    def __listen(self):
        ...
        for event in pygame.event.get():
            ...
            # Event: Mouse-Button Down
                ...
                if pos[0] < self.__grid_width_in_pixels and pos[
                    1] < self.__grid_height_in_pixels and event.button == 1:
                    cell_location = self.__pixel_to_cell(pos)
                    self._logging.debug("gamegrid.__listen__() : " + str(cell_location))
                    column = cell_location[0]
                    row = cell_location[1]
                    cell_clicked = (column, row)
                    self.__listen_all("mouse_left", cell_location)
                    self._logging.debug("gamegrid.__listen__(): Mouseclick left at grid-position:" +
                                       str(cell_clicked))
                elif pos[0] < self.__grid_width_in_pixels and pos[
                    1] < self.__grid_height_in_pixels and event.button == 3:
                    cell_location = self.__pixel_to_cell(pos)
                    self._logging.debug("gamegrid.__listen__() : " + str(cell_location))
                    column = cell_location[0]
                    row = cell_location[1]
                    ...
                    self._logging.info("gamegrid.__listen__(): - Mouseclick right at grid-position:" +
                                       str(cell_clicked))
                    self.__listen_all("mouse_right", cell_location)
                # Click is in status_bar
                elif pos[1] >= self.__grid_height_in_pixels:
                    self.actionbar.dirty = 1
                    if pos[0] > 5 and pos[0] < 60:
                        if not self._running:
                            self._logging.debug("gamegrid.__listen__(): : Act")
                            return True
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 60 and pos[
                        0] < 120 and not self._running:
                        self._running = True
                        self._logging.debug("gamegrid.__listen__(): : Play")
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 60 and pos[0] < 120 and self._running:
                        self._running = False
                        self._logging.debug("gamegrid.__listen__() : Pause")
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 120 and pos[0] < 220:
                        self._logging.debug("gamegrid.__listen__() : Reset")
                        self._running = False
                        self.reset()
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 220 and pos[0] < 280:
                        ...
                        if self._info == True:
                            ...
                            self.dirty = 1
                            self._info = True
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 285 and pos[0] < 345:
                        if self.speed > 0:
                            self.speed = self.speed - 1
                    elif pos[1] >= self.__grid_height_in_pixels and pos[0] > 345 and pos[0] < 395:
                        if self.speed<60:
                            self.speed = self.speed + 1
                elif pos[0] > self.__grid_width_in_pixels:
                    toolbar_event = self.toolbar.listen("mouse_left", position=(pos[0], pos[1]))
                    if toolbar_event:
                        self.__listen_all(toolbar_event[0], toolbar_event[1])
            elif event.type == pygame.KEYDOWN:
                keys_pressed = pygame.key.get_pressed()
                self.__listen_all("key_down", keys.key_pressed_to_key(keys_pressed))
        if pygame.key.get_pressed().count(1) != 0:
            keys_pressed = pygame.key.get_pressed()
            self.__listen_all("key", keys.key_pressed_to_key(keys_pressed))
            self.__listen_all("key_pressed", keys.key_pressed_to_key(keys_pressed))
        mouse_x, mouse_y = pygame.mouse.get_pos()
        if mouse_x > self.__grid_width_in_pixels:
    ...
    def __update(self, no_logic: bool = False, no_drawing: bool = False):
        ''' Part 1:
    ...
    def reset(self):
        ...
        self.__update(no_logic=True, no_drawing=True)
