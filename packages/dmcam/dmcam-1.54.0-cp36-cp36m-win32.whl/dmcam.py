...
DEV_REG_MCU = _dmcam.DEV_REG_MCU
...
def dev_open_replay(filename):
    return _dmcam.dev_open_replay(filename)
dev_open_replay = _dmcam.dev_open_replay
...
def cap_get_frame_buffer(dev, frame_buf, frame_buf_size):
    return _dmcam.cap_get_frame_buffer(dev, frame_buf, frame_buf_size)
cap_get_frame_buffer = _dmcam.cap_get_frame_buffer
...
def cap_wait(dev, timeout_msec):
    return _dmcam.cap_wait(dev, timeout_msec)
cap_wait = _dmcam.cap_wait
...
DMCAM_FILTER_ID_GAUSS = _dmcam.DMCAM_FILTER_ID_GAUSS
...
def filter_enable(dev, fid, args, arg_len):
    return _dmcam.filter_enable(dev, fid, args, arg_len)
filter_enable = _dmcam.filter_enable

def filter_disable(dev, fid):
    return _dmcam.filter_disable(dev, fid)
...
def cmap_float(dst, src, outfmt, min_val, max_val):
    return _dmcam.cmap_float(dst, src, outfmt, min_val, max_val)
...
DMCAM_FRAME_SAVE_PCL = _dmcam.DMCAM_FRAME_SAVE_PCL

def frame_save_raw(fd, save_fmt, raw, raw_len, img_w, img_h, dcs_cnt, raw_tag):
    return _dmcam.frame_save_raw(fd, save_fmt, raw, raw_len, img_w, img_h, dcs_cnt, raw_tag)
...
def frame_load_raw(fd, dst, dst_len, dst_w, dst_h, dst_dcsn, dst_tag, dst_tag_len):
    return _dmcam.frame_load_raw(fd, dst, dst_len, dst_w, dst_h, dst_dcsn, dst_tag, dst_tag_len)
...
def gauss2d_iir_u16(dst, src, sigma):
    return _dmcam.gauss2d_iir_u16(dst, src, sigma)
gauss2d_iir_u16 = _dmcam.gauss2d_iir_u16

def gauss2d_iir_f32(dst, src, sigma):
    return _dmcam.gauss2d_iir_f32(dst, src, sigma)
gauss2d_iir_f32 = _dmcam.gauss2d_iir_f32
