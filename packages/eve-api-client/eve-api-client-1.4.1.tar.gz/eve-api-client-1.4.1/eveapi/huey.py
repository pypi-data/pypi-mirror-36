...
def log_data_age(r):
    ...
    if 'Last-Modified' in r.headers:
        time = datetime.datetime.now() - datetime.datetime.strptime(r.headers['Expires'], '%a, %d %b %Y %H:%M:%S %Z')
        time = time.total_seconds()
        logging.info(f'ESI data is {time} seconds old')

    if 'Expires' in r.headers:
        time = datetime.datetime.strptime(r.headers['Expires'], '%a, %d %b %Y %H:%M:%S %Z') - datetime.datetime.now()
        time = time.total_seconds()
        logging.info(f'New ESI data in {time} seconds')
