...
def esi(method, url, headers, params, data, fetch_pages, auth_callback, cache, page=False):
    ...
    if 200 <= r.status_code < 300: # is 2xx
        ...
        if 'Expires' in r.headers:
            cache_diff = datetime.datetime.strptime(r.headers['Expires'], '%a, %d %b %Y %H:%M:%S %Z') - datetime.datetime.now()
            cache_diff = cache_diff.total_seconds()
            logging.debug(f'New ESI data in {cache_diff} seconds')
        ...
        if fetch_pages and total_pages > 1:
            ...
            pages = [
                esi(method, url, headers, params, data, False, auth_callback, cache, page=p + 2)
        ...
        if r.json()['error'] == 'expired':
            return esi(method, url, headers, params, data, fetch_pages, auth_callback, cache, page=page).get(blocking=True)
