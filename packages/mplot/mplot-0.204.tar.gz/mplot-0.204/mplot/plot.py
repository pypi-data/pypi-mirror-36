...
def stackhchartup(obj,overlay,width,categories,offset):
    ...
    for i in range(0,obj.lenny):
        ...
        if y not in overlay:
            overlay[y] = 0
        overlay[y] += obj.x[i]
        newx.append(overlay[y])
        newy.append(y)
        newxs.append([0,overlay[y],overlay[y],0,0])
        newys.append([y+offset,y+offset,y+offset+width,y+offset+width,y+offset])                
    obj.xs = newxs
    obj.ys = newys
    obj.x = newx
...
def makeaxis(ax1,ax2,objects,cats,mindiff,xlabel,xticks,xsubticks,xrotation,xalign,xminmax,xzero,rawxfontsize,ylabel,yticks,ysubticks,yrotation,yalign,yminmax,yzero,rawyfontsize,second_ylabel,second_yticks,second_ysubticks,second_yrotation,second_yalign,second_yminmax,second_yzero,rawsecond_yfontsize,axisfontsize):
    ...
    minx,maxx,xlocs,xvals = makeaxis_sub(ax1,'x',objects,xlabel,xticks,xsubticks,xrotation,xalign,xminmax,xzero,xfontsize,categories = cats['x'],mindiff = mindiff['x'])
    miny,maxy,ylocs,yvals = makeaxis_sub(ax1,'y',objects,ylabel,yticks,ysubticks,yrotation,yalign,yminmax,yzero,yfontsize,categories = cats['y'],mindiff = mindiff['y'])
