...
def destroy_infra(docker_image: str, bucket_name: str):
    ...
    container = client.containers.create(docker_image,
                                         environment=["AWS_ACCESS_KEY_ID=" + os.environ.get("AWS_ACCESS_KEY_ID", None),
...
def start(config: tuple):
    ...
    while True:
        ...
        for bucket in get_buckets(config.bucket_prefix):
            try:
                if not is_valid(bucket + "s", config.max_age):
                    old_infra += 1
...
def main():
    ...
    arguments = docopt(__doc__, version='Marketplace PR Laundromat')
    if arguments["--config"]:
