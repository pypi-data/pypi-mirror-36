...
class CoingateAPI(object):
    ...
    def checkout(self, order_id, pay_currency='BTC'):
        ...
        if len(pay_currency) != 3:
            raise APIError('Pay currency name length must be exactly 3 symbols')
    ...
    def create_order(self, price_amount, price_currency, receive_currency, **kwargs):
        ...
        if len(receive_currency) != 3:
            raise APIError('Receive currency name length must be exactly 3 symbols')
