...
from dpaypybase.operations import Comment_options
...
class Post(dict):
    """ This object gets instanciated by DPay.streams and is used as an
        abstraction layer for Comments in DPay
    ...
    def refresh(self):
        ...
        post = self.dpay.rpc.get_content(post_author, post_permlink)
        ...
        for p in bbd_amounts:
            post[p] = Amount(post.get(p, "0.000 %s" % self.dpay.symbol("BBD")))
    ...
    def get_comments(self, sort="total_payout_value"):
        ...
        """
        post_author, post_permlink = resolveIdentifier(self.identifier)
        posts = self.dpay.rpc.get_content_replies(post_author, post_permlink)
        r = []
        for post in posts:
    ...
    def reply(self, body, title="", author="", meta=None):
        ...
        """
        return self.dpay.reply(self.identifier, body, title, author, meta)
    ...
    def vote(self, weight, voter=None):
        ...
        """
        return self.dpay.vote(self.identifier, weight, voter=voter)

    @property
    def reward(self):
        """Return a float value of estimated total BBD reward.
        """
        if not self.loaded:
            self.refresh()
    ...
    def export(self):
        ...
        # Remove DPay instance object
        safe_dict = remove_from_dict(self, ['dpay'])
    ...
    def set_comment_options(self, options):
        op = Comment_options(
            **{"author": self["author"],
               ...
               "percent_dpay_dollars": int(
                   options.get("percent_dpay_dollars",
                               self["percent_dpay_dollars"] / 100
                               ) * 100),
               "allow_votes":
        ...
        return self.dpay.finalizeOp(op, self["author"], "posting")
