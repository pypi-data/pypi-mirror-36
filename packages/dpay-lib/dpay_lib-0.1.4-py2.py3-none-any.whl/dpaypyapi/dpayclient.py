from dpaypyapi.dpaywalletrpc import DPayWalletRPC
    ...
    """ The behavior of your program (e.g. reactions on messages) can be
        ...
        **RPC-Only Connections**:
        ...
        .. code-block:: python
            ...
            dpay = DPayClient(Config)
            print(dpay.rpc.info())
            print(dpay.rpc.get_account("init0"))
            print(dpay.rpc.get_asset("USD"))

        All methods within ``dpay.rpc`` are mapped to the corresponding
        ...
        **Additional Websocket Connections**:
        ...
        All methods within ``dpay.ws`` are mapped to the corresponding
...
class DPayClient():
    """ The ``DPayClient`` class is an abstraction layer that makes the use of the
    ...
    def __init__(self, config, **kwargs):
        ...
        if ("wallet_host" in available_features and
            ...
            self.rpc = DPayWalletRPC(self.wallet_host,
                                      self.wallet_port,
                                      self.wallet_user,
        ...
        if "witness_url" in available_features:
            ...
            self.ws = DPayNodeRPC(self.witness_url,
                                   self.witness_user,
