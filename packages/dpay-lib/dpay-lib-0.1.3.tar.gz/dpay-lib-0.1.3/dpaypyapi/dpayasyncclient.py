...
class Config():
    """ The ``Config`` class is used to contain all the parameters needed by ``DPayAsyncClient`` in
        a neat and simple object. The main parameters needed are the witness connection parameters and
        the mapping of aliases used in code to dPay APIs that are requested to be initialized by
        ``DPayAsyncClient`` on behalf of the code.
...
class DPayAsyncClient(object):
    """ DPay Asynchronous Client

        The ``DPayAsyncClient`` class is an abstraction layer that makes asynchronous
        use of the RPC API of either dpayd (witness) or cli_wallet (wallet)  easy to use.
        ...
        .. code-block:: python
            ...
            from dpaypyasyncclient import DPayAsyncClient, Config

            @asyncio.coroutine
            def print_block_number(dpay):
               res = yield from dpaypy.database.get_dynamic_global_properties()
               print(res["head_block_number"])

            dpay = DPayAsyncClient(Config(witness_url="ws://localhost:8090",
                                            witness_apis=["database"]))
            dpay.run([print_block_number])
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "wallet"):
            self.wallet = DPayAsyncClient.WalletRPCDispatch(self)

    class WalletRPCDispatch(object):
        # Internal implementation specific class. Users of DPayAsyncClient do not need to use this class.
        def __init__(self, dpay):
            self._dpay = dpay

        def __getattr__(self, method_name):
            """ Map field names to dPay wallet methods
            """
            if method_name[0] == '_':
            ...
            def method(*args, **kwargs):
                call_id = self._dpay._wallet_call_id
                self._dpay._wallet_call_id += 1
                query = {"jsonrpc": "2.0", "id": call_id, "method": method_name, "params": args}
                self._dpay._wallet_pending_rpc[call_id] = asyncio.Future()
                yield from self._dpay._wallet_ws.send(json.dumps(query))
                if "future" in kwargs and kwargs["future"]:
                    return self._dpay._wallet_pending_rpc[call_id]
                else:
                    ret = yield from self._dpay._wallet_pending_rpc[call_id]
                    return ret
            return method

    class WitnessRPCDispatch(object):
        # Internal implementation specific class. Users of DPayAsyncClient do not need to use this class.
        def __init__(self, dpay, api_id):
            self._dpay = dpay
            self._api_id = api_id

        def __getattr__(self, method_name):
            """ Map field names to dPay methods within the specific dPay API that this
            ...
            @asyncio.coroutine
            def method(*args, **kwargs):
                call_id = self._dpay._witness_call_id
                self._dpay._witness_call_id += 1
                query = {"jsonrpc": "2.0", "id": call_id, "method": "call", "params": [self._api_id, method_name, args]}
                self._dpay._witness_pending_rpc[call_id] = asyncio.Future()
                yield from self._dpay._witness_ws.send(json.dumps(query))
                if "future" in kwargs and kwargs["future"]:
                    return self._dpay._witness_pending_rpc[call_id]
                else:
                    ret = yield from self._dpay._witness_pending_rpc[call_id]
                    return ret
            return method

    def __getattr__(self, api_name):
        """ Map field names to dPay APIs using the map provided in Config.
        """
        if api_name == "wallet":
            raise RPCClientError("Wallet RPC was not specified when initializing DPayAsyncClient")
        if not self._witness_ws:
            raise RPCClientError("Witness RPC was not specified when initializing DPayAsyncClient")
        ...
        return DPayAsyncClient.WitnessRPCDispatch(self, api_id)
    ...
    def _initialize(self, coroutines):
        if self._witness_ws:
            ...
            if not l:
                raise RPCClientError("Could not login to dpayd node")
