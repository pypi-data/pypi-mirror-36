...
class Operation(GrapheneOperation):
    ...
    def _getklass(self, name):
        module = __import__("dpaypybase.operations", fromlist=["operations"])
        ...
        if isArgsThisClass(self, args):
            ...
            if "json_metadata" in kwargs and kwargs["json_metadata"]:
                if isinstance(kwargs["json_metadata"], dict):
                    ...
                    meta = kwargs["json_metadata"]
            # HF 18 requires liquid BEX to be multiplied by 30 for creation
            # f = Amount(kwargs["fee"])
            # fee = '{} BEX'.format(f.amount * 30)
            # print(fee)
            super().__init__(OrderedDict([
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            super().__init__(OrderedDict([
                ...
                ('percent_dpay_dollars', Uint16(int(kwargs["percent_dpay_dollars"]))),
