...
from dpaypyapi.dpaynoderpc import DPayNodeRPC, NoAccessApi
from dpaypybase import memo
from dpaypybase import operations
from dpaypybase.account import PrivateKey, PublicKey
...
BEXIT_100_PERCENT = 10000
BEXIT_1_PERCENT = (BEXIT_100_PERCENT / 100)
...
class DPay(object):
    """ Connect to the dPay network.
        ...
        * **Force keys**: This more is for advanced users and
        ...
        .. code-block:: python
            ...
            dpay = DPay()
            dpay.post("Testing dPay library", "I am testing dPay", category="spam")

        All that is requires is for the user to have added a posting key with dpaypy
        ...
        .. code-block:: bash
            ...
            dpaypy set default_author jared
    ...
    def finalizeOp(self, ops, account, permission):
        ...
        """
        tx = TransactionBuilder(dpay_instance=self)
        tx.appendOps(ops)
    ...
    def sign(self, tx, wifs=[]):
        """ Sign a provided transaction witht he provided key(s)
            ...
            :param string wifs: One or many wif keys to use for signing
                ...
                of the transactions.
        """
        tx = TransactionBuilder(tx, dpay_instance=self)
    ...
    def edit(self,
        ...
        """
        original_post = Post(identifier, dpay_instance=self)
    ...
    def post(self,
        ...
        if not author:
            raise ValueError(
                "Please define an author. (Try 'dpaypy set default_author'"
            )
        ...
        # Default "app"
        if "app" not in meta:
            version = pkg_resources.require("dpay-lib")[0].version
            meta["app"] = "dpay-lib/{}".format(version)

        # Identify the comment options
        options = {}
        if "max_accepted_payout" in meta:
            options["max_accepted_payout"] = meta.pop("max_accepted_payout", None)
        if "percent_dpay_dollars" in meta:
            options["percent_dpay_dollars"] = meta.pop("percent_dpay_dollars", None)
        ...
        if options:
            default_max_payout = "1000000.000 %s" % self.symbol("BBD")
            op.append(
                operations.Comment_options(**{
                    "author": author,
                    "permlink": permlink,
                    "max_accepted_payout": options.get("max_accepted_payout", default_max_payout),
                    "percent_dpay_dollars": int(
                        options.get("percent_dpay_dollars", 100) * BEXIT_1_PERCENT
                    ),
    ...
    def vote(self,
        ...
        """ Vote for a post
            ...
            .. code-block:: python
                ...
                dpaypy set default_voter <account>
        """
        if not voter:
        ...
        op = operations.Vote(
            **{"voter": voter,
               ...
               "weight": int(weight * BEXIT_1_PERCENT)}
        )
    ...
    def create_account(self,
        ...
        if not creator:
            raise ValueError(
                ...
                "creator=x, or set the default_author using dpaypy")
        ...
        account = None
        try:
            account = Account(account_name, dpay_instance=self)
        except:
            pass
        if account:
        ...
        from dpaypybase.account import PasswordKey, PublicKey
        if password:
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        """ Transfer BBD or BEX to another account.

            :param str to: Recipient
            :param float amount: Amount to transfer
            :param str asset: Asset to transfer (``BBD`` or ``BEX``)
        ...
        if memo and memo[0] == "#":
            ...
            to_account = Account(to, dpay_instance=self)
    ...
    def transfer_to_vesting(self, amount, to=None, account=None):
        """ Vest BEX
            ...
            :param float amount: number of BEX to vest
    ...
    def convert(self, amount, account=None, requestid=None):
        """ Convert DPayDollars to DPay (takes one week to settle)
    ...
    def transfer_to_savings(self, amount, currency, memo, to=None, account=None):
        """ Transfer BBD or BEX into a 'savings' account.
            ...
            :param float currency: 'BEX' or 'BBD'
    ...
    def witness_feed_publish(self, dpay_usd_price, quote="1.000", account=None):
        """ Publish a feed price as a witness.

            :param float dpay_usd_price: Price of BEX in USD (implied price)
    ...
    def _valid_currency(currency):
        if currency not in [
            DPay.rpc.chain_params["bbd_symbol"],
            DPay.rpc.chain_params["dpay_symbol"]
        ]:
    ...
    def get_content(self, identifier):
        """ Get the full content of a post.
            ...
            :param str identifier: Identifier for the post to upvote Takes
                                   the form ``@author/permlink``
        """
        return Post(identifier, dpay_instance=self)
    ...
    def get_replies(self, author, skipown=True):
        ...
        for reply in replies:
            ...
            if skipown and post["author"] == author:
                continue
            discussions.append(Post(post, dpay_instance=self))
        return discussions
    ...
    def get_promoted(self):
        ...
        for p in promoted:
            ...
            r.append(Post(post, dpay_instance=self))
        return r
    ...
    def get_posts(self, limit=10,
        ...
        for p in func(discussion_query):
            r.append(Post(p, dpay_instance=self))
        return r
    ...
    def get_comments(self, identifier):
        """ Return **first-level** comments of a post.
            ...
            :param str identifier: Identifier of a post. Takes an
                                   identifier of the form ``@author/permlink``
        """
        return Post(identifier, dpay_instance=self).get_comments()
    ...
    def get_balances(self, account=None):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        a = Account(account, dpay_instance=self)
        ...
        vesting_shares_dpay = Amount("%f %s" % (
            float(vesting_shares) / 1e6 * dpay_per_mvest,
            "BEX"
        ))
        ...
        return {
            ...
            "bbd_balance": Amount(a["bbd_balance"]),
            "savings_balance": Amount(a["savings_balance"]),
            "savings_bbd_balance": Amount(a["savings_bbd_balance"]),
            # computed amounts
            "vesting_shares_dpay": Amount(vesting_shares_dpay),
        }

    def get_account_history(self, account, **kwargs):
        return Account(account, dpay_instance=self).rawhistory(**kwargs)
    ...
    def stream_comments(self, *args, **kwargs):
        ...
        for c in Blockchain(
            ...
            yield Post(c, dpay_instance=self)
    ...
    def interest(self, account):
        ...
        last_payment = formatTimeString(account["bbd_last_interest_payment"])
        next_payment = last_payment + timedelta(days=30)
        interest_rate = self.info()["bbd_interest_rate"] / 100  # the result is in percent!
        interest_amount = (interest_rate / 100) * int(
            int(account["bbd_seconds"]) / (60 * 60 * 24 * 356)
        ) * 10 ** -3
    ...
    def set_withdraw_vesting_route(self, to, percentage=100,
        ...
        op = operations.Set_withdraw_vesting_route(
            **{"from_account": account,
               "to_account": to,
               "percent": int(percentage * BEXIT_1_PERCENT),
               "auto_vest": auto_vest
               }
        )
    ...
    def allow(self, foreign, weight=None, permission="posting",
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, dpay_instance=self)
    ...
    def comment_options(self, identifier, options, account=None):
        ...
        op = operations.Comment_options(
            **{
                ...
                "permlink": permlink,
                "max_accepted_payout": options.get("max_accepted_payout", default_max_payout),
                "percent_dpay_dollars": options.get("percent_dpay_dollars", 100) * BEXIT_1_PERCENT,
                "allow_votes": options.get("allow_votes", True),
                "allow_curation_rewards": options.get("allow_curation_rewards", True),
            }
        )
    ...
    def claim_reward_balance(self,
        ...
        set desired claim amount by setting any of `reward_dpay`, `reward_bbd` or `reward_vests`.
        ...
        if not reward_dpay:
            reward_dpay = account['reward_dpay_balance']
        if not reward_bbd:
            reward_bbd = account['reward_bbd_balance']
        if not reward_vests:
        ...
        op = operations.Claim_reward_balance(
            **{
                "account": account["name"],
                "reward_dpay": str(reward_dpay),
                "reward_bbd": str(reward_bbd),
