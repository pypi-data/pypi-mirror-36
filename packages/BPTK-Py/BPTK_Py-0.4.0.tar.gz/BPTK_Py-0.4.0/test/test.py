...
bptk = bptk()
bptk.scenario_manager_factory.reset_scenario(scenario_manager="ScenarioManager2", scenario="MakeYourStartUpGrow-x")


scenarios = bptk.scenario_manager_factory.get_scenarios(
    scenarios=["MakeYourStartUpGrow_strategy", "MakeYourStartUpGrow"], scenario_managers=["ScenarioManager2"])

from BPTK_Py.widgets.dashboard import Dashboard

widget_gen = Dashboard(bptk)

config.configuration["log_modes"]= [""]

bptk.dashboard(scenario_managers=["ScenarioManager1"],

               scenarios=["MakeYourStartUpGrow_strategy"],
               kind="line",
               equations=["cash.cash"],
               stacked=False,
               strategy=True,
