            ...
            if len(images) > 0 and not isinstance(images[0], np.ndarray) and os.path.isdir(str(images[0])):
                ...
                if len(labels) == 1:
                    ...
                    for i in range(len(images)):
                        if isinstance(labels, np.ndarray) and len(labels.shape) == 1 and type(labels[i]) in [np.uint8, np.uint16, np.uint32, np.int8, np.int16, np.int32]:
                            label = int(labels[i])
                        else:
                            label = labels[i]
