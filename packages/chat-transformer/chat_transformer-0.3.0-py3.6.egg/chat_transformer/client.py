...
class TransformerClient(AioSimpleIRCClient):
    ...
    def __init__(
        ...
        for key, value in output_data:
            output_cls_str = value.pop('class', DEFUALT_OUTPUT_DATA[key])
            output_cls = class_from_string(output_cls_string)
    ...
    def connect(self, irc_server, irc_port, irc_nickname, *args, **kwargs):
        ...
        for output in self.outputs.value():
            output.connect()
