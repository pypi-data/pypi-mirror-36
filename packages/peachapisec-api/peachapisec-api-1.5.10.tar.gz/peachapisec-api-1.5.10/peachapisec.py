...
def __check_semver(response):
    ...
    if api_ver.major == cli_version.major:
        ...
        if api_ver.minor == cli_version.minor:
            ...
            logger.warn('Client API version {} is compatible with server API version {} but consider upgrading.'.format(cli_version, api_ver)) 
            return
        elif api_ver.minor > cli_version.minor:
            # client minor behind, warn but ok
            logger.warn('Client API version {} is compatible with server API version {} but consider upgrading the client to version {}'.format(cli_version, api_ver, server_version))
            return
        else:
