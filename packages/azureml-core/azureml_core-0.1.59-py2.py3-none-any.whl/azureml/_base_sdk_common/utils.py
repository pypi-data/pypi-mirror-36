...
def convert_list_to_dict(input_list):
    """Converts a list of items to a Dictionary with all the odd numbered items in the list as its keys
    and all the even numbered items as the corresponding values.
    """
    output_dict = {}
    if not input_list:
        return output_dict
...
def convert_dict_to_list(input_dict):
    ...
    [output_list.extend([key, value]) for
        key, value in input_dict.items()]
    return output_list


def update_list_from_dict(input_list, input_dict):
    """Update the List from the given Dictionary."""
    list_as_dict = convert_list_to_dict(input_list)
    list_as_dict.update(input_dict)
    return convert_dict_to_list(list_as_dict)


def merge_dict(dict1, dict2):
    """
    merge two Dictionary and dict2 will overwirte dict1
    """
    merged_dict = {}
    if dict1:
