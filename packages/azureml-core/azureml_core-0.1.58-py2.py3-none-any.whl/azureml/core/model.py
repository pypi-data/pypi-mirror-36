...
from azureml._model_management._util import match_mms_payload_kv_tags
...
class Model(object):
    ...
    :type version: int
    """
    _expected_payload_keys = ['createdTime', 'description', 'id', 'mimeType', 'name', 'tags',
    ...
    def _get(workspace, name=None, id=None, tags=None, properties=None, version=None):
        ...
        if resp.status_code == 200:
            ...
            if id:
                match_mms_payload_kv_tags(model_payload)
                return model_payload
            else:
                paginated_results = get_paginated_results(model_payload, headers)

                for model_dict in paginated_results:
                    match_mms_payload_kv_tags(model_dict)
    ...
    def get_model_path(model_name, version=None, _workspace=None):
        ...
        Model will be searched in the following order:
        If version is None:
        1) download from remote to cache
        2) load from cache azureml-models/<model name>/<latest version>/
        3) ./<model name>

        If version is not None:
        1) load from cache azureml-models/<model name>/<latest version>/
    ...
    def download(self, target_dir="."):
        ...
        if len(sas_to_relative_download_path) == 0:
            raise Exception("No files to download. Did you upload files?")
        ...
        if self.unpack:
            ...
            if not os.path.exists(tar_path):
                raise Exception("tar file not found at {}. Paths in scope:\n{}".format(
                    tar_path,
        ...
        if len(file_paths) != 1:
            raise Exception("Illegal state. Paths in target_dir is {}".format(file_paths))
    ...
    def _get_latest_version(versions):
        ...
        if not len(versions) > 0:
            raise Exception("versions is empty")
