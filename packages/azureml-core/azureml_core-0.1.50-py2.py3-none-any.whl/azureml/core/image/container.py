...
# ---------------------------------------------------------
"""Module including ContainerImage class, ContainerImageConfig class, and TargetRuntime class"""
...
from .image import Image, ImageConfig, Asset
...
class ContainerImage(Image):
    ...
    _expected_payload_keys = ['assets', 'createdTime', 'creationState', 'description', 'driverProgram', 'id',
                              'imageLocation', 'imageType', 'modelIds', 'name', 'tags', 'properties',
...
class ContainerImageConfig(ImageConfig):
    ...
    @staticmethod
    def __validate_path_exists_or_throw(member, name):
    ...
    def validate_configuration(self):
        ...
        self.__validate_path_exists_or_throw(self.execution_script, "Driver file")
        ...
        if self.conda_file:
            self.__validate_path_exists_or_throw(self.conda_file, "Conda file")

        if self.docker_file:
            self.__validate_path_exists_or_throw(self.docker_file, "Docker file")

        if self.dependencies:
            for dependency in self.dependencies:
                self.__validate_path_exists_or_throw(dependency, "Dependency")

        if self.schema_file:
            self.__validate_path_exists_or_throw(self.schema_file, "Schema file")
...
class TargetRuntime(object):
    ...
    def __init__(self, properties, runtime_type):
        """
    ...
    def serialize(self):
        ...
        :rtype: dict
        """
        return {'properties': self.properties, 'runtimeType': self.runtime_type}
    ...
    def deserialize(payload_obj):
        ...
        return TargetRuntime(payload_obj['properties'], payload_obj['runtimeType'])
