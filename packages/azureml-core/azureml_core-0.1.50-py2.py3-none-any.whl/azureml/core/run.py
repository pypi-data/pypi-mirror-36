...
Use Run inside of your experimentation code to log metrics and artifacts to the Run History service.

Use Run outside of your experiments to query and analyze the metrics that were written.

Functionality includes:

*  Storing data through log_* commands and retrieving them
...
*  Loading the current run from a remote environment with Run.get_submitted_run()
...
class Run(_RunBase):
    ...
    This class works with the :class:`azureml.core.Experiment` and is used in three scenarios

    1. Creating a run by executing code or logging in a notebook
    2. Logging metrics and uploading artifacts in your experiment code
    3. Reading metrics and downloading artifacts when analyzing experimental results.

    To create a run use :func:`azureml.core.Experiment.submit` or :func:`azureml.core.Experiment.start_logging`
    To submit a run, create a configuration object that describes how the experiment is run.  These objects are:
    ...
    * :class:`azureml.core.scriptrunconfig`
    ...
    def _dto_to_run(experiment, run_dto, outputs=None,
        ...
        :type experiment: azureml.core.Experiment
        :param run_dto:
        :type azureml._restclient.models.run_dto.RunDto:
        :param outputs: Outputs directory to be tracked
        :type outputs: str
        :param _child_auth:
        :type azureml.core.authentication.AzureMlTokenAuthentication:
        ...
        child = Run(experiment, run_id,
                    outputs=outputs, _run_dto=run_dto, **kwargs)
        return child
    ...
    def get_context(cls):
        """
        Returns the run that was created by an experiment submission.  This method is used inside the experiment code.
        ...
        return _SubmittedRun(experiment, run_id, _cluster_url=url)

    @classmethod
    def get_submitted_run(cls):
        """
    ...
    @staticmethod
    def list(experiment, type=None, tags=None, properties=None, _rehydrate_runs=True):
        """
        ...
        run_dtos = RunClient.get_runs(experiment, runtype=type, tags=tags, properties=properties)
    ...
    def number(self):
        """
        Return the Run Number - a monotonically increasing number representing the order of runs within an experiment.
    ...
    def get_children(self, recursive=False, tags=None, properties=None, type=None, _rehydrate_runs=True):
        """
    ...
    def log(self, name, value, description=""):
        ...
        :type value: Undefined, The value to be posted to the service
        ...
        if any([isinstance(value, list),
            ...
            self._client.metrics.log_list(self.id, name, value, description=description)
        else:
            self._client.metrics.log_scalar(self.id, name, value, description=description)
    ...
    def log_list(self, name, value, description=""):
        ...
        :type description: str
        """
        self._client.metrics.log_list(self.id, name, value, description)
    ...
    def log_row(self, name, description=None, **kwargs):
        ...
        :type description: str
        """
        self._client.metrics.log_row(self.id, name, value=kwargs, description=description)
    ...
    def log_table(self, name, value, description=""):
        ...
        :type description: str
        """
        self._client.metrics.log_table(self.id, name, value, description)
    ...
    def log_image(self, name, path=None, plot=None):
        ...
        """
        self._client.log_image(name, path=path, plot=plot)
