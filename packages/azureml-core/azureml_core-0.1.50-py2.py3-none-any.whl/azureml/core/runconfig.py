...
DEFAULT_CPU_IMAGE = "mcr.microsoft.com/azureml/base:0.1.2"
DEFAULT_GPU_IMAGE = "mcr.microsoft.com/azureml/base-gpu:0.1.2"
...
class RunConfiguration(AbstractRunConfigElement):
    """
    This class encapsulates the execution details of a run.
    It describes how the run is invoked, where the run is executed and which platform to be used.
    The RunConfiguration also contains a list of resources and settings made available to the run during execution.

    .. attribute:: script

        The relative path to the python script file to be invoked.
        The file path is relative to the source_directory passed to :func:`azueml.core.experiment.Experiment`.

    .. attribute:: arguments

        Command line arguments to the script specified in a list.

    .. attribute:: framework

        The supported frameworks are Python and PySpark.
        When running on Batch AI managed clusters TensorFlowParameterServer and PythonMPI
        are also supported for distributed training runs.

    .. attribute:: environment

        The environment definition, This field describes which python environment to use or to setup for executing the script.
        Type of :class:`EnvironmentDefinition`

    .. attribute:: auto_prepare_environment

        Defaulted to True, but if set to False the run will fail if no environment matching the environment definition requirements already exists.
        User can set this value to False if they prefer to fail fast when the environment is not found instead of automatically creating and caching it.

    .. attribute:: max_run_duration_seconds

        Maximum allowed time for the run. If the run exceeded this value the system will attempt to automatically cancel it.

    .. attribute:: history

        This section is used to disable and enable experiment history logging features.
        Type of :class:`HistoryConfiguration`

    .. attribute:: spark

        When the platform is set to Pyspark,
        The spark configuration is used to set the default sparkconf for the submitted job.
        Type of :class:`SparkConfiguration`

    .. attribute:: hdi

        This parameter takes effect only when the target is set to an Azure HDI compute.
        The HDI Configuration is used to set the YARN deployment mode.
        The configuration is defaulted to cluster mode.
        Type of :class:`HDIConfiguration`

    .. attribute:: batchai

        The attribute is used to configure the required node count for job.
        This parameter takes effect only when the target is set to an Azure Batch AI compute.
        Type of :class:`BatchAIConfiguration`

    .. attribute:: tensorflow

        The attribute is used to configure the distributed tensorflow job parameters.
        ...
        Type of :class:`TensorFlowConfiguration`
    ...
    .. attribute:: mpi
        ...
        Azure Batch AI is the only supported compute type for this configuration.
        Type of :class:`MPIConfiguration`

    .. attribute:: container_instance

        The attribute is used to configure a new Azure Container Instance for this run.
        ...
        Type of :class:`ContainerInstanceConfiguration`

    .. attribute:: data_references

        A list of :class:`DataReferenceConfiguration`, all the data sources are made available to the run
    ...
    def __init__(self, script=None, arguments=None, framework=None,
        ...
        """
        Initializes a RunConfiguration with many default settings.
