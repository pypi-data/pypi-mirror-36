...
class ClientBase(ChainedIdentity):
    ...
    def __init__(self, host, auth, workerpool=None, logger=None, **kwargs):
        """
    ...
    def __del__(self):
        ...
        if hasattr(self, "_pool") and self._pool is not None:
            with self._log_context("ThreadPoolShutDown"):
                self._pool.shutdown()
    ...
    def _get_thread_pool(self):
        if not self._pool:
            self._pool = WorkerPool(_parent_logger=self._logger)
            self._logger.info("Created a threadpool for first use")
        else:
            self._logger.info("Access an existing threadpool")
        return self._pool
    ...
    def _call_api(self, func, *args, **kwargs):
        ...
        if is_async:
            ...
            future = pool.submit(self._execute_with_base_arguments, func, *args, **kwargs)
            ident = func.__name__
...
3. You have multiple objects/login session opened, please close all session and try again.
                    ...
                    raise UserErrorException(error_meg)
                raise
