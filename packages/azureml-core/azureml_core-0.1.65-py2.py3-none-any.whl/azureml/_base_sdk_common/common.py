...
def fetch_tenantid_from_aad_token(token):
    import base64
    jwt_token_body = token.split(".")[1]
    missing_padding = len(jwt_token_body) % 4
    if missing_padding != 0:
        jwt_token_body += '=' * (4 - missing_padding)
    aad_token = base64.b64decode(jwt_token_body)
    json_obj = aad_token.decode('utf8')
    token_dict = json.loads(json_obj)
    return token_dict['tid']
