...
def write_frame(file, frame, position, orientation):
    file.writestr('{0}/position'.format(frame), position.tostring());
    file.writestr('{0}/orientation'.format(frame), orientation.tostring());

def read_frame(file, frame):
    position = file.read('{0}/position'.format(frame));
    orientation = file.read('{0}/orientation'.format(frame));
...
def run_benchmarks(N, size):
    ...
    compress_mode = zipfile.ZIP_STORED;
    if compression:
    ...
    start = time.time();
    file = zipfile.ZipFile('test.zip', mode='w', compression=compress_mode, allowZip64=True);
    ...
    start = time.time();
    file = zipfile.ZipFile('test.zip', mode='r', compression=compress_mode, allowZip64=True);
    end = time.time();
