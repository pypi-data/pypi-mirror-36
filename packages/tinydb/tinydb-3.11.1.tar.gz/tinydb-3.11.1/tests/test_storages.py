...
def test_json_encoding(tmpdir):
    ...
    db_file = tmpdir.join('test.db')
    db_file.write_text('''{"_default": {"1": {"a": "%s"}}}''' % (utf8,), 'utf-8')

    with TinyDB(str(db_file)) as db:
        db.insert({'a': 'ברי צקלה'})

    with io.open(str(db_file), 'r+', encoding='utf-8') as fp:
        data = json.loads(fp.read())
        fp.seek(0)
        fp.write(json.dumps(data, ensure_ascii=False))
        fp.truncate()

    assert isinstance(TinyDB(str(db_file)), TinyDB)
