...
class _GfeedClientFactory(WebSocketClientFactory, ReconnectingClientFactory):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.on_message_fundamental_data = None
...
class GfeedClient(object):
    ...
    def connect(self):
        ...
        self.factory.on_message_fundamental_data = self._on_message_fundamental_data
    ...
    def get_fundamental_data(self, exchange):
        str_message = {}
        str_message["MessageType"] = "GetFundamentalInstruments"
        str_message["Exchange"] = exchange
        payload = (json.dumps(str_message)).encode('utf8')
        self.base_client.sendMessage(payload, isBinary=False)
    ...
    def _on_message_fundamental_data(self, l_i):
        if self.on_message_fundamental_data:
            self.on_message_fundamental_data(l_i)
    ...
    def _on_message(self,base_client, payload, isBinary):
        ...
        if message.find("\"Complete\":true") != -1 or message.find("\"AllowVMRunning\":false") != -1 \
            ...
            self._on_message_fundamental_data(message)
