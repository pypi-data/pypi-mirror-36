...
class TD_IPSO_3304_01(CoAPTestCase):
    """
testcase_id: TD_IPSO_3304_01
uri : http://openmobilealliance.org/iot/lightweight-m2m-lwm2m
configuration: LWM2M_CFG_01
objective: 
  - Querying Humidity Sensor Object instance (ID = 3304) representation in JSON format on LwM2M client
pre_conditions: Device is registered at the LWM2M server
sequence:
  - step_id: 'TD_IPSO_3304_01_step_01'
    type: stimuli
    node : lwm2m_server
    description:
      - 'Server sends a READ request (COAP GET) on Humidity Sensor Object'
      - - Type = 0 (CON)
        - Code = 1 (GET)

  - step_id: 'TD_IPSO_3304_01_step_02'
    type: check
    description:
      - 'The request sent by the server contains'
      - - Type=0 and Code=1
        - Accept option = application/vnd.oma.lwm2m+json
        - Uri-Path option = 3304/0

  - step_id: 'TD_IPSO_3304_01_step_03'
    type: check
    description:
  ...
  - step_id: 'TD_IPSO_3304_01_step_04'
    type: verify
    node: lwm2m_server
    description:
    ...
    def run(self):
        self.match('server', CoAP(type='con', code='get', opt=self.uri('/3304/0')), 'fail')
