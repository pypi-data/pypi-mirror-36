...
async def router(
    ...
    try:
        if isinstance(req_params, List):
            handler_result = await handler(*req_params)
            
            if isinstance( handler_result, Dict ) \
                    and 'code' in handler_result \
                    and 'message' in handler_result:
                result = error_dict( request_id=req_id, **handler_result )
            else:
...
def error_object( code:int, message:str, data=None ) -> Dict:
    """Return the Dict for represent JSON-RPC Error object returned from handler.
    For situation when server handler want to return 'error' instead of 'result'.
    ...
    Keyword Arguments:
        data  -- json representable value for additional info about error (default: {None})
