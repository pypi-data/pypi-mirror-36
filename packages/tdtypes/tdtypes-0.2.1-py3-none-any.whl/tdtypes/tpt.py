...
class FileReaderOp(ProducerOp):
	...
	def __init__(self, tbl, dir=None, dlm=None, esc='\\', quote=None, fit=False, empty=False, skip1=False, **kwargs):
		super().__init__('FILE_READER', sch=tbl, dlm=(dlm!=None), **kwargs)

		if dlm is None:
			self.vars['DCPFormat']        = 'Binary'
			self.vars['DCPIndicatorMode'] = True
		else:
			self.vars['DCPFormat']        = 'Delimited'
			self.vars['DCPEscapeTextDelimiter'] = esc
			if empty:
				self.vars['DCPAcceptMissingColumns'] = empty
				self.vars['DCPNullColumns'] = empty

		if dir: self.vars['DCPDirectoryPath'] = dir
		if fit: self.vars['DCPTruncateColumnData'] = True

		if dlm:
			try:
				self.vars['DCPTextDelimiterHEX'] = format(int(dlm), '02X')
			except ValueError:
				self.vars['DCPTextDelimiter'] = dlm

		if quote is not None:
			self.vars['DCPQuotedData'] = quote

		self.attrs['FileName'] = tbl.src
		if skip1: self.attrs['SkipRows'] = 1
...
class TPTJob:
	...
	def run(self, jobvar: str = None, chkpt: str = None) -> int:
		...
		# No native TTU for Cygwin, translate paths and use cmd shell
		if platform.system().startswith('CYGWIN'):
			cmd = 'cmd /c '
			fpath = lambda p: "$(cygpath -w " + p + ")"
		else:
			cmd = ''
			fpath = lambda p:p

		cmd += 'tbuild -f {} '.format(fpath(script_file))

		if jobvar:
			cmd += '-v {} '.format(fpath(jobvar))
		if chkpt:
			cmd += '-z {} '.format(chkpt)
		cmd += self.name
		...
		try:
			...
			logger.error("tbuild command failed with error code: {}. Manually remove '{}'".format(msg.returncode, script_file))
			return 1
	...
	def __str__(self):
		...
		jvars = '\n\n'.join(v.as_decl() for v in self.varlist if v)
		steps = '\n\n'.join([step2str(s) for s in self.steps])
...
(
    ...
    {}
);""".format(self.name, jvars.replace('\n','\n    '), steps.replace('\n','\n    '))
