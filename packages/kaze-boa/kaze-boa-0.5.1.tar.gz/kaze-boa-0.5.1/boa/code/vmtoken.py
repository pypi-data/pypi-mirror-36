...
#            pdb.set_trace()
    ...
    def is_sys_call(op):
        ...
        :return:
        """
        if op is not None and kaze_SC_FRAMEWORK in op:
            return True
        return False
    ...
    def convert_sys_call(self, op, pytoken=None):
        ...
        if 'TriggerType.ApplicationR' in op:
            ...
            if op == 'kaze.Enumerator':
                op = 'kaze.Enumerator.Create'

        syscall_name = op.replace(kaze_SC_FRAMEWORK, '').encode('utf-8')
    ...
    def convert_built_in(self, op, pytoken):
        ...
        syscall_name = None
        if op == 'print':
            syscall_name = 'kaze.Runtime.Log'.encode('utf-8')

        elif op == 'enumerate':
            syscall_name = b'kaze.Enumerator.Create'
        elif op == 'iter':
            syscall_name = b'kaze.Iterator.Create'
        elif op == 'next':
            syscall_name = b'kaze.Enumerator.Next'
    ...
    def convert_notify_event(self, pytoken):
        ...
        # insert syscall
        syscall_name = 'kaze.Runtime.Notify'.encode('utf-8')
        length = len(syscall_name)
        ba = bytearray([length]) + bytearray(syscall_name)
        vmtoken = self.convert1(VMOp.SYSCALL, pytoken, data=ba)
#        self.insert1(VMOp.NOP)
