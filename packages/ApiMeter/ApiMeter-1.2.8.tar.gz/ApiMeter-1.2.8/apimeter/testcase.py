...
class TestcaseLoader(object):
        ...
        for item in FileUtils.load_file(file_path):
            ...
            if key == "config":
                ...
                if "api" in test_block:
                    ...
                    def_block = TestcaseLoader._get_block_by_name(ref_call, "api")  # override the passed parameter values
                    TestcaseLoader._override_block(def_block, test_block)  # override the passed extractors and validators
                    ...
                    # testset["config"].update(block["config"])  # TODO: don't need -> "path": file_path
                else:
                    testset["testcases"].append(test_block)
    ...
    def _get_block_by_name(ref_call, ref_type):
        """ get test content by reference name and use the passed parameter values
        @params:
