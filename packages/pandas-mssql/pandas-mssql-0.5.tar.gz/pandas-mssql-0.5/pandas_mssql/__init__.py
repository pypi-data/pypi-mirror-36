...
def read_mssql(
            table, user, password, server, database, params=None
            ):
    """Read MSSQL query or database table into a DataFrame."""
    # table = ('' if schema is None else schema + '.') + table # removed for now
...
def callproc(proc, user, password, server, database, params=None):
    if params is None:
        params = []
    engine = get_engine(user, password, server, database)
    ...
    try:
        ...
        cursor.execute(proc, params) # changed from callproc
        try:
