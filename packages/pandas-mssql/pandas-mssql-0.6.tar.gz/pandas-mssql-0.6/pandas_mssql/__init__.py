...
def read_mssql(
            table, user, password, server, database, schema=None, params=None
            ):
    """Read MSSQL query or database table into a DataFrame."""
    if schema is not None:
        table = f'SELECT * FROM {schema}.{table}'
...
def execute(stmt, user, password, server, database, params=None):
    if params is None:
        params = []
    engine = get_engine(user, password, server, database)
    ...
    try:
        ...
        cursor.execute(stmt, params) # changed from callproc
        try:
