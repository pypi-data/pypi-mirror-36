...
def workflow_selector(input_file, ID, atlas_select, network, node_size, mask, thr, uatlas_select, multi_nets,
    ...
    if input_file:
        ...
        if k_clustering > 0:
            ...
            meta_wf.get_node("%s%s" % (wf_selected, '.clustering_node'))._mem_gb = 6
        meta_wf.get_node("%s%s" % (wf_selected, '.get_conn_matrix_node'))._n_procs = 2
        meta_wf.get_node("%s%s" % (wf_selected, '.get_conn_matrix_node'))._mem_gb = 6
...
def functional_connectometry(func_file, ID, atlas_select, network, node_size, mask, thr, uatlas_select, conn_model,
    ...
    get_conn_matrix_node.interface.n_procs = 2
    get_conn_matrix_node._mem_gb = 4
    get_conn_matrix_node.n_procs = 2
