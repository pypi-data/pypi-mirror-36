...
def load_conf(xfile, options = None):
    ...
    if not conf.get('endpoints'):
        raise FdReplayConfigurationError("Missing 'endpoints' section in configuration")

    for name, ept_cfg in conf['endpoints'].iteritems():
        cfg     = {'general':  dict(conf['general']),
                   'fd-replay': {'endpoint_name': name,
                                 'config_dir':    config_dir},
                   'vars' :    {},
                   'config':   {}}

        if 'plugin' not in ept_cfg:
            raise FdReplayConfigurationError("Missing 'plugin' option in endpoint: %r" % name)

        if ept_cfg['plugin'] not in PLUGINS:
            raise FdReplayConfigurationError("Invalid plugin %r in endpoint: %r"
                                             % (ept_cfg['plugin'],
                                                name))
