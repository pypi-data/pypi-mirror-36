...
class JobParser(object):
    ...
    def _build_run_args(self):
        ...
        input_data = {
            ...
            'image': self.kwargs.pop('image', 'missinglinkai/tensorflow:gce'),
    ...
    def submit_job(self, job_env):
        return self.call_api('put', '{}/{}/invoke'.format(job_env.org, job_env.project), job_env.input_data)

    @classmethod
    def _command_extent_with_secure(cls, command, job_env):
        cls._command_extend_from_iter(command, 'env', list(job_env.secure_env.items()))
        if job_env.docker_creds:
            cls._command_extend_from_iter(command, 'docker-auth', [(job_env.docker_creds['endpoint'], job_env.docker_creds['user'], job_env.docker_creds['password'])])

    @classmethod
    def _command_extend_from_iter(cls, command, arg_name, iter_):
        for itm in iter_:
    ...
    def build_local_command(self, job_env):
        ...
        command.extend(['--job', job_response['invocation']])
        ...
        self._command_extend_from_iter(command, 'env', env.items())
        self._command_extend_from_iter(command, 'mount', list(volumes.items()))
        ...
        return job_response['invocation'], dict(command=' '.join([str(x) for x in command]), image=self.docker_tools.config.rm_manager_image,
                                                auto_remove=True, volumes=rm_volumes, environment=rm_env, detach=True)
    ...
    def _build_local_docker_env(self, prefix):
        rm_env = {}
        rm_env['ML_CONFIG_VOLUME'] = self.docker_tools.config.rm_config_volume
        if prefix:
            rm_env['ML_PIP_INDEX'] = 'https://test.pypi.org/simple/'
        else:
            rm_env['ML_PIP_INDEX'] = 'https://pypi.org/simple/'
        rm_env['ML_PIP_VERSION'] = '>=2.4'
        return rm_env
