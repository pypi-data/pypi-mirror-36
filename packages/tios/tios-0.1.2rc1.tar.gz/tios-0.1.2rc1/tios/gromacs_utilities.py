...
def trajectory_from_tpr_file(tprfile):
    """
    Create an MDTraj Trajectory from the tprfile.
    ...
    """
    try:
        topfile = tempfile.NamedTemporaryFile(suffix='.pdb', delete=False).name
        result2 = None
...
def gromacs_check_version():
    ...
    if gmx is None:
        return None
    try:
        output = subprocess.check_output([gmx, '-version'], stderr=subprocess.STDOUT)
    except:
        return None
    version = 'unknown'
    for line in output.split(b'\n'):
        words = line.split()
        if b'version:' in words:
            version = words[-1].decode()
            break
