...
class BacktestResult:
    ...
    def benchmark(self):
        ...
        df = pd.DataFrame(
            ...
            columns=st['data']['dates']).T
        columns = ['portfolio_value', 'cash', 'positions_value', 'returns', 'pnl',
        ...
        df = df.reset_index()
        df = Util.convert_df(
            df, date_columns=["index"], number_columns=columns)
    ...
    def symbol_summary(self):
        ...
        df = df.merge(pd.DataFrame(data, columns=columns,
                                   index=df.index), left_index=True, right_index=True)

        return Util.convert_df(df, date_columns=[], number_columns=["txn_count", "position_count", "valuation", "return"])
    ...
    def trade_history_by_symbol(self, symbol):
        ...
        df = Util.convert_df(df, date_columns=["order_date", "complete_date"],
