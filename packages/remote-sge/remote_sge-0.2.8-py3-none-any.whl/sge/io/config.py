...
if 'PYTHONPATH' in environ:
    DEV_CONF = join(abspath(getenv('PYTHONPATH')), 'etc/config.ini')
else:
    DEV_CONF = join(getcwd(), 'etc/config.ini')
...
def load_config():
    """
        ...
        Returns:
            :class:`~configobj.ConfigObj` object containing the aggregate settings
    ...
    files.reverse()
    return ConfigObj(infile=next(file for file in files if exists(file)))
