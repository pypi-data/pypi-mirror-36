...
class IniFile1L(object):
    ...
    def __init__(self, fname):
        ...
        self.encode = dict(encoding= 'utf-8')
        if OP.exists(self.fname):
            with copen(self.fname, 'r', **self.encode) as f:
                self.data = json.load(f)
        else:
            self.data = dict()
    ...
    def save(self):
        with open(self.fname, 'w') as f:
            json.dump(self.data, f, indent= 2, **self.encode)
...
def update_package(package_name, minvers= '', pypi_name=None, deps= False, 
    ...
    if vers:
        if not minvers:
            if verbose: print(package_name, 'is already installed')
            return False
