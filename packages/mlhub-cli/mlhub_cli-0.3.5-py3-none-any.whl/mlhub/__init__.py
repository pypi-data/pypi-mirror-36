...
def deploy(name):
    ...
    answers = prompt([
        dict(type='input', name='desc', message='Input a description for this deployment', validate=validate_desc),
    ...
    try:
        ...
        creds = service_account.Credentials.from_service_account_file(package_path + '/sa.json')
        bucket = storage.Client(project='mlhub-firebase', credentials=creds).bucket('mlhub-firebase.appspot.com')
