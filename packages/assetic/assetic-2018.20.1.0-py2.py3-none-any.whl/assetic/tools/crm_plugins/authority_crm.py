...
class AuthorityCRM(CRMPluginBase):
    def __init__(self, **kwargs):
        ...
        self._wr_sub_type_id_mappings = list()
        ...
        self._wr_sub_type_id_representation = AuthorityAsseticWRSubTypeCode

        if self._db_type == "informix":
            self._sql_get_new_crm = """
            select distinct d.fmt_acc, d.wrk_typ, d.dda_cd1, d.dda_cd2
            ,d.doc_num,d.rec_dte,d.cre_dte_tme, d.doc_pre
            ,trim(d.ext_ref) as ext_ref
            ,d.mod_opr
            ,p.str_nme as prop_st, p.sbr_nme as prop_suburb
            ...
            from aualdocs d 
            ,outer auallnks l
            ,outer(auallnks l2, outer aunrmast cn)
            ...
            --create an address string from address fields
            ...
            + ' ' +
            RTRIM(ISNULL(p.str_nme,'')) + ' ' + RTRIM(ISNULL(p.str_typ,'')) + ' ' + RTRIM(ISNULL(p.sbr_nme,'')) as prop_addr
            --address string created - represents location of defect as entered in CRM
    ...
    @property
    def wr_sub_type_id_representation(self):
        return self._wr_sub_type_id_representation

    @property
    def wr_sub_type_id_mappings(self):
        return self._wr_sub_type_id_mappings

    @wr_sub_type_id_mappings.setter
    def wr_sub_type_id_mappings(self, value):
        if isinstance(value, list):
            self._wr_sub_type_id_mappings = [
                i if isinstance(i, AuthorityAsseticWRSubTypeCode)
                else
                self.logger.warning("wr_sub_type_id_mappings list must contain"
                                    "AuthorityAsseticWRSubTypeCode objects.  "
                                    "Ignoring invalid list item.")
                for i in value]
        else:
            self.logger.error("wr_sub_type_id_mappings should be a list")
    ...
    def get_resolution_code_for_status(self, wr_status, crm_obj):
        ...
        for row in self.resolution_codes:
            ...
            if not row.crm_category1: row.crm_category1 = crm_obj.category1
            if not row.crm_category2: row.crm_category2 = crm_obj.category2
            if not row.crm_task: row.crm_task = crm_obj.task_key
            if not row.crm_workflow: row.crm_workflow = crm_obj.workflow
            if row.wr_status.lower() == wr_status.lower() \
                    ...
                    and row.crm_task == crm_obj.task_key:
                return row
    ...
    def _crm_sql_to_object_list(self, sql, params=None):
        ...
        for row in result:
            ...
            try:
                ...
                pass
            try:
                authority_obj.propertyref = row.prop_addr
            except AttributeError:
                pass
            try:
    ...
    def clear_cancelled_wr_from_crm(self, wr_crm_obj):
        ...
        if wr_crm_obj.authority_obj.assetic_wr_guid_del:
            wr_guid = "{0};{1}".format(
                wr_crm_obj.authority_obj.assetic_wr_guid_del,wr_guid)
...
class AuthorityCrmDTO(object):
    ...
    def __init__(self, formatted_account=None, workflow=None, category1=None
                 , category2=None, crm_number=None,received=None, prop_st=None
                 , prop_sub=None, propertyref=None, road=None, road_type=None
    ...
    def get_crm_requests(self, integrated=False):
        ...
        if integrated:
            ...
            wr = self.authority_common.prepare_new_wr_object(
                authority_obj, self.wr_sub_type_id_mappings)
        wr_crm_obj.assetic_wr_representation = wr
...
class AuthorityCommon(object):
    ...
    def prepare_new_wr_object(self, authority_obj, wr_sub_type_id_mappings):
        ...
        # set WR subtype
        subtype_id = self._get_wr_subtype(
    ...
    def _get_wr_subtype(self,crm_obj, wr_sub_type_id_mappings):
        """
        For the given workflow and task get the wr sub type ID utilising mapping
        set when adapter initialised
        :param crcm_obj: The Authority CRM object
        :param wr_sub_type_id_mappings: user defined mapping of subtask id to
        """
        for row in wr_sub_type_id_mappings:
            # since the resolution filters are optional set the undefined ones
            # to the current value so simplify the test
            if not row.crm_category1: row.crm_category1 = crm_obj.category1
            if not row.crm_category2: row.crm_category2 = crm_obj.category2
            ...
            if row.crm_workflow == crm_obj.workflow\
                    ...
                    and row.crm_category2 == crm_obj.category2 \
                    and row.crm_task == crm_obj.task_key:
                return row.wr_sub_type_id

        # No setting found so return default
        return 0
...
class AuthorityAsseticWRSubTypeCode(object):
    ...
    def __init__(self, wr_sub_type_id=None, crm_workflow=None
                 , crm_category1=None, crm_category2=None, crm_task=None):
