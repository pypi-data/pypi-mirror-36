from kazerpc.Settings import settings as rpc_settings
import requests
import binascii


class kazeRPCException(Exception):
    pass
...
class RPCClient():
    ...
    def get_block(self, height_or_hash, id=None, endpoint=None):
        ...
        Returns:
            block: a json object or the ``kazerpc.Core.Block.Block`` object
        """
    ...
    def get_block_sysfee(self, height, id=None, endpoint=None):
        """
        Get the system fee of a block by height.  This is used in calculating STREAM claims
        Args:
            height: (int) height of the block to lookup
            id: (int, optional) id to use for response tracking
            endpoint: (RPCEndpoint, optional) endpoint to specify to use
    ...
    def get_storage(self, contract_hash, storage_key, id=None, endpoint=None):
        ...
        try:
            ...
            raise kazeRPCException("could not decode result %s " % e)
    ...
    def _call_endpoint(self, method, params=None, id=None, endpoint=None):
        ...
        try:
            ...
            raise kazeRPCException("Could not call method %s with endpoint: %s : %s " % (method, endpoint.addr, e))
