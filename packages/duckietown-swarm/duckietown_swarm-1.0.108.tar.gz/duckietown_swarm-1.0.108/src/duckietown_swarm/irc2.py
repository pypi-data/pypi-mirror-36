...
from .udp_interface import UDP_PORT, get_suitable_udp_interfaces, \
...
class MessagingInterface(Dispatcher):
    ...
    def __init__(self, key, ipfs_path):
        ...
        for name, x in get_suitable_udp_interfaces().items():
            ...
            a = '/udp/%s/%s/%s' % (name, broadcast, UDP_PORT)
...
def duckietown_swarm_main(args=[]):
    ...
    parser.add_argument('--low-profile', default=False,
                        action='store_true',
                        dest='low_profile',
    ...
    if DSwarmConstants.use_udp:
        for x in mi.udps:
            ...
            pm = ProcessManager(udp_broadcaster,
                                (mi.copy_as(x), interface, UDP_PORT), x,
                                restart=True)
            pm.start()
