...
def get_suitable_udp_interfaces():
    ...
    for iface in ifaces:
        ...
        if netifaces.AF_INET in addrs:
            ...
            for _ in ip4:
                if 'broadcast' in _:
                    badd =  str(_['broadcast'])

                    if badd in badd2ifaces:
                        msg = 'Found that interfaces %s and %s are on the same subnet.' % (iface,
                                                                                           badd2ifaces[badd])
                        msg += 'Disabling broadcast on %s' % iface
                        print(msg)
                    else:
                        B[iface] = {'address': str(_['addr']),
                                    'broadcast': badd}

                    badd2ifaces[badd].append(iface)
