...
def get_returns_table(alpha_beta,
    ...
    risk_adjusted_rts_df = pd.DataFrame.from_dict(risk_adjusted_rts)
    returns_table = returns_table.append(risk_adjusted_rts_df)
...
def plot_returns_table(returns_table, benchmark_name=None):
    if benchmark_name is None:
        print(
            "Returns Analysis against Equal-Weight Factor Universe Portfolio")
    else:
        print("Returns Analysis against {}".format(benchmark_name))
...
def plot_quantile_statistics_table(factor_data):
    quantile_stats = get_quantile_statistics_table(factor_data)
...
def plot_ic_ts(ic, ax=None):
    ...
    for a, (period_num, ic) in zip(ax, ic.iteritems()):
        ...
        a.legend(['IC', 'moving avg (window_size=5)'], loc='upper right')
...
def plot_performance_comparison(combined_performance,
                                benchmark_name,
                                period,
                                ax=None,
                                title_suffix=None):
    for col in combined_performance.columns:
        ax.plot(combined_performance[col].dropna())
    title_template = "Performance Comparison against {} with rebalance period {} {}"
    title = title_template.format(benchmark_name, period, title_suffix)
    ax.set_title(title)
...
def plot_mean_quantile_returns_spread_time_series(mean_returns_spread,
    ...
    mean_returns_spread_bps.rolling(window=5).mean().plot(
        color='orangered',
        alpha=0.7,
        ax=ax
    )
    ax.legend(['mean returns spread', 'moving avg (window_size=5)'],
              loc='upper right')
...
def plot_ic_by_group(ic_group, ax=None):
    ...
    ax.set_xticklabels(ic_group.index, rotation=90)
...
def _to_label(category_map, category):
...
def plot_cumulative_returns_by_category(returns_by_category, period,
                                        title=None, ax=None,
                                        category_map=None):
    if ax is None:
        f, ax = plt.subplots(1, 1, figsize=(18, 6))

    def tail_mean(series, num_entries=6):
        return series[-num_entries:].mean()

    tail_mean_map = {}

    factor_return_map = {}

    for category, factor_return in returns_by_category.items():
        factor_return = factor_return[period]
        factor_return = perf.cumulative_returns(factor_return, period)
        t_mean = tail_mean(factor_return)
        tail_mean_map[category] = t_mean
        factor_return_map[category] = factor_return

    should_split = len(tail_mean_map) >= 5
    split_size = math.ceil(len(tail_mean_map) * 0.1)

    if should_split:
        sorted_mean = sorted(tail_mean_map.items(), key=lambda pair: pair[1])
        bottom_categories = [k for k, v in sorted_mean[0: split_size + 1]]
        top_categories = [k for k, v in sorted_mean[-split_size:]]
        categories_in_split = bottom_categories + top_categories

    for category, factor_return in returns_by_category.items():
        if should_split and category not in categories_in_split:
            continue
        factor_return = factor_return_map[category]
        factor_return.plot(ax=ax, lw=1.5, alpha=0.6,
                           label=_to_label(category_map, category),
                           c=numpy.random.rand(3, ))
    ax.legend(frameon=True, fancybox=True, fontsize='xx-small')
...
def plot_cumulative_returns(factor_returns, period, title=None, ax=None,
                            benchmark_return=None, result_tables=None):
    """
    ...
    if benchmark_return is not None:
        benchmark_returns = perf.cumulative_returns(benchmark_return, period)
        benchmark_returns.plot(ax=ax, lw=3, color='orange', alpha=0.6,
                               label='benchmark')
    if result_tables is not None:
        if 'cum_return' not in result_tables:
            result_tables['cum_return'] = {}
        result_tables['cum_return'][period] = factor_returns
