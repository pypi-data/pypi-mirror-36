...
import pandas as pd
from alphalens.utils import match_date
...
class GridFigure(object):
    ...
    def __init__(self, rows, cols, width=14, heights_per_row=7):
        self.rows = rows
        self.cols = cols
        self.fig = plt.figure(figsize=(width, rows * heights_per_row))
...
def create_summary_tear_sheet(factor_data,
    ...
    benchmark_return : pd.DataFrame
        benchmark return dataframe to compute alpha-beta against
    benchmark_name : str
        benchmark name to use
    """
    ...
    returns_table = plotting.get_returns_table(alpha_beta,
                                               mean_quant_rateret,
                                               mean_ret_spread_quant)
    plotting.plot_returns_table(returns_table, benchmark_name)
...
def _min_base_period(periods):
    return min([pd.Timedelta(p) for p in periods])


@plotting.customize
def create_returns_tear_sheet_by_categories(combined_factor,
                                            categorized_factors,
                                            long_short=True,
                                            group_neutral=False,
                                            category_map=None):
    combined_factor_returns = perf.factor_returns(combined_factor,
                                                  long_short,
                                                  group_neutral)
    returns_by_category = {}
    for category, factor_data in categorized_factors.items():
        factor_returns = perf.factor_returns(factor_data,
                                             long_short,
                                             group_neutral)

        returns_by_category[category] = factor_returns

    factor_return = list(returns_by_category.values())[0]
    fr_cols = len(factor_return.columns)
    vertical_sections = 2 + fr_cols * 3
    gf = GridFigure(rows=vertical_sections, cols=1, width=30,
                    heights_per_row=14)

    for p in factor_return:
        title = ('Categorized Factor Weighted '
        ...
        plotting.plot_cumulative_returns(combined_factor_returns[p],
                                         period=p,
                                         title=title,
                                         ax=ax)

        plotting.plot_cumulative_returns_by_category(returns_by_category,
                                                     period=p,
                                                     title=title,
                                                     ax=ax,
...
def create_returns_tear_sheet(factor_data,
    ...
    benchmark_return: pf.DataFrame, optional
        Period wise benchmark return
    benchmark_name: str, optional
        Benchmark name
    benchmark_price: pd.DataFramem, optional
        Original Benchmark price
    factor_name: str, optional
        Factor name
    show_performance: bool,optional
        If true, show cumulative return chart without averaging out on
        start date
    batch_mode: bool, optional
        If true, plotting is turned off, only performance result will be
        recorded in the result tables
    result_tables: dict, optional
        Dict to store the performance results
    """

    top_quantile = factor_data['factor_quantile'].unique().max()
    bottom_quantile = factor_data['factor_quantile'].unique().min()

    period_cols = utils.get_forward_returns_columns(factor_data.columns)

    result_tables['periods'] = period_cols
    result_tables['top_quantile'] = top_quantile
    result_tables['bottom_quantile'] = bottom_quantile

    factor_returns = perf.factor_returns(factor_data,
                                         long_short,
                                         group_neutral,
                                         result_tables=result_tables)
    result_tables['factor_returns'] = factor_returns
    ...
    mean_quant_rateret = \
        mean_quant_ret.apply(utils.rate_of_return, axis=0,
                             base_period=_min_base_period(
                                 mean_quant_ret.columns))
    ...
    mean_quant_rateret_bydate = mean_quant_ret_bydate.apply(
        ...
        base_period=_min_base_period(mean_quant_ret_bydate.columns)
    )

    compstd_quant_daily = \
        std_quant_daily.apply(utils.std_conversion, axis=0,
                              base_period=_min_base_period(std_quant_daily))

    if benchmark_return is not None:
        risk_adjusted_rts = \
            perf.risk_adjusted_returns(factor_returns, benchmark_return)
    ...
    if show_performance and benchmark_return is not None:
        compute_performance_sheet(factor_data=factor_data,
    ...
    vertical_sections = 2 + fr_cols * 3
    if benchmark_return is not None:
        vertical_sections += len(period_cols)
    gf = GridFigure(rows=vertical_sections, cols=1)
    ...
    if by_group:
        ...
        mean_quant_rateret_group = mean_return_quantile_group.apply(
            utils.rate_of_return, axis=0,
            base_period=_min_base_period(mean_return_quantile_group.columns)
        )
...
def compute_performance_sheet(factor_data, factor_returns, result_tables,
                              top_quantile, bottom_quantile,
                              benchmark_price, periods,
                              long_short, group_neutral):
    top_port_perf = perf.factor_returns(factor_data,
                                        long_short,
                                        group_neutral,
                                        quantile=top_quantile)
    bot_port_perf = perf.factor_returns(factor_data,
                                        long_short,
                                        group_neutral,
                                        quantile=bottom_quantile,
                                        is_bottom_quantile=True)

    result_tables['perf_dfs'] = {}
    for period in periods:
        top_cum_prod = perf._cum_prod(top_port_perf, period)
        bot_cum_prod = perf._cum_prod(bot_port_perf, period)
        factor_cum_prod = perf._cum_prod(factor_returns, period)
        ben_perf, = match_date(top_cum_prod, benchmark_price)
        combined_index = top_cum_prod.index.union(ben_perf.index)
        combined_df = pd.DataFrame(index=combined_index)
        combined_df['factor_top_quantile'] = top_cum_prod
        combined_df['factor_bottom_quantile'] = bot_cum_prod
        combined_df['factor'] = factor_cum_prod
        combined_df['benchmark'] = perf._to_percent_index(ben_perf)
        result_tables['perf_dfs'][period] = combined_df
...
@plotting.customize
def create_full_tear_sheet_by_categories(combined_factor,
                                         categorized_factors,
                                         long_short=True,
                                         group_neutral=False,
                                         by_group=False,
                                         category_map=None):
    create_returns_tear_sheet_by_categories(combined_factor,
                                            categorized_factors,
                                            long_short,
                                            group_neutral,
                                            by_group,
