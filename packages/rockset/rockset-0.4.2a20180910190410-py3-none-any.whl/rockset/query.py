...
class QueryStringSexpr(Query):
    ...
    def sqlbuild(self, sqlsel):
        raise ValueError(
            'cannot autogen SQL from s-expression: \'{}\''.format(self._sexpr)
        )
...
class QueryStringSQLText(Query):
    ...
    def sexpression(self):
        raise ValueError(
            'cannot autogen s-expression from SQL: \'{}\''
...
class FieldEqQuery(FieldOpQuery):
    def __init__(self, field, v):
        ...
        if field._is_array_field():
            parts = self._split_array_path(field)
            if len(parts) != 2:
                raise NotImplementedError(
                    'Field path with more than 1 array '
                    'fields not supported yet!'
                )
            unnest_t = _sqlexp([Symbol('UNNEST('), parts[0], Symbol(')')])
            self.unnest_query = SubQuery(
                QueryStringResource(Symbol(unnest_t), alias='u1')
                .select(parts[1])
            )
    ...
    def sqlexpression(self, **kwargs):
        if not self.field._is_array_field():
            return super(FieldEqQuery, self).sqlexpression(**kwargs)
        return _sqlexp(
            [self.value_p, Symbol('IN'), self.unnest_query], **kwargs
        )
...
class UnaryOpQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(self.op, self.field)
...
class FieldIsDefinedQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldIsDefinedQuery, self).__init__(field, 'is_defined',
            post_sqlop='IS NOT NULL')


class FieldIsNullQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldIsNullQuery, self).__init__(field, 'not-implemented',
            post_sqlop='IS NULL')


class FieldExistsQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldExistsQuery, self).__init__(field, 'not-implemented',
            pre_sqlop='EXISTS')
        self.field = field
...
class FieldIsNotNullQuery(UnaryOpQuery):
    ...
    def sqlexpression(self, **kwargs):
        return _sqlexp(
            FieldIsDefinedQuery(self.field) &
            FieldOpQuery(self.field, Symbol('NULL_VALUE'), 'ne', 'IS DISTINCT FROM'), **kwargs
        )
...
class ApplyQuery(Query):
    ...
    def sexpression(self):
        ...
        return _sexp_query(
            'apply', self.to_field, self.child.get_source(), self.child
        )
...
class ProximityQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(
            'proximity', self.search_field, self.analyzer, self.search_query
        )
...
class LookupQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(
            'lookup', self.new_field, self.target_query, self.local_field,
            self.child
        )
...
class AggregateQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(
            'aggregate', self.dimension_fields, self.aggregate_fields,
            self.child
        )
...
class LimitQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(
            'limit', self.max_results, self.skip_results, self.child
        )
...
class ScoreQuery(Query):
    ...
    def sexpression(self):
        return _sexp_query(
            'score', self.name, self.code, self.context, self.child
        )
