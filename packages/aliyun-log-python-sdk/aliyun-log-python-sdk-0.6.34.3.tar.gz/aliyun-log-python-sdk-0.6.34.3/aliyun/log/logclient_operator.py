...
def manage_shard(client, project, logstore, count):
    res = client.list_shards(project, logstore)
    rw_shards = [shard for shard in res.get_shards_info() if shard['status'] == 'readwrite']
    if count == len(rw_shards):
        return

    if count < len(rw_shards):
        # need to split shard
        rw_shard_range = [{'id': shard['shardID'],
                           'start': int('0x'+shard['inclusiveBeginKey'], base=16),
                           'end': int('0x'+shard['exclusiveEndKey'], base=16) }
                          for shard in rw_shards]
        for shard in rw_shard_range:
