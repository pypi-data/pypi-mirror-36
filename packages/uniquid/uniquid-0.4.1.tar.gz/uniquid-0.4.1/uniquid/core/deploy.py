...
class Deploy:
    ...
    def _aws_deploy(self, in_orch_info):
        ...
        aws_cmd = ('aws ec2 describe-images --filters \"Name=architecture,Values=x86_64\" \"Name=state,Values=available\" ')
        ...
        if ami_id is None:
            self._console.exception(constants.ERR_NO_AWS_IMAGE)
        self._console.ok(_TAB + 'AWS EC2 image choosen: ' + ami_id)
        # create instance using the selected image
        ...
        linux_cmd = 'ssh -i ' + keypair_path + ' ubuntu@' + ec2_public_ip + ' \"uname -a && exit 0\"'
        ...
        appconfig = appconfig.replace('ORGANIZATION', self._login_mgr.get_org_id())
