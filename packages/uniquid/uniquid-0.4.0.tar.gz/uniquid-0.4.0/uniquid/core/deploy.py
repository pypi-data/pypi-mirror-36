...
# AWS EC2 instance type to use for agent
_AWS_AMI_ID = 'ami-0552e3455b9bc8d50'
...
_aws_virtdev_config = {
    ...
    'mqttTopic' : '',
    'registryUrl': '',
    'awsAuthorizerName': 'UniquIDCustomAuth',
    'awsPrivateKey': [],
...
class Deploy:
    ...
    def deploy(self, in_platform):
        ...
        if in_platform not in constants.PLATFORMS:
            self._console.exception(constants.ERR_BAD_PLATFORM)
    ...
    def _aws_deploy(self, in_orch_info):
        ...
        if not self._console.confirm("Do you want to start the deployment?",
                                 in_default=False):
            self._console.exception(constants.TXT_AWS_EXIT_EARLY)
        ...
        if result.returncode != 0:
            ...
            self._console.exception(constants.ERR_MISSING_DEPENDENCY)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        if self._console.confirm("Do you want to create another AWS Security Group?",
            ...
            sec_group_name = self._console.prompt('Enter the name of the new AWS security group name',
                                             in_type=str,
                                             in_default='uniquid-agent-sg',
                                             in_show_default=True)
            for group in sec_groups:
                if sec_group_name in group:
                    self._console.exception(constants.ERR_AWS_SEC_GROUP_EXISTS)
            ...
            if 'GroupId' in sec_group_dict:
                ...
                self._console.ok(constants.TXT_AWS_SEC_GROUP_CREATED +
                                 sec_group_id)
            else:
                self._console.exception(constants.ERR_AWS_REPLY_MISSING_DATA + 'GroupId')
        else:
            ...
            for group in sec_groups:
                if sec_group_name == group['Name']:
                    ...
                    isMatch = True
            if not isMatch:
                self._console.exception(constants.ERR_AWS_SEC_GROUP_NOT_EXISTS)
        # open ports for security group
        self._console.ok('Incoming TCP ports must be open for the security group. Port numbers: ' + ', '.join(constants.AWS_SEC_GROUP_PORTS))
        if self._console.confirm('Do you want to open inbound TCP ports of '
                                 'the Security Group ' + sec_group_name + '?',
                                 in_default=False):
            for portnum in constants.AWS_SEC_GROUP_PORTS:
                ...
                self._console.ok(constants.TXT_AWS_PORT_OPENED + portnum)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        if self._console.confirm('Do you want to create a key-pair '
            ...
            if (not os.path.exists(keypair_path) or
                ...
                self._console.exception(constants.ERR_AWS_KEYPAIR_FILE_READ)
        ...
        if (keypair_name is None or
                len(keypair_name) == 0):
            self._console.exception(constants.ERR_AWS_INVALID_KEYPAIR_NAME)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        if self._console.confirm('Do you want to create a new AWS EC2 Instance'
                                 ...
                                 in_default=False):
            aws_cmd = ('aws ec2 run-instances --image-id ' + _AWS_AMI_ID +
                       ' --security-group-ids ' + sec_group_id +
                       ' --count 1 --instance-type t2.micro --key-name ' +
                       keypair_name + ' --query \"Instances[0].InstanceId\"')
        ...
        if (ec2_public_ip is None or
            len(ec2_public_ip) == 0):
            self._console.exception(constants.ERR_AWS_CANNOT_FIND_INSTANCE +
                                    ec2_id)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        result = self._run_command(aws_cmd)
        ec2_public_dns = result.stdout.decode('utf-8').strip().replace('\"','')
        if (ec2_public_dns is None or
            len(ec2_public_dns) == 0):
            self._console.exception(constants.ERR_AWS_CANNOT_FIND_INSTANCE +
                                    ec2_id)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        while status != 'running':
            self._console.ok(constants.TXT_AWS_WAIT_INSTANCE_RUNNING)
            time.sleep(30)
            status = self._aws_get_instance_status(ec2_id)
        # connect to ec2 instance
        step = self._print_step(step, 'Verify login to AWS EC2 instance.')
        linux_cmd = 'ssh -i ' + keypair_path + ' ubuntu@' + ec2_public_ip + ' \"uname -a && exit\"'
        result = self._run_command(linux_cmd)
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        package_url = ('http://' + constants.UNIQUID_STATIC_IP + ':' +
                       constants.UNIQUID_STATIC_PORT +
                       constants.AWS_AGENT_URL_PATH)
        ...
        if orch_props is None:
            self._console.exception(constants.ERR_ORCH_REQ_FAIL)
        config_path = os.path.join(_OUTPUT_DIR, _AGENT_TAR_SUBDIR, 'appconfig.properties')
        appconfig = ''
        ...
        appconfig = appconfig.replace('TOPIC', orch_props.get('mqttTopic', '[Error]'))
        appconfig = appconfig.replace('NODE_NAME', _AWS_AGENT_PREFIX + aws_account_id)
        ...
        os.chdir(node_dir)
        result = self._run_command('npm install response')
        self._console.ok(_TAB + constants.TXT_PASS)
        # zip package
        ...
        with zipfile.ZipFile('UniquidLambda.zip', 'w') as zf:
            zf.write(os.path.join('lambda', 'UniquidLambda.js')) # FIXME hardcoded
        ...
        time.sleep(20)
        cmd = 'aws lambda create-function --function-name UniquidLambda --runtime nodejs8.10 --role ' + aws_role_arn + ' --handler UniquidLambda.handler --zip-file fileb://' + node_dir + '/UniquidLambda.zip --region ' + aws_region
        result = self._run_command(cmd)
        result_string = result.stdout.decode('utf-8').strip()
        result_json = json.loads(result_string)
        ...
        time.sleep(30)
        cmd = 'aws lambda invoke --invocation-type RequestResponse --function-name UniquidLambda --region ' + aws_region + ' --log-type Tail --payload \'{\"key1\":\"value1\", \"key2\":\"value2\", \"key3\":\"value3\"}\' ' + node_dir + '/lambda-out.txt'
        result = self._run_command(cmd)
        # TODO check the result from invocation
        self._console.ok(_TAB + constants.TXT_PASS)
        # generate key pair
        ...
        with open('virtual_device_key_pair.pem', 'w') as f:
            f.write(rsa_private_key + '\n')
            f.write(rsa_public_key)
            self._console.ok('Key pair file written: virtual_device_key_pair.pem') # FIXME hardcoded path
        self._console.ok(_TAB + constants.TXT_PASS)
        # create authorizer
        step = self._print_step(step, 'Create the IoT Authorizer.')
        cmd = 'aws iot create-authorizer --authorizer-name UniquIDCustomAuth --authorizer-function-arn ' + aws_function_arn + ' --token-key-name UniquIDToken --token-signing-public-keys FIRST_KEY=\"' + rsa_public_key + '\" --status ACTIVE --region ' + aws_region
        result = self._run_command(cmd)
        result_string = result.stdout.decode('utf-8').strip()
        result_json = json.loads(result_string)
        ...
        cmd = 'aws lambda add-permission --function-name UniquidLambda --principal iot.amazonaws.com --source-arn ' + aws_authorizer_arn + ' --statement-id UniquID-Id-123 --action \"lambda:InvokeFunction\"'
        self._console.ok(_TAB + constants.TXT_PASS)
        ...
        _aws_virtdev_config['mqttTopic'] = orch_props.get('mqttTopic', '[Error]')
        _aws_virtdev_config['registryUrl'] = 'http://' + self._login_mgr.get_api_ip() + ':8060' # demo work around # orch_props.get('registryUrl', '[Error]')
        _aws_virtdev_config['awsPrivateKey'] = rsa_private_key.split('\n')
        _aws_virtdev_config['awsAgentName'] = _AWS_AGENT_PREFIX + aws_account_id
        _aws_virtdev_config['awsEndpointAddress'] = ec2_public_dns
    ...
    def _run_command(self, in_command):
        """Run a shell command.
        ...
        if result.returncode != 0:
            ...
            self._console.exception(constants.ERR_SHELL_COMMAND_FAILED)
        return result
    ...
    def _aws_configure_get(self, in_param_name):
        ...
        if (result.returncode != 0 or
            ...
            self._console.exception(constants.ERR_AWSCLI_NOT_CONFIGURED)
    ...
    def _aws_get_instance_status(self, in_ec2_id):
        ...
        """
        retval = None
        query_cmd = ('aws ec2 describe-instance-status --instance-ids ' +
                     in_ec2_id)
        arg_list = shlex.split(query_cmd)
        result = subprocess.run(arg_list,
                                stdout=subprocess.PIPE,
    ...
    def _aws_list_security_groups(self):
        ...
        retlist = []
        query_cmd = 'aws ec2 describe-security-groups'
        arg_list = shlex.split(query_cmd)
        result = subprocess.run(arg_list,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        if (result.returncode != 0):
            self._console.error(result.stderr.decode('utf-8').strip())
            self._console.exception(constants.ERR_AWS_COMMAND_FAILED)
        else:
            json_obj = json.loads(result.stdout.decode('utf-8').strip())
            if 'SecurityGroups' in json_obj:
                ...
                self._console.exception(constants.ERR_AWS_REPLY_MISSING_DATA)
        return retlist
    ...
    def _aws_list_key_pairs(self):
        ...
        retlist = []
        query_cmd = 'aws ec2 describe-key-pairs'
        arg_list = shlex.split(query_cmd)
        result = subprocess.run(arg_list,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
    ...
    def _write_keypair_file(self, in_keypair):
        ...
        with fd as outfile:
            ...
            self._console.ok(_TAB + constants.TXT_AWS_KEYPAIR_FILE_WRITTEN +
                             fullpath)
        return fullpath
    ...
    def _get_file_handle(self, in_fullpath, in_mode='r'):
        ...
        try:
            ...
            self._console.exception(constants.ERR_AWS_KEYPAIR_FILE_CREATE +
                                    in_fullpath)
        return retval
