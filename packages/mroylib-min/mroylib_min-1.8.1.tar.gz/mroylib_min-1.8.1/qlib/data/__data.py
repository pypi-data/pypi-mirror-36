...
c.query(XXX, '~2', time=1512124620.0)   # time before 2017-12-1 18:37:00 +- 2 sec 's XXX
    ...
    def save(self, con=None, tp=None, check_same=False):
        ...
        _insert_str = "INSERT INTO %s (%s) VALUES(" % (self.__class__.__name__, ",".join([f'`{i}`' for i in __dict.keys()]))
        if tp == 'mysql':
        ...
        for k in __dict:
            ...
            if isinstance( v, str):
                _create_str += f"`{k}`" + " TEXT,"
            elif isinstance( v, bytes):
                _create_str += f"`{k}`" + " BLOB,"
            elif isinstance( v, (int, float)):
                if 'time' in k:
                    _create_str += f"`{k}`" + " DATETIME,"
                    ...
                    if isinstance(v, float):
                        _create_str += f"`{k}`" + " FLOAT,"
                    else:
                        _create_str += f"`{k}`" + " INTEGER,"
...
c.query(XXX, '~2', time=1512124620.0)   # time before 2017-12-1 18:37:00 +- 2 sec 's XXX
    ...
    def query(self,obj, eq="=", m='or', timestamp=None,**kargs):
        ...
        try:
            ...
            logging.error(f"{__query_str}  line:{710}")
    ...
    def remove(self, Obj, **kargs):
        if not kargs:
            ...
            try:
                ...
                logging.error(f"{_delete_str} line:804")
            except Exception as e:
                logging.error(f"{_delete_str} {vals} line:806")
    ...
    def delete(self, obj):
        ...
        try:
            ...
            logging.error(f"{_delete_str} line:820")
            raise e
        except Exception as e:
            logging.error(f"{_delete_str} {vals} line:823")
