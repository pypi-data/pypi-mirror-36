...
def find_double_edge_growth(tx, input_str, heavy_atom_diff_min=3, heavy_atom_diff_max=6,
                            c_heavy_atom_diff_min=3, c_heavy_atom_diff_max=6):
    return tx.run(
        "MATCH (sta:F2 {smiles:$smiles})-[nm:F2EDGE]-(mid:F2)-[ne:F2EDGE]-(end:EM) where"
        " sta.hac-end.hac > $hacmin and sta.hac-end.hac <= $hacmax"
        " and sta.hac-end.chac > $chacmin and sta.hac-end.hac <= $chacmax"
        " and sta.smiles <> end.smiles "
        "RETURN sta, nm, mid, ne, end "
        "order by split(nm.label, '|')[4], split(ne.label, '|')[2];",
        smiles=input_str, hacmin=heavy_atom_diff_min, hacmax=heavy_atom_diff_max,
        chacmin=c_heavy_atom_diff_min, chacmax=c_heavy_atom_diff_max
    )
