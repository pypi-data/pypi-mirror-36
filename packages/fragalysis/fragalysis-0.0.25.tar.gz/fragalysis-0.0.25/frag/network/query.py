...
def define_double_edge_type(record):
    ...
    ret_obj = ReturnObject(mol_one["smiles"], mol_three["smiles"], label, 2, change_frag, iso_label)
    if "." in label:
...
def define_proximal_type(record):
    ...
    mol_two = record["m"]
    ret_obj = ReturnObject(mol_one["smiles"], mol_two["smiles"], label, 1, change_frag, iso_label)
    if "." in label:
        ret_obj.frag_type = "LINKER"
