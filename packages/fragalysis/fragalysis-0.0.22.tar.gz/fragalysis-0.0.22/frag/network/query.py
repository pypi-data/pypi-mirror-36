...
class ReturnObject(object):
    ...
    def __init__(self, start_smi, end_smi, label, edge_count, change_frag=None):
        """
        Build this object.
        :param start_smi:
        :param end_smi:
...
def define_double_edge_type(record):
    ...
    change_frag = str(record["ne"]["label"].split("|")[1])
    mol_two = record["mid"]
    mol_three = record["end"]
    diff_one = mol_one["hac"] - mol_two["hac"]
    diff_two = mol_two["hac"] - mol_three["hac"]
    ret_obj = ReturnObject(mol_one["smiles"], mol_three["smiles"], label, 2, change_frag)
    if "." in label:
...
def define_proximal_type(record):
    ...
    change_frag = str(record["nm"]["label"].split("|")[1])
    mol_two = record["m"]
    ret_obj = ReturnObject(mol_one["smiles"], mol_two["smiles"], label, 1, change_frag)
    if "." in label:
        ret_obj.frag_type = "LINKER"
...
def organise(records, num_picks):
    ...
    for rec in records:
        rec_key = str(rec)
        if rec_key in out_d:
            out_d[rec_key].append(rec.change_frag)
        else:
            out_d[rec_key] = [rec.change_frag]
