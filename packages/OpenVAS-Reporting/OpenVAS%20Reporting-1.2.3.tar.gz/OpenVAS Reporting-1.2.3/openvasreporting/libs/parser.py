...
def openvas_parser(input_files, min_level=Config.levels()["n"]):
    ...
    for f_file in input_files:
        ...
        for vuln in root.findall(".//results/result"):
            ...
            #
            # VULN_TAGS
            # Remove double whitespace including whitespace, this does mess up lists
            # vuln_tags_text = re.sub(r"\s+", " ", nvt_tmp.find("./tags").text)
            # Remove double whitespace but not newlines, this does result in akward line breaks
