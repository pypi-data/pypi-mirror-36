...
class BinaryReader(object):
    ...
    def ReadVarInt(self, max=sys.maxsize):
        ...
        The KAZE network protocol supports encoded storage for space saving. See: http://docs.kaze.org/en-us/node/network-protocol.html#convention
    ...
    def ReadSerializableArray(self, class_name, max=sys.maxsize):
        ...
        Args:
            class_name (str): a full path to the class to be deserialized into. e.g. 'kaze.Core.Block.Block'
            max (int): (Optional) maximum number of bytes to read.
