...
def _parse_type(parameter_type, argument: dict) -> dict:
    ...
    if inspect.isclass(parameter_type) and issubclass(parameter_type, (list, tuple)):
        ...
        argument["nargs"] = '*'
    elif inspect.isclass(parameter_type) and issubclass(parameter_type, dict):
        # When adding a argument type, dict doesn't support string so we have to use a replacement function.
        # yaml lib is the best solution since it's already a dependency and allows for multiple input formats of a dict.
        argument["type"] = yaml.load
