...
x = h.sample(min_steps=1, max_steps=10)
# Sample vectors x[n, :] are now (nearly) independent samples,
...
# The sample method can be called repeatedly, to get new batches of samples,
# approximately independent of the previous batch.
...
class HamiltonianSampler:
    ...
    def __init__(self, fun, jac, x,
        ...
        :param min_accept_rate: (optional) error limit for accept rate
       """
        self._fun = fun
        self._jac = jac
    ...
    def safe_sample(self, n_samples=None,
        ...
        Check that accept_rate is above limit,
        and adjust epsilon until accept_rate is good.
        ...
        while self.epsilon > epsilon_start / 10.:
            try:
                ...
                logger.debug(e)
                self.epsilon *= (0.7 + 0.2 * uniform.rvs())
        # Giving up: no success even with reduced epsilon:
        msg = f'Accept_rate ={self.accept_rate:.1%}. epsilon = {self.epsilon}'
    ...
    def init_batch(self, n_samples):
        ...
        """
        i = randint.rvs(0, self.n_samples, size=n_samples)
        self.x = self.x[:,i] if VECTOR_AXIS == 0 else self.x[i, :]
        self.x += self.epsilon * norm.rvs(size=self.x.shape)
    ...
    def one_sample_step(self):
        ...
        accept = uniform.rvs(size=len(h1)) < np.exp(h0 - h1)
        if VECTOR_AXIS == 0:
    ...
    def trajectory(self, x, p, dt, leap_steps):
        """Run one leapfrog trajectory in parallel with all particles in the sample batch.
    ...
    def unstable(self):
        ...
        # logger.debug('Unstable test: abs_d={}; std_d={}'.format( abs_d, std_d))
    ...
    def random_epsilon(self):
        ...
        r = uniform.rvs(size=1) * 2 * r_range - r_range
...
if __name__ == '__main__':
    ...
    x0 = np.zeros((2,2000)) # + np.array([2., -2.]).reshape((-1,1))
    if VECTOR_AXIS == 1:
        x0 = x0.T
