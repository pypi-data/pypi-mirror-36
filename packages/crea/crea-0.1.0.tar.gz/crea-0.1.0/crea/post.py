...
from creabase.exceptions import (
    PostDoesNotExist,
    VotingInvalidOnArchivedPost,
)
from creabase.operations import CommentOptions
...
class Post(dict):
    """ This object gets instantiated by Crea.streams and is used as an
        abstraction layer for Comments in Crea
        ...
        Args:
            ...
            cread_instance (Cread): Cread node to connect to

    """

    def __init__(self, post, cread_instance=None):
        self.cread = cread_instance or shared_cread_instance()
        self.commit = Commit(cread_instance=self.cread)
    ...
    def refresh(self):
        ...
        post = self.cread.get_content(post_author, post_permlink)
        ...
        for p in cbd_amounts:
            post[p] = Amount(post.get(p, "0.000 CBD"))
    ...
    def get_replies(self):
        ...
        """
        post_author, post_permlink = resolve_identifier(self.identifier)
        replies = self.cread.get_content_replies(post_author, post_permlink)
    ...
    @property
    def reward(self):
        """Return a float value of estimated total CBD reward.
        """
        return Amount(self.get("total_payout_value", "0 CBD")) + \
               Amount(self.get("pending_payout_value", "0 CBD"))
    ...
    def export(self):
        ...
        # Remove Crea instance object
        safe_dict = remove_from_dict(self, ['cread', 'commit'])
        ...
        op = CommentOptions(
            **{
                ...
                "percent_crea_dollars":
                    int(
                        options.get("percent_crea_dollars",
                                    self["percent_crea_dollars"] / 100) * 100),
                "allow_votes":
