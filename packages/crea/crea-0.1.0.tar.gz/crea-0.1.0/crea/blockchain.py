...
from .instance import shared_cread_instance, stm
...
class Blockchain(object):
    ...
    Args:
        cread_instance (Cread): Cread() instance to use when accessing a RPC
        mode (str): `irreversible` or `head`. `irreversible` is default.
    """

    def __init__(self, cread_instance=None, mode="irreversible"):
        self.crea = cread_instance or shared_cread_instance()
    ...
    def info(self):
        ...
        """
        return self.crea.get_dynamic_global_properties()

    def config(self):
        return self.crea.get_config()
    ...
    def get_current_block(self):
        """ This call returns the current block
        """
        return self.crea.get_block(self.get_current_block_num())
    ...
    def stream_from(self,
        ...
        operations, return raw, unedited blocks as provided by cread. This
       ...
       """
        ...
        while True:
            ...
            for block_num in range(start_block, head_block + 1):
                ...
                if full_blocks:
                    yield self.crea.get_block(block_num)
                elif batch_operations:
                    yield self.crea.get_ops_in_block(block_num, False)
                else:
                    for ops in self.crea.get_ops_in_block(block_num, False):
                        yield ops
    ...
    def reliable_stream(self,
        ...
        until getting a response from cread, allowing catching up after
        server downtime.

        Warnings: To ensure reliability, this method does some weird
        none-standard things with the cread client
        ...
        provided, will attempt to query cread for this value
        ...
        timeout (int): Time to wait on response from cread before assuming
        ...
        def get_reliable_client(_timeout):
            ...
            return stm.cread.Cread(
                nodes=self.crea.nodes,
                retries=1,
                timeout=_timeout,
                re_raise=True)
        ...
        for ops in self.stream_from(*args, **kwargs):
            ...
            for event in events:
                ...
                if not filter_by or op_type in filter_by:
                    # return unmodified cread output
    ...
    def get_all_usernames(self, *args, **kwargs):
        ...
        _ = args, kwargs
        warnings.warn(
            'Blockchain.get_all_usernames() is now Cread.get_all_usernames().'
        )
        return self.crea.get_all_usernames()
