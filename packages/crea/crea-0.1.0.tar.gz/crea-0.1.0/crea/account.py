...
from creabase.exceptions import AccountDoesNotExistsException
...
class Account(dict):
    ...
    def __init__(self, account_name, cread_instance=None):
    ...
    def refresh(self):
        account = self.cread.get_account(self.name)
        if not account:
    ...
    def converter(self):
        if not self._converter:
            self._converter = Converter(self.cread)
    ...
    def get_balances(self):
        available = {
            'CREA': Amount(self['balance']).amount,
            'CBD': Amount(self['cbd_balance']).amount,
            'VESTS': Amount(self['vesting_shares']).amount,
        }

        savings = {
            'CREA': Amount(self['savings_balance']).amount,
            'CBD': Amount(self['savings_cbd_balance']).amount,
        }

        rewards = {
            'CREA': Amount(self['reward_crea_balance']).amount,
            'CBD': Amount(self['reward_cbd_balance']).amount,
            'VESTS': Amount(self['reward_vesting_balance']).amount,
        }

        totals = {
            'CREA':
                sum([available['CREA'], savings['CREA'], rewards['CREA']]),
            'CBD':
                sum([available['CBD'], savings['CBD'], rewards['CBD']]),
            'VESTS':
    ...
    def _get_followers(self, direction="follower", last_user=""):
        if direction == "follower":
            ...
            followers = self.cread.get_followers(self.name, last_user, "blog",
                                                  100)
        elif direction == "following":
            followers = self.cread.get_following(self.name, last_user, "blog",
                                                  100)
    ...
    def virtual_op_count(self):
        try:
            last_item = self.cread.get_account_history(self.name, -1, 0)[0][0]
        except IndexError:
            return 0
        else:
            return last_item
    ...
    def get_account_votes(self):
        return self.cread.get_account_votes(self.name)

    def get_withdraw_routes(self):
        return self.cread.get_withdraw_routes(self.name, 'all')

    def get_conversion_requests(self):
        return self.cread.get_conversion_requests(self.name)
    ...
    def get_account_history(self,
                            ...
                            filter_by=None,
                            raw_output=False):
        """ A generator over cread.get_account_history.
        ...
        Args:
            ...
            raw_output (bool): (Defaults to False). If True, return history in
                cread format (unchanged).
        """
        history = self.cread.get_account_history(self.name, index, limit)
