...
from creabase import bip38
from creabase.account import PrivateKey
from creabase.exceptions import (InvalidWifError, WalletExists)
...
class Wallet:
    """ The wallet is meant to maintain access to private keys for
        ...
        :param Crea rpc: RPC connection to a Crea node
        ...
        * **Wallet Database**: Here, crealibs loads the keys from the
    ...
    def __init__(self, cread_instance=None, **kwargs):
        ...
        # Prefix
        if self.cread:
            self.prefix = self.cread.chain_params["prefix"]
        else:
        ...
        if "keys" in kwargs:
            ...
            """ If no keys are provided manually we load the SQLite
                keyStorage
            """
            from creabase.storage import (keyStorage, KeyEncryptionKey)
            self.keyEncryptionKey = KeyEncryptionKey
            self.keyStorage = keyStorage

    def setKeys(self, loadkeys):
        """ This method is strictly only for in memory keys that are
            passed to Wallet/Crea with the ``keys`` argument
        """
        log.debug(
    ...
    def addPrivateKey(self, wif):
        ...
        # it could be either graphenebase or creapybase so we can't check
        # the type directly
    ...
    def getOwnerKeyForAccount(self, name):
        ...
        if "owner" in Wallet.keyMap:
            ...
            account = self.cread.get_account(name)
            if not account:
                return
