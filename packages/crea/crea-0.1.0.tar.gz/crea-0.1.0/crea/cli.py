...
def legacyentry():
    ...
    parser.add_argument(
        ...
        help='URL for public Crea API (default: "https://cread.creativechain.net")'
    )
    ...
    parser.add_argument(
        ...
        version='%(prog)s {version}'.format(
            version=pkg_resources.require("crea")[0].version))
    ...
    parser_info = subparsers.add_parser(
        'info', help='Show basic CREA blockchain info')
    ...
    parser_transfer.add_argument(
        ...
        type=str,
        choices=["CREA", "CBD"],
        help='Asset to transfer (i.e. CREA or SDB)')
    parser_transfer.add_argument(
    ...
    parser_powerup = subparsers.add_parser(
        'powerup', help='Power up (vest CREA as CREA POWER)')
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing CREA from CREA POWER)')
    ...
    parser_convert = subparsers.add_parser(
        'convert',
        help='Convert CREA Dollars to CREA (takes a week to settle)')
    parser_convert.set_defaults(command="convert")
    parser_convert.add_argument(
        'amount', type=float, help='Amount of CBD to convert')
    parser_convert.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_buy = subparsers.add_parser(
        'buy', help='Buy CREA or CBD from the internal market')
    ...
    parser_buy.add_argument(
        'price', type=float, help='Limit buy price denoted in (CBD per CREA)')
    parser_buy.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_sell = subparsers.add_parser(
        'sell', help='Sell CREA or CBD from the internal market')
    ...
    parser_sell.add_argument(
        ...
        type=str,
        choices=["CREA", "CBD"],
        help='Asset to sell (i.e. CREA or SDB)')
    parser_sell.add_argument(
        'price',
        type=float,
    ...
    parser_repost.add_argument(
        ...
        required=False,
        default=configStorage["default_account"],
        help='Repost as this user (requires to have the ' +
    ...
    crea = stm.Crea(no_broadcast=args.no_broadcast, **options)
    ...
    if args.command == "set":
        ...
        if not args.objects:
            ...
            median_price = crea.get_current_median_history_price()
            crea_per_mvest = (
            ...
            # Account name
                ...
                try:
                    ...
                    for key in sorted(witness):
                        value = witness[key]
                        if key in ["props", "cbd_exchange_rate"]:
                            value = json.dumps(value, indent=4)
                        t.add_row([key, value])
                    print(t)
        ...
        if args.unsafe_import_key:
            ...
            while True:
                ...
                if not wifkey:
                    break
                try:
                    crea.commit.wallet.addPrivateKey(wifkey)
                except Exception as e:
                    print(str(e))
                    continue

                installed_keys = crea.commit.wallet.getPublicKeys()
                if len(installed_keys) == 1:
                    name = crea.commit.wallet.getAccountFromPublicKey(
                        installed_keys[0])
                    print("=" * 30)
                    print("Would you like to make %s a default user?" % name)
                    print()
                    print("You can set it with with:")
                    print("    creacli set default_account <account>")
                    print("=" * 30)
        ...
        if confirm("Are you sure you want to delete keys from your wallet?\n"
            ...
            for pub in args.pub:
                crea.commit.wallet.removePrivateKeyFromPublicKey(pub)
        ...
        print(crea.commit.wallet.getPrivateKeyForPublicKey(args.pub))

    elif args.command == "listkeys":
        t = PrettyTable(["Available Key"])
        t.align = "l"
        for key in crea.commit.wallet.getPublicKeys():
            t.add_row([key])
        print(t)
        ...
        t.align = "l"
        for account in crea.commit.wallet.getAccounts():
            t.add_row([
        ...
        print_json(
            crea.commit.transfer_to_vesting(
                args.amount, account=args.account, to=args.to))

    elif args.command == "powerdown":
        print_json(
            crea.commit.withdraw_vesting(
                args.amount,
                account=args.account,
            ))

    elif args.command == "convert":
        print_json(crea.commit.convert(
            args.amount,
            account=args.account,
        ))
        ...
        print_json(
            crea.commit.set_withdraw_vesting_route(
                args.to,
        ...
        if args.account and isinstance(args.account, list):
            for account in args.account:
                ...
                t.add_row([
                    'Available',
                    a.balances['available']['CREA'],
                    a.balances['available']['CBD'],
                    a.balances['available']['VESTS'],
                ])
                t.add_row([
                    'Rewards',
                    a.balances['rewards']['CREA'],
                    a.balances['rewards']['CBD'],
                    a.balances['rewards']['VESTS'],
                ])
                t.add_row([
                    'Savings',
                    a.balances['savings']['CREA'],
                    a.balances['savings']['CBD'],
                    'N/A',
                ])
                t.add_row([
                    'TOTAL',
                    a.balances['total']['CREA'],
                    a.balances['total']['CBD'],
                    a.balances['total']['VESTS'],
                ])
                print(t)
        else:
            print("Please specify an account: creacli balance <account>")
        ...
        if not args.key:
            ...
            if not args.no_broadcast:
                crea.commit.wallet.addPrivateKey(memo_privkey)
        print_json(
            crea.commit.update_memo_key(args.key, account=args.account))
        ...
        print_json(
            crea.commit.create_account(
                args.accountname,
                creator=args.account,
                password=pw,
                delegation_fee_crea=args.fee,
            ))
        ...
        if "owner" in args.roles:
            ...
            if owner_pubkey in [x[0] for x in account["owner"]["key_auths"]]:
                ...
                crea.commit.wallet.addPrivateKey(owner_privkey)
                imported = True
        ...
        if "active" in args.roles:
            ...
            if active_pubkey in [x[0] for x in account["active"]["key_auths"]]:
                ...
                crea.commit.wallet.addPrivateKey(active_privkey)
                imported = True
        ...
        if "posting" in args.roles:
            ...
            if posting_pubkey in [
                ...
                crea.commit.wallet.addPrivateKey(posting_privkey)
                imported = True
        ...
        if args.file and args.file != "-":
            ...
            tx = sys.stdin.read()
        tx = eval(tx)
        print_json(crea.commit.sign(tx))
        ...
        print_json(
            crea.commit.approve_witness(args.witness, account=args.account))

    elif args.command == "disapprovewitness":
        print_json(
            crea.commit.disapprove_witness(
                args.witness, account=args.account))

    elif args.command == "repost":
        print_json(crea.commit.repost(args.identifier, account=args.account))
        ...
        print_json(
            crea.commit.update_account_profile(
        ...
        if args.account_creation_fee:
            props["account_creation_fee"] = str(
                Amount("%f CREA" % args.account_creation_fee))
        ...
        props = {
            "account_creation_fee":
                str(Amount("%f CREA" % args.account_creation_fee)),
            "maximum_block_size":
                args.maximum_block_size,
            "cbd_interest_rate":
                int(args.cbd_interest_rate * 100)
        }
        print_json(
            crea.commit.witness_update(
...
def creatailentry():
    parser = argparse.ArgumentParser(
        description="UNIX tail(1)-like tool for the Crea blockchain")
    parser.add_argument(
        '-f',
        '--follow',
