...
import certifi
import urllib3
from creabase.exceptions import RPCError, RPCErrorRecoverable
...
class HttpClient(object):
    """ Simple Crea JSON-HTTP-RPC API

    This class serves as an abstraction layer for easy use of the Crea API.

    Args:
      nodes (list): A list of Crea HTTP RPC nodes to connect to.

    .. code-block:: python

       from crea.http_client import HttpClient

       rpc = HttpClient(['https://cread.creativechain.net',
       'https://d.creadev.io'])
    ...
    def json_rpc_body(name, *args, **kwargs):
        """ Build request body for cread RPC requests.
    ...
    def call(self,
             ...
             *args,
             **kwargs):
        """ Call a remote procedure in cread.
        ...
        while True:
            ...
            #       define exceptions for which we refuse to retry.
                ...
                logger.error('Unexpected exception! Please report at ' +
                             'https://github.com/creativechain/crea-python/issues' +
    ...
    def sanitize_nodes(self, nodes):
        ...
        1. a string of a single node url. ie nodes='https://cread.creativechain.net'
        2. a comma separated string of several node url's.
            nodes='https://cread.creativechain.net,<<community node>>,<<your personal node>>'
