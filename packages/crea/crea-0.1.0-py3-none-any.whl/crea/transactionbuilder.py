...
from creabase.exceptions import (InsufficientAuthorityError, MissingKeyError,
                                  InvalidKeyFormat)
from creabase import operations
from creabase.operations import Operation
from creabase.transactions import SignedTransaction, fmt_time_from_now, \
    get_block_params
...
class TransactionBuilder(dict):
    ...
    def __init__(self,
        ...
        self.wallet = wallet_instance or Wallet(self.cread)
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(
                        authority[0], cread_instance=self.cread)
    ...
    def constructTx(self):
        ...
        ref_block_num, ref_block_prefix = get_block_params(self.cread)
    ...
    def sign(self):
        ...
        # presented wrongly!
        if self.cread:
            operations.default_prefix = self.cread.chain_params["prefix"]
        ...
        signedtx.sign(self.wifs, chain=self.cread.chain_params)
    ...
    def broadcast(self):
        ...
        try:
            if not self.cread.verify_authority(self.json()):
        ...
        try:
            self.cread.broadcast_transaction(self.json())
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        accountObj = Account(account, cread_instance=self.cread)
        ...
        for account_auth in authority["account_auths"]:
            account_auth_account = Account(
                account_auth[0], cread_instance=self.cread)
        ...
        self["blockchain"] = self.cread.chain_params
