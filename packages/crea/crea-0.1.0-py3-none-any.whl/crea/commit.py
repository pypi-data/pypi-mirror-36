...
from creabase import memo
from creabase import operations
from creabase.account import PrivateKey, PublicKey
from creabase.exceptions import AccountExistsException, MissingKeyError
...
DPAY_1_PERCENT = (DPAY_100_PERCENT / 100)
...
class Commit(object):
    """ Commit things to the Crea network.
        ...
        :param Cread cread: Cread node to connect to*
    ...
    def __init__(self, cread_instance=None, no_broadcast=False, **kwargs):
        ...
        self.wallet = Wallet(self.cread, **kwargs)
    ...
    def post(self,
        ...
        if comment_options or beneficiaries:
            options = keep_in_dict(comment_options or {}, [
                'max_accepted_payout', 'percent_crea_dollars', 'allow_votes',
            ...
            default_max_payout = "1000000.000 CBD"
            comment_op = operations.CommentOptions(
                **{
                    ...
                    "percent_crea_dollars":
                        int(options.get("percent_crea_dollars", 10000)),
                    "allow_votes":
    ...
    def vote(self, identifier, weight, account=None):
        """ Vote for a post
            ...
            .. code-block:: python
                ...
                creacli set default_account <account>
        """
        if not account:
        ...
        op = operations.Vote(
            **{
                ...
                "weight": int(weight * DPAY_1_PERCENT)
            })
    ...
    def create_account(
            ...
            delegation_fee_crea='0 CREA',
            creator=None,
    ):
        """ Create new account in Crea

            The brainkey/password can be used to recover all generated keys
            (see `creabase.account` for more details.
            ...
            .. note:: Account creations cost a fee that is defined by
               ...
               **You can partially pay that fee by delegating VESTS.**
               ...
               To pay the fee in full in CREA, leave
               ``delegation_fee_crea`` set to ``0 CREA`` (Default).

               To pay the fee partially in CREA, partially with delegated
               VESTS, set ``delegation_fee_crea`` to a value greater than ``1
               CREA``. `Required VESTS will be calculated automatically.`

               To pay the fee with maximum amount of delegation, set
               ``delegation_fee_crea`` to ``1 CREA``. `Required VESTS will be
        ...
        you lose your password!
            ...
            :param str delegation_fee_crea: (Optional) If set, `creator` pay a
            fee of this amount, and delegate the rest with VESTS (calculated
            automatically). Minimum: 1 CREA. If left to 0 (Default), full fee
        ...
        if not creator:
            raise ValueError(
                ...
                "creator=x, or set the default_account using creacli")
        ...
        try:
            Account(account_name, cread_instance=self.cread)
        ...
        from creabase.account import PasswordKey, PublicKey
        if password:
            ...
            posting_pubkey = PublicKey(
                posting_key, prefix=self.cread.chain_params["prefix"])
            active_pubkey = PublicKey(
                active_key, prefix=self.cread.chain_params["prefix"])
            owner_pubkey = PublicKey(
                owner_key, prefix=self.cread.chain_params["prefix"])
            memo_pubkey = PublicKey(
                memo_key, prefix=self.cread.chain_params["prefix"])
        else:
            raise ValueError(
                "Call incomplete! Provide either a password or public keys!")

        owner = format(owner_pubkey, self.cread.chain_params["prefix"])
        active = format(active_pubkey, self.cread.chain_params["prefix"])
        posting = format(posting_pubkey, self.cread.chain_params["prefix"])
        memo = format(memo_pubkey, self.cread.chain_params["prefix"])
        ...
        props = self.cread.get_chain_properties()
        required_fee_crea = Amount(props["account_creation_fee"]).amount * 30

        required_fee_vests = 0
        delegation_fee_crea = Amount(delegation_fee_crea).amount
        if delegation_fee_crea:
            # creating accounts without delegation requires 30x
            # account_creation_fee creating account with delegation allows one
            # to use VESTS to pay the fee where the ratio must satisfy 1 CREA
            # in fee == 5 CREA in delegated VESTS

            delegated_sp_fee_mult = 5

            if delegation_fee_crea < 1:
                raise ValueError(
                    "When creating account with delegation, at least " +
                    "1 CREA in fee must be paid.")

            # calculate required remaining fee in vests
            remaining_fee = required_fee_crea - delegation_fee_crea
            if remaining_fee > 0:
        ...
        s = {
            'creator': creator,
            'fee': '%s CREA' % (delegation_fee_crea or required_fee_crea),
            ...
            'prefix': self.cread.chain_params["prefix"]
        }
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        ...
        if memo and memo[0] == "#":
            ...
            to_account = Account(to, cread_instance=self.cread)
    ...
    """
        ...
        op = operations.TransferToVesting(
            **{
                ...
                "amount":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=3, asset='CREA')
            })

        return self.finalizeOp(op, account, "active")

    def convert(self, amount, account=None, request_id=None):
        """ Convert BexDollars to CREA (takes one week to settle)
    ...
    def claim_reward_balance(self,
        ...
        `reward_crea`, `reward_cbd` or `reward_vests`.

        Args:
            reward_crea (string): Amount of CREA you would like to claim.
            reward_cbd (string): Amount of CBD you would like to claim.
        ...
        if none(
                float(first(x.split(' ')))
                for x in [reward_cbd, reward_crea, reward_vests]):
            a = Account(account)
            reward_crea = a['reward_crea_balance']
            reward_cbd = a['reward_cbd_balance']
            reward_vests = a['reward_vesting_balance']

        op = operations.ClaimRewardBalance(
            **{
                "account": account,
                "reward_crea": reward_crea,
                "reward_cbd": reward_cbd,
    ...
    def interest(self, account):
        ...
        """
        account = Account(account, cread_instance=self.cread)
        last_payment = fmt_time_string(account["cbd_last_interest_payment"])
        next_payment = last_payment + timedelta(days=30)
        interest_rate = self.cread.get_dynamic_global_properties()[
                            "cbd_interest_rate"] / 100  # percent
        interest_amount = (interest_rate / 100) * int(
            int(account["cbd_seconds"]) / (60 * 60 * 24 * 356)) * 10 ** -3
    ...
    def allow(self,
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, cread_instance=self.cread)
    ...
    def disallow(self,
        ...
        try:
            pubkey = PublicKey(
                foreign, prefix=self.cread.chain_params["prefix"])
            affected_items = list(
        ...
        op = operations.CommentOptions(
            **{
                ...
                "max_accepted_payout":
                    options.get("max_accepted_payout", default_max_payout),
                "percent_crea_dollars":
                    options.get("percent_crea_dollars", 100) * DPAY_1_PERCENT,
                "allow_votes":
                    options.get("allow_votes", True),
                "allow_curation_rewards":
                    options.get("allow_curation_rewards", True),
