...
class DisplayUI(object):
	...
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None, 
		...
		if rowset and column_headers:
			...
			if sys.version_info < (3,):
				datawidthtbl = [[len(rowset[i][j] if isinstance(rowset[i][j], stringtypes) else u"Binary data (%s bytes)" % len(rowset[i][j]) if type(rowset[i][j]) in (buffer, bytearray) else rowset[i][j]) for i in nrows] for j in ncols]
			else:
			...
			for i, row in enumerate(rowset):
				if sys.version_info < (3,):
					enc_row = ['' if c is None else u"Binary data (%s bytes)" % len(c) if type(c) in (buffer, bytearray) else c for c in row]
				else:
					enc_row = ['' if c is None else "Binary data (%s bytes)" % len(c) if type(c) in (type(memoryview(b'')), bytes, bytearray) else c for c in row]
...
def x_sub_add(**kwargs):
	...
	subvars.increment_by(subvar, increment_val)
	return None
