...
class senseBoxSensor(ReprObject):
    ...
    def clear_metadata(self):
        ...
        """
        for attr in ("title","type","unit","icon","last_time","last_value"):
    ...
    def delete(self, really = False):
        ...
        assert really, ("Refusing to delete sensor '{}' "
            "without really=True").format(self.id)
        assert self.box,"this sensor does not know what senseBox it belongs to"
        self.clear_metadata()
    ...
    def from_json(cls, d):
        ...
        """
        sensor = cls()
        sensor.update_from_json(d)
        return sensor

    def to_json(self, with_id = None, edited = False,
    ...
    def upload_measurement(self):
        ...
        :any:`senseBox.upload_measurement`.
        """
        assert self.last_value is not None, "last_value needs to be defined"
        assert self.box,"this sensor does not know what senseBox it belongs to"
        return self.box.upload_measurement(self)
    ...
    def get_measurements(self, **kwargs):
        ...
        data.name = "{} ({}) [{}]".format(self.title, self.type, self.unit)
        return data
    ...
    def delete_measurements(self, **kwargs):
        ...
        assert self.box.client, \
            "This sensor's senseBox does not have an associated client"
        assert hasattr(self.box.client, "delete_sensor_measurements"), \
...
class senseBoxSensorCollection(collections.abc.MutableSequence, ReprObject):
    ...
    def by_id(self):
        ...
        i = itertools.count(0)
        return {
            sensor.id if sensor.id else ("new_sensor_{}".format(next(i))):\
                sensor for sensor in self}
        ...
        for sensor in self:
            if sensor.title:
                if sensor.title in d:
                    ...
                    d["{} Nr. {}".format(sensor.title,
                        duplicates[sensor.title] + 1)] = sensor
                else:
