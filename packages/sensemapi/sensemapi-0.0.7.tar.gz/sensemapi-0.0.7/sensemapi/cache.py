...
class AuthentiCache(ABC, ReprObject):
    ...
    @simplegetter
    def directory(self):
        with XDGPackageDirectory("XDG_CACHE_HOME") as d:
            return d

    @simplesetter(directory)
    def directory(self, new):
        with Directory(new) as d:
            return d
    ...
    @abstractmethod
    def get(self, conditions, all_inner=True, all_outer=True,
        match_inner=True, match_outer=True):
        """
    ...
    def delete(self, condition):
        """
        Delete cache entr(y/ies) matching condition
        """
        pass
...
class SQLiteAuthentiCache(AuthentiCache):
    ...
    def sanitize(string, brutally = False):
        return re.sub("\W" if brutally else "['\";]","",string)
    ...
    def table_exists(self):
        with self.connection as conn:
            return any(
                self.table in row for row in conn.execute(
    ...
    def tableheader(self):
        self.create_table()
        with self.connection as conn:
            cursor = conn.execute("SELECT * from {cache.table}"
                .format(cache=self))
    ...
    def sql_update(self, old_row, new_row):
        ...
        return "SET {assignments} WHERE {condition}".format(
            assignments = ",".join(map("{} = ?".format, new_row.keys())),
            condition = sql_cond,
            ), tuple(new_row.values()) + sql_cond_params

    def sql_condition(self, conditions=tuple(), all_inner=True, all_outer=True,
        match_inner=True, match_outer=True):
        """
        ...
        conds = {pref: args.get(pref) \
            for pref in ("all_inner","all_outer","match_inner","match_outer")}
        conds = { # properly cycle all settings
            pref: itertools.cycle(val if hasattr(val,"__iter__") else(val,)) \
            for pref,val in conds.items()}
        conds["values"] = (conditions,) if hasattr(conditions, "items") \
            else conditions
        sql_conditions, sql_params = [], []
        for cond in listdict_to_dictlist(conds):
            sanitized_values = {self.sanitize(k, brutally=True):
                self.sanitize(v, brutally=False) if isinstance(v, str) else v
                    for k,v in cond["values"].items() if k in self.tableheader}
            inner_sql_cond = (" AND " if cond["all_inner"] else " OR ").join(
                map("{} {} ?".format,
                    sanitized_values.keys(),
                    itertools.cycle(("IS" if cond["match_inner"]else"IS NOT",))
                    ))
            sql_conditions.append("{}({})".format(
                "" if cond["match_outer"] else "NOT ", inner_sql_cond))
            sql_params.extend(tuple(sanitized_values.values()))
        sql_condition = functools.reduce(
            lambda x,y:
                (" AND " if next(conds["all_outer"]) else " OR ").join((x,y)),
            sql_conditions,
    ...
    def sync(self, cache):
        ...
        primary_key_equal = {k:v for k,v in cache.items()
        ...
        if len(matching_rows) > 1:
            logger.warning("There are duplicate rows:\n{}\n"
        ...
        if matching_row:
            ...
            our_retoti=matching_row.get("refresh_token_time",ancient)or ancient
            ...
            logger.debug("Given token '{}' from '{}' is {}newer than "
                "ours from ('{}').".format(their_to, their_toti,
                    "" if their_to_younger else "not ", our_toti))
            if their_to_younger:
            ...
            logger.debug("Given refresh token '{}' from '{}' is {}newer than "
                "ours from ('{}').".format(their_reto, their_retoti,
                    "" if their_reto_younger else "not ", our_retoti))
            ...
            if to_save.items() <= matching_row.items():
                ...
                return matching_row
            logger.debug("Will update\n{}\nwith\n{}".format(
                matching_row,to_save))
