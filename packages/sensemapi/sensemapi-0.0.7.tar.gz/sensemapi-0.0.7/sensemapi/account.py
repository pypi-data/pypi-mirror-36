...
class SenseMapAccount(SenseMapClient):
    ...
    Args:
        ...
        refresh_token_time (str, optional): retrieval time of the refresh JSON
            Web Token
        cache (AuthentiCache, optional): The authentication cache to use.
            Defaults to no cache.
        cache_password (bool, optional): cache the password (in plain text)?
            Defaults to ``False``. Only used if ``cache=True``.
    """
    def __init__(self,
        email = None,
        username = None,
    ...
    def sync_cache_afterwards(decorated_function):
        """
        Decorator for methods to call :meth:`sync_cache` after completition
        """
        @functools.wraps(decorated_function)
        def wrapper(self, *args, **kwargs):
            ret = decorated_function(self, *args, **kwargs)
            if self.cache:
                logger.debug("calling sync_cache after {}"
    ...
    def cache(self, new):
        if new:
            assert all(map(lambda x: hasattr(new, x), ("get","sync"))), \
                "{} object is not suitable as cache".format(type(new).__name__)
        self._cache = new
    ...
    def authenticache(self):
        ...
        for attr in ("token", "refresh_token",
            "token_time", "refresh_token_time", "password"):
    ...
    def delete_cache(self):
        ...
        """
        login = {attr:getattr(self, attr) \
            for attr in ("email","username","api") if getattr(self, attr)}
        full_cache = self.authenticache
        full_cache.update(login)
        if self.cache:
            self.cache.delete(full_cache)

    def sync_cache(self):
        """
        Synchronize with the :any:`cache`
        """
        if not self.cache:
            return False
        login = {attr:getattr(self, attr) \
            for attr in ("email","username","api") if getattr(self, attr)}
        matching = self.cache.get(conditions = login)
        our_cache = self.authenticache
        our_cache.update(login)
        updated = self.cache.sync(our_cache)
    ...
    def request(self, *args, **kwargs):
        ...
        if response.status_code == 403:
            ...
            raise OpenSenseMapAPIAuthenticationError(
                (code + ": " if code else "" + message)) # pragma: no cover
        return response
            ...
            if code == "Forbidden": # pragma: no cover
                if re.search("token.+invalid.+too.+old",message.lower()):
                    raise OpenSenseMapAPIOutdatedTokenError(message \
                        + ". You should actually never see this.")
    ...
    def new_box(self, box):
        ...
        try:
            message = response_json.get("message","")
            if not "successfully" in message: # pragma: no cover
                raise OpenSenseMapAPIError(
        ...
        try:
            ...
            if self.username != username:
                if self.username is not None:
                    logger.warning("The real username '{}' is different from "
                    "the configured username '{}'"
                    .format(username, self.username))
                self.username = username
            if self.email != email:
                if self.email is not None:
                    logger.warning("The real email '{}' is different from "
                    "the configured email '{}'"
                    .format(email, self.email))
                self.email = email
            if not user_details["emailIsConfirmed"]:
                logger.warning("The email '{}' is unconfirmed!"
                    .format(email))
    ...
    def __del__(self):
        ...
        if not self.cache:
            try:
                self.sign_out()
            except SenseMapiError: # pragma: no cover
                pass
