import binascii
import json
from kaze.Blockchain import GetBlockchain
from kaze.VM.ScriptBuilder import ScriptBuilder
from kaze.VM.InteropService import InteropInterface
from kaze.Network.NodeLeader import NodeLeader
from kaze.Prompt.Utils import parse_param, get_asset_attachments, lookup_addr_str, get_owners_from_params, gather_param, get_parse_addresses
...
from kaze.Core.TX.InvocationTransaction import InvocationTransaction
from kaze.Core.TX.TransactionAttribute import TransactionAttribute, TransactionAttributeUsage
from kaze.Core.TX.Transaction import TransactionOutput
...
from kaze.SmartContract.ContractParameterContext import ContractParametersContext
...
from kaze.VM.OpCode import PACK
...
def TestInvokeContract(wallet, args, withdrawal_tx=None,
    ...
    if contract:
        ...
        params, kaze_to_attach, stream_to_attach = get_asset_attachments(params)
...
def test_invoke(script, wallet, outputs, withdrawal_tx=None,
    ...
    try:
        ...
        if success:
            ...
            # this will be removed in favor of kaze.EventHub
            if len(service.notifications) > 0:
                for n in service.notifications:
                    Blockchain.Default().OnNotify(n)

            print("Used %s stream " % engine.streamConsumed().ToString())

            consumed = engine.streamConsumed() - Fixed8.FromDecimal(10)
            ...
            if consumed <= Fixed8.Zero():
                net_fee = min_fee
                tx_stream = Fixed8.Zero()
            else:
                tx_stream = consumed
                net_fee = Fixed8.Zero()

            # set the amount of stream the tx will need
            wallet_tx.stream = tx_stream
...
def test_deploy_and_invoke(deploy_script, invoke_args, wallet,
    ...
    if d_success:
        ...
        invoke_args, kaze_to_attach, stream_to_attach = get_asset_attachments(invoke_args)
        ...
        if i_success:
            ...
            logger.info("Used %s stream " % engine.streamConsumed().ToString())
