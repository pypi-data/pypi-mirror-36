...
A common use case for implementing kaze-python is to interact with smart contracts. Typical smart contract events
include ``Runtime.Notify``, ``Runtime.Log``, execution success or failure and ``Storage.GET/PUT/DELETE``.

Event Types
"""""""""""

This is a list of smart contract event types which can currently be handled with kaze-python:
...
When such events occur in received blocks, a ``SmartContractEvent`` instance is dispatched through ``kaze.EventHub``.

SmartContractEvent
""""""""""""""""""

The event handlers always receive a single argument, an instance of ``kaze.EventHub.SmartContractEvent``, which
...
===================== ========= ==========
``event_type``        str       One of the event types in ``kaze.EventHub.SmartContractEvent``
...
kaze.contrib.smartcontract.SmartContract
"""""""""""""""""""""""""""""""""""""""

Developers can easily subscribe to these events by using ``kaze.contrib.smartcontract.SmartContract``.
...
::
    ...
    from kaze.contrib.smartcontract import SmartContract
...
::
    ...
    from kaze.EventHub import SmartContractEvent
...
One of the most enjoyable features of ``kaze-python`` is the ability to quickly build, test, import, and invoke smart contracts on the kaze platform.
...
Build Your Contract
^^^^^^^^^^^^^^^^^^^
The first step to using SC ( Smart Contracts ) within the prompt is to build one.  This is a convienience method which uses the ``kaze-boa`` compiler to compiler your SC and save it in the ``.avm`` format.

When building or importing a file or contract within the prompt, it is always best to use a relative path ( relative to the ``kaze-python`` installation directory), though an absolute path will most likely work as well.
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample1.py
  Saved output to docs/source/example/sample1.avm

This command simply compiles the file and nothing else.  You could now use the compiled ``.avm`` file to import in a later stage, or use another tool such as kaze-Gui to import your contract.
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample1.py test '' 01 False false
  Saved output to docs/source/example/sample1.avm
...
.. code-block:: sh
  ...
  kaze> open wallet Wallets/awesome
  [password]> ***********
  Opened wallet at Wallets/awesome
  kaze> build docs/source/example/sample1.py test '' 01 False false
  Saved output to docs/source/example/sample1.avm
  [I 180302 22:22:58 Invoke:482] Used 0.016 stream
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample1.py test '' 02 False False
  Saved output to docs/source/example/sample1.avm
  [I 180302 22:25:09 Invoke:482] Used 0.016 stream
...
.. code-block:: sh
  ...
  [I 180302 22:56:20 Invoke:482] Used 0.016 stream
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample2.py test 070202 02 False False
  ...
  Traceback (most recent call last):
    File "/Users/thomassaunders/Workshop/kaze-python/kaze/VM/ExecutionEngine.py", line 823, in StepInto
      self.ExecuteOp(op, self.CurrentContext)
    File "/Users/thomassaunders/Workshop/kaze-python/kaze/VM/ExecutionEngine.py", line 276, in ExecuteOp
      estack.PushT(estack.Remove(n))
    File "/Users/thomassaunders/Workshop/kaze-python/kaze/VM/RandomAccessStack.py", line 57, in Remove
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample2.py test 070202 02 False False add 1 2
  Saved output to docs/source/example/sample2.avm
  [I 180302 22:32:06 Invoke:482] Used 0.033 stream
  ...
  kaze>
  kaze> build docs/source/example/sample2.py test 070202 02 False False mul -1 20000
  Saved output to docs/source/example/sample2.avm
  [I 180302 22:33:36 Invoke:482] Used 0.041 stream
...
.. code-block:: python3
  ...
  from boa.interop.kaze.Storage import Get,Put,Delete,GetContext
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample3.py test 070502 02 True False add AG4GfwjnvydAZodm4xEDivguCtjCFzLcJy 3
  Saved output to docs/source/example/sample3.avm
  [I 180302 23:04:33 Invoke:482] Used 1.174 stream
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample3.py test 070502 02 True False remove AG4GfwjnvydAZodm4xEDivguCtjCFzLcJy 2
  Saved output to docs/source/example/sample3.avm
  [I 180302 23:09:21 Invoke:482] Used 1.176 stream
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample3.py test 070502 02 True False add bytearray(b'\x00\x01\x02\x03') 4
  Saved output to docs/source/example/sample3.avm
  [I 180302 23:12:43 Invoke:482] Used 0.041 stream
...
.. code-block:: sh
  ...
  kaze> build docs/source/example/sample3.py test 070502 02 True False balance bytearray(b'\x03\x19\xe0)\xb9%\x85w\x90\xe4\x17\x85\xbe\x9c\xce\xc6\xca\xb1\x98\x96') 0
  Saved output to docs/source/example/sample3.avm
  [I 180302 23:16:23 Invoke:482] Used 0.162 stream
...
.. code-block:: sh
  ...
  Results:
  [<kaze.Core.State.ContractState.ContractState object at 0x11435d2e8>]
  Deploy Invoke TX stream cost: 90.0
...
Here is where, if you really really want to spend the stream to deploy your contract, you can enter your password and the real magic begins:
...
.. code-block:: sh
  ...
  kaze> tx f8ad261d28bf4bc5544e47f9bc3fff85f85ee674f14162dac81dd56bf73cf0a3
  {
  ...
  kaze> contract 0x86d58778c8d29e03182f38369f0d97782d303cc0
  {
      "version": 0,
      "code": {
          "hash": "0x86d58778c8d29e03182f38369f0d97782d303cc0",
...
.. code-block:: sh
  ...
  kaze> testinvoke 0x86d58778c8d29e03182f38369f0d97782d303cc0 add 1 2
  Used 0.033 stream
...
.. code-block:: sh
  ...
  kaze>
  kaze> [I 180303 07:39:31 EventHub:89] [SmartContract.Execution.Success][433122] [86d58778c8d29e03182f38369f0d97782d303cc0] [tx e0f4251a83f7081fb6fd94ce884d12b0bb597c1c1b3f1a89f07db68e114f4fa2] [3]
  kaze>
