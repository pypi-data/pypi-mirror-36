...
.. code-block:: python
  ...
  from boa.interop.kaze.Runtime import GetTrigger,CheckWitness
  from boa.interop.kaze.Storage import Get,Put,Delete,GetContext
  from boa.interop.kaze.TriggerType import Application, Verification
...
You will notice that the ``OWNER`` is the same ByteArray that we used before, as it is the address that is in the wallet I am using.  This example will use the ``boa.interop.kaze.Runtime.CheckWitness`` method to verify that the wallet that has signed the InvocationTransaction is the same as ``OWNER``.  First, lets build and test to make sure its working.
We will turn on ``sc-events`` to make sure we can see exactly what is going on.

.. code-block:: sh

  kaze>
  kaze> build docs/source/kaze/example/sample4.py test 070202 02 True False add AG4GfwjnvydAZodm4xEDivguCtjCFzLcJy 7
  Saved output to docs/source/kaze/example/sample4.avm
  ...
  [I 180303 08:25:12 Invoke:482] Used 1.191 stream

  -----------------------------------------------------------
  Calling docs/source/kaze/example/sample4.py with arguments ['add', 'AG4GfwjnvydAZodm4xEDivguCtjCFzLcJy', '7']
...
.. code-block:: sh
  ...
  kaze> contract 2e80ee491a0a54c9bbb0f791672050f9ab367767
  {
      "version": 0,
      "code": {
...
.. code-block:: sh
  ...
  kaze> testinvoke 0x2e80ee491a0a54c9bbb0f791672050f9ab367767 add AMUUgxnLhGxNSATinNp8gKmndqM1BxDZHR 42
...
.. code-block:: sh
  ...
  kaze>
  kaze>
  kaze> [I 180303 08:37:29 EventHub:89] [SmartContract.Runtime.Log][433333] [562d6c29209dc96432c6868621fe489cedd05222] [tx 0fd755e847a5ce54f9894c0c0bbf9303730ac28d8aeacdaddb2f912a2a3fcd40] [b'Running Sample v4']
