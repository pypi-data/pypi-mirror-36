...
from kaze.Prompt.Utils import get_asset_id, get_from_addr, get_arg
from kazecore.Fixed8 import Fixed8
from kazecore.UInt160 import UInt160
from prompt_toolkit import prompt
from kaze.Implementations.Wallets.peewee import UserWallet
import binascii
import json
import math
...
def Claimstream(wallet, require_password=True, args=None):
    ...
    # the following can be used to claim stream that is in an imported contract_addr
    ...
    print("\n---------------------------------------------------------------")
    print("Will make claim for %s stream" % available_bonus.ToString())
    if max_coins_per_claim and unclaimed_count > max_coins_per_claim:
        print("NOTE: You are claiming stream on %s unclaimed coins. %s additional claim transactions will be required to claim all available stream." % (max_coins_per_claim, math.floor(unclaimed_count / max_coins_per_claim)))
    print("------------------------------------------------------------------\n")
...
def SplitUnspentCoin(wallet, args, prompt_passwd=True):
    ...
    example ``wallet split Ab8RGQEWetkhVqXjPHeGN9LJdbhaFLyUXz kaze 1 100``
    this would split the second unspent kaze vin into 100 vouts
