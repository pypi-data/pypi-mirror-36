...
from kaze.Wallets.Coin import CoinState
from kaze.Core.TX.Transaction import TransactionInput
from kaze.Core.TX.TransactionAttribute import TransactionAttribute, TransactionAttributeUsage
from kaze.SmartContract.ContractParameter import ContractParameterType
...
def get_asset_attachments(params):
    to_remove = []
    kaze_to_attach = None
    stream_to_attach = None

    for item in params:
        if type(item) is str:
            if '--attach-kaze=' in item:
                to_remove.append(item)
                try:
                    kaze_to_attach = Fixed8.TryParse(int(item.replace('--attach-kaze=', '')))
                except Exception as e:
                    pass
            if '--attach-stream=' in item:
                to_remove.append(item)
                try:
                    stream_to_attach = Fixed8.FromDecimal(float(item.replace('--attach-stream=', '')))
                except Exception as e:
                    pass
    for item in to_remove:
        params.remove(item)

    return params, kaze_to_attach, stream_to_attach
...
def get_asset_id(wallet, asset_str):
    ...
    if asset_str.lower() == 'kaze':
        assetId = Blockchain.Default().SystemShare().Hash
    elif asset_str.lower() == 'stream':
...
def get_input_prompt(message):
    from kaze.bin.prompt import PromptInterface
