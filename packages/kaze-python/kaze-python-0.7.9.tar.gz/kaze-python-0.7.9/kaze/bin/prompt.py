...
from kaze import __version__
from kaze.Core.Blockchain import Blockchain
from kaze.SmartContract.ContractParameter import ContractParameter
...
from kaze.Implementations.Blockchains.LevelDB.DebugStorage import DebugStorage
from kaze.Implementations.Wallets.peewee.UserWallet import UserWallet
from kaze.Implementations.Notifications.LevelDB.NotificationDB import NotificationDB
from kaze.Network.NodeLeader import NodeLeader, kazeClientFactory
from kaze.Prompt.Commands.BuildNRun import BuildAndRun, LoadAndRun
from kaze.Prompt.Commands.Invoke import InvokeContract, TestInvokeContract, test_invoke
from kaze.Prompt.Commands.LoadSmartContract import LoadContract, GatherContractDetails, ImportContractAddr, \
    ImportMultiSigContractAddr
from kaze.Prompt.Commands.Send import construct_and_send, parse_and_sign
from kaze.contrib.nex.withdraw import RequestWithdrawFrom, PrintHolds, DeleteHolds, WithdrawOne, WithdrawAll, \
    CancelWithdrawalHolds, ShowCompletedHolds, CleanupCompletedHolds

from kaze.Prompt.Commands.Tokens import token_approve_allowance, token_get_allowance, token_send, token_send_from, \
    token_mint, token_crowdsale_register, token_history
from kaze.Prompt.Commands.Wallet import CreateAddress, DeleteAddress, ImportWatchAddr, ImportToken, Claimstream, DeleteToken, AddAlias, \
    ShowUnspentCoins, SplitUnspentCoin

from kaze.Prompt.Utils import get_arg, get_from_addr, get_tx_attr_from_args, get_owners_from_params
from kaze.Prompt.InputParser import InputParser
from kaze.Settings import settings, PrivnetConnectionError
from kaze.UserPreferences import preferences
...
class PromptInterface:
    ...
    commands = ['quit',
                ...
                'wallet tkn_mint {token symbol} {mint_to_addr} (--attach-kaze={amount}, --attach-stream={amount})',
                'wallet tkn_register {addr} ({addr}...) (--from-addr={addr})',
                'wallet tkn_history {token symbol}',
                'wallet unspent (kaze/stream)',
                ...
                'testinvoke {contract hash} [{params} or --i] (--attach-kaze={amount}, --attach-stream={amount}) (--from-addr={addr}) --no-parse-addr (parse address strings to script hash bytearray)',
    ...
    def __init__(self, history_filename=None):
        ...
        self.token_style = Style.from_dict({
            ...
            "kaze": preferences.token_style['kaze'],
    ...
    def get_completer(self):
        ...
        standard_completions = ['block', 'tx', 'header', 'mem', 'kaze', 'stream',
    ...
    def show_wallet(self, arguments):
        ...
        if item in ['v', '--v', 'verbose']:
            ...
            Claimstream(self.Wallet, True, arguments[1:])
        elif item == 'rebuild':
            self.stop_wallet_loop()
            try:
                self.Wallet.Rebuild()
    ...
    def test_invoke_contract(self, args):
        ...
        if args and len(args) > 0:
            ...
            if tx is not None and results is not None:
                ...
                print("Invoke TX stream cost: %s" % (tx.stream.value / Fixed8.D))
        ...
        if function_code:
            ...
            if contract_script is not None:
                ...
                if tx is not None and results is not None:
                    ...
                    print("Deploy Invoke TX stream cost: %s " % (tx.stream.value / Fixed8.D))
    ...
    def run(self):
        ...
        tokens = [("class:kaze", 'kaze'), ("class:default", ' cli. Type '),
...
def main():
    ...
    # Show the kaze-python version
    parser.add_argument("--version", action="version",
                        version="kaze-python v{version}".format(version=__version__))
