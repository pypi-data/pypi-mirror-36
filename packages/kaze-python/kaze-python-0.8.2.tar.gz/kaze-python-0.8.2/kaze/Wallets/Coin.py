...
from kazecore.IO.Mixins import TrackableMixin
...
class Coin(TrackableMixin):
    ...
    def CoinFromRef(coin_ref, tx_output, state=CoinState.Unconfirmed, transaction=None):
        ...
        Args:
            coin_ref (kaze.Core.CoinReference): an object representing a single UTXO / transaction input.
            tx_output (kaze.Core.Transaction.TransactionOutput): an object representing a transaction output.
            state (kaze.Core.State.CoinState):
    ...
    def __init__(self, prev_hash=None, prev_index=None, tx_output=None, coin_reference=None,
        ...
        Args:
            prev_hash (kazecore.UInt256): (Optional if coin_reference is given) the hash of the previous transaction.
            prev_index (UInt16/int): (Optional if coin_reference is given) index of the previous transaction.
            tx_output (kaze.Core.Transaction.TransactionOutput): an object representing a transaction output.
            coin_reference (kaze.Core.CoinReference): (Optional if prev_hash and prev_index are given) an object representing a single UTXO / transaction input.
            state (kaze.Core.State.CoinState):
        """
    ...
    def State(self):
        ...
        Returns:
            kaze.Core.State.CoinState: the coins state.
        """
        return self._state
    ...
    def State(self, value):
        ...
        Args:
            value (kaze.Core.State.CoinState): the new coin state.
        """
        self._state = value
