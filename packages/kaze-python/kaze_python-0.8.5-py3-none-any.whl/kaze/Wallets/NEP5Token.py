...
from kaze.Core.VerificationCode import VerificationCode
from kazecore.Cryptography.Crypto import Crypto
from kazecore.Fixed8 import Fixed8
from kaze.Prompt.Commands.Invoke import TestInvokeContract, test_invoke
from kaze.Prompt.Utils import parse_param
...
from kaze.Core.Mixins import SerializableMixin
...
class NEP5Token(VerificationCode, SerializableMixin):
    ...
    def FromDBInstance(db_token):
        ...
        Args:
            db_token (kaze.Implementations.Wallets.peewee.Models.NEP5Token):
    ...
    def Query(self):
        ...
        Args:
            wallet (kaze.Wallets.Wallet): a wallet instance.
        ...
        engine = None
        try:
            engine = ApplicationEngine.Run(sb.ToArray(), exit_on_error=True, stream=Fixed8.FromDecimal(10.0), test_mode=False)
    ...
    def Transfer(self, wallet, from_addr, to_addr, amount, tx_attributes=None):
        ...
        Returns:
            tuple:
                ...
                list: the kaze VM evaluationstack results.
        """
    ...
    def TransferFrom(self, wallet, from_addr, to_addr, amount):
        ...
        Returns:
            tuple:
                InvocationTransaction: the transaction.
                int: the transaction fee.
                list: the kaze VM evaluation stack results.
        """
    ...
    def Mint(self, wallet, mint_to_addr, attachment_args, invoke_attrs=None):
        ...
        Args:
            ...
            attachment_args: (list): a list of arguments used to attach kaze and/or stream to an invoke, eg ['--attach-stream=10.0','--attach-kaze=3']
    ...
    def Serialize(self, writer):
        ...
        Args:
            writer (kazecore.IO.BinaryWriter): binary writer to write serialization data to
    ...
    def Deserialize(self, reader):
        ...
        Args:
            reader (kazecore.IO.BinaryReader): reader to read byte data from
        """
