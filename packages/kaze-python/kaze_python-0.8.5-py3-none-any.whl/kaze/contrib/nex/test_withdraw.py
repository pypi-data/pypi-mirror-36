...
from kaze.Implementations.Wallets.peewee.Models import VINHold
from kaze.contrib.nex.withdraw import WithdrawAll, WithdrawOne, PrintHolds, \
...
class WithdrawWalletTestCase(WalletFixtureTestCase):
    FIXTURE_REMOTE_LOC = 'https://s3.us-east-2.amazonaws.com/KAZEBLOCKCHAIN/fixtures/withdraw_fixture042518.tar.gz'
    FIXTURE_FILENAME = os.path.join(settings.DATA_DIR_PATH, 'Chains/withdraw_fixture042518.tar.gz')

    @classmethod
    def leveldb_testpath(self):

        path = os.path.join(settings.DATA_DIR_PATH, 'fixtures/withdraw_fixture042418')
        return path
    ...
    def test_3_make_withdrawl_request(self):
        ...
        res = RequestWithdrawFrom(wallet, 'kaze', self.withdraw_hash, self.wallet_1_addr, 100)

        self.assertFalse(res)

        res2 = RequestWithdrawFrom(wallet, 'kaze', self.withdraw_hash, self.wallet_1_addr, 1, require_password=False)
