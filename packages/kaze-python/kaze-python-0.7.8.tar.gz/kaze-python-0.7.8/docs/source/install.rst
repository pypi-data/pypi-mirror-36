...
You should install platform specific items before installing ``kaze-python``.
...
The installation on the Windows operating system needs a set of unique steps and several common issues that might require troubleshooting, for this reason we dedicated a whole separate page for installing ``kaze-python`` on windows.
...
.. code-block:: sh
    ...
    ImportError: dlopen(kaze-python/venv/lib/python3.6/site-packages/plyvel/_plyvel.cpython-35m-darwin.so, 2): Symbol not found: __ZN7leveldb2DB4OpenERKNS_7Options
    ERKSsPPS0_
    Referenced from: kaze-python/venv/lib/python3.6/site-packages/plyvel/_plyvel.cpython-35m-darwin.so
...
The easiest way to install ``kaze-python`` on your machine is to download it and install from PyPi using ``pip``. First, we recommend you to create a virtual environment in order to isolate this installation from your system directories and then install it as you normally would do:
...
Clone the repository at `https://github.com/KAZEBLOCKCHAIN/kaze-python/ <https://github.com/KAZEBLOCKCHAIN/kaze-python/>`_ and navigate into the project directory.
...
Updating kaze-python from Git
""""""""""""""""""""""""""""

If you are updating kaze-python with ``git pull``, make sure you also update the dependencies with ``pip install -r requirements.txt``.


Bootstrapping the Blockchain
============================

If you use kaze-python for the first time, you need to synchronize the blockchain, which may take a long time. Included in this project is ``bootstrap.py`` to automatically download a chain directory for you.
