"""
Example of running a kaze node, receiving smart contract notifications and
...
http://kaze-python.readthedocs.io/en/latest/smartcontracts.html

This example requires the environment variable kaze_REST_API_TOKEN, and can
optionally use kaze_REST_LOGFILE and kaze_REST_API_PORT.

Example usage (with "123" as valid API token):

    kaze_REST_API_TOKEN="123" python examples/smart-contract-rest-api.py
...
from kaze.Network.api.decorators import json_response, gen_authenticated_decorator, catch_exceptions
...
API_PORT = os.getenv("kaze_REST_API_PORT", 8080)

# If you want to enable logging to a file, set the filename here:
LOGFILE = os.getenv("kaze_REST_LOGFILE", None)
...
API_AUTH_TOKEN = os.getenv("kaze_REST_API_TOKEN", None)
if not API_AUTH_TOKEN:
    raise Exception("No kaze_REST_API_TOKEN environment variable found!")
