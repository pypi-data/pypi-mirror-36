...
from kaze.Implementations.Blockchains.LevelDB.TestLevelDBBlockchain import TestLevelDBBlockchain
from kaze.Core.TX.Transaction import Transaction
from kaze.Core.TX.MinerTransaction import MinerTransaction
...
class kazeClientFactory(ReconnectingClientFactory):
    maxRetries = 1
    ...
    def __init__(self, incoming_client=False):
        ...
        Args:
            incoming_client (bool): true to create a kazeNode for an incoming client that initiates the P2P handshake.
        """
        self.incoming = incoming_client
        super(kazeClientFactory, self).__init__()

    def buildProtocol(self, addr):
        return kazeNode(self.incoming)
...
class NodeLeader:
    ...
    def Start(self):
        ...
        if settings.ACCEPT_INCOMING_PEERS:
            reactor.listenTCP(settings.NODE_PORT, kazeClientFactory(incoming_client=True))
    ...
    def SetupConnection(self, host, port):
        if len(self.Peers) < settings.CONNECTED_PEER_MAX:
            reactor.connectTCP(host, int(port), kazeClientFactory(), timeout=120)
    ...
    def InventoryReceived(self, inventory):
        ...
        Args:
            inventory (kaze.Network.Inventory): expect a Block type.
    ...
    def AddTransaction(self, tx):
        ...
        Args:
            tx (kaze.Core.TX.Transaction): instance.
