...
Description:
    Wallet
Usage:
    from kaze.Wallets.Wallet import Wallet
"""
import traceback
import hashlib
...
from kaze.Core.TX.Transaction import TransactionType, TransactionOutput
from kaze.Core.State.CoinState import CoinState
from kaze.Core.Blockchain import Blockchain
from kaze.Core.CoinReference import CoinReference
from kaze.Core.TX.ClaimTransaction import ClaimTransaction
from kazecore.Cryptography.Helper import scripthash_to_address
from kazecore.Cryptography.Crypto import Crypto
from kaze.Wallets.AddressState import AddressState
from kaze.Wallets.Coin import Coin
from kazecore.KeyPair import KeyPair
from kaze.Wallets.NEP5Token import NEP5Token
...
class Wallet:
    ...
    def __init__(self, path, passwordKey, create):
        ...
        Args:
            ...
            passwordKey (aes_key): A password that has been converted to aes key with kaze.Wallets.utils.to_aes_key
    ...
    def AddContract(self, contract):
        ...
        Args:
            contract (Contract): a contract of type kaze.SmartContract.Contract.
    ...
    def AddNEP5Token(self, token):
        ...
        Args:
            token (NEP5Token): an instance of type kaze.Wallets.NEP5Token.
    ...
    def DeleteAddress(self, script_hash):
        ...
        Returns:
            tuple:
                ...
                list: a list of any ``kaze.Wallet.Coin`` objects to be removed from the wallet.
        """
        coin_keys_toremove = []
        coins_to_remove = []
    ...
    def FindCoinsByVins(self, vins):
        ...
        Args:
            vins: A list of ``kaze.Core.CoinReference`` objects.

        Returns:
            list: A list of ``kaze.Wallet.Coin`` objects.
        """
        ret = []
    ...
    def FindUnspentCoins(self, from_addr=None, use_standard=False, watch_only_val=0):
        ...
        Returns:
            list: a list of ``kaze.Wallet.Coins`` in the wallet that are not spent.
        """
        ret = []
        for coin in self.GetCoins():
    ...
    def FindUnspentCoinsByAsset(self, asset_id, from_addr=None, use_standard=False, watch_only_val=0):
        ...
        Returns:
            list: a list of ``kaze.Wallet.Coin`` in the wallet that are not spent
        """
    ...
    def FindUnspentCoinsByAssetAndTotal(self, asset_id, amount, from_addr=None, use_standard=False, watch_only_val=0, reverse=False):
        ...
        Returns:
            list: a list of ``kaze.Wallet.Coin`` in the wallet that are not spent. this list is empty if there are not enough coins to satisfy the request.
        """
    ...
    def GetUnclaimedCoins(self):
        """
        Gets coins in the wallet that have not been 'claimed', or redeemed for their stream value on the blockchain.

        Returns:
            list: a list of ``kaze.Wallet.Coin`` that have 'claimable' value
        """
        unclaimed = []

        kaze = Blockchain.SystemShare().Hash

        for coin in self.GetCoins():
            if coin.Output.AssetId == kaze and \
    ...
    def GetAvailableClaimTotal(self):
        """
        Gets the total amount of stream that this wallet is able to claim at a given moment.

        Returns:
            Fixed8: the amount of stream available to claim as a Fixed8 number.
        """
    ...
    def GetUnavailableBonus(self):
        """
        Gets the total claimable amount of stream in the wallet that is not available to claim
        because it has not yet been spent.

        Returns:
            Fixed8: the amount of stream unavailable to claim.
        """
    ...
    def GetTokenBalance(self, token, watch_only=0):
        ...
        Args:
            token (NEP5Token): an instance of type kaze.Wallets.NEP5Token to get the balance from.
    ...
    def GetBalance(self, asset_id, watch_only=0):
        ...
        Args:
            asset_id (NEP5Token|TransactionOutput): an instance of type kaze.Wallets.NEP5Token or kaze.Core.TX.Transaction.TransactionOutput to get the balance from.
            watch_only (bool): True, to limit to watch only wallets.
    ...
    def ProcessNewBlock(self, block):
        ...
        Args:
            block: (kaze.Core.Block) a block on the blockchain.
        """
        added = set()
        changed = set()
        deleted = set()
    ...
    def IsWalletTransaction(self, tx):
        ...
        Args:
            tx (TransactionOutput):an instance of type kaze.Core.TX.Transaction.TransactionOutput to verify.
    ...
    def GetDefaultContract(self):
        ...
        Returns:
            contract (Contract): if Successful, a contract of type kaze.SmartContract.Contract, otherwise an Exception.
    ...
    def GetCoins(self):
        ...
        Returns:
            list: a list of kaze.Wallets.Coin objects.
        """
    ...
    def GetContracts(self):
        ...
        Returns:
            list: a list of kaze.SmartContract.Contract objects.
        """
    ...
    def MakeTransaction(self,
        ...
        be used when creating a transaction that involves an exchange of system assets, ( kaze, stream, etc ).
