...
from azureml.train.hyperdrive.policy import MedianStoppingPolicy, NoTerminationPolicy
...
from azureml._base_sdk_common.utils import convert_list_to_dict, convert_dict_to_list, list_remove_empty_items
...
class HyperDriveRunConfig(object):
    ...
    :param policy: The early termination policy to use. If None, MedianTermination
                   with delay_evaluation of 5 is used as default termination policy.
                   These are conservative settings, that can provide 25%-35% savings
                   with no loss on primary metric (based on our evaluation data).
    ...
    def __init__(self,
        ...
        if policy is None:
            # We don't support any early termination with Bayesian currently
            if isinstance(hyperparameter_sampling, BayesianParameterSampling):
                self._policy_config = NoTerminationPolicy().to_json()
            else:
                self._policy_config = MedianStoppingPolicy(delay_evaluation=5).to_json()
        else:
    ...
    def _remove_duplicate_estimator_arguments(self):
        ...
        estimator_args_dict = convert_list_to_dict(run_config_copy.arguments)
        input_params = copy.deepcopy(estimator_args_dict).keys()
        ...
        if warning:
            warnings.warn("Same input parameter(s) specified in estimator script params "
