...
class GridParameterSampling(HyperParameterSampling):
    ...
    def __init__(self, parameter_space):
        ...
        :type parameter_space: dict
        """
        super().__init__("GRID", parameter_space)
   ...
   """
    ...
    def __init__(self, parameter_space):
        ...
        if parameter_space is None:
            raise AzureMLException("parameter_space can't be None")

        allowed_distributions = {'choice', 'quniform', 'uniform'}
        for distribution in parameter_space.values():
            if distribution[0] not in allowed_distributions:
                raise AzureMLException("Distribution '{}' not supported. "
                                       "Only 'choice', 'quniform' and 'uniform' are "
                                       "supported for Bayesian optimization.", distribution)
