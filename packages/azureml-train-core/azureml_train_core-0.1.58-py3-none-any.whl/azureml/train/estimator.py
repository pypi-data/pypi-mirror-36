...
from azureml.exceptions import ComputeTargetException, UserErrorException, TrainingException, \
    AzureMLException
from azureml._base_sdk_common.utils import update_list_from_dict, convert_dict_to_list, merge_dict, merge_list
...
class MMLBaseEstimator(ABC):
    ...
    def _override_params(self, script_params=None, inputs=None):
        data_inputs = []
        if script_params:
            self._estimator_config.arguments = update_list_from_dict(self._estimator_config.arguments, script_params)
    ...
    def _update_environment_definition(conda_packages, custom_docker_base_image, environment_definition,
        ...
        if is_user_manged_environment and pip_packages:
            raise Exception('If environment_definition parameter is specified with user managed python'
                            ' dependencies set to True, pip_packages parameter cannot '
                            'be specified')
        if is_user_manged_environment and conda_packages:
            raise Exception('If environment_definition parameter is specified with user managed python'
                            ' dependencies set to True, conda_packages parameter cannot be specified')
        if custom_docker_base_image:
            raise Exception(
                'If environment_definition parameter is specified custom_docker_base_image parameter '
                'cannot be specified')
        if use_gpu:
            raise Exception('If environment_definition parameter is specified use_gpu parameter'
                            'cannot be specified')
...
class MMLBaseEstimatorRunConfig(RunConfiguration):
    ...
    :type framework: str
    """
    def __init__(self, compute_target, entry_script=None, script_params=None, node_count=None,
                 ...
                 environment_definition=None, inputs=None, framework=None):
        """
        ...
        if node_count > 1 and compute_target.type.lower() != BatchAITarget._BATCH_AI_TYPE.lower():
            raise Exception("Compute target should be Batch AI for distributed training (node_count > 1).")

        if node_count < 1:
            raise Exception("Node count should be at least 1.")
        if process_count_per_node < 1:
            raise Exception("Process count per node should be at least 1.")
        ...
        if environment_definition:
            ...
            if use_docker is False and custom_docker_base_image:
                raise Exception('If use_docker parameter is set to false, custom_docker_base_image parameter '
                                'is not allowed')
    ...
    @staticmethod
    def _get_data_references(inputs, data_inputs):
        ...
        for item in merged_inputs:
            if isinstance(item, AbstractAzureStorageDatastore):
                ...
                data_references[item_ref.data_reference_name] = item_ref._to_config()
            elif isinstance(item, DataReference):
                data_references[item.data_reference_name] = item._to_config()
            else:
    ...
    def _get_framework(backend, node_count, process_count_per_node):
        ...
        if not framework:
            ...
            raise Exception("Could not determine framework based on the backend value passed: {}.".format(backend))
        return framework
...
class Estimator(MMLBaseEstimator):
    ...
    def __init__(self,
        ...
        if not distributed_backend and (node_count > 1 or process_count_per_node > 1):
            raise Exception("Distributed backend must be specified when node_count > 1" +
                            " or process_count_per_node > 1.")

        if distributed_backend and distributed_backend.lower() != "mpi":
            raise Exception("Unsupported distributed backend value:" +
