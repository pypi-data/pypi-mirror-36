...
class Visualize(object):
    ...
    def __init__(self, feature_vector=None, model=None, label=None):
        ...
        if self.label is None:
            print("Label column is not specified for this knowledpack, specify as label=XXX")
    ...
    def feature_vector(self, value):
        self._feature_vector = value[[ i for i in value.columns.tolist() if ('gen_' == i[:4])]+[self.label]]
    ...
    def plot_features(self, features_list=None,
        ...
        if not(features_list):
            features_list =  [ i for i in self.feature_vector.columns.tolist() if ('gen_' == i[:4])]
    ...
    def neuron_feature_map(self,
        ...
        self.columns_index[1] = self.feature_vector.columns.tolist().index(self.pivot_feature)
        ...
        for feature in features_list:
            ...
            if neuron_figure and self.knowledgepack:
                if norm is None:
                    norm = self.knowledgepack.device_configuration.get('distance_mode', 0)
                else:
                    norm = norm
    ...
    def _plot_hist(self,Xs, Ys, clr):
        ...
        self.axHisty.hist(Ys, bins=bins, color=clr, alpha = self.vector_alpha, orientation='horizontal')
    ...
    def _set_initial_figure(self):
        ...
        if self.knowledgepack:
            self.label_indx_list = np.unique([self.knowledgepack.class_map[str(i)] for i in
                                        np.sort(map(int, self.knowledgepack.class_map.keys()))] + \
        ...
        for indx in range(len(self.label_indx_list)):
            self.neuron_scatter.scatter(0, 0, c=self.color_list[indx], label = self.label_indx_list[indx] )
    ...
    def _set_final_figure_settings(self, versus_feature):
        ...
        if self.hist_figure:
            ...
            self.neuron_scatter.legend(loc='center left', bbox_to_anchor=(1.02, 1.26), fontsize = 12   )

        else:
            self.neuron_scatter.legend(loc='center left', bbox_to_anchor=(1, 0.945), fontsize = 12   )
