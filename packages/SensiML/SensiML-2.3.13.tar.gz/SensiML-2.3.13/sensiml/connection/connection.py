    ...
    def __init__(self, server=None, username=None, password=None, **kwargs):
        ...
        if re.match(r'^https?://', server):
            ...
            try:
                ...
                choice = input('No configuration found, would you like to create one? (y)/n ') or 'y'
        ...
        if not self.session.token:
            try:
                ...
                if isinstance(e, InvalidGrantError):
                    ...
                    print('No token received: The remote server did not return a valid authorization response.')
                if os.path.exists(self._token_filename):
    ...
    def file_request(self, path, file_path, data, read_type='rb', method='post', **kwargs):
        ...
        with open(file_path, read_type) as f:
            ...
            # TODO: Make this send params as body instead of headers.
                ...
                request = partial(self.request, method, path, data=data, files={'file': f})
...
class PagedRequest(object):
    ...
    def next(self):
        ...
        if not self.response:
            self.response = self.session.request(self._method, self._path, self._json, self._headers, **self._kwargs)
        else:
            json = self.response.json()
            path = json['next']
            if not path:
                raise StopIteration
            self.response = self.session.request(self._method, path, self._json, self._headers, **self._kwargs)
