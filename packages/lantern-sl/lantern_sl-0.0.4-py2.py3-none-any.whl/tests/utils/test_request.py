...
def test_http_error(message, detail, e):
    _res = http_error(message=message, detail=detail, e=e)
    assert "status" in _res, "status column in response"
    assert "message" in _res, "message column in response"
    assert "detail" in _res, "detail column in response"
    if e:
        assert "raw" in _res, "exception detail"