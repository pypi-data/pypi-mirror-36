...
class ServerlessCrudBase(ServerlessBase):
	...
	def __init__(self, event, context, table_name, debug=False):
		...
		self.controller = DynamoController(table_name=table_name, debug=debug)
	...
	def get_primary_keys(self, use_event_if_none=True, error_on_empty=False):
		...
		if crud_node:
			primary_keys = crud_node.get("primary_keys",None)
		else:
			primary_keys = self.event.get("primary_keys",None)
	...
	def get_filters(self, error_on_empty=True):
		...
		crud_node = self.event.get("crud", None)
		if crud_node:
			filters = crud_node.get("filters",None)
		else:
			filters = self.event.get("filters", None)
