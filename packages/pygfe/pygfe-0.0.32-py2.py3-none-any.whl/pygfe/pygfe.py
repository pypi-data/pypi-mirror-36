...
class pyGFE(object):
    ...
    def type_from_seq(self, locus: str=None,
        ...
        if sequence_typing:
            ...
            gfe_o = self.gfe_create(locus, sequence, ac_object.imgtdb_version,
                                    alignseqs, nseqs, rerun, full)
            # if(not 'annotation' in gfe_o
            #    or not 'gfe' in gfe_o or
            #    not 'structure' in gfe_o):
            ...
            #     raise Exception("Failed to create annotation! {} {}".format(ac_object.pygfe_version, str(imgtdb_version)))

            if(not 'gfe' in gfe_o):
                self.logger.error(self.logname + "Failed to create annotation!! {}|{}|{}|{}".format(ac_object.pygfe_version, str(imgtdb_version), sequence, locus))
                #error = Error("Failed to create annotation!!", ac_object.pygfe_version, ac_object.gfedb_version, imgtdb_version)
                raise Exception("Failed to create gfe annotation! {}|{}|{}|{}".format(ac_object.pygfe_version, str(imgtdb_version), sequence, locus))
            ...
            if(not 'structure' in gfe_o):
                self.logger.error(self.logname + "Failed to create structure in annotation!!")
                return ac_object
    ...
    def gfe_create(self, locus, sequence, imgtdb_version,
        ...
        # TODO: Add try
        annotation = self.seqann[imgtdb_version].annotate(sequence=seq_rec, locus=locus, nseqs=nseqs,
                                                          alignseqs=nalign, rerun=rerun, full=full)
        if not hasattr(annotation, 'gfe'):
            return {}
        else:
