from typing import Any, List, Dict, Callable, Union, TYPE_CHECKING, Optional
...
class Task:
    ...
    def _reached_multi_objective_threshold_if_defined(objectives: List[Dict],
        ...
        for objective in objectives:
            if objective['id'] not in score_threshold:
                goal = Goal[objective['goal']]
                best_known_score = objective.get('minKnownScore' if goal == Goal.min else 'maxKnownScore')
        ...
        def reached_all_thresholds(scores: ScoreDict) -> bool:
            for objective in objectives:
                score = scores.get(objective['id'])
                if score is None:
                    return False

                threshold = score_threshold[objective['id']]
                goal = Goal[objective['goal']]
                if goal == Goal.min and score > threshold:
                    return False
                if goal == Goal.max and score < threshold:
                    return False
            return True
    ...
    def get_results(self, limit: int = None, best_first: bool = False, include_configurations: bool = False,
        ...
        if best_first and self.json.get('objectives'):
            raise ValueError('Getting the best results is not currently supported for multi-objective Tasks')
        ...
        if include_configurations or as_dataframe:
            query_params['includeConfigurations'] = 'true'
        response = self._session.get(url, query_params=query_params)
        results = [StoredResult(json) for json in response.body['results']]
        if as_dataframe:
    ...
    def get_best_result_and_configuration(self) -> StoredResult:
        ...
        """
        results = self.get_results(best_first=True, limit=1, include_configurations=True)
        if results:
            return results[0]
