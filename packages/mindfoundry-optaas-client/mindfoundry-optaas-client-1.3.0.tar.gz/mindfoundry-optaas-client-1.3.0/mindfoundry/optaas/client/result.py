...
class StoredResult:
    ...
    Attributes:
        ...
        configuration (Union[str, Configuration]):
            Either the unique id for the :class:`.Configuration` used to obtain the Result,
            or (if you specified `include_configurations=True`) the Configuration itself.
        pipeline (Optional[Pipeline]):
            The Pipeline corresponding to the Configuration (only available with SkelarnTasks).
    ...
    def as_pandas_row(self) -> Dict:
        ...
        Assumes that you have used include_configuration=True, so that the configuration values are available.
        """
        return {
