...
def fit_generator(model_id, model, generator, **kwargs):
    # TODO generator requires the original dataset first
    """
    checkpoint = FitchainCheckpoint(model_id)
    callbacks_list = []
    # -- check if there are already callbacks defined
    ...
    callbacks_list.append(checkpoint)
    return model.fit(x_train, y_train, callbacks=callbacks_list, **kwargs)
    return model.fit_generator(generator(all_images, all_labels, batch_size),
