...
from ava_engine.ava.engine_core_pb2 import BoundingBox, SAVE_ALL, SAVE_NOTHING, SAVE_RESULT
...
class _FaceRecognitionFeature:
    ...
    def add_group(self, name, custom_id=''):
        return self._stub.AddGroup(AddGroupRequest(name=name, custom_id=custom_id))

    def get_group(self, id):
        return self._stub.GetGroup(GetGroupRequest(id=id))

    def list_groups(self):
        return self._stub.ListGroups(ListGroupsRequest())

    def remove_group(self, id):
        return self._stub.RemoveGroup(RemoveGroupRequest(
            id=id,
        ))
    
    def update_group(self, update):
        return self._stub.UpdateGroup(UpdateGroupRequest(
            id=update.get('id'),
    ...
    def get_identity(self, identity_id):
        return self._stub.GetIdentity(GetIdentityRequest(identity_id=identity_id))
    ...
    def compute_cluster_set(self, group_id, min_height, min_width, number_faces, custom_id=''):
    ...
    def list_cluster_sets(self, group_id, custom_id=''):
        return self._stub.ListClusterSets(ListClusterSetsRequest(group_id=group_id, custom_id=custom_id))
