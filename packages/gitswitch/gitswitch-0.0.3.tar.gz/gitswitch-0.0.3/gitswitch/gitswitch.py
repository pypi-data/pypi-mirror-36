...
def handler():
  ...
  parser.add_argument('--current', help='Show current environment', required=False, action='store_true')
...
def read_current():
   with open(os.path.join(os.environ.get('HOME'),'.ssh/current'),'r') as f:
     current_name = f.readlines()
     if len(current_name) > 0:
       print "Current environment is {}".format(current_name[0])
...
def activate_environment(environment):
  ...
  if os.path.exists(environment_folder):
    ...
    if os.path.exists(os.path.join(os.environ.get('HOME'),'.ssh/id_rsa')):
      os.chmod(os.path.join(os.environ.get('HOME'),'.ssh/id_rsa'),0400)
    with open(os.path.join(os.environ.get('HOME'),'.ssh/current'),'w') as f:
      f.write(environment)
  else:
