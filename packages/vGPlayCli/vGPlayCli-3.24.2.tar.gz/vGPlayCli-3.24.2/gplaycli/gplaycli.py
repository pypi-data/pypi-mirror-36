...
class GPlaycli:
	...
	def retrieve_token(self, force_new=False):
		...
		if token is not None and not force_new:
			...
			return token, gsfid
		logger.info("Retrieving token ..., token_url:%s,device_codename:%s", self.token_url, self.device_codename)
		start = time.time()
		response = requests.get(self.token_url, params={"device_codename":self.device_codename})
