...
import pandas as pd
from pyfi.mapper import mapper, da_mapper
...
class WindHelper(object):
    ...
    @classmethod
    def wsd(cls, code, paras, begin_date, end_date, options="credibility=1", adjust=False, shift=0):
        """单代码多维日期序列
        或者多代码单维日期序列
    ...
    @classmethod
    def edb(cls, codes, begin_date, end_date, options="fill=perious", adjust=False, shift=0, fill=False):
        """多代码单维时间序列
        :param shift:
        :param fill:
        :param adjust: 是否对日期进行调整，td表示最近的交易日期
        :param codes:
        :param begin_date:
        :param end_date:
        :param options:
