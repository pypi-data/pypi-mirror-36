...
from modellingService.Utils import generateDirName, getEstimator, getParamGrid, getEvaluator, getValidator, getScorer, \
    reshapeParams, getParamInits, getDefaultParams, getParamBounds, generateReport
import Estimators as e


def train(spark, inputDF, algorithm, hyperparams={}, metric='areaUnderROC', numFolds=3, seed=None, numInitPoints=5, numIterations=2, acquisitionFunction='poi', gpParams={'alpha': 1e-5}, baseDir='output', workDir='output', outputDir=uuid.uuid4(), idx=1, binary=True, smart=False, optimize=False, persist=True):
    """
    ...
    evaluator = getEvaluator(metric, binary)

    def getWrapperFunction():
        def optimizer(**params):
            paramGrid = getParamGrid(estimator, params)
            validator = getValidator(estimator, evaluator, paramGrid, numFolds=numFolds, seed=seed)
            model = validator.fit(inputDF)

            if (persist):
                model.bestModel.save('{}/{}/{}'.format(baseDir, _dirName, _counter.next() + 1))
    ...
    if (optimize):
        ...
        report = generateReport('{}/{}'.format(baseDir, _dirName), workDir, algorithm, smartParams, bo.space.keys, bo.space.X, bo.space.Y, metric=metric, persist=persist)
        ...
        report = generateReport('{}/{}'.format(baseDir, _dirName), workDir, algorithm, smartParams, keys, X, Y, metric=metric, persist=persist)
