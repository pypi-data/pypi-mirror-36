...
def getEvaluator(metric='areaUnderROC', binary=True):
    """
...
def getParamInits(algorithm, customParams):
    ...
    if (c.inits.has_key(algorithm)):
        ...
        raise Exception('{} does not have paramInit defined yet'.format(algorithm))
...
def generateReport(outputDir, workDir, algorithm, customParams, headers, params, values, metric='areaUnderROC', persist=True):
    """
    ...
    if (persist):
        np.savetxt('{}/report.csv'.format(workDir), points, header=header, delimiter=',', fmt='%s', comments='')
