...
class DTr(object):
     def results(self, x, y, z):  
        ...
        self.y = y
        self.z = z
        self.clr = tree.DecisionTreeRegressor()
        self.clr = self.clr.fit(self.x, self.y)
        self.inp = tree.export_graphviz(self.clr, out_file=None) 
        self.g1 = graphviz.Source(self.inp) 
        self.g1.render("rdiagram")
        with open("rdiagram.dot", "w") as f:
             self.inp = tree.export_graphviz(self.clr, out_file=f, 
                             filled=True, rounded=True,  
                             special_characters=True) 
        print (self.clr.predict(self.z))
...
class DTc(object):
     def results(self, x, y, z):  
        ...
        self.cls = self.cls.fit(self.x, self.y)
        self.inp = tree.export_graphviz(self.cls, out_file=None) 
        self.g1 = graphviz.Source(self.inp) 
        self.g1.render("cdiagram")
        with open("cdiagram.dot", "w") as f:
             self.inp = tree.export_graphviz(self.cls, out_file=f, 
                             filled=True, rounded=True,  
                             special_characters=True) 
        print(self.cls.predict(self.z))
