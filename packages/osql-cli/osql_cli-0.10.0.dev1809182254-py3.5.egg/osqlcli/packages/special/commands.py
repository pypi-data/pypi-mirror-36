...
from osqlcli.packages.special.main import special_command, NO_QUERY
from osqlcli.packages.special.namedqueries import named_queries
from osqlcli.packages.special import export

logger = logging.getLogger('osqlcli.commands')
...
def list_indexes(osqlcliclient, pattern, verbose):
    pattern = pattern.replace('"', '').split('.')[-1]
    base_query = '''
SELECT
     TableName = t.name,
...
def list_functions(osqlcliclient, pattern, verbose):
    pattern = pattern.replace('"', '').split('.')[-1]
    base_query = '''
SELECT {cols}
...
def show_function_definition(osqlcliclient, pattern, verbose):
    if not pattern:
...
def describe_object(osqlcliclient, pattern, verbose):
    if not pattern:
...
def list_logins(osqlcliclient, pattern, verbose):
    base_query = '''
SELECT {cols}
FROM {table}
WHERE TYPE IN ('U', 'S', 'G')
...
'''
    base_query = base_query.format(cols='{cols}',
                                   ...
                                   if osqlcliclient.is_cloud else 'sys.server_principals',
                                   pattern='{pattern}')

    if verbose:
        if osqlcliclient.is_cloud:
...
def execute_named_query(osqlcliclient, pattern, **__):
    if pattern == '':
    ...
    if query is None:
        ...
        if arg_error:
            ...
            return osqlcliclient.execute_query(query)
