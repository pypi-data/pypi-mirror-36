...
def getInputsFromCSV(configFile, infilename):
  ...
  inputVarNames = striplist(csvReader.next())  #The first row gives the sinter names of variables
...
def writeJson2CSV(configFile, inputJson, csvInFilename, csvOutFilename):
  ...
  csvOutFile.close;
  csvOutFile = open(csvOutFilename, "wb") #Prep for writing outputs (to the same file)
  ...
  for sample in inputJson:
    ...
    if(sample["Output"] != ""):
      writeCSVOutputs(configFile, outputVarNames, sample["Status"], loadJsonObjFromJsonObj("Output", sample), csvWriter)
    else:
      writeCSVOutputs(configFile, outputVarNames, sample["Status"], None, csvWriter)
