...
def enqueue_tpu_embedding_sparse_batch(sample_indices, embedding_indices, aggregation_weights, device_ordinal=-1, combiners=[], name=None):
  ...
  Args:
    ...
    combiners: An optional list of `strings`. Defaults to `[]`.
      A list of string scalars whose values are 'mean', 'sum', or 'sqrtn'
...
def enqueue_tpu_embedding_sparse_tensor_batch(sample_indices, embedding_indices, aggregation_weights, table_ids, device_ordinal=-1, combiners=[], name=None):
  ...
  Args:
    ...
    device_ordinal: An optional `int`. Defaults to `-1`.
      ...
      device.
    combiners: An optional list of `strings`. Defaults to `[]`.
      A list of strings, one for each embedding table, specifying the
...
def send_tpu_embedding_gradients(inputs, config, name=None):
  ...
  if _ctx is None or not _ctx._eager_context.is_eager:
    ...
    config = _execute.make_str(config, "config")
    _, _, _op = _op_def_lib._apply_op_helper(
        "SendTPUEmbeddingGradients", inputs=inputs, config=config, name=name)
    return _op
    _result = None
    return _result
    ...
    try:
      ...
      return send_tpu_embedding_gradients_eager_fallback(
          inputs, config=config, name=name, ctx=_ctx)
    except _core._NotOkStatusException as e:
      if name is not None:
        message = e.message + " name: " + name
      else:
...
def send_tpu_embedding_gradients_eager_fallback(inputs, config, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  ...
  _inputs_flat = list(inputs)
  _attrs = ("N", _attr_N, "config", config)
