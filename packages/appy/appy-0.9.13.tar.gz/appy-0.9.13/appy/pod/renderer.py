...
class Renderer:
    ...
    def __init__(self, template, context, result, pythonWithUnoPath=None,
      ...
      pdfOptions='ExportNotes=True', deleteTempFolder=True):
        ...
        # LibreOffice to perform a conversion. If p_forceOoCall is True, even if
        # p_result ends with .odt, LibreOffice will be called, not for
        ...
        # LibreOffice. In both cases, we will try to connect to LibreOffice in
        # server mode on p_ooServer:p_ooPort.

        # p_stream dictates the communication between the renderer and
        # LibreOffice. By default (p_stream is "auto"), if p_ooServer is
        # "localhost", exchanging p_template and p_result between the renderer
        # and LibreOffice will be made via files on disk. Else, the renderer
        # will consider that LibreOffice runs on another machine and the
        # exchange will be made via streams over the network. If you want to
        # force the type of exchange, specify p_stream being True (exchange
        # forced via streams) or False (exchange forced via files on disk).
    ...
    def getCompleteContext(self, context):
        ...
        r.update({'xhtml': self.renderXhtml, 'test': self.evalIfExpression,
          'document': self.importDocument, 'pod': self.importPod,
    ...
    def callLibreOffice(self, resultName, format=None):
        '''Call LibreOffice in server mode to convert or update the result. If
           p_format is specified, it is used instead of p_self.resultType.'''
        loOutput = ''
        resultType = format or self.resultType
        try:
            ...
            try:
                ...
                try:
                    Converter(resultName, resultType, self.ooServer,self.ooPort,
