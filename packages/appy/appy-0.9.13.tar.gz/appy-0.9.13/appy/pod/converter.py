...
HELP_STREAM = 'By default (stream = "auto"), if you specify "localhost" as ' \
  ...
  'template and result files via the disk. If you specify anything else, the ' \
  'converter will consider that LibreOffice runs on a distant server: ' \
  'template and result files will be carried as streams via the network. ' \
  'If you want to bypass this logic and force exchange as streams or files ' \
  'on disk, set this option to "True" for stream or "False" for disk.'
...
class ConverterScript:
    ...
    def run(self):
        ...
        converter = Converter(args[0], args[1], options.server, options.port,
          ...
          pdfOptions, options.ppp, stream, options.verbose)
        try:
            converter.run()
