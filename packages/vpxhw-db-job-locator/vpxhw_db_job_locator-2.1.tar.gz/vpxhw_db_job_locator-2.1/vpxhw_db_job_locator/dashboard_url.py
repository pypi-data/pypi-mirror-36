...
url_template='''http://35.241.37.91/?{{jobIds}}
...
class DashboardUrl(object):
  ...
  def add_single_job(self, commit, encoder, usecase, test_suite, model_name):
    ...
    r = requests.post('http://35.241.37.91/be/locate/single', data = {'commit':commit, 'encoder':encoder, 'usecase':usecase, 'test_suite':basename(test_suite), 'model_name':model_name,
    'FPGA':'0',
    })
  ...
  def add_latest_job(self, encoder, usecase, test_suite, model_name=None):
    ...
    data={'encoder':encoder, 'usecase':usecase, 'test_suite':basename(test_suite), 
    'FPGA':'0',
    }
  ...
  def get_dashboard_url(self):
    ...
    jobIdsString='&'.join(['job='+urllib.quote_plus(id) for id in self._jobIds])
    return template.render({'jobIds':jobIdsString, 'encoder':self._encoder, 'model_name': self._model_name})
