...
class CustomNode(object):
    def __init__(self, in_data):
        ...
        if type(in_data) in (str,) or not hasattr(in_data,
                                                          '__getitem__'):
            self._data = [in_data]
...
class CustomModel(QtCore.QAbstractItemModel):
    ...
    def index(self, in_row, in_column, in_parent=None):
        ...
        if not QtCore.QAbstractItemModel.hasIndex(self, in_row, in_column,
                                                  in_parent):
            return QtCore.QModelIndex()

        child = parent.child(in_row)
        if child:
            return QtCore.QAbstractItemModel.createIndex(self, in_row,
                                                         in_column, child)
        else:
            return QtCore.QModelIndex()
    ...
    def parent(self, in_index):
        if in_index.isValid():
            ...
            if p:
                return QtCore.QAbstractItemModel.createIndex(self, p.row(), 0,
                                                             p)
        return QtCore.QModelIndex()
