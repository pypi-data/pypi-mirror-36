...
def sampling_policy(allow_eval_sampling=False, max_dim_clustering=50, explain_subset=None, **kwargs):
...
def kernel_policy(nsamples="auto", silent=False, **kwargs):
    ...
    Parameters:
        nsamples : "auto" or int
            Default to 'auto'. Number of times to re-evaluate the model when explaining
            each prediction. More samples lead to lower variance estimates of the feature
            importance values, but incur more computation cost. When "auto" is provided,
            the number of samples is computed according to a heuristic rule.

        silent : bool
            Default to 'False'.  Determines whether to display the explanation status bar
