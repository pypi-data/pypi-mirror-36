...
def main(**kwargs):
        ...
        if _tessellate :
                tessellate(local, method, output_file=rwa_file, \
                        label=method, verbose=verbose, min_location_count=30, \
                        strict_min_location_count=7)

        # plot locations and meshes
        pt_size_low, pt_size_high = 8, 6
        res_low, res_high = 50, 100
        other_plots = False
        if kwargs.get('ll', False):
                other_plots = True
                cell_plot(rwa_file, label=method, output_file=basename+'_locations_low.png',
                        verbose=verbose, voronoi=False,
                        locations=dict(color=['k']*9999, size=pt_size_low), dpi=res_low)
        if kwargs.get('lh', False):
                other_plots = True
                cell_plot(rwa_file, label=method, output_file=basename+'_locations_high.png',
                        verbose=verbose, voronoi=False,
        ...
        if kwargs.get('ml', False):
                ...
                cell_plot(rwa_file, label=method, output_file=basename+'_{}_low.png'.format(method),
                        verbose=verbose,
                        voronoi=dict(color='yyyy', linewidth=.5, centroid_style=None,
                                negative=True), dpi=res_low)
        if kwargs.get('mh', False):
                other_plots = True
                cell_plot(rwa_file, label=method, output_file=basename+'_{}_high.png'.format(method),
                        verbose=verbose,
        ...
        def img(mode):
                return '{}.{}.{}.png'.format(basename, method, mode)

        # infer and plot maps
        map_plot_args = dict(show=True, point_style=dict(color='y', alpha=.02), clip=4.,
