...
class OneHotEncodingBuilder:
    ...
    def learn(self) -> None:
        ...
        if result is None or len(result) == 0:
            raise ValueError('Failed to get categorical labels. The current upper limit for labels is 10000 distinct values.')
    ...
    def prefix(self) -> str:
        ...
        If no prefix is provided, source_column with an underscore will be the default prefix (e.g. source_column_label1, source_column_label2, ...).
        """
...
class LabelEncoderBuilder:
    ...
    def encoded_labels(self) -> Dict[str, int]:
        """
        Returns a dictionary of encoded labels. encoded_labels can be assigned by calling learn(), which will generate and assign the labels for you.
    ...
    def encoded_labels(self, value):
        ...
        self._arguments['encodedLabelsMap'] = [{'valueToLabel': key, 'encodedLabel': val} for key, val in self._encoded_labels.items()]
...
class JoinBuilder:
    ...
    def __init__(self,
                 engine_api: EngineAPI,
                 left_dataflow: 'dataprep.api.dataflow.DataflowReference',
                 right_dataflow: 'dataprep.api.dataflow.DataflowReference',
    ...
    def _pprint_suggestion(self, s: JoinSuggestionResult) -> str:
        return dedent("""
            Suggestion:
                Left:
                    Needs transform: {0!s}
    ...
    def detect_column_info(self) -> None:
        ...
        column_info_response = self._engine_api.anonymous_send_message_to_block(
            AnonymousSendMessageToBlockMessageArguments(blocks=[join_block],
                                                        ...
                                                        message_arguments={'leftPrefixBase': self._arguments['leftColumnPrefix'],
...
class QuantileTransformBuilder:
    ...
    def quantiles(self):
        ...
        """
        return self._arguments.get('quantiles')
    ...
    def quantiles_values(self):
        ...
        """
        return self._arguments.get('quantilesValues')
    ...
    def learn(self) -> None:
        ...
        new_block = self._engine_api.add_block_to_list(AddBlockToListMessageArguments(
            blocks=blocks,
            new_block_arguments=BlockArguments(self._arguments, 'Microsoft.DPrep.QuantileTransformBlock'),
            project_context=self._dataflow.parent_package_path
        ))
        learned_arguments = new_block.arguments.to_pod()
        self._arguments['quantiles'] = learned_arguments['quantiles']
        self._arguments['quantilesValues'] = learned_arguments['quantilesValues']
    ...
    def _need_learning(self) -> bool:
        return QuantileTransformBuilder._none_or_empty(self._arguments.get('quantiles')) \
            or QuantileTransformBuilder._none_or_empty(self._arguments.get('quantilesValues'))
