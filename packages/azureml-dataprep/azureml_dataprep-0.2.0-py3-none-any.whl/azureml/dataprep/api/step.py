...
class Step:
    def __init__(self, step_type: str, arguments: Dict[str, Any]):
        self.id = uuid.uuid4()
    ...
    def __deepcopy__(self, memodict=None):
        import copy
        new_step = Step(self.step_type, copy.deepcopy(self.arguments))
        new_step.id = self.id
...
def step_to_block_data(step: Step) -> AnonymousBlockData:
    return AnonymousBlockData(id=step.id, type=step.step_type, arguments=PropertyValues.from_pod(step.arguments))
