...
# Unitracc-Tools-Unterpaket tools.pdf:
from .PDFreactor import PDFreactor
from .config import pdfreactor_kwargs, LICENSE_DATA
from .utils import ApiFilter, acceptable_method_name

# Logging und Debugging:
from visaplan.plone.tools.log import getLogSupport
logger, debug_active, DEBUG = getLogSupport(fn=__file__, default='2')
...
class PDFCreator(object):
    ...
    def __init__(self, mydict):
        ...
        self._createReactor(bool(debug_active))

    def _createReactor(self, debug=None):
        kwargs = dict(pdfreactor_kwargs)
        if debug is not None:
            kwargs['debug'] = debug
        self.reactor = PDFreactor(**kwargs)
    ...
    def set_key(self):
        if LICENSE_DATA is not None:
            ...
            DEBUG('PDFreactor-Lizenzdaten gesetzt (%d Bytes)', len(LICENSE_DATA))
        else:
            logger.warn('Keine PDFreactor-Lizenzdaten!')
