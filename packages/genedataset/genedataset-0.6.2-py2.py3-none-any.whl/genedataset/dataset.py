...
# Utility functions 
# ------------------------------------------------------------
def tpm(rawCounts, totalExonLength):
	"""
		Return TPM values given raw count values. TPM is obtained by first dividing each raw count value by total exon length,
		then diving by the library size (with some constant multiplicative factors in between).
				
		Parameters:
		rawCounts: either a pandas DataFrame or Series, where values are unnormalised summarised counts and index are gene ids
		medianTranscriptLength: pandas Series, where values are the sum of the non overlapping length of all the exons in a gene
			and index are gene ids.
				
		Returns:
		DataFrame if rawCounts is DataFrame, Series if it is Series.

		Note that only gene ids common to both rawCounts.index and totalExonLength.index are used.
		Hence a subset of rawCounts may be returned if totalExonLength has missing gene ids compared to rawCounts.
		"""
	commonIndex = set(rawCounts.index).intersection(set(totalExonLength.index))
	tpms = rawCounts.loc[commonIndex].divide(totalExonLength[commonIndex], axis="index")*1000
	return tpms.divide(tpms.sum())*1000000
...
def createDatasetFile(destDir, **kwargs):
	...
	If platform_type=='rna-seq', we need summarised counts, cpm and tpm values.
	...
	Parameters:
		...
		tpm: pandas DataFrame of TPM values for each gene and sample (rna-seq only)
	...
	if attributes['platform_type']=='microarray':
		...
		tpm.to_hdf(filepath, '/dataframe/tpm')
...
class Dataset(object):
	...
	def __init__(self, pathToHdf):
		...
		if self.isRnaSeqData:
			self._expression = {'counts': pandas.read_hdf(self.filepath, '/dataframe/counts'),
								...
								'tpm': pandas.read_hdf(self.filepath, '/dataframe/tpm')}
		else:
	...
	# ------------------------------------------------------------
	def expressionMatrix(self, geneIds=None, datatype='tpm', sampleGroupForMean=None):
		...
		Parameters:
			...
			datatype: one of ['counts','tpm','cpm'] if self.isRnaSeqData is True, ignored if False.
		...
		if geneIds is not None and len(geneIds)>0:
	...
	def expressionValues(self, geneIds=[]):
		...
		if self.isRnaSeqData:
			rowIds = set(self._expression['tpm'].index).intersection(set(geneIds))
			df = self._expression['tpm'].loc[rowIds]
	...
	def valueRange(self):
		...
		"""
		df = self._expression['tpm'] if self.isRnaSeqData else self._expression
	...
	def correlation(self, featureId):
		...
		if self.isRnaSeqData:	# use log2 tpm values
			df = numpy.log2(df+1)		
...
def test_utilityFunctions():
    # tpm
	counts = pandas.DataFrame([[724,779],[0,0],[43,48]], index=['g1','g2','g3'], columns=['c1','c2'])
	hsgenesetexon = pandas.Series([2252.0,2372.0,6079.0], index=['g1','g2','g3'])
	df = tpm(counts, hsgenesetexon)
	assert round(df.loc['g3','c2']) == 22317
