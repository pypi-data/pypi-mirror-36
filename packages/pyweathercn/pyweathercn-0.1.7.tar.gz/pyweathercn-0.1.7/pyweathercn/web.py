...
class WeatherHandler(BaseHandler):
    ...
    def run_request(self):
        ...
        if city is None:
            self.set_status(400)
            response = make_json(4)
        # day, return specified day.
        elif day:
            data = make_json(city)
            try:
                sp = {"status": "success", "message": CODE.get(0), "data": data['data']['forecast'][int(day)]}
            except IndexError:
                sp = {"status": "error", "message": CODE.get(5)}
            response = json.dumps(sp)
        # return whole json.
        else:
            response = json.dumps(make_json(city))
        # set http code and response
        self.set_status(HTTP.get(json.loads(response)['status'], 418))
        return response
