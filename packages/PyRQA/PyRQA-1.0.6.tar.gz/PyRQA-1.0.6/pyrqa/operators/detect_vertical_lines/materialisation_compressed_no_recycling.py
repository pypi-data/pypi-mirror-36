...
def detect_vertical_lines(sub_matrix,
                          ...
                          kernels):
    """
    Detect vertical lines within a sub matrix that adheres to the CSC matrix representation.
    ...
    vertical_length_carryover_buffer = cl.Buffer(context,
                                                 cl.mem_flags.READ_WRITE,
                                                 sub_matrix.vertical_length_carryover.nbytes)

    transfer_to_device_events.append(cl.enqueue_copy(command_queue,
                                                     vertical_length_carryover_buffer,
                                                     sub_matrix.vertical_length_carryover,
                                                     device_offset=0,
                                                     wait_for=None,
                                                     is_blocking=False))

    vertical_index_carryover_buffer = cl.Buffer(context,
                                                cl.mem_flags.READ_WRITE,
                                                sub_matrix.vertical_index_carryover.nbytes)

    transfer_to_device_events.append(cl.enqueue_copy(command_queue,
                                                     vertical_index_carryover_buffer,
                                                     sub_matrix.vertical_index_carryover,
                                                     device_offset=0,
                                                     wait_for=None,
                                                     is_blocking=False))

    white_vertical_flag_carryover_buffer = cl.Buffer(context,
                                                     cl.mem_flags.READ_WRITE,
                                                     sub_matrix.white_vertical_flag_carryover.nbytes)

    transfer_to_device_events.append(cl.enqueue_copy(command_queue,
                                                     white_vertical_flag_carryover_buffer,
                                                     sub_matrix.white_vertical_flag_carryover,
                                                     device_offset=0,
                                                     wait_for=None,
                                                     is_blocking=False))
    ...
    detect_vertical_lines_args = [indices_buffer,
                                  indptr_buffer,
