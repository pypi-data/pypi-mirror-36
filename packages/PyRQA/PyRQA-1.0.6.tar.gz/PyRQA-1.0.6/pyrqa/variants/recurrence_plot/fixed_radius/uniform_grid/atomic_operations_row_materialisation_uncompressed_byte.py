...
"""
Create recurrence plot using an uniform grid.
...
from pyrqa.operators.create_uniform_grid.atomic_operations_row import create_uniform_grid
from pyrqa.operators.create_matrix.fixed_radius.uniform_grid.atomic_operations_row_materialisation_uncompressed_byte import create_matrix


class AtomicOperationsRowMaterialisationUncompressedByte(BaseSettings):
    """
    ...
    def __initialize(self):
        ...
        if not self.kernels_created:
            self.kernels = self.opencl.create_kernels(self.program,
                                                      (self.settings.clear_buffer_kernel_name(np.uint8),
    ...
    def process_sub_matrix(self,
        ...
        sub_matrix_buffer, \
            create_matrix_runtimes = create_matrix(self.settings,
                                                   ...
                                                   self.opencl.contexts[self.device],
                                                   self.opencl.command_queues[self.device],
                                                   (self.kernels[self.settings.clear_buffer_kernel_name(np.uint8)],
