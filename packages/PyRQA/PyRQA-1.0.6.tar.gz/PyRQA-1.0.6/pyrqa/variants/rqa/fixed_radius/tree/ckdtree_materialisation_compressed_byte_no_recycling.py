...
Recurrence matrix representation: Compressed
...
from pyrqa.operators.copy_matrix.materialisation_compressed import copy_matrix
from pyrqa.operators.create_matrix.fixed_radius.tree.ckdtree_materialisation_byte import create_matrix
from pyrqa.operators.detect_diagonal_lines.materialisation_compressed import detect_diagonal_lines
from pyrqa.operators.detect_vertical_lines.materialisation_compressed_no_recycling import detect_vertical_lines
from pyrqa.runtimes import FlavourRuntimesMultipleOperators
from pyrqa.types import MatrixRepresentation, Tree


class CKDTreeMaterialisationCompressedByteNoRecycling(BaseSettings):
    """"
    ...
    def process_sub_matrix(self,
        ...
        indices_buffer, \
            indptr_buffer, \
            copy_matrix_runtimes = copy_matrix(sub_matrix_data,
                                               self.device,
                                               self.opencl.contexts[self.device],
                                               self.opencl.command_queues[self.device])

        variant_runtimes.create_matrix_runtimes += copy_matrix_runtimes
