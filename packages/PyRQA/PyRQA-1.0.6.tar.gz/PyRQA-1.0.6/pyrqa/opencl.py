...
class OpenCL(BaseVerbose):
    ...
    def get_kernel_sources(self,
        ...
        for element in self.config_data:
            if element['operator_module'] == operator.__module__:
                kernel_sources = ''

                for kernel_file_name in tuple(element['kernel_file_names']):
                    kernel_file_path = os.path.join(kernel_base_dir, *operator.__module__.split('.')[2:-1])

                    try:
                        kernel_sources += FileReader.file_as_string(kernel_file_path, kernel_file_name)
                    except IOError:
                        ...
                        try:
                            ...
                            raise NoOpenCLKernelFoundException("Kernel with file name '%s' not found in path '%s'." % (kernel_file_name,
                                                                                                                       kernel_file_path))
    ...
    def create_program(self,
        ...
        for operator in operators:
            program_source += self.get_kernel_sources(operator,
                                                      settings_sub_dir)
