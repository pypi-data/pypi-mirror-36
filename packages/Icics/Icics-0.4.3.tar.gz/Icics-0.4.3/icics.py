...
class Allot():
    ...
    def __str__(self):
        return "欢迎体验联信智能分案 مرحبا بكم في  برنامج  ذكي приветствие к Интеллектуальному опыту"
    __repr__ = __str__

    

    def EuclidDis1(self,A,B):
        '''
        ==============================================================
    ...
    def recomm(self,dfold,df,bh,ywy,topn,ncluster,epsilon=0.001,init='k-means++',random_state=123,max_iter=1000,algorithm="auto"):
        '''
        =============================================================================================
    ...
    def icic(self,dfold0,df0,topn0,bhywy0,ncluster,shuffle=0,epsilon=0.001,init='k-means++',random_state=123,max_iter=1000,algorithm="auto",path=0):
        '''
        ==========================================================================
        ...
        Arguments:
                ...
                topn0:       A DataFrame,inclouding two cols like: 'ywy0' and 'topn_ywy',topn_ywy means 
                             how many cases should assign to this ywy0.+ topn_money
        ...
        dfold0 = pd.DataFrame(dfold0)
        topn0 = pd.DataFrame(topn0)
        bhywy0 = pd.DataFrame(bhywy0)

        if shuffle==1:
            nrowdf0 = range(df0.shape[0])
            random.shuffle(list(nrowdf0))
            df0 = df0.iloc[nrowdf0]

            nrowbhywy0 = range(bhywy0.shape[0])
            random.shuffle(list(nrowbhywy0))
        ...
        for ywy in ywylist:
            if len(list(df0.ajbh)) >=1:
                ...
                recommend = self.recomm(dfold2,df2,bh,ywy,topn,ncluster,epsilon=0.001,init='k-means++',random_state=123,max_iter=1000,algorithm="auto")
        ...
        #return pd.DataFrame(recommend_last1)
    ...
    def acc_mean(self,dfold0,bhywy0,topn0,ncluster,n_folds=5,shuffle=True,random_state=33,shuffle_icic=0,epsilon=0.001,init='k-means++',max_iter=1000,algorithm="auto",path=0):
    ...
    #  输入的参数中包含，法务列表，在手案量和户数，产能系数，参数分案的手次
    # 考虑户数均衡

    def Confunc(self,ywy_table,new_case_capcity,new_case_money):
        '''
        ywy_table:
            ywy0: 业务员编号
            coef : 产能系数（0-2）
        ...
        返回： 每个法务本次应该分配的具体金额和户数(作为输入参数)
        '''

        #inhand_hs = np.sum(list(ywy_table_history['case_c'])) + new_case_capcity
        inhand_hs = new_case_capcity
        n_ywy = len(list(ywy_table['ywy0']))
        # base_hs = (np.ceil(np.float(inhand_hs) / n_ywy * np.array(list(ywy_table['coef']))) - 
        #     np.array(list(ywy_table_history['case_c'])))
        base_hs = np.ceil(np.float(inhand_hs) / n_ywy * np.array(list(ywy_table['coef'])))

        # inhand_money = np.sum(list(ywy_table_history['caseMoney'])) + new_case_money
        
        inhand_money = new_case_money
        base_money = np.ceil(np.float(inhand_money) / n_ywy * np.array(list(ywy_table['coef_money']))) 

        return pd.DataFrame({'ywy0':list(ywy_table.ywy0),'topn_ywy':list(base_hs),'topn_money':list(base_money),"newornot":list(ywy_table.newornot)}) 
