...
class SurfMesh(object):
    ...
    def low_points_local_flood_fill(self, its=99999, scale=1.0, smoothing_steps=2):
        ...
        if self.rank==0 and self.verbose:
            print( "Low point local flood fill {} seconds ".format(clock()-t))
    ...
    def low_points_local_patch_fill(self, its=1, smoothing_steps=1):
        ...
        if self.rank==0 and self.verbose:
            print("Low point local patch fill - {} seconds".format( clock()-t))
    ...
    def low_points_swamp_fill(self, its=1000, saddles=True, ref_height=0.0):
        ...
        if self.rank==0:
            print("Build low point catchments - {} seconds".format(clock() - t))

        ## Can only be sure to find the saddle points in the downhill algorithm if
        ## the number of neighbours tested is the number sampled in cedges
        ##

        if self.downhill_neighbours==2:  # Find saddle points on the catchment edge
            cedges = np.where(ctmt[self.down_neighbour[2]] != ctmt )[0] ## local numbering

        elif self.downhill_neighbours>2:
            cedges2 = np.logical_or (ctmt[self.down_neighbour[2]] != ctmt,
                                     ctmt[self.down_neighbour[3]] != ctmt) ## local numbering
            cedges = np.where(cedges2)[0]

        else: # Find all edge points and cross fingers
        ...
        if self.rank == 0:
            print(" Sort spills - {} seconds".format(clock() - t))
        ...
        if rank == 0:
            print(" Gather spill data - {} seconds".format(clock() - t))
        ...
        if self.rank == 0:
            ...
            print(" Sort all spills - {} seconds".format(clock() - t))
        ...
        if self.rank==0 and self.verbose:
            print("Low point swamp fill - {} seconds".format(clock() - t))
