...
def cli(ctx, database, host, port, user, password, superuserpassword, protocol, timeout, config, load, mode,
    ...
    database = database or (config_obj.get(load, 'database') if load_from_config else database)
    host = host or (config_obj.get(load, 'host') if load_from_config else host)
    port = int(port) or (config_obj.getint(load, 'port') if load_from_config else port)
    user = user or (config_obj.get(load, 'user') if load_from_config else user)
    password = password or (config_obj.get(load, 'password') if load_from_config else password)
    protocol = protocol or (config_obj.get(load, 'protocol') if load_from_config else protocol)
    mode = mode or (config_obj.get(load, 'mode') if load_from_config else mode)
    superuserpassword = superuserpassword or (
        config_obj.get(load, 'superuserpassword') if load_from_config else superuserpassword)
    if prompt_connect:
...
def __section_list(ctx, arg, connect, login, short, version, sections):
    ...
    for index, section in enumerate(sections):
        ...
        protocol = config.get(section, 'protocol') or ''
        database = config.get(section, 'database') or ''
        host = config.get(section, 'host') or ''
        port = config.get(section, 'port') or ''
        user = config.get(section, 'user') or ''
        password = config.get(section, 'password') or ''
        superuserpassword = config.get(section, 'superuserpassword') or ''
        mode = config.get(section, 'mode') or ''
        if not arg and not short:
            data = [
                str(index),
                section_label,
