...
class APIWrapperHelpers(object):
    ...
    def get_curation_by_slot_number(self, slot_position, auth=True, device_type=None):
...
class APIWrapper(AuthHelpers):
    ...
    def find_watch_tray_item_with_long_synopsis(
            self, type='Feature', category='Title', char=50):
        ...
        for i, item in enumerate(data_list.get('Items')):
            ...
            if media.get('Type') == type and i != 0:
                switcher['Media Info'] = media
                switcher['Position'] = i + 1
                filtered_data.append(switcher)
        ...
        for item in filtered_data:
            ...
            if len(media.get(category).encode('utf8')) > char:
                switcher = item
                break
        if not switcher.get('Media Info'):
            raise Exception(
    ...
    def get_genre_all_metadata(self, genre='TV', auth=False, alpha=None, device_type=None):
        ...
        if device_type == 'roku':
            endpoint = '/Service.svc/browse/{0}/full/all/alpha-{1}/{2}'.format(
                genre, alpha, self.configuration.geo_code)
        else:
