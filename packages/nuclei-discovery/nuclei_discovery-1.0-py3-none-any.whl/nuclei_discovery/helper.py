...
def images(root, path, mode='DAPI'):
    '''
    Image picker rule:
        DAPI mode :=
            #1: <- single file
            #2: <- filename w/ '_UV' substring
            #3: <- filename w/ 'DAPI' substring
            #4: <- first file
        RGB mode :=
            #1: <- filename w/ 'RGB' substring
            #2: <- [R, G, B]: '_Blue-Red', '_Blue', '_UV'
            #3: <- [R, G, B]: '_Green', '_Blue', '_UV'
            #4: <- secondary file
    '''
    ...
    if mode == 'DAPI':
        return _r(_isin(['_UV.', 'DAPI'], files[0]))
    elif mode == 'RGB':
        fn = _isin(['RGB'])
        if fn:
            return _r(fn)
        r, g, b = _isin(['_Blue-Red.', '_Green.']), _isin(['_Blue.']), _isin(['_UV.'])
