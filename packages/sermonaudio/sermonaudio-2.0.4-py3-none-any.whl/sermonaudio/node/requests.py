...
class Node(API):
    ...
    get_sermon_list_node = functools.partialmethod(get_node,
                                                   parse_func=lambda res: [models.Sermon.parse(rec)
                                                                           for rec in parse_node(res).results])
    ...
    class PaginatedResponse:
        def __init__(self,
                     ...
                     result_processing_func=None):
            self.page = page
            self.page_size = page_size
            self.next_url = response.next
            self.total_count = response.total_count
            self.results = [result_processing_func(result)
                            for result in response.results] if result_processing_func else response.results
    ...
    def get_sermons(cls,
        ...
        """
        path = path or posixjoin(URL_PATH, 'sermons')
        ...
        return cls.get_node(path,
                            parse_func=lambda res: cls.PaginatedResponse(page,
                                                                         page_size,
                                                                         parse_node(res),
                                                                         models.Sermon.parse),
                            **kwargs)
    ...
    def get_sermon(cls, sermon_id: str, **kwargs) -> Optional[models.Sermon]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'sermons', str(sermon_id)),
    ...
    def delete_media(cls, sermon_id: str, media_type: models.MediaClass, **kwargs) -> Response:  # pragma: no cover
        ...
        return cls.post(posixjoin('jobs', 'delete'),
                        params=params,
                        **kwargs)
    ...
    def get_broadcaster(cls, broadcaster_id: str, **kwargs) -> Optional[models.Broadcaster]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'broadcasters', str(broadcaster_id)),
                       parse_func=lambda res: models.Broadcaster.parse(res.json()) if res.ok else None,
                       **kwargs)

    @classmethod
    def get_broadcasters_near_location(cls,
                                       latitude: float,
                                       longitude: float,
                                       meters: int,
                                       **kwargs) -> List[models.RelativeBroadcasterLocation]:
        ...
        return cls.get(posixjoin(URL_PATH, 'broadcasters', 'near_location'),
                       parse_func=lambda res: [models.RelativeBroadcasterLocation.parse(rec)
                                               for rec in parse_node(res).results],
                       **kwargs)
    ...
    def get_series_list(cls,
                        broadcaster_id: str = None,
                        **kwargs) -> List[models.SermonSeries]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'broadcasters', broadcaster_id, 'filter_options', 'series'),
                       parse_func=lambda res: [models.SermonSeries.parse(rec) for rec in parse_node(res).results],
                       **kwargs)

    @classmethod
    def get_sermon_event_types(cls,
                               broadcaster_id: str = None,
                               **kwargs) -> List[models.SermonEventTypeDetail]:
        """Fetches a sermon series list for a broadcaster.
        ...
        return cls.get(posixjoin(URL_PATH, 'filter_options', 'sermon_event_types'),
                       parse_func=lambda res: [models.SermonEventTypeDetail.parse(rec)
                                               for rec in parse_node(res).results],
                       **kwargs)
    ...
    def get_speaker(cls, speaker_name: str, **kwargs) -> Optional[models.Speaker]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'speakers', speaker_name),
    ...
    def get_speakers(cls,
        ...
        return cls.get(posixjoin(URL_PATH, 'speakers'),
                       parse_func=lambda res: [models.Speaker.parse(rec) for rec in parse_node(res).results],
                       **kwargs)

    @classmethod
    def get_webcasts_in_progress(cls,
                                 broadcaster_id: str = None,
                                 **kwargs) -> List[models.Webcast]:
        ...
        return cls.get(posixjoin(URL_PATH, 'webcasts'),
                       parse_func=lambda res: [models.Webcast.parse(rec) for rec in parse_node(res).results],
                       **kwargs)

    @classmethod
    def get_spurgeon_devotional(cls,
                                kind: models.SpurgeonDevotionalType,
                                month: int,
                                day_of_month: int,
                                **kwargs) -> Optional[models.SpurgeonDevotional]:
        ...
        return cls.get(posixjoin(URL_PATH, 'spurgeon_devotional'),
                       parse_func=parse_spurgeon,
                       **kwargs)
    ...
    def get_filter_options(cls, broadcaster_id: str, **kwargs) -> Optional[models.FilterOptions]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'broadcasters', broadcaster_id, 'filter_options'),
    ...
    def get_all_languages(cls, **kwargs) -> List[models.Language]:
        ...
        """
        return cls.get(posixjoin(URL_PATH, 'filter_options', 'languages'),
