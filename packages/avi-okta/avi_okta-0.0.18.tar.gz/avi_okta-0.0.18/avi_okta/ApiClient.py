...
def _giveup_handler(details):
    rsp = details['value']
    try:
        raise OktaError.factory(rsp.json())
    except ValueError:
...
class ApiClient(object):
    ...
    def __init__(self, base_url, token=None, username=None, password=None):
        ...
        url = base_url + SESSION_COOKIE_PATH
        rsp = self._request(self.session.get, url, params=params)
    ...
    def _init_internal_api_session(self):
        ...
        url = self.base_url + "/app/admin/sso/saml"
        rsp = self._request(session.get, url)
        ...
        url = self.base_admin_url + "/admin/sso/request"
        rsp = self._request(session.post, url, data=data)
        match = re.search(r'xsrfToken">(.+?)<', rsp.content)
        csrf_token = match.group(1)
    ...
    def get(self, path, params=None):
        ...
        return self._request(self.session.get, url, params=params)

    def post(self, path, params=None, data=None):
        url = self.api_url + path
        if data:
            data = json.dumps(data, cls=Serializer, separators=(',', ':'))
        return self._request(self.session.post, url, params=params, data=data)

    def put(self, path, params=None, data=None):
        url = self.api_url + path
        if data:
            data = json.dumps(data, cls=Serializer)
        return self._request(self.session.put, url, params=params, data=data)

    def delete(self, path, params=None):
        url = self.api_url + path
        return self._request(self.session.delete, url, params=params)
