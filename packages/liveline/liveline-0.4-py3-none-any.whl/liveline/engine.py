...
class LiveLine:
    ...
    def start(self, debug=False, web_server=True):
        if web_server:
            ...
            flask_thread = Thread(target=self.app.run, kwargs={
                                  'port': port}, name='flask')
        ...
        if not debug:
            ...
            debug_thread = Thread(target=self.__day_start__, args=[debug])
