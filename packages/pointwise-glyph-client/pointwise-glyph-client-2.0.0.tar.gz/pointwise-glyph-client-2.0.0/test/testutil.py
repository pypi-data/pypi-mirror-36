...
class TestBase(unittest.TestCase):
    ...
    def assertEqual(self, v1, v2):
        if not isinstance(v1, (list, tuple, Vector2, Vector3)):
            super().assertEqual(v1, v2)
        else:
            for i,j in zip(tuple(v1), tuple(v2)): super().assertEqual(i, j)

    def assertAlmostEqual(self, v1, v2):
        if not isinstance(v1, (list, tuple, Vector2, Vector3)):
            super().assertAlmostEqual(v1, v2)
        else:
            for i,j in zip(tuple(v1), tuple(v2)): super().assertAlmostEqual(i, j)
