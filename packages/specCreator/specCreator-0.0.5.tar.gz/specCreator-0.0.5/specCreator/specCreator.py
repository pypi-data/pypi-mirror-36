...
noSubSpec = """
  ...
  if ENV['IS_SOURCE'] || ENV["#{s.name}_SOURCE"]
    ...
    # 如果有自己依赖的库，请写在这里，并且把依赖的.a 或者 .framework 放到Classes 同级别的Assets目录下
    ...
    s.vendored_libraries = "#{s.name}/Archive/#{configuration}/lib#{s.name}.a","#{s.name}/Assets/**/*.a"
    s.vendored_frameworks= "#{s.name}/Assets/**/*.framework"
  else
    s.public_header_files = "#{s.name}/Framework/#{configuration}/#{s.name}.framework/Headers/*.h"
    s.source_files = "#{s.name}/Framework/#{configuration}/#{s.name}.framework/Headers/*.h"
    s.vendored_frameworks = "#{s.name}/Framework/#{configuration}/#{s.name}.framework","#{s.name}/Assets/**/*.framework"
    s.vendored_libraries = "#{s.name}/Assets/**/*.a"
  end
...
packageBySubspec = """
  ...
  _Foundation = {:spec_name => "Foundation", :resources => "#{s.name}/Assets/**/*.{bundle,lic,png,jpg,plist}" }
  # :dependency => 字典元素的数组， 里面每个字典包含两个元素 :name=> 第三方库名字， :version => 版本号
  _UIKit = {:spec_name => "UIKit", :dependency => [{:name => "AFNetworking", :version => "~> 3.0"}]}
  ...
  _subspecs.each do |spec|
    if ENV["#{s.name}_#{spec[:spec_name]}_SOURCE"] || ENV['IS_SOURCE'] || ENV["#{s.name}_SOURCE"]
      #源码部分
      spec[:source_files] = "#{s.name}/Classes/#{spec[:spec_name]}/*.{h,m}"
      spec[:public_header_files] = "#{s.name}/Classes/#{spec[:spec_name]}/*.h"
    elsif ENV["#{s.name}_#{spec[:spec_name]}_ARCHIVE"] || ENV['IS_ARCHIVE'] || ENV["#{s.name}_ARCHIVE"]
      spec[:source_files] = "#{s.name}/Archive/#{configuration}/#{spec[:spec_name]}/*.h"
      spec[:public_header_files] = "#{s.name}/Archive/#{configuration}/#{spec[:spec_name]}/*.h"
      spec[:vendored_libraries] = "#{s.name}/Archive/#{configuration}/#{spec[:spec_name]}/lib#{s.name}#{spec[:spec_name]}.a","#{s.name}/Assets/**/*.a"
      spec[:vendored_frameworks] = "#{s.name}/Assets/**/*.framework"
    else
      spec[:source_files] = "#{s.name}/Framework/#{configuration}/#{spec[:spec_name]}/#{s.name}#{spec[:spec_name]}.framework/Headers/*.h"
      spec[:public_header_files] = "#{s.name}/Framework/#{configuration}/#{spec[:spec_name]}/#{s.name}#{spec[:spec_name]}.framework/Headers/*.h"
      spec[:vendored_frameworks] = "#{s.name}/Framework/#{configuration}/#{spec[:spec_name]}/#{s.name}#{spec[:spec_name]}.framework","#{s.name}/Assets/**/*.framework"
      spec[:vendored_libraries] = "#{s.name}/Assets/**/*.a"
    end
  end
...
def printTips():
    formatPrint("""脚本参数错误，请参考以下提示：
        ...
        需要先看看相关python环境有没有安装好。请运行
            > python specCreator.py env
        参数：
            --gerritUser            gerrit用户名
            --projectPath           工程地址
            --podName               pod库名（需要再根目录)
            --version               要打包的版本
            --subPackage            打包的时候是否要每个subspec单独打包。（默认为False，一期暂时不让用)
        ...
        本脚本的功能：
            ...
            5：对已经存在的podspec文件进行源码和静态库的适配。
                > python specCreator.py initSpec
                    --projectPath=/Users/handa/Documents/LJABCD
                    --podName=LJABCD
        
        对服务端如果完全自动化，请运行：
                > python specCreator.py autoPackage
                    --projectPath=/Users/handa/Documents/LJCache
                    --podName=LJCache
                    --branch=master
                    --version=0.1.27
                    ...
                    --repoSource=LJComponentPodSpecs
                    --framework
                    --archive
                    --check
                    --debugPackage
...
def checkEnv():
    if not fileExist("/usr/local/bin/pip"):
        ...
        excommandUntilDone("sudo easy_install pip")
    if not fileExist("/Library/Python/2.7/site-packages/tornado") and not fileExist("/usr/local/lib/python2.7/site-packages/tornado"):
        ...
        excommandUntilDone("sudo pip install tornado")
    if not fileExist("/Library/Python/2.7/site-packages/nose") and not fileExist("/usr/local/lib/python2.7/site-packages/nose"):
        ...
        excommandUntilDone("sudo pip install nose")
    if not fileExist("/System/Library/Frameworks/Python.framework/Versions/2.7/Extras/lib/python/matplotlib") and not fileExist("/usr/local/lib/python2.7/site-packages/matplotlib"):
        ...
        excommandUntilDone("sudo pip install matplotlib")
    if not fileExist("/Library/Python/2.7/site-packages/pexpect") and not fileExist("/usr/local/lib/python2.7/site-packages/pexpect"):
        ...
        excommandUntilDone("sudo pip install pexpect")
    if not fileExist("/Library/Ruby/Gems/2.3.0/gems/cocoapods-packager-1.5.0"):
        formatPrint("正在安装打包脚本cocoapods-packager")
        excommandUntilDone("sudo gem install cocoapods-packager")
    if not fileExist("/Library/Python/2.7/site-packages/yaml"):
        formatPrint("正在安装打包脚本PyYAML")
        excommandUntilDone("sudo pip install PyYAML")


def packageA(projectPath,podName, subSpecName, version, commitId, debug, dependencyJSON):
    "打包.a"
    return package(projectPath, podName, subSpecName, version, commitId, debug, dependencyJSON, isFrameWork=False)

def package(projectPath, podName, subSpecName, version, commitId, debug, dependencyJSON, isFrameWork=True):
    """
    打包
    ...
    createSubSpecFile(podName, subSpecName, version, commitId, dependencyJSON)
    cmd = ''
    prefixString = ''
    suffixString = ''
    productDir = ''
    descFileName = ''
    if isFrameWork:
        cmd = "IS_SOURCE=1 pod package " + podspecName + " --force --no-mangle --verbose --exclude-deps --spec-sources=http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs,http://git.lianjia.com/mobile_ios/Lianjia_component_Podspec.git,https://github.com/CocoaPods/Specs.git "
        suffixString = ".framework"
        productDir = podName + "/Framework"
    else:
        cmd = "IS_SOURCE=1 pod package " + podspecName + " --library --force --verbose --no-mangle --exclude-deps --spec-sources=http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs,http://git.lianjia.com/mobile_ios/Lianjia_component_Podspec.git,https://github.com/CocoaPods/Specs.git "
    ...
    if returnCode > 0:
        ...
        returnError()
    podspecString = readFile(podspecName)
    versionMathList = matchList(r"s.version.*= [\'\"]([0-9]+\.[0-9]+\.[0-9]+)[\'\"]", podspecString)
    if not versionMathList:
        formatPrint("找不到版本信息，请检查podspec里的 s.version 字段")
    version = str(versionMathList[0])
    formatPrint(version)
    ...
    #     excommandUntilDone("cp -rf " + productPath + " " + backupPath)
    formatPrint("即将把" + descFileName + "放入发布目录：" + productDir)
    excommandUntilDone("cp -rf " + binaryFilePath + " " + productPath)
    if not isFrameWork:
        formatPrint("即将把.a 的头文件移动到目标目录：" + productPath)
        # 移动头文件
        sourcePath = os.path.join(podName, "Classes")
    ...
    if len(subSpecName) > 0:
        excommandUntilDone("rm -r " + podspecName)
    formatPrint("成功生成静态库")
    print "- " * 30 + '\n'
    return content
...
def changePodSpec(podName, version, commitId=""):
    ...
    replaceContent = readFile(originSpecPath)
    for lineStr in readFileLines(originSpecPath):
        if str(lineStr).startswith("#"):
            continue
        lineStr = lineStr.replace("\n", "")
        lineStr = lineStr.strip(" ")
        if str(lineStr).startswith("s.version"):
            originLine = lineStr
            if version:
                formatPrint("即将替换行换版本行" + lineStr)
                replaceVersion = "s.version          = \'" + version + "\'"
                formatPrint("替换为：" + replaceVersion)
                replaceContent = replaceContent.replace(originLine, replaceVersion, 1)
        elif lineStr.find("s.source") > -1 and lineStr.find(":git") > -1:
            formatPrint("找到要替换的source行：" + lineStr + ",\n")
            originSourceLine = lineStr
            sourceStr = "s.source           = { :git => 'http://gerrit.lianjia.com/mobile_ios/" + lineStr.split(",")[0].split("/")[-1].strip(" ").strip("\"").strip("\'") + "\'"
            if lineStr.find(":commit") and commitId:
                commitId = commitId.strip("\n")
                replaceSource = sourceStr + ", :commit => \"" + commitId + "\" }"
            else:
                replaceSource = sourceStr + ", :tag => s.name.to_s + \"-\" + s.version.to_s }"
            replaceContent = replaceContent.replace(originSourceLine, replaceSource, 1)
            formatPrint("s.source 替换成：" + replaceSource)
...
def createSubSpecFile(podName, subspecName, version, commitId, dependencyJSON):
    ...
    for lineStr in readFileLines(originSpecPath):
        ...
        if str(lineStr).startswith("#"):
            continue
        lineStr = lineStr.replace("\n", "")
        if str(lineStr).startswith("s.name") and lineStr.find(podName) > -1:
            formatPrint("找到要替换的name行：" + lineStr)
            orginSpecName = lineStr
            replaceSpecName = lineStr.replace(podName, podName + subspecName, 1)
            formatPrint("s.name 替换成：" + podName + subspecName)
            replaceContent = replaceContent.replace(orginSpecName, replaceSpecName)
        elif str(lineStr).startswith("s.version"):
            originLine = lineStr
            if version:
                formatPrint("即将替换行换版本行" + lineStr)
                replaceVersion = "  s.version          = \'" + version + "\'"
                formatPrint("替换为：" + replaceVersion)
                replaceContent = replaceContent.replace(originLine, replaceVersion, 1)
        elif lineStr.startswith("s.source") and lineStr.find(":git") > -1:
            formatPrint("找到要替换的source行：" + lineStr + "，\n\n即将用最新的提交打包：\n")
            originSourceLine = lineStr
            sourceStr = "s.source           = { :git => 'http://gerrit.lianjia.com/mobile_ios/" + \
                        lineStr.split(",")[0].split("/")[-1].strip(" ").strip("\"").strip("\'") + "\'"
            if lineStr.find(":commit") and commitId:
                commitId = commitId.strip("\n")
                replaceSource = sourceStr + ", :commit => \"" + commitId + "\" }"
            else:
                replaceSource = lineStr.split(",")[0].strip("}") + " }"
            replaceContent = replaceContent.replace(originSourceLine, replaceSource, 1)
            formatPrint("s.source 替换成：" + replaceSource)
        # elif lineStr.find("ss.dependency") > -1:
        #     formatPrint("找到内部依赖的行" + lineStr)
        #     originLine = lineStr
        #     replaceLine = lineStr.replace(podName, podName + subspecName)
        #     replaceContent = replaceContent.replace(originLine, replaceLine, 1)
        elif lineStr.find(":dependency => [{") > -1:
            # TODO subspec 打包的时候改动
            pass
        elif lineStr.find("#{s.name}") > -1:
            originLine = lineStr
            replaceLine = lineStr.replace("#{s.name}", podName)
            formatPrint("把库名：" + lineStr + "\n变成：   " + replaceLine + "\n")
            replaceContent = replaceContent.replace(originLine, replaceLine, 1)
        elif lineStr.find(".dependency ") > -1:
            formatPrint("修改dependency：" + lineStr)
            originLine = lineStr
            # s.dependency 'LJHTTPService'
            specName = lineStr.split(" ")[-1].strip("\'")
            if specName not in dependencyDict.keys():
                continue
            replaceLine = lineStr + ", \'" + dependencyDict[specName] + "\'"
            replaceContent = replaceContent.replace(originLine, replaceLine, 1)
    # 把原podspec重命名
    renamePodspec(originSpecPath, originSpecPath+".old")
    writeToFile(replaceContent, podName + subspecName + ".podspec")
...
def checkSpec(podName, dependencyJSON):
    ...
    createSubSpecFile(podName, "", "", "", dependencyJSON)
    cmd = "IS_SOURCE=1 pod lib lint " + podspecName + " --allow-warnings --verbose --use-libraries --sources=http://gerrit.lianjia.com/mobile_ios/LJComponentPodSpecs,http://git.lianjia.com/mobile_ios/Lianjia_component_Podspec.git,https://github.com/CocoaPods/Specs.git  --fail-fast --no-clean"
    # pexpect = importlib.import_module('pexpect')
    # pexpect.run(cmd)
...
def returnError():
    # excommandUntilDone("git add .;git stash")
    exit(1)
...
def initProject(user, projectPath, subPackage):
    ...
    formatPrint("git ignore Example/Pods 文件夹")
    excommandUntilDone("touch Example/.gitignore")
    writeToFile(ignoreFileString, "Example/.gitignore")
...
def autoPackage(projectPath, podName, branch, version, subPackage=False, commitId="", dependencyJSON="", framework=True,
    ...
    # 判断branch和commit
    if commitId:
        returnCode, content = excommandUntilDone("git branch --contains " + commitId)
        if returnCode > 0:
            formatPrint(" 请确认commtid是否正确", " *")
            exit(1)
        realBranch = content.strip("\n").strip(" ").strip("* ")
        if branch and branch not in realBranch:
            formatPrint("commit 所在分支并非提供的分支" + branch + "，而是" + realBranch, " *")
            exit(1)
        if not branch:
            formatPrint("即将用在commitID" + commitId + "所在的分支进行打包", " -")
            branch = realBranch
    else:
        if not branch:
    ...
    dependencyDict = {}
    if framework and debugPackage:
        formatPrint("正在打 framework debug 二进制库", "- ")
        versionHistory += "| Debug | ✔️️ |"
        contentDict = packageFramework(projectPath, podName, version, subPackage, commitId, True, dependencyJSON)
    ...
    if archive and debugPackage:
        formatPrint("正在打 archive debug 二进制库","- ")
        versionHistory += "✔️ | \n"
        contentDict = packageArchive(projectPath, podName, version, subPackage, commitId, True, dependencyJSON)
        if len(dependencyDict.keys()) == 0:
            dependencyDict = processDependency(podName, contentDict)
        resultDict["debugArchivePath"] = os.path.join(archivePath, "Debug")
    else:
        versionHistory += " ️ |\n"
    if framework and releasePackage:
        formatPrint("正在打 framework release 二进制库","- ")
        versionHistory += "|Release | ✔️ |"
        contentDict = packageFramework(projectPath, podName, version, subPackage, commitId, False, dependencyJSON)
        if len(dependencyDict.keys()) == 0:
            dependencyDict = processDependency(podName, contentDict)
    ...
    if archive and releasePackage:
        formatPrint("正在打 archive release 二进制库","- ")
        versionHistory += "✔️ | \n"
        contentDict = packageArchive(projectPath, podName, version, subPackage, commitId, False, dependencyJSON)
        if len(dependencyDict.keys()) == 0:
            dependencyDict = processDependency(podName, contentDict)
    ...
    if autoPush:
        ...
        if "nothing to commit, working tree clean" not in content:
            excommandUntilDone("git add .; git commit -a -m \'" + commitMessage + "\'")
        # changeSpec(podName, subPackage, version)
        changePodSpec(podName, version)
        returnCode, content = excommandUntilDone("git status")
        if "nothing to commit, working tree clean" not in content:
            excommandUntilDone("git add .; git commit -a -m \'发布" + podName + "-" + version + "\'")
    ...
    if autoPushRepo:
        ...
        excommandUntilDone("git push origin  " + tagVersion)
        excommandUntilDone("IS_SOURCE=1 pod repo push " + repoSource + " " + podName +".podspec --use-libraries --allow-warnings")
...
def getarguments(type=""):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[2:], "facdrp",
                                   ["help", "gerritUser=", "projectPath=", "subPackage=", "podName=", "branch=",
                                    ...
                                    "repoSource=", "resultPath=", "whiteListPath="])
    ...
    if type == "autoPackage":
        return projectPath, podName, branch, version, subPackage, commitId, dependencyJSON, framework, archive, check,\
               debugPackage, releasePackage, autoPush, autoPushRepo, repoSource, resultPath, whiteListPath
    elif type == "init":
...
def packageFramework(projectPath, podName, version, subPackage, commitId, debug, dependencyJSON):
    contentDict = {}
    if subPackage:
        subspecNames = subspecList(podName)
        for subspec in subspecNames:
            content = package(projectPath, podName, subspec, version, commitId, debug, dependencyJSON)
...
def packageArchive(projectPath, podName, version, subPackage, commitId, debug, dependencyJSON):
    contentDict = {}
    if subPackage:
        subspecNames = subspecList(podName)
        for subspec in subspecNames:
            content = packageA(projectPath, podName, subspec, version, commitId, debug, dependencyJSON)
            contentDict[subspec] = content
    else:
...
def checkopts():
    ...
    if arg == "env":
        ...
        debugPackage, releasePackage, autoPush, autoPushRepo, repoSource, resultPath, whiteListPath = getarguments(arg)
        if whiteListPath:
        ...
        autoPackage(projectPath, podName, branch, version, subPackage, commitId, dependencyJSON, framework, archive, check,\
               debugPackage, releasePackage, autoPush, autoPushRepo, repoSource, resultPath)
    elif arg == "init":
...
def getSpecDict(podName):
    ...
    if returnCode > 0 or not contentList:
        formatPrint("podspec 有问题，不能转换成HSON数据")
        returnError()
