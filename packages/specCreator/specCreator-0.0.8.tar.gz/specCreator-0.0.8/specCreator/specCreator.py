...
packageBySubspec = """
  ...
  _subspecs.each do |spec|
    if ENV["#{s.name}_#{spec[:spec_name]}_SOURCE"] || ENV['IS_SOURCE']
      ...
      # spec[:vendored_libraries] = "#{s.name}/Archive/#{spec[:spec_name]}/#{configuration}/*.a","#{s.name}/Assets/**/*.a"
      ...
      # 有外部依赖的静态库的话请注释上面一行，换下面的
      # spec[:vendored_frameworks] = "#{s.name}/Framework/#{spec[:spec_name]}/#{configuration}/*.framework","#{s.name}/Assets/**/*.framework"
...
def create_tmp_podspec(podName, subSpecName, version, commitId, dependencyJSON):
    ...
    excommandUntilDone("mkdir -p " + tmpPodspecPath)
    excommandUntilDone("mv ./" + tmpPodspecFileName + " " + tmpPodspecPath + "/" + tmpPodspecFileName)


def clean_tmp_podspec(podName, tmp_subSpec_name):
    userPath = os.path.expanduser('~')
    repoName = "LJComponentPodSpecs"
    repoPath = userPath + "/.cocoapods/repos/" + repoName
    tmpPodspecFileName = podName + tmp_subSpec_name + ".podspec"
    tmpPodspecPath = repoPath + "/" + podName + tmp_subSpec_name
    if fileExist(tmpPodspecPath):
        formatPrint("正在清理创建的临时podspec" + tmpPodspecPath , " -")
        excommandUntilDone("rm -rf " + tmpPodspecPath)
...
def package(projectPath, podName, subSpecName, version, commitId, debug, dependencyJSON, isFrameWork=True):
    ...
    formatPrint("即将把.a 的头文件移动到目标目录：" + productDir)
    # 移动头文件
    sourcePath = os.path.join(podName, "Classes")
    if subSpecName:
        sourcePath = os.path.join(sourcePath, subSpecName)
    ...
    clean_tmp_podspec(podName, subSpecName + "Tmp")
    formatPrint("成功生成静态库")
    print "- " * 30 + '\n'
    return content
...
def changePodSpec(podName, version, commitId=""):
    ...
    for lineStr in readFileLines(originSpecPath):
        ...
        if str(tmpLine).startswith("s.version"):
            ...
            sourceStr = "s.source           = { :git => 'http://gerrit.lianjia.com/mobile_ios/" + tmpLine.split(",")[0].split("/")[-1].strip(" ").strip("\"").strip("\'") + "\'"
            if commitId:
    ...
    for lineStr in readFileLines(originSpecPath):
        ...
        if str(tmpLineStr).startswith("s.name") and tmpLineStr.find(podName) > -1:
            ...
            sourceStr = "s.source           = { :git => 'http://gerrit.lianjia.com/mobile_ios/" + \
                        tmpLineStr.split(",")[0].split("/")[-1].strip(" ").strip("\"").strip("\'") + "\'"
            if tmpLineStr.find(":commit") and commitId:
                commitId = commitId.strip("\n")
                replaceSource = sourceStr + ", :commit => \"" + commitId + "\" }"
            else:
                replaceSource = tmpLineStr.split(",")[0].strip("}") + " }"
            formatPrint("把" + lineStr + "替换成" + replaceSource, "-")
            lineStr = lineStr.replace(originSourceLine, replaceSource)
            ...
            originLine = tmpLineStr
            specName = tmpLineStr.split(" ")[-1].strip("\"").strip("\'")
...
def autoPackage(projectPath, podName, branch, version, subPackage=False, commitId="", dependencyJSON="", framework=True,
    ...
    checkParams(podName, subPackage, whiteListPath, version, subspecList)
    ...
    if autoPush:
        ...
        changePodSpec(podName, version, newCommitId)
        returnCode, content = excommandUntilDone("git status")
        if "nothing to commit, working tree clean" not in content:
    ...
    if autoPushRepo:
        ...
        repoName = repoSource.strip(" ").strip(".git").split("/")[-1]
        # 获取用户目录
        userPath = os.path.expanduser('~')
        if not fileExist(userPath + "/.cocoapods/repos/" + repoName):
            if "http://" not in repoSource:
                formatPrint("repoSource 不是一个url, 请检查", " *")
                returnError()
            if not repoSource.endswith(".git"):
                repoSource += ".git"
            excommandUntilDone("pod repo add " + repoName + " " + repoSource)
        # 最好先清空下repo，防止repo污染
        returnCode, content = excommandUntilDone(
            "IS_SOURCE=1 pod repo push " + repoName + " " + podName + ".podspec --use-libraries --allow-warnings --skip-import-validation --skip-tests")
        if returnCode > 0:
            formatPrint("发布repo失败，请检查", " *")
            returnError()
...
def checkParams(podName, subPackage, whiteListPath, version, subspecs=[]):
    if whiteListPath:
