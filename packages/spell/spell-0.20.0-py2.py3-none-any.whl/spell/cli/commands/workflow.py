...
def workflow(ctx, command, args, local, repo_paths,
    ...
    utf8 = ctx.obj["utf8"]
    click.echo(with_emoji(u"💫", "Casting workflow #{}".format(workflow.id) + ellipses(utf8)))
    if not local:
        if background:
            click.echo("View logs with `spell logs {}`".format(workflow.managing_run.id))
        else:
            click.echo(with_emoji(u"✨", "Following workflow at run {}.".format(workflow.managing_run.id)))
            click.echo(with_emoji(u"✨", "Stop viewing logs with ^C"))
