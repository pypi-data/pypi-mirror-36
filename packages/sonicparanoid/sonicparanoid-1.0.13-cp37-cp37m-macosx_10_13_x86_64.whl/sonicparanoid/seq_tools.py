...
def checkMoleculeType(inSeq, debug=True):
    """Check if the input fasta sequences are DNA or PROTEIN."""
    if debug:
    ...
    if fmt != 'fasta':
        ...
        sys.exit(-4)
    #now we can start checking the format
    from collections import OrderedDict
    #DNA -> A,T,C,G,N
    #RNA -> A,U,C,G
    symbolsDict = OrderedDict()
    seqCnt = 0
    #open and analyse the input sequence
    fd = open(inSeq, 'r')
    for ln in fd:
        if ln[0] == '>':
            if seqCnt == 50: #check only the first 20 seqs
                break
            continue
        else:
            seqCnt = seqCnt + 1
            ln = ln.strip()
            ln = ln.lower()
            for el in ln: #read the single element
                if not el in symbolsDict:
                    symbolsDict[el] = None #add the key to the dictionary
    fd.close()
    symbLen = len(symbolsDict)
    if debug:
        print('Symbols in input sequence:\t%s'%str(symbLen))
        for k in symbolsDict.keys():
            print(k, end=' ')
    #let's now decide what it is
    if symbLen <= 5:
        if not 'u' in symbolsDict: #then it is a DNA sequence
            molType = 'dna'
        else: #then it is probably a RNA sequence
            sys.stderr.write('\nERROR: Your sequence seem to be an RNA senquence and it is not supported by this program')
            sys.stderr.write('\nPlease input a fasta file of either DNA or Protein sequences\n')
            sys.exit(-4)
    elif symbLen <= 25: #then we consider it a proteome
        #21 codes for amino-acid plus 3 special codes (X, * and -)
        molType = 'protein'
    elif symbLen > 25: #too many symbols, something is wrong in the input file, note that we count 21 because the translated proteins can have the '*' symbol
        sys.stderr.write('\nERROR: Too many symbols in the alphabeth of your seuqnence')
        sys.stderr.write('\nPlease input a fasta file of either DNA or Protein sequences\n')
        sys.exit(-4)
    return molType
