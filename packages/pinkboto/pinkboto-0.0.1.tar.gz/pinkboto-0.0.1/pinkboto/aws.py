...
class aws(object):
    def __init__(self, profile=None, region=None, cache=True):
        """
        Create connection.
        ...
        from pinkboto.cache import caching, clean_cache
        # self.__cachefolder__ = self.__package_folder__+'/.cache'
        # if not os.path.exists(self.__cachefolder__):
        #     os.makedirs(self.__cachefolder__)
    ...
    def find(self, query=None, projection=None):
        ...
        if 'list_parameters' in resource:
            method_params = dict([(k, v) for k, v in query.items() if k in resource['list_parameters']])
        ...
        del query['resource']
        results = self.caching(self.pagination, method_params)
        ...
        if projection:
            if len(projection) > 1:
                output = [{k: v for k, v in obj.items() if k in projection} for obj in results]
            else:
                output = [[v for k, v in obj.items() if k in projection] for obj in results]
