...
@click.option(
    '-s', '--silent',
    is_flag=True,
    help='Minimize chatter from commands executed.'
)
@click.pass_context
@login_required
def clone(ctx, path, directory, include, exclude,silent):
    entity_type = get_entity_type(path)
    if entity_type == 'project':
        projects_clone(ctx, path, directory, include, exclude)
    elif entity_type == 'dataset':
        datasets_clone(ctx, path, directory, include, exclude,silent)
    else:
...
@click.pass_context
@login_required
def download(ctx, path, directory,archive,silent):
    entity_type = get_entity_type(path)
    if entity_type == 'dataset':
        datasets_download(ctx, path, directory,silent)
...
@click.pass_context
@login_required
def push(ctx, message, name,silent):
    home = os.getcwd()
    ...
    if os.path.isfile(ProjectViewController.PROJECT_FILE):
        ...
        general_push(ctx, message, name,silent)
    else:
