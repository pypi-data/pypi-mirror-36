...
class DynamicModelView(ModelFormMixin):
    ...
    def get_form_class(self):
        if self.model is None:
            self.app_label = self.kwargs.get("app", self.base_model._meta.app_label)
            self.model_name = self.kwargs.get("model", self.base_model._meta.model_name)
...
class DynamicSingleObjectMixin(SingleObjectMixin):
        ...
        def get_object(self, queryset=None):
            ...
            return getattr(obj, getattr(obj, "subclass", "__class__").lower(), )
...
class DynamicTemplate(SingleObjectTemplateResponseMixin):
    ...
    def get_template_names(self):
        ...
        file_name = self.model._meta.model_name + ".html"
        # orion_manager/forms/waste_collection/waste.html
        names.append(os.path.join(self.template_path, self.model._meta.app_label, file_name))
        ...
        while backup_models:
            ...
            file_name = next_model._meta.model_name + ".html"
            # orion_manager/forms/waste_collection/waste.html
            names.append(os.path.join(self.template_path, next_model._meta.app_label, file_name))
            # orion_manager/forms/waste.html
            names.append(os.path.join(self.template_path, file_name))
