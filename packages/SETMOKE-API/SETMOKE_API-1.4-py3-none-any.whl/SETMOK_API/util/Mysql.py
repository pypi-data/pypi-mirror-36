...
class MySql:
    ...
    def add_kwd(self, keyword, result, user_id, required_keyword=None, optional_keyword=None, excluded_keyword=None):
        ...
        for items in result:
            ...
            post_content=str(html2text.html2text(post_content))
    ...
    def update_data_base(self, result, keyword_id,userid):
        for items in result:
            ...
            self.cursor.execute(
                """Insert into Post(Keyword_id, PostUser_id, statusID,Content,CreatedAt, Source, ResharerCount) VALUES (%s,%s,%s,%s,%s,%s,%s)""",
                (keyword_id, postUserID, post_id, post_content, time, source, resharer_count))
            self.db.commit()
