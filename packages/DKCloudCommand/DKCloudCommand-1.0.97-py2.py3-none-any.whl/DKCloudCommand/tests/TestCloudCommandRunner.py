...
class TestCloudCommandRunner(BaseTestCloud):
    ...
    def test_recipe_get_negative(self):
        ...
        self.assertTrue('Recipe "%s" not found in kitchen %s' % (recipe_name, kitchen_name) in rc.get_message())
        shutil.rmtree(temp_dir, ignore_errors=True)
...
1 files are remote only:
    ...
    def test_merge_kitchens_source_remote_modified_resolved(self):
        ...
        expected_result = ('Kitchen %s has new changes since last kitchen-merge-preview. Please run kitchen-merge-preview again.\n') % from_kitchen
        self.assertTrue(expected_result in rc.get_message())
